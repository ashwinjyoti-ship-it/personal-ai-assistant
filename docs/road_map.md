...
**Date**: 2026-08-08
**Target repo**: [`poppin-browser`](https://github.com/ashwinjyoti-ship-it/poppin-browser) — all file paths below are relative to that repo, not this one.
**Companion docs** (this repo): `docs/successor-app-design.md`, `docs/cloudflare-compute-integration.md`
...
| Store convention | `src/main/workspace/workspace-store.ts` | Constructor runs `CREATE TABLE IF NOT EXISTS` inline — **no separate migrations folder**. Typed `*Row` interfaces map to snapshot types. Follow this exactly for new stores; don't introduce Tandem's numbered-migration-file style. |
...
Tandem source to port **content and behavior** from (not architecture — Tandem is Workers/D1, we're
translating to local SQLite):
...
- Port the useful half of `personal-ai-assistant`'s `src/services/memory.ts` — the compaction/decay
  logic — stripped of anything Cloudflare-specific.
...
- **PDF**: port `web/src/lib/pageExport.ts` close to verbatim — it's already client-side HTML→PDF, no
  server dependency to remove.
