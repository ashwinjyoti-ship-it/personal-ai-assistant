...
chat.post('/send', async (c) => {
  ...
  try {
    const confirmed = await tryConfirmPendingApproval(
      c.env, user, message.trim(), activeThreadId, channel,
    );
    if (confirmed) {
      return c.json({
        response: confirmed,
...
chat.post('/stream', async (c) => {
  ...
  try {
    ...
    if (confirmed) {
      const confirmStream = new ReadableStream({
        start(controller) {
          const encoder = new TextEncoder();
          controller.enqueue(encoder.encode(formatSSE({
            type: 'chunk',
            data: { text: confirmed, threadId: activeThreadId },
          })));
          controller.enqueue(encoder.encode(formatSSE({
            type: 'done',
            data: { threadId: activeThreadId, provider: 'approval' },
          })));
          controller.close();
        },
      });
      return new Response(confirmStream, {
        headers: {
