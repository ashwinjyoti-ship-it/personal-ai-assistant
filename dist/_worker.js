...
Sources:
${_}`,2048),sources:a,pagesRead:v.length}}const o=await tn(t,"You are a research planning expert.",`I need to research: ${e}

Generate exactly 4-5 specific sub-queries that together cover all important angles (definition, current state, comparisons, recent developments, expert analysis). Return ONLY a JSON array of strings, nothing else.`,400),l=vs(o,[e,`${e} overview`,`${e} examples`,`${e} latest`]).slice(0,5),c=await Promise.all(l.map(v=>i(v,5,"thorough"))),d=new Set,u=[];for(const v of c)for(const _ of v)d.has(_.result.url)||u.length>=20||(d.add(_.result.url),u.push(_));let p="";for(let v=0;v<u.length;v++){const _=u[v],T=`[${v+1}] ${_.result.title}
${_.result.url}

${_.content}

`;if(p.length+T.length>18e4)break;p+=T,a.push({url:_.result.url,title:_.result.title})}const y=await tn(t,"You are a research analyst identifying information gaps.",`I'm researching: ${e}

Here's what I've found so far:
${p.slice(0,6e4)}

Identify 2-3 specific information gaps or important angles not yet covered. Return ONLY a JSON array of follow-up search queries.`,300),g=vs(y,[]).slice(0,3);if(g.length>0){let v=0;const _=await Promise.all(g.map(T=>i(T,4,"thorough")));for(const T of _)for(const x of T){if(v>=5||d.has(x.result.url))continue;d.add(x.result.url);const N=`[${a.length+1}] ${x.result.title}
${x.result.url}

${x.content}

`;if(p.length+N.length>18e4)break;p+=N,a.push({url:x.result.url,title:x.result.title}),u.push(x),v++}}return{report:await tn(t,`You are an expert research analyst producing a comprehensive report. Use this exact structure:
...
Sources:
${p}`,4096),sources:a,pagesRead:u.length}}async function Ar(e,t,n={}){const s=n.depth||"quick",r=async(f,v)=>(await At(f,{num:v,site:n.site,googleApiKey:n.googleApiKey,googleCseId:n.googleCseId})).results||[];let a;if(n.anthropicKey)try{const f=await fo(e,n.anthropicKey,n.tavilyKey||null,s,r);if(f.report.trim())return{report:f.report,sources:f.sources.map(v=>({title:v.title,url:v.url})),pagesRead:f.pagesRead}}catch(f){a=f.message,console.warn("[conductResearch] Opus path failed:",f.message)}if(n.tavilyKey)try{const f=await Or(e,n.tavilyKey,s);if(f.results.length>0){const v=s==="thorough"?8:5,T=(await Promise.all(f.results.slice(0,v).map(async M=>({result:M,content:await Cr(M)})))).filter(M=>M.content.length>50),x=T.map(M=>({title:M.result.title,url:M.result.url}));if(T.length>0){const M=T.map((z,W)=>`--- SOURCE ${W+1}: ${z.result.title} ---
${z.content}
--- END SOURCE ${W+1} ---`).join(`

`);return{report:await nn(e,M,t,"full"),sources:x,pagesRead:T.length}}const L=f.results.map((M,$)=>`[${$+1}] ${M.title}
${M.content}
Source: ${M.url}`).join(`

`);return{report:await nn(e,L,t,"snippets"),sources:f.results.map(M=>({title:M.title,url:M.url})),pagesRead:0}}}catch(f){console.warn("[conductResearch] Tavily path failed:",f.message)}const i=n.maxPages||(s==="thorough"?5:3),o=n.maxResults||(s==="thorough"?8:5),l=await At(e,{num:o,site:n.site,googleApiKey:n.googleApiKey,googleCseId:n.googleCseId});if(l.error){const f=a?` (Opus: ${a})`:"";return{report:"",sources:[],pagesRead:0,error:`Search failed: ${l.error}${f}`}}if(l.results.length===0)return{report:`No web results found for "${e}".`,sources:[],pagesRead:0};const d=l.results.slice(0,i).map(async f=>{const v=await Fn(f.link);return{title:f.title,url:f.link,displayLink:f.displayLink,snippet:f.snippet,content:v.text,error:v.error}}),p=(await Promise.all(d)).filter(f=>f.content.length>50);if(p.length===0){const f=l.results.map((_,T)=>`[${T+1}] ${_.title}
${_.snippet}
Source: ${_.link}`).join(`

`);return{report:await nn(e,f,t,"snippets"),sources:l.results.map(_=>({title:_.title,url:_.link})),pagesRead:0}}const y=p.map((f,v)=>`--- SOURCE ${v+1}: ${f.title} (${f.displayLink}) ---
${f.content}
--- END SOURCE ${v+1} ---`).join(`

`);return{report:await nn(e,y,t,"full"),sources:p.map(f=>({title:f.title,url:f.url})),pagesRead:p.length}}async function nn(e,t,n,s){const a=`You are a research analyst. Your job is to synthesize web research into clear, actionable reports.
...
If a captcha was encountered, set captcha_required to true and populate whatever tracking data was visible before the captcha appeared.`}async function jr(e){const t=e instanceof Buffer?new Uint8Array(e):e,n=new DataView(t.buffer,t.byteOffset,t.byteLength);let s=0;for(;s<t.length-30&&n.getUint32(s,!0)===67324752;){const r=n.getUint16(s+6,!0),a=n.getUint16(s+8,!0),i=n.getUint32(s+18,!0),o=n.getUint32(s+22,!0),l=n.getUint16(s+26,!0),c=n.getUint16(s+28,!0),d=new TextDecoder().decode(t.slice(s+30,s+30+l)),u=s+30+l+c;if(d==="word/document.xml"){const p=t.slice(u,u+i);let y;if(a===0)y=p;else{const v=new DecompressionStream("deflate-raw"),_=v.writable.getWriter();_.write(p),_.close();const T=v.readable.getReader(),x=[];let L=!1;for(;!L;){const $=await T.read();$.done?L=!0:x.push($.value)}const N=x.reduce(($,z)=>$+z.length,0);y=new Uint8Array(o||N);let M=0;for(const $ of x)y.set($,M),M+=$.length}return new TextDecoder().decode(y).replace(/<\/w:p>/g,`
`).replace(/<\/w:tr>/g,`
...
`;return C+=I.results.map((O,P)=>`${P+1}. [${O.title}](${O.link})
   ${O.snippet}`).join(`
...
`)){const p=u.trim().split("|");if(p.length<4)continue;const[y,g,f,v]=p,_=["fact","preference","context","decision","summary","task"].find(x=>x===y.trim().toLowerCase());if(!_||!(g!=null&&g.trim())||!(f!=null&&f.trim()))continue;const T=Math.min(10,Math.max(1,parseInt(v)||5));await s.store(n.id,_,g.trim(),f.trim(),T,"long_term")}}const Ts={"claude-opus-4-8":1e6,"claude-sonnet-4-6":1e6,"claude-haiku-4-5":2e5,"gpt-4o":128e3,"gpt-4o-mini":128e3,"grok-3-mini":131072,"grok-3":131072,"deepseek-chat":64e3,"gemini-2.0-flash":1e6,default:32e3};function Xo(e){for(const[t,n]of Object.entries(Ts))if(e.toLowerCase().includes(t.toLowerCase()))return n;return Ts.default}function Qo(e,t,n,s){const r=Xo(s),a=Math.floor(r*.75),i=[];let o=0,l=!1;const c=kn(e);i.push({role:"system",content:e}),o+=c;const d=kn(n);o+=d;const u=a-o,p=[];let y=0;for(let g=t.length-1;g>=0;g--){const f=t[g],v=kn(f.content);if(y+v<=u)p.unshift({role:f.role,content:f.content}),y+=v;else{l=!0;break}}return i.push(...p),i.push({role:"user",content:n}),o+=y,{maxTokens:r,usedTokens:o,messages:i,wasTruncated:l}}async function*el(e,t,n,s,r,a){var he,m,E;const i=new Q(t),o=(he=e.metadata)==null?void 0:he.thread_id,l=Date.now();yield{type:"thinking",data:{threadId:o,provider:n.name}};const[c,d,u,p]=await Promise.all([i.buildContext(s.id),Un(t,s.id),ns(t,s.id),zr(t,s.id)]),y=await i.getRecentConversations(s.id,30,o);await sa(i,y,s.id,e.channel,o);const g=Qr(s,c,e.channel,u,p,d),f=Qn(y),v=es([...Xn(y)]);let _=e.text;f&&(_=`${Ln}

${e.text}`);const T=Qo(g,v.map(h=>({role:h.role,content:h.content})),_,n.name);await i.storeMessage(s.id,e.channel,"user",e.text,"{}",o);const x=await ts(t,s.id),L=10;let N="",M=0;const $=[...T.messages],z=[];let W,ne=0,j=0;const Y={hasActiveTask:!1,persistSession:!1,threadId:o,channel:e.channel},q=[/\bmy (address|phone|email|budget|preferences?|settings?|rules?|sheet|doc|folder|password|login|account)\b/i,/\b(do you remember|last time|what did i|you said|i told you|my usual|like before|same as last|remind me what|what was the|previously)\b/i],Z=(c.match(/^- /gm)||[]).length;if(q.some(h=>h.test(e.text))||Z<3||f)try{const h=await i.searchLongTerm(s.id,e.text,5);if(h.length>0){const w=h.map(b=>`- [${b.type}] ${b.title}: ${b.content}`).join(`
...
`;for(const c of a)i+=`- ${c.name} (${c.type}, ${Math.round(c.size/1024)}KB, file_id: ${c.file_id})`,c.text_preview&&(i+=`
  Preview: ${c.text_preview.substring(0,300)}...`),i+=`
`;i+="Use parse_document tool to read full file contents. Use drive_upload to upload to Google Drive.]"}let o=r;if(!o){const c=await e.env.DB.prepare("INSERT INTO threads (user_id, title) VALUES (?, ?) RETURNING *").bind(t.id,n.trim().substring(0,60)+(n.trim().length>60?"...":"")).first();o=c==null?void 0:c.id}const l={userId:t.id,username:t.username,channel:s,text:n.trim()+i,sessionId:e.get("sessionId"),timestamp:new Date().toISOString(),metadata:{thread_id:o}};try{const{provider:c,rotation:d}=await Qe(e.env.DB,t.id,t.pin_hash),u=tl(l,e.env.DB,c,t,d,{GOOGLE_CLIENT_ID:e.env.GOOGLE_CLIENT_ID,GOOGLE_CLIENT_SECRET:e.env.GOOGLE_CLIENT_SECRET,GOOGLE_API_KEY:e.env.GOOGLE_API_KEY,GOOGLE_CSE_ID:e.env.GOOGLE_CSE_ID,DOCUMENTS_BUCKET:e.env.DOCUMENTS_BUCKET}),p=(async function*(){for await(const v of u)v.data.threadId||(v.data.threadId=o),yield v})(),y=await sl(e.env.DB,t.id,o),g=v=>{try{e.executionCtx.waitUntil(v)}catch{}};if(y){il(e.env.DB,y,p,{waitUntil:g});const v=new ReadableStream({async start(_){const T=new TextEncoder,x=await ia(e.env.DB,y);if(!x){_.enqueue(T.encode(at({type:"error",data:{error:"Run not found"}}))),_.close();return}const{replay:L,tail:N,status:M}=oa(x);for(const $ of L)_.enqueue(T.encode(at($)));if(M!=="running"){_.close();return}try{for await(const $ of N)_.enqueue(T.encode(at($)))}catch{}_.close()}});return new Response(v,{headers:{"Content-Type":"text/event-stream","Cache-Control":"no-cache",Connection:"keep-alive","X-Thread-Id":String(o||""),"X-Run-Id":String(y)}})}const f=new ReadableStream({async start(v){const _=new TextEncoder;try{for await(const T of p)v.enqueue(_.encode(at(T)));o&&await e.env.DB.prepare("UPDATE threads SET message_count = message_count + 2, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(o).run(),v.close()}catch(T){v.enqueue(_.encode(at({type:"error",data:{error:T.message||"An error occurred",threadId:o}}))),v.close()}}});return new Response(f,{headers:{"Content-Type":"text/event-stream","Cache-Control":"no-cache",Connection:"keep-alive","X-Thread-Id":String(o||"")}})}catch(c){console.error("Stream setup error:",c);const d=c.message||"";return d.includes("No LLM provider configured")?e.json({error:"No AI provider configured. Please add at least one API key in Settings → Keys.",type:"no_provider",thread_id:o},400):d.includes("429")||d.includes("limit reached")||d.includes("rate limit")||d.includes("Too Many Requests")?e.json({error:"Rate limit reached across all configured providers. Add a second AI provider in Settings → API Keys (Gemini and DeepSeek offer free tiers) to enable automatic failover and avoid this.",type:"rate_limit",thread_id:o},429):e.json({error:"Something went wrong setting up the stream.",details:d,thread_id:o},500)}});le.get("/runs/:id/resume",async e=>{const t=e.get("user"),n=e.req.param("id");if(!n)return e.json({error:"Run id required"},400);if(!await al(e.env.DB,n,t.id))return e.json({error:"Run not found"},404);const r=await ia(e.env.DB,n);if(!r)return e.json({error:"Run not found"},404);const{replay:a,tail:i,status:o}=oa(r),l=new ReadableStream({async start(c){const d=new TextEncoder;for(const u of a)c.enqueue(d.encode(at(u)));if(o!=="running"){c.close();return}try{for await(const u of i)c.enqueue(d.encode(at(u)))}catch{}c.close()}});return new Response(l,{headers:{"Content-Type":"text/event-stream","Cache-Control":"no-cache",Connection:"keep-alive","X-Thread-Id":String(r.threadId||""),"X-Run-Id":String(n)}})});le.get("/threads/:id/active-run",async e=>{const t=e.get("user"),n=parseInt(e.req.param("id"));if(!Number.isFinite(n))return e.json({error:"Invalid thread id"},400);const s=await rl(e.env.DB,t.id,n);return s?e.json({run:{runId:s.runId,status:s.status,threadId:s.threadId}}):e.json({run:null})});le.get("/threads/:id/messages",async e=>{var a;const t=e.get("user"),n=parseInt(e.req.param("id")),s=parseInt(e.req.query("limit")||"50"),r=await e.env.DB.prepare(`SELECT id, role, content, channel, created_at FROM conversations 
...
`+y;if(d&&await e.env.DB.prepare("INSERT INTO conversations (user_id, channel, role, content, metadata) VALUES (?, ?, ?, ?, ?)").bind(r.user_id,"system","assistant",g,JSON.stringify({type:"cron",job_id:r.id})).run(),r.pin_hash){const{channel:f}=await We(e.env.DB,r.user_id,o,y,{pinHash:r.pin_hash,priority:"default",tags:["reminder","karna"]});f==="ntfy-failed"?console.warn(`[run-task] job ${r.id}: Ntfy push failed — in-app notification still delivered. Check ntfy_url/ntfy_token in Settings.`):f==="in-app"&&console.warn(`[run-task] job ${r.id}: Ntfy not configured — delivered in-app only.`)}else await e.env.DB.prepare("INSERT INTO notifications (user_id, type, title, body, source, is_read) VALUES (?, 'reminder', ?, ?, ?, 0)").bind(r.user_id,o,y,"cron:"+r.id).run();return e.json({job_id:s,status:"completed",response_length:c.length})});async function da(e){var s;const t=(s=e.req.header("Authorization"))==null?void 0:s.replace("Bearer ","");if(!t)return null;const n=await e.env.DB.prepare("SELECT user_id FROM sessions WHERE id = ? AND expires_at > datetime('now')").bind(t).first();return(n==null?void 0:n.user_id)||null}Ae.get("/health/tools",async e=>{var n;const t=await da(e);if(!t)return e.json({error:"Not authenticated"},401);try{const s=await e.env.DB.prepare(`SELECT tool_name,
       ...
       FROM tool_execution_log
       WHERE user_id = ?
       ORDER BY id DESC LIMIT 50`).bind(t).all();return e.json({logs:n.results})}catch(n){return e.json({error:n.message},500)}});const Pt=`
...
You MUST call gmail_list or gmail_search immediately.${Pt}`:n==="check_calendar"?`[Autonomous Scheduled Task] Execute this task NOW using tools — do NOT just describe what you'd do.
Task: "${e}"
Instructions: ${t||"Check calendar for upcoming events."}
You MUST call list_calendar_events immediately.${Pt}`:n==="check_sheet"?`[Autonomous Scheduled Task] Execute this task NOW using tools — do NOT just describe what you'd do.
Task: "${e}"
Instructions: ${t}
You MUST call read_sheet immediately with the relevant spreadsheet.${Pt}`:n==="custom"&&t?`[Autonomous Scheduled Task] Execute this task NOW using tools — do NOT just describe what you'd do.
Task: "${e}"
Instructions: ${t}
...
`];for(const g of o){let f="";if(g.due_date){const v=g.due_date.slice(0,10);v<c?f=" ⚠️ _overdue_":v===c?f=" 🔴 _due today_":v===u?f=" 🟡 _due tomorrow_":f=` _${new Date(g.due_date).toLocaleDateString("en-GB",{day:"numeric",month:"short"})}_`}p.push(`☐ ${g.title}${f}`)}p.push(`
...
`),"Markdown",r,s.id);y.success||console.warn(`[/tasks] Failed to send message: ${y.errors.join(" | ")}`)}catch(i){const o=await ae(n,t,"❌ Could not fetch tasks: "+i.message,"Markdown",r,s==null?void 0:s.id);o.success||console.warn(`[/tasks error] Failed to send message: ${o.errors.join(" | ")}`)}return!0}case"/notifications":case"/notif":{if(!s){const i=await ae(n,t,"⚠️ Account not linked.","Markdown",r);return i.success||console.warn(`[/notifications] Failed to send message: ${i.errors.join(" | ")}`),!0}try{const o=(await r.prepare(`
          ...
          LIMIT 5
        `).bind(s.id).all()).results||[];if(o.length===0){const c=await ae(n,t,"🎉 No pending notifications. You're all caught up.","Markdown",r,s.id);return c.success||console.warn(`[/notifications] Failed to send message: ${c.errors.join(" | ")}`),!0}const l=await ae(n,t,`📬 *${o.length} pending notification${o.length>1?"s":""}:*`,"Markdown",r,s.id);l.success||console.warn(`[/notifications] Failed to send header: ${l.errors.join(" | ")}`);for(const c of o)await xl(n,t,c)}catch(i){const o=await ae(n,t,"❌ Could not fetch notifications: "+i.message,"Markdown",r,s==null?void 0:s.id);o.success||console.warn(`[/notifications error] Failed to send message: ${o.errors.join(" | ")}`)}return!0}default:return!1}}async function Rl(e,t){var r,a,i,o,l,c;const n=t.db,s=t.env;console.log(`[telegram webhook] envVars keys=${Object.keys(s).join(",")}`);try{if(e.callback_query){await Cl(n,e.callback_query);return}const d=e.message;if(!d)return;const u=!!d.text,p=!!d.voice,y=!!d.document,g=!!d.photo,f=!!d.caption;if(!u&&!p&&!y&&!g)return;const v=String(d.chat.id);let _=d.text||"";const T=await n.prepare("SELECT * FROM users WHERE telegram_chat_id = ?").bind(v).first();let x=null;if(T){const j=await n.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(T.id,"telegram_bot_token").first();j&&(x=await G(j.encrypted_value,T.pin_hash))}if(!x){const j=await n.prepare(`SELECT c.encrypted_value, u.pin_hash FROM credentials c
         JOIN users u ON c.user_id = u.id
         WHERE c.service = 'telegram_bot_token' LIMIT 1`).first();j&&(x=await G(j.encrypted_value,j.pin_hash))}if(!x||_.startsWith("/")&&await Dl(_,v,x,T,n))return;if(!T){const j=await ae(x,v,`⚠️ Your account isn't linked yet.

Your Telegram Chat ID is: \`${v}\`

Go to the web app → Settings → Profile → set your Telegram Chat ID to this value.`,"Markdown",n);j.success||console.warn(`Failed to send unlinked account message: ${j.errors.join(" | ")}`);return}if(d.voice&&x&&T)try{if((d.voice.file_size??0)>20*1024*1024){const Z=await ae(x,v,"⚠️ Voice note is too large to process (max 20 MB).","Markdown",n,T.id);Z.success||console.warn(`[voice size] Failed to send message: ${Z.errors.join(" | ")}`);return}const j=await ae(x,v,"🎤 Processing voice note...","Markdown",n,T.id);j.success||console.warn(`[voice start] Failed to send message: ${j.errors.join(" | ")}`);const q=await(await Oe(`https://api.telegram.org/bot${x}/getFile?file_id=${d.voice.file_id}`)).json();if(q.ok&&((r=q.result)!=null&&r.file_path)){const te=await(await Is(`https://api.telegram.org/file/bot${x}/${q.result.file_path}`)).blob(),oe=await wl(n,T.id,T.pin_hash);if(!oe){const h=await ae(x,v,"⚠️ To use voice notes, add an OpenAI or Groq API key in Settings → Keys (LLM slot or legacy openai/groq).","Markdown",n,T.id);h.success||console.warn(`[voice no stt] Failed to send message: ${h.errors.join(" | ")}`);return}const re=new FormData;re.append("file",te,"voice.ogg"),re.append("model",oe.model),re.append("language","en");const he=await fetch(oe.url,{method:"POST",headers:{Authorization:`Bearer ${oe.apiKey}`},body:re});if(!he.ok){const h=await he.text(),w=await ae(x,v,`⚠️ Transcription failed: ${he.status} ${h}`,"Markdown",n,T.id);w.success||console.warn(`[voice transcription error] Failed to send message: ${w.errors.join(" | ")}`);return}_=(await he.json()).text;const E=await ae(x,v,`🗣️ *You said:* ${_}`,"Markdown",n,T.id);E.success||console.warn(`[voice transcript echo] Failed to send message: ${E.errors.join(" | ")}`)}}catch(j){const Y=await ae(x,v,`⚠️ Failed to process voice note: ${j.message}`,"Markdown",n,T==null?void 0:T.id);Y.success||console.warn(`[voice processing error] Failed to send message: ${Y.errors.join(" | ")}`);return}if((y||g)&&x&&T)try{let j,Y="unknown",q="unknown",Z=0;if(y)j=d.document.file_id,Y=d.document.file_name||"document",q=d.document.mime_type||"unknown",Z=d.document.file_size||0;else if(g){const te=d.photo[d.photo.length-1];j=te.file_id,Y="photo.jpg",q="image/jpeg",Z=te.file_size||0}if(j){const oe=await(await Oe(`https://api.telegram.org/bot${x}/getFile?file_id=${j}`)).json();let re="";if(oe.ok&&((a=oe.result)!=null&&a.file_path)&&(/\.(txt|csv|json|md|xml|html|log|yaml|yml|tsv|ini|cfg|conf|py|js|ts|sh|sql)$/i.test(Y)||/^text\/|application\/json|application\/xml|application\/csv/i.test(q))&&Z<5e4)try{re=await(await Is(`https://api.telegram.org/file/bot${x}/${oe.result.file_path}`)).text()}catch{}const he=d.caption||"",m=`[Telegram file received: "${Y}" (${q}, ${Math.round(Z/1024)}KB)]`;re?_=`${he?he+`
...
`,i);if(l>a+1800/2)i=l+2;else{const c=e.lastIndexOf(". ",i);c>a+1800/2&&(i=c+2)}}const o=e.slice(a,i).trim();o.length>=100&&r.push(o),a=i-200,a<=0&&(a=i)}return r}async function Kc(e,t,n,s){if(!e.AI||!e.VECTORIZE)return;const r=Pa(s);if(r.length===0)return;const a=await e.DB.prepare("SELECT vector_id FROM document_chunks WHERE document_id = ?").bind(n).all();a.results.length>0&&(await e.VECTORIZE.deleteByIds(a.results.map(d=>d.vector_id)),await e.DB.prepare("DELETE FROM document_chunks WHERE document_id = ?").bind(n).run());const o=(await e.AI.run("@cf/baai/bge-large-en-v1.5",{text:r})).data,l=r.map((d,u)=>`doc_${n}_${u}`);await e.VECTORIZE.insert(r.map((d,u)=>({id:l[u],values:o[u],metadata:{userId:String(t),documentId:String(n)}})));const c=e.DB.prepare("INSERT INTO document_chunks (user_id, document_id, chunk_index, text, vector_id) VALUES (?, ?, ?, ?, ?)");await e.DB.batch(r.map((d,u)=>c.bind(t,n,u,d,l[u])))}async function Yc(e,t,n,s=5){if(!e.AI||!e.VECTORIZE)return[];const a=(await e.AI.run("@cf/baai/bge-large-en-v1.5",{text:[n]})).data[0],i=await e.VECTORIZE.query(a,{topK:s*3,filter:{userId:String(t)}});if(!i.matches||i.matches.length===0)return[];const o=i.matches.map(f=>f.id),l=new Map(i.matches.map(f=>[f.id,f.score])),c=o.map(()=>"?").join(","),u=((await e.DB.prepare(`SELECT dc.text, dc.vector_id, dc.document_id, dl.name, dc.chunk_index
     ...
     LIMIT ?`).bind(t,`%${n.substring(0,80)}%`,s*2).all()).results||[]).map(f=>({filename:f.name,relevance_score:.55,chunk:f.text,document_id:f.document_id,chunk_index:f.chunk_index,retrieval_method:"keyword"})),g=new Map;for(const f of[...u,...y]){const v=`${f.document_id}:${f.chunk_index}`;if(!g.has(v))g.set(v,f);else{const _=g.get(v);g.set(v,{..._,relevance_score:Math.max(_.relevance_score,f.relevance_score),retrieval_method:"hybrid"})}}return[...g.values()].sort((f,v)=>v.relevance_score-f.relevance_score).slice(0,s)}const Ve=Object.freeze(Object.defineProperty({__proto__:null,chunkText:Pa,indexDocumentChunks:Kc,semanticDocumentSearch:Yc},Symbol.toStringTag,{value:"Module"}));export{Ps as default};
