...
- Unread errors: ${(x==null?void 0:x.cnt)||0}`}case"read_sheet":{if(!n)return"Authentication context unavailable.";try{const d=new te(a,r,n,s||"",o||""),y=t.spreadsheet_id;let p=t.range;const h=await d.sheets.getMetadata(y),x=h.sheets;p.includes("!")||(p=`${x[0]}!${p}`);let T;try{T=await d.sheets.readRange(y,p)}catch(k){if((f=k.message)!=null&&f.includes("Unable to parse range")||(v=k.message)!=null&&v.includes("400")){const I=p.includes("!")?p.split("!")[1]:p;p=`${x[0]}!${I}`,T=await d.sheets.readRange(y,p)}else throw k}let O=`[Spreadsheet: "${h.title}" | Reading tab: "${p.split("!")[0]}" | All tabs in this spreadsheet: ${x.map(k=>`"${k}"`).join(", ")}]
`;return x.length>1&&(O+=`[To read a different tab, call read_sheet again with range like "${x[1]}!A1:Z500"]
...
(Could not move to folder "${t.folder_name}": ${x.message})`}try{await new J(a).store(r,"context",`Spreadsheet: ${t.title}`,`Spreadsheet ID: ${p.spreadsheetId} | URL: ${p.url} | Sheets: ${(t.sheet_names||["Sheet1"]).join(", ")}`,7,"working")}catch{}return`Spreadsheet created: "${t.title}"${h}
ID: ${p.spreadsheetId}
URL: ${p.url}`}catch(d){return await C(a,r,"google","create_sheet",d.message),`Failed to create spreadsheet: ${d.message}`}}case"list_calendar_events":{if(!n)return"Authentication context unavailable.";try{const d=new te(a,r,n,s||"",o||""),y=t.calendar_id||"primary",p=t.days_ahead||7,h=new Date,x=new Date(h.getTime()+p*24*60*60*1e3),T=await d.calendar.listEvents(y,{timeMin:h.toISOString(),timeMax:x.toISOString(),query:t.query});return T.length===0?`No events found in the next ${p} days.`:T.map(O=>{var $;const k=O.start.dateTime||O.start.date||"TBD",I=O.end.dateTime||O.end.date||"",D=O.location?` 📍 ${O.location}`:"",L=(($=O.attendees)==null?void 0:$.map(z=>z.email).join(", "))||"";return`• ${O.summary} — ${k} to ${I}${D}${L?`
  Attendees: ${L}`:""}`}).join(`
...
(Could not move to folder "${t.folder_name}": ${x.message})`}try{await new J(a).store(r,"context",`Document: ${t.title}`,`Document ID: ${p.documentId} | URL: ${p.url}`,6,"working")}catch{}return`Document created: "${t.title}"${h}
ID: ${p.documentId}
URL: ${p.url}`}catch(d){return await C(a,r,"google","create_doc",d.message),`Failed to create document: ${d.message}`}}case"read_doc":{if(!n)return"Authentication context unavailable.";try{const y=await new te(a,r,n,s||"",o||"").docs.readDocument(t.document_id);return`Document: "${y.title}"

${y.content}`}catch(d){return await C(a,r,"google","read_doc",d.message),`Failed to read document: ${d.message}`}}case"append_to_doc":{if(!n)return"Authentication context unavailable.";try{const d=new te(a,r,n,s||"",o||"");if(!(await d.isConnected()).connected)return'Google account not connected. Please go to Settings → Keys → Google Workspace and click "Connect Google Account" first.';await d.docs.appendText(t.document_id,t.content);let p=t.document_id;try{p=(await d.docs.readDocument(t.document_id)).title}catch{}return`Content appended to "${p}".
URL: https://docs.google.com/document/d/${t.document_id}/edit`}catch(d){return await C(a,r,"google","append_to_doc",d.message),`Failed to append to document: ${d.message}`}}case"gmail_list":{if(!n)return"Authentication context unavailable.";try{const y=await new ne(a,r,n,s||"",o||"").listMessages({maxResults:t.max_results||10,query:t.query});return y.length===0?"No messages found.":y.map((p,h)=>`${p.isUnread?"● ":"  "}${h+1}. **${p.subject}**
   From: ${p.from}
   Date: ${p.date}
   ${p.snippet}
   [id: ${p.id}]`).join(`

`)}catch(d){return await C(a,r,"gmail","list",d.message),(g=d.message)!=null&&g.includes("not connected")?d.message:`Gmail list error: ${d.message}`}}case"gmail_read":{if(!n)return"Authentication context unavailable.";try{const d=new ne(a,r,n,s||"",o||""),y=await d.getMessage(t.message_id);if(!y)return"Message not found.";const p=await d.getMessageBody(t.message_id);return`**${y.subject}**
From: ${y.from}
To: ${y.to}
Date: ${y.date}

${p}`}catch(d){return await C(a,r,"gmail","read",d.message),`Gmail read error: ${d.message}`}}case"gmail_search":{if(!n)return"Authentication context unavailable.";try{const y=await new ne(a,r,n,s||"",o||"").search(t.query,t.max_results||10);return y.length===0?`No results for: ${t.query}`:y.map((p,h)=>`${p.isUnread?"● ":"  "}${h+1}. **${p.subject}**
   From: ${p.from}
   Date: ${p.date}
   ${p.snippet}
   [id: ${p.id}]`).join(`

`)}catch(d){return await C(a,r,"gmail","search",d.message),`Gmail search error: ${d.message}`}}case"gmail_send":{if(!n)return"Authentication context unavailable.";try{const y=await new ne(a,r,n,s||"",o||"").send(t.to,t.subject,t.body,{cc:t.cc});return`Email sent successfully to ${t.to}. Subject: "${t.subject}" [Message ID: ${y.id}]`}catch(d){return await C(a,r,"gmail","send",d.message),`Gmail send error: ${d.message}`}}case"gmail_draft":{if(!n)return"Authentication context unavailable.";try{const y=await new ne(a,r,n,s||"",o||"").createDraft(t.to,t.subject,t.body);return`Draft created. To: ${t.to}, Subject: "${t.subject}" — Review and send from Gmail. [Draft ID: ${y.id}]`}catch(d){return await C(a,r,"gmail","draft",d.message),`Gmail draft error: ${d.message}`}}case"gmail_modify":{if(!n)return"Authentication context unavailable.";try{return await new ne(a,r,n,s||"",o||"").modifyMessage(t.message_id,t.action),`Message ${t.message_id} successfully ${t.action}ed.`}catch(d){return await C(a,r,"gmail","modify",d.message),`Gmail modify error: ${d.message}`}}case"gmail_unread_count":{if(!n)return"Authentication context unavailable.";try{const y=await new ne(a,r,n,s||"",o||"").getUnreadCount();return`You have ${y} unread email${y!==1?"s":""} in Gmail.`}catch(d){return(_=d.message)!=null&&_.includes("not connected")?d.message:`Gmail error: ${d.message}`}}case"drive_list":{if(!n)return"Authentication context unavailable.";try{const{token:d}=await(await Promise.resolve().then(()=>st)).getGoogleAuth(a,r,n,s||"",o||""),y=new URLSearchParams;y.set("pageSize",String(t.max_results||10)),y.set("fields","files(id,name,mimeType,modifiedTime,size,webViewLink)"),y.set("orderBy","modifiedTime desc");let p="";t.folder_id?p=`'${t.folder_id}' in parents and trashed = false`:t.query?p=`${t.query} and trashed = false`:p="trashed = false",y.set("q",p);const h=await fetch(`https://www.googleapis.com/drive/v3/files?${y}`,{headers:{Authorization:`Bearer ${d}`}});if(!h.ok)throw new Error(`Drive API error (${h.status})`);const x=await h.json();return(w=x.files)!=null&&w.length?x.files.map((T,O)=>{var L,$;const k=((L=T.mimeType)==null?void 0:L.split(".").pop())||T.mimeType,I=T.size?`${(parseInt(T.size)/1024).toFixed(1)} KB`:"",D=(($=T.modifiedTime)==null?void 0:$.split("T")[0])||"";return`${O+1}. **${T.name}** (${k})
   ${I} · Modified: ${D}
   ${T.webViewLink||""}`}).join(`

`):"No files found."}catch(d){return await C(a,r,"google","drive_list",d.message),`Drive list error: ${d.message}`}}case"drive_search":{if(!n)return"Authentication context unavailable.";try{const{token:d}=await(await Promise.resolve().then(()=>st)).getGoogleAuth(a,r,n,s||"",o||""),y=`fullText contains '${t.query.replace(/'/g,"\\'")}' and trashed = false`,p=new URLSearchParams;p.set("q",y),p.set("pageSize",String(t.max_results||10)),p.set("fields","files(id,name,mimeType,modifiedTime,size,webViewLink)"),p.set("orderBy","modifiedTime desc");const h=await fetch(`https://www.googleapis.com/drive/v3/files?${p}`,{headers:{Authorization:`Bearer ${d}`}});if(!h.ok)throw new Error(`Drive API error (${h.status})`);const x=await h.json();return(E=x.files)!=null&&E.length?x.files.map((T,O)=>{var D,L;const k=((D=T.mimeType)==null?void 0:D.split(".").pop())||T.mimeType,I=((L=T.modifiedTime)==null?void 0:L.split("T")[0])||"";return`${O+1}. **${T.name}** (${k}) — Modified: ${I}
   ${T.webViewLink||""}`}).join(`
...
`)}catch(d){return await C(a,r,"search","web_search",d.message),`Web search error: ${d.message}`}case"read_url":try{const d=t.url;if(!d||!d.startsWith("http://")&&!d.startsWith("https://"))return"Invalid URL. Please provide a full URL starting with http:// or https://";const y=Math.min(t.max_length||8e3,15e3),{fetchPageContent:p}=await Promise.resolve().then(()=>rn),h=await p(d,y);return h.error?`Failed to read page: ${h.error}`:!h.text||h.text.length<20?`Page at ${d} returned no readable content.`:`Content from ${d} (${h.text.length} chars):

${h.text}`}catch(d){return await C(a,r,"search","read_url",d.message),`Read URL error: ${d.message}`}case"research":{if(!u)return"Research tool requires an LLM provider but none is available.";try{const y=za(t.query,u,{depth:t.depth||"quick",site:t.site}),p=new Promise(T=>setTimeout(()=>T(null),45e3)),h=await Promise.race([y,p]);if(h===null){const{webSearch:T}=await Promise.resolve().then(()=>Zr),O=await T(t.query,{num:5});if(O.error||O.results.length===0)return"Research timed out and fallback search returned no results. Try rephrasing or asking a more specific question.";let k=`Research took too long, but here are the top search results:

`;return k+=O.results.map((I,D)=>`${D+1}. **${I.title}**
   ${I.snippet}
   ${I.link}`).join(`

`),k}if(h.error)return`Research failed: ${h.error}`;let x=h.report;return h.sources.length>0&&(x+=`

---
**Sources** (`+h.pagesRead+` pages read):
`,x+=h.sources.map((T,O)=>`[${O+1}] ${T.title}
    ${T.url}`).join(`
`)),x}catch(d){return await C(a,r,"research","research",d.message),`Research error: ${d.message}`}}case"search_places":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured. Add it in Settings → Keys → Google API Key.";const y=await M(d.encrypted_value,n),p=await Na(y,t.query,{type:t.type});return p.error?`Places search failed: ${p.error}`:p.results.length===0?`No places found for "${t.query}".`:p.results.map((h,x)=>{const T=h.rating?` ★${h.rating} (${h.userRatingsTotal||0} reviews)`:"",O=h.openNow!==void 0?h.openNow?" · Open now":" · Closed":"",k=h.googleMapsUri?`
   ${h.googleMapsUri}`:"";return`${x+1}. **${h.name}**${T}${O}
   ${h.address}${k}
   [place_id: ${h.placeId}]`}).join(`

`)}catch(d){return await C(a,r,"google_api","search_places",d.message),`Places search error: ${d.message}`}}case"get_place_details":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const y=await M(d.encrypted_value,n),p=await $a(y,t.place_id);if(p.error)return`Details lookup failed: ${p.error}`;if(!p.details)return"No details found.";const h=p.details;let x=`**${h.name}**
...
Recent Reviews:`;for(const T of h.reviews)x+=`
— ${T.author} (★${T.rating}, ${T.time}): "${T.text}"`}return x}catch(d){return await C(a,r,"google_api","place_details",d.message),`Place details error: ${d.message}`}}case"get_directions":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const y=await M(d.encrypted_value,n),p=await Ma(y,t.origin,t.destination,{mode:t.mode||"driving"});if(p.error)return`Directions failed: ${p.error}`;if(!p.route)return"No route found.";const h=p.route;let x=`**${h.startAddress}** → **${h.endAddress}**
`;return x+=`📏 ${h.distance} · ⏱️ ${h.duration}`,h.durationInTraffic&&(x+=` (with traffic: ${h.durationInTraffic})`),x+=`
via ${h.summary}`,x+=`

Steps:`,h.steps.forEach((T,O)=>{x+=`
${O+1}. ${T.instruction} (${T.distance}, ${T.duration})`}),x}catch(d){return await C(a,r,"google_api","directions",d.message),`Directions error: ${d.message}`}}case"get_travel_time":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const y=await M(d.encrypted_value,n),p=await Ua(y,t.origin,t.destination,t.mode||"driving");if(p.error)return`Travel time lookup failed: ${p.error}`;let h=`${t.origin} → ${t.destination}: ${p.distance}, ${p.duration}`;return p.durationInTraffic&&(h+=` (with traffic: ${p.durationInTraffic})`),h}catch(d){return await C(a,r,"google_api","travel_time",d.message),`Travel time error: ${d.message}`}}case"translate_text":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const y=await M(d.encrypted_value,n),p=await Ba(y,t.text,t.target_language,t.source_language);return p.error?`Translation failed: ${p.error}`:`[${p.detectedSourceLang||t.source_language||"auto"} → ${t.target_language}]

${p.translatedText}`}catch(d){return await C(a,r,"google_api","translate",d.message),`Translation error: ${d.message}`}}case"search_youtube":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const y=await M(d.encrypted_value,n),p=await Pa(y,t.query,{maxResults:t.max_results||5,order:t.order||"relevance"});return p.error?`YouTube search failed: ${p.error}`:p.results.length===0?`No YouTube results for "${t.query}".`:p.results.map((h,x)=>{var T;return`${x+1}. **${h.title}**
   ${h.channelTitle} · ${((T=h.publishedAt)==null?void 0:T.split("T")[0])||""}
   ${h.description}
   ${h.url}`}).join(`

`)}catch(d){return await C(a,r,"google_api","youtube_search",d.message),`YouTube search error: ${d.message}`}}case"geocode_address":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const y=await M(d.encrypted_value,n),p=await ja(y,t.address);return p.error?`Geocoding failed: ${p.error}`:p.results.length===0?`Location not found: "${t.address}"`:p.results.map((h,x)=>`${x+1}. ${h.address}
   Coordinates: ${h.lat}, ${h.lng}`).join(`
`)}catch(d){return await C(a,r,"google_api","geocode",d.message),`Geocoding error: ${d.message}`}}default:return`Unknown tool: ${e}`}}async function qt(e,t,a,r,n,s){var E;const o=new J(t),i=(E=e.metadata)==null?void 0:E.thread_id,l=await o.buildContext(r.id),c=await o.getRecentConversations(r.id,25,i),m=[{role:"system",content:Ka(r,l)},...c.map(d=>({role:d.role,content:d.content})),{role:"user",content:e.text}];await o.storeMessage(r.id,e.channel,"user",e.text,"{}",i);const f=10;let v="",g=0;const _=[];for(let d=0;d<f;d++)try{const y=await a.chat(m,{tools:gt});if(y.usage&&(g+=y.usage.promptTokens+y.usage.completionTokens),y.toolCalls&&y.toolCalls.length>0){y.content&&m.push({role:"assistant",content:y.content});for(const p of y.toolCalls){_.push(p.name);try{const h=await Ne(p.name,p.arguments,t,r.id,{agentType:"full",providerName:a.name,channel:e.channel},r.pin_hash,s==null?void 0:s.GOOGLE_CLIENT_ID,s==null?void 0:s.GOOGLE_CLIENT_SECRET,s==null?void 0:s.GOOGLE_API_KEY,s==null?void 0:s.GOOGLE_CSE_ID,r.timezone,a);m.push({role:"user",content:`[Tool Result for ${p.name}]: ${h}`})}catch(h){await C(t,r.id,"tool",p.name,h.message||"Tool execution failed"),m.push({role:"user",content:`[Tool Error for ${p.name}]: ${h.message||"Execution failed"}`})}}continue}v=y.content;break}catch(y){if(n){const p=y.message||"",h=p.includes("401")||p.includes("403")||p.includes("authentication")||p.includes("credit balance"),x=p.includes("429"),T=h?1440:x?10:5;await n.recordError(a.name,p,T)}throw await C(t,r.id,"llm","provider_error",y.message||"Unknown LLM error",{provider:a.name,turn:d}),y}if(n&&g>0)try{await n.recordUsage(a.name,g)}catch{}const w=_.length>0?`[TOOLS_USED: ${[...new Set(_)].join(", ")}] `:"";return await o.storeMessage(r.id,e.channel,"assistant",w+v,"{}",i),await o.compactHistory(r.id,30),v}const Kt={"claude-sonnet-4-20250514":2e5,"claude-haiku-4-20250514":2e5,"gpt-4o":128e3,"gpt-4o-mini":128e3,"grok-3-mini":131072,"grok-3":131072,"deepseek-chat":64e3,"gemini-2.0-flash":1e6,default:32e3};function un(e){for(const[t,a]of Object.entries(Kt))if(e.toLowerCase().includes(t.toLowerCase()))return a;return Kt.default}function pn(e,t,a,r){const n=un(r),s=Math.floor(n*.75),o=[];let i=0,l=!1;const c=Tt(e);o.push({role:"system",content:e}),i+=c;const u=Tt(a);i+=u;const m=s-i,f=[];let v=0;for(let g=t.length-1;g>=0;g--){const _=t[g],w=Tt(_.content);if(v+w<=m)f.unshift({role:_.role,content:_.content}),v+=w;else{l=!0;break}}return o.push(...f),o.push({role:"user",content:a}),i+=v,{maxTokens:n,usedTokens:i,messages:o,wasTruncated:l}}async function*mn(e,t,a,r,n,s){var w;const o=new J(t),i=(w=e.metadata)==null?void 0:w.thread_id;yield{type:"thinking",data:{threadId:i,provider:a.name}};const l=await o.buildContext(r.id),c=await o.getRecentConversations(r.id,20,i),u=Ka(r,l),m=pn(u,c,e.text,a.name);await o.storeMessage(r.id,e.channel,"user",e.text,"{}",i);const f=10;let v="",g=0;const _=[...m.messages];for(let E=0;E<f;E++)try{E>0&&(yield{type:"thinking",data:{threadId:i}});const d=await a.chat(_,{tools:gt});if(d.usage&&(g+=d.usage.promptTokens+d.usage.completionTokens),d.toolCalls&&d.toolCalls.length>0){d.content&&(yield{type:"chunk",data:{text:d.content,threadId:i}},_.push({role:"assistant",content:d.content}));for(const p of d.toolCalls){yield{type:"tool_start",data:{tool:p.name,toolArgs:p.arguments,threadId:i}};try{const h=await Ne(p.name,p.arguments,t,r.id,{agentType:"full",providerName:a.name,channel:e.channel},r.pin_hash,s==null?void 0:s.GOOGLE_CLIENT_ID,s==null?void 0:s.GOOGLE_CLIENT_SECRET,s==null?void 0:s.GOOGLE_API_KEY,s==null?void 0:s.GOOGLE_CSE_ID,r.timezone,a);yield{type:"tool_end",data:{tool:p.name,toolResult:h.substring(0,500)+(h.length>500?"...":""),threadId:i}},_.push({role:"user",content:`[Tool Result for ${p.name}]: ${h}`})}catch(h){await C(t,r.id,"tool",p.name,h.message||"Tool execution failed"),yield{type:"tool_end",data:{tool:p.name,toolResult:`Error: ${h.message||"Execution failed"}`,threadId:i}},_.push({role:"user",content:`[Tool Error for ${p.name}]: ${h.message||"Execution failed"}`})}}continue}v=d.content;const y=50;for(let p=0;p<v.length;p+=y)yield{type:"chunk",data:{text:v.substring(p,p+y),threadId:i}},p+y<v.length&&await new Promise(x=>setTimeout(x,10));break}catch(d){if(n){const y=d.message||"",p=y.includes("401")||y.includes("403")||y.includes("authentication")||y.includes("credit balance"),h=y.includes("429"),x=p?1440:h?10:5;await n.recordError(a.name,y,x)}await C(t,r.id,"llm","provider_error",d.message||"Unknown LLM error",{provider:a.name,turn:E}),yield{type:"error",data:{error:d.message||"An error occurred",threadId:i}};return}if(n&&g>0)try{await n.recordUsage(a.name,g)}catch{}await o.storeMessage(r.id,e.channel,"assistant",v,"{}",i),await o.compactHistory(r.id,30),yield{type:"done",data:{threadId:i,provider:a.name,tokenCount:g}}}async function Mt(e,t,a,r,n,s){var u;const o=new J(t),i=(u=e.metadata)==null?void 0:u.thread_id,l=await o.buildContext(r.id),c=Fa(e.text,l);if(c.agent==="conversation")return gn(e,t,a,r,l,n,i);if(c.agent==="multi"||c.confidence<.5)return qt(e,t,a,r,n,s);try{return await hn(c.agent,e,t,a,r,l,n,s,i)}catch(m){return await C(t,r.id,"router","subagent_fallback",`${c.agent} failed: ${m.message}`,{route:c}),qt(e,t,a,r,n,s)}}async function hn(e,t,a,r,n,s,o,i,l){const c=new J(a),u=ft(n.timezone),m=dt(e,n,s,n.timezone,u),f=Wa(e,gt),v=(await c.getRecentConversations(n.id,25,l)).map(k=>({...k,content:k.content.replace(/^\[TOOLS_USED: [^\]]+\]\s*/i,"")})),g=[{role:"system",content:m},...v.map(k=>({role:k.role,content:k.content})),{role:"user",content:t.text}];await c.storeMessage(n.id,t.channel,"user",t.text,"{}",l);const _=10;let w="",E=0,d="",y=!1;const p=[];for(let k=0;k<_;k++)try{const I=await r.chat(g,{tools:f.length>0?f:void 0});if(I.usage&&(E+=I.usage.promptTokens+I.usage.completionTokens),I.toolCalls&&I.toolCalls.length>0){y=!0,I.content&&(d=I.content,g.push({role:"assistant",content:I.content}));for(const D of I.toolCalls){p.push(D.name);try{const L=await Ne(D.name,D.arguments,a,n.id,{agentType:e,providerName:r.name,channel:t.channel},n.pin_hash,i==null?void 0:i.GOOGLE_CLIENT_ID,i==null?void 0:i.GOOGLE_CLIENT_SECRET,i==null?void 0:i.GOOGLE_API_KEY,i==null?void 0:i.GOOGLE_CSE_ID,n.timezone,r);g.push({role:"user",content:`[Tool Result for ${D.name}]: ${L}`})}catch(L){await C(a,n.id,"tool",D.name,L.message||"Tool execution failed"),g.push({role:"user",content:`[Tool Error for ${D.name}]: ${L.message||"Execution failed"}`})}}continue}w=I.content,!w&&d&&(w=d);break}catch(I){if(o){const D=I.message||"",L=D.includes("401")||D.includes("403")||D.includes("authentication")||D.includes("credit balance"),$=D.includes("429"),z=L?1440:$?10:5;await o.recordError(r.name,D,z)}throw await C(a,n.id,"llm","subagent_error",I.message||"Unknown error",{agent:e,provider:r.name,turn:k}),I}const h=["scheduler","workspace","research"],x=e==="scheduler"&&!p.includes("create_schedule")&&!p.includes("list_schedules")&&!p.includes("toggle_schedule")&&!p.includes("update_schedule_state")&&!p.includes("delete_schedule")&&/\b(remind|in\s+\d+|at\s+\d{1,2}[:.]\d{0,2}|timer|alarm|schedule|tell\s+me\s+in|notify|alert|ping)\b/i.test(t.text);if(!y&&h.includes(e)&&f.length>0||x){try{await a.prepare(`INSERT INTO tool_execution_log (user_id, agent_type, provider_name, tool_name, tool_args, tool_result, success, error_message, latency_ms, was_enforcement_retry, channel)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(n.id,e,r.name,"__enforcement_trigger",JSON.stringify({userMessage:t.text.substring(0,200)}),w.substring(0,200),0,"LLM narrated without calling tools",0,0,t.channel||"web").run()}catch{}try{g.push({role:"assistant",content:w}),g.push({role:"user",content:e==="scheduler"&&x?`[SYSTEM OVERRIDE] You called ${p.join(", ")||"no tools"} but did NOT call create_schedule. The user wants a reminder or schedule created. Call create_schedule NOW with the correct parameters. Do NOT respond with text saying a reminder is set — actually create it.`:`[SYSTEM OVERRIDE] You responded with text but did NOT call any tool. This is a ${e} request — you MUST use your tools. Do NOT repeat your text response. Call the appropriate tool NOW (e.g., ${f.slice(0,3).map(I=>I.name).join(", ")}). The user is waiting for an actual action, not a description of what you would do.`});const k=await r.chat(g,{tools:f});if(k.usage&&(E+=k.usage.promptTokens+k.usage.completionTokens),k.toolCalls&&k.toolCalls.length>0){y=!0;for(const D of k.toolCalls){p.push(D.name);try{const L=await Ne(D.name,D.arguments,a,n.id,{agentType:e,providerName:r.name,channel:t.channel,isEnforcementRetry:!0},n.pin_hash,i==null?void 0:i.GOOGLE_CLIENT_ID,i==null?void 0:i.GOOGLE_CLIENT_SECRET,i==null?void 0:i.GOOGLE_API_KEY,i==null?void 0:i.GOOGLE_CSE_ID,n.timezone,r);g.push({role:"user",content:`[Tool Result for ${D.name}]: ${L}`})}catch(L){g.push({role:"user",content:`[Tool Error for ${D.name}]: ${L.message||"Execution failed"}`})}}const I=await r.chat(g,{tools:f});I.content&&(w=I.content),I.usage&&(E+=I.usage.promptTokens+I.usage.completionTokens)}if(!p.includes("create_schedule")&&e==="scheduler")try{const I=cn(t.text);if(I){const D=await Ne("create_schedule",I.args,a,n.id,{agentType:"scheduler",providerName:"programmatic_fallback",channel:t.channel,isEnforcementRetry:!0},n.pin_hash,i==null?void 0:i.GOOGLE_CLIENT_ID,i==null?void 0:i.GOOGLE_CLIENT_SECRET,i==null?void 0:i.GOOGLE_API_KEY,i==null?void 0:i.GOOGLE_CSE_ID,n.timezone,r);y=!0,p.push("create_schedule"),w=D}}catch(I){await C(a,n.id,"tool_enforcement","scheduler_fallback",`Programmatic scheduler fallback failed: ${I.message||"Unknown"}`)}}catch(k){await C(a,n.id,"tool_enforcement",e,`Tool enforcement retry failed: ${k.message||"Unknown"}`,{originalResponse:w.substring(0,200)})}}if(o&&E>0)try{await o.recordUsage(r.name,E)}catch{}const O=p.length>0?`[TOOLS_USED: ${[...new Set(p)].join(", ")}] `:"";return await c.storeMessage(n.id,t.channel,"assistant",O+w,"{}",l),await c.compactHistory(n.id,30),w}async function gn(e,t,a,r,n,s,o){const i=new J(t),l=ft(r.timezone),c=dt("conversation",r,n,r.timezone,l),u=await i.getRecentConversations(r.id,25,o),m=[{role:"system",content:c},...u.map(g=>({role:g.role,content:g.content})),{role:"user",content:e.text}];await i.storeMessage(r.id,e.channel,"user",e.text,"{}",o);let f=0,v="";try{const g=await a.chat(m,{temperature:.8});g.usage&&(f=g.usage.promptTokens+g.usage.completionTokens),v=g.content}catch(g){if(s){const _=g.message||"",w=_.includes("401")||_.includes("403")||_.includes("authentication")||_.includes("credit balance"),E=_.includes("429"),d=w?1440:E?10:5;await s.recordError(a.name,_,d)}throw await C(t,r.id,"llm","conversation_error",g.message,{provider:a.name}),g}if(s&&f>0)try{await s.recordUsage(a.name,f)}catch{}return await i.storeMessage(r.id,e.channel,"assistant",v,"{}",o),await i.compactHistory(r.id,30),v}async function*fn(e,t,a,r,n,s){var d;const o=new J(t),i=(d=e.metadata)==null?void 0:d.thread_id,l=await o.buildContext(r.id),c=Fa(e.text,l);if(yield{type:"thinking",data:{threadId:i,provider:a.name}},c.agent==="multi"||c.confidence<.5){yield*mn(e,t,a,r,n,s);return}const u=ft(r.timezone),m=c.agent==="conversation"?dt("conversation",r,l,r.timezone,u):dt(c.agent,r,l,r.timezone,u),f=Wa(c.agent,gt),v=await o.getRecentConversations(r.id,25,i),g=[{role:"system",content:m},...v.map(y=>({role:y.role,content:y.content})),{role:"user",content:e.text}];await o.storeMessage(r.id,e.channel,"user",e.text,"{}",i);const _=10;let w="",E=0;for(let y=0;y<_;y++)try{y>0&&(yield{type:"thinking",data:{threadId:i}});const p=await a.chat(g,{tools:f.length>0?f:void 0});if(p.usage&&(E+=p.usage.promptTokens+p.usage.completionTokens),p.toolCalls&&p.toolCalls.length>0){p.content&&(yield{type:"chunk",data:{text:p.content,threadId:i}},g.push({role:"assistant",content:p.content}));for(const x of p.toolCalls){yield{type:"tool_start",data:{tool:x.name,toolArgs:x.arguments,threadId:i}};try{const T=await Ne(x.name,x.arguments,t,r.id,{agentType:c.agent,providerName:a.name,channel:e.channel},r.pin_hash,s==null?void 0:s.GOOGLE_CLIENT_ID,s==null?void 0:s.GOOGLE_CLIENT_SECRET,s==null?void 0:s.GOOGLE_API_KEY,s==null?void 0:s.GOOGLE_CSE_ID,r.timezone,a);yield{type:"tool_end",data:{tool:x.name,toolResult:T.substring(0,500)+(T.length>500?"...":""),threadId:i}},g.push({role:"user",content:`[Tool Result for ${x.name}]: ${T}`})}catch(T){await C(t,r.id,"tool",x.name,T.message||"Tool execution failed"),yield{type:"tool_end",data:{tool:x.name,toolResult:`Error: ${T.message||"Execution failed"}`,threadId:i}},g.push({role:"user",content:`[Tool Error for ${x.name}]: ${T.message||"Execution failed"}`})}}continue}w=p.content;const h=50;for(let x=0;x<w.length;x+=h)yield{type:"chunk",data:{text:w.substring(x,x+h),threadId:i}},x+h<w.length&&await new Promise(O=>setTimeout(O,10));break}catch(p){if(n){const h=p.message||"",x=h.includes("401")||h.includes("403")||h.includes("authentication")||h.includes("credit balance"),T=h.includes("429"),O=x?1440:T?10:5;await n.recordError(a.name,h,O)}await C(t,r.id,"llm","subagent_stream_error",p.message||"Unknown error",{agent:c.agent,provider:a.name,turn:y}),yield{type:"error",data:{error:p.message||"An error occurred",threadId:i}};return}if(n&&E>0)try{await n.recordUsage(a.name,E)}catch{}await o.storeMessage(r.id,e.channel,"assistant",w,"{}",i),await o.compactHistory(r.id,30),yield{type:"done",data:{threadId:i,provider:a.name,tokenCount:E}}}const B=new ve;async function yn(e,t){var n;const a=(n=e.req.header("Authorization"))==null?void 0:n.replace("Bearer ","");if(!a)return e.json({error:"Authentication required"},401);const r=await e.env.DB.prepare(`SELECT s.*, u.* FROM sessions s JOIN users u ON s.user_id = u.id 
...
`;for(const c of s)o+=`- ${c.name} (${c.type}, ${Math.round(c.size/1024)}KB, file_id: ${c.file_id})`,c.text_preview&&(o+=`
  Preview: ${c.text_preview.substring(0,300)}...`),o+=`
`;o+="Use parse_document tool to read full file contents. Use drive_upload to upload to Google Drive.]"}let i=n;if(!i){const c=await e.env.DB.prepare("INSERT INTO threads (user_id, title) VALUES (?, ?) RETURNING *").bind(t.id,a.trim().substring(0,60)+(a.trim().length>60?"...":"")).first();i=c==null?void 0:c.id}const l={userId:t.id,username:t.username,channel:r,text:a.trim()+o,sessionId:e.get("sessionId"),timestamp:new Date().toISOString(),metadata:{thread_id:i}};try{const{provider:c,rotation:u}=await tt(e.env.DB,t.id,t.pin_hash),m=new ReadableStream({async start(f){const v=new TextEncoder;try{const g=fn(l,e.env.DB,c,t,u,{GOOGLE_CLIENT_ID:e.env.GOOGLE_CLIENT_ID,GOOGLE_CLIENT_SECRET:e.env.GOOGLE_CLIENT_SECRET,GOOGLE_API_KEY:e.env.GOOGLE_API_KEY,GOOGLE_CSE_ID:e.env.GOOGLE_CSE_ID});for await(const _ of g)_.data.threadId||(_.data.threadId=i),f.enqueue(v.encode(Yt(_)));i&&await e.env.DB.prepare("UPDATE threads SET message_count = message_count + 2, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(i).run(),f.close()}catch(g){const _={type:"error",data:{error:g.message||"An error occurred",threadId:i}};f.enqueue(v.encode(Yt(_))),f.close()}}});return new Response(m,{headers:{"Content-Type":"text/event-stream","Cache-Control":"no-cache",Connection:"keep-alive","X-Thread-Id":String(i||"")}})}catch(c){console.error("Stream setup error:",c);const u=c.message||"";return u.includes("No LLM provider configured")?e.json({error:"No AI provider configured. Please add at least one API key in Settings → Keys.",type:"no_provider",thread_id:i},400):u.includes("limit reached")?e.json({error:u,type:"cost_limit",thread_id:i},429):e.json({error:"Something went wrong setting up the stream.",details:u,thread_id:i},500)}});B.get("/threads/:id/messages",async e=>{var s;const t=e.get("user"),a=parseInt(e.req.param("id")),r=parseInt(e.req.query("limit")||"50"),n=await e.env.DB.prepare(`SELECT id, role, content, channel, created_at FROM conversations 
...
Status: ✅ Online`;await G(a,t,u)}catch{await G(a,t,"✅ Online — but had trouble fetching stats.")}return!0}case"/new":return r?(await n.prepare("UPDATE threads SET is_archived = 1 WHERE user_id = ? AND channel = 'telegram' AND is_archived = 0").bind(r.id).run(),await G(a,t,"🆕 Starting fresh conversation. Your next message begins a new thread."),!0):(await G(a,t,"⚠️ Account not linked."),!0);default:return!1}}at.post("/webhook",async e=>{var t,a,r,n,s;try{const o=await e.req.json();if(o.callback_query)return await In(e.env.DB,o.callback_query),e.json({ok:!0});const i=o.message;if(!i)return e.json({ok:!0});const l=!!i.text,c=!!i.voice,u=!!i.document,m=!!i.photo,f=!!i.caption;if(!l&&!c&&!u&&!m)return e.json({ok:!0});const v=String(i.chat.id);let g=i.text||"";const _=await e.env.DB.prepare("SELECT * FROM users WHERE telegram_chat_id = ?").bind(v).first();let w=null;if(_){const h=await e.env.DB.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(_.id,"telegram_bot_token").first();h&&(w=await M(h.encrypted_value,_.pin_hash))}if(!w){const h=await e.env.DB.prepare(`SELECT c.encrypted_value, u.pin_hash FROM credentials c 
         JOIN users u ON c.user_id = u.id 
         WHERE c.service = 'telegram_bot_token' LIMIT 1`).first();h&&(w=await M(h.encrypted_value,h.pin_hash))}if(!w)return e.json({ok:!0,message:"Bot token not configured"});if(g.startsWith("/")&&await Sn(g,v,w,_,e.env.DB))return e.json({ok:!0});if(!_)return await G(w,v,`⚠️ Your account isn't linked yet.

Your Telegram Chat ID is: \`${v}\`

Go to the web app → Settings → Profile → set your Telegram Chat ID to this value.`),e.json({ok:!0});if(i.voice&&w&&_)try{await G(w,v,"🎤 Processing voice note...");const x=await(await fetch(`https://api.telegram.org/bot${w}/getFile?file_id=${i.voice.file_id}`)).json();if(x.ok&&((t=x.result)!=null&&t.file_path)){const O=await(await fetch(`https://api.telegram.org/file/bot${w}/${x.result.file_path}`)).blob();let k="",I="",D="whisper-1";const L=await e.env.DB.prepare("SELECT service, encrypted_value FROM credentials WHERE user_id = ? AND (service IN ('llm_slot_1', 'llm_slot_2', 'llm_slot_3', 'openai'))").bind(_.id).all();for(const W of L.results){const De=await M(W.encrypted_value,_.pin_hash);if(W.service==="openai"){k="https://api.openai.com/v1/audio/transcriptions",I=De;break}else if(W.service.startsWith("llm_slot_"))try{const ue=JSON.parse(De);if(ue.provider==="openai"){k="https://api.openai.com/v1/audio/transcriptions",I=ue.apiKey;break}else if(ue.provider==="groq"){k="https://api.groq.com/openai/v1/audio/transcriptions",I=ue.apiKey,D="whisper-large-v3";break}}catch{}}if(!k)return await G(w,v,"⚠️ To use voice notes, configure an OpenAI API key in your LLM slots (Settings → Keys)."),e.json({ok:!0});const $=new FormData;$.append("file",O,"voice.ogg"),$.append("model",D),$.append("language","en");const z=await fetch(k,{method:"POST",headers:{Authorization:`Bearer ${I}`},body:$});if(!z.ok){const W=await z.text();return await G(w,v,`⚠️ Transcription failed: ${z.status} ${W}`),e.json({ok:!0})}g=(await z.json()).text,await G(w,v,`🗣️ *You said:* ${g}`)}}catch(h){return await G(w,v,`⚠️ Failed to process voice note: ${h.message}`),e.json({ok:!0})}if((u||m)&&w&&_)try{let h,x="unknown",T="unknown",O=0;if(u)h=i.document.file_id,x=i.document.file_name||"document",T=i.document.mime_type||"unknown",O=i.document.file_size||0;else if(m){const k=i.photo[i.photo.length-1];h=k.file_id,x="photo.jpg",T="image/jpeg",O=k.file_size||0}if(h){const I=await(await fetch(`https://api.telegram.org/bot${w}/getFile?file_id=${h}`)).json();let D="";if(I.ok&&((a=I.result)!=null&&a.file_path)&&(/\.(txt|csv|json|md|xml|html|log|yaml|yml|tsv|ini|cfg|conf|py|js|ts|sh|sql)$/i.test(x)||/^text\/|application\/json|application\/xml|application\/csv/i.test(T))&&O<5e4)try{D=await(await fetch(`https://api.telegram.org/file/bot${w}/${I.result.file_path}`)).text()}catch{}const L=i.caption||"",$=`[Telegram file received: "${x}" (${T}, ${Math.round(O/1024)}KB)]`;D?g=`${L?L+`
...
`:""}${$}
Note: This file type cannot be read directly. I can see it was sent but cannot extract the content. For PDFs, images, or Office docs — suggest uploading to Google Drive via the web app where I can process them.`}}catch(h){if(f&&i.caption)g=i.caption;else return await G(w,v,`⚠️ Received your file but couldn't process it: ${h.message}`),e.json({ok:!0})}if(!g)return e.json({ok:!0});await Tn(w,v);let E=await e.env.DB.prepare("SELECT id FROM threads WHERE user_id = ? AND channel = 'telegram' AND is_archived = 0 ORDER BY updated_at DESC LIMIT 1").bind(_.id).first();E?await e.env.DB.prepare("UPDATE threads SET updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(E.id).run():E={id:(await e.env.DB.prepare("INSERT INTO threads (user_id, title, channel) VALUES (?, 'Telegram', 'telegram')").bind(_.id).run()).meta.last_row_id};const d=_n(_.id,_.username,g,v);d.metadata={thread_id:E.id};let y,p;try{const h=await tt(e.env.DB,_.id,_.pin_hash);y=h.provider,p=h.rotation}catch(h){console.error("Telegram provider setup error:",h);const x=(r=h.message)!=null&&r.includes("No LLM provider")?`⚠️ No AI provider configured yet.

Go to the web app → Settings → Keys → add at least one API key (DeepSeek, Grok, Abacus AI, etc.).`:(n=h.message)!=null&&n.includes("Daily usage limit")?"⚠️ Daily usage limit reached. Your limit resets at midnight.":`⚠️ AI provider error: ${h.message||"Unknown error"}`;return await G(w,v,x),e.json({ok:!0})}try{const h=await Mt(d,e.env.DB,y,_,p,{GOOGLE_CLIENT_ID:e.env.GOOGLE_CLIENT_ID,GOOGLE_CLIENT_SECRET:e.env.GOOGLE_CLIENT_SECRET,GOOGLE_API_KEY:e.env.GOOGLE_API_KEY,GOOGLE_CSE_ID:e.env.GOOGLE_CSE_ID}),x=xn(h,"telegram");await G(w,v,x||"(empty response)")}catch(h){console.error("Telegram agent error:",h);const x=(s=h.message)!=null&&s.includes("API error")?`⚠️ AI provider returned an error. The provider (${y.name}) may be temporarily unavailable. Your message was saved — try again shortly.`:`⚠️ Something went wrong processing your message. Error: ${(h.message||"Unknown").substring(0,200)}`;await G(w,v,x);try{const{logError:T}=await Promise.resolve().then(()=>Dt);await T(e.env.DB,_.id,"telegram","agent_error",h.message||"Agent error",{provider:y.name})}catch{}}return e.json({ok:!0})}catch(o){console.error("Telegram webhook error:",o);try{const{logError:i}=await Promise.resolve().then(()=>Dt);await i(e.env.DB,null,"telegram","webhook_error",o.message||"Unknown telegram error")}catch{}return e.json({ok:!0,error:o.message})}});at.post("/setup-webhook",async e=>{var l;const t=(l=e.req.header("Authorization"))==null?void 0:l.replace("Bearer ","");if(!t)return e.json({error:"Auth required"},401);const a=await e.env.DB.prepare(`SELECT s.user_id, u.pin_hash FROM sessions s JOIN users u ON s.user_id = u.id
...
`)}function Nn(e){const t=[];let a=0;for(const r of e.calendar.google)t.push({type:"calendar",key:r.id,text:`${r.title} - ${new Date(r.startTime).toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"})}`,metadata:{event:r},sortOrder:a++});e.emails.gmail.unreadCount>0&&t.push({type:"email",key:"gmail-unread",text:`Review ${e.emails.gmail.unreadCount} unread Gmail messages`,metadata:{source:"gmail",count:e.emails.gmail.unreadCount},sortOrder:a++});for(const r of e.tasks.items)t.push({type:"task",key:`task-${r}`,text:r,metadata:{},sortOrder:a++});for(const r of e.news.items)t.push({type:"news",key:`news-${r.url}`,text:`📰 ${r.title}`,metadata:{url:r.url,source:r.source},sortOrder:a++});return t}async function $n(e,t){const a=await e.prepare("SELECT * FROM briefing_preferences WHERE user_id = ?").bind(t).first();if(!a)return{components:{google_calendar:!0,gmail:!0,tasks:!0,news:!0},newsTopics:["AI","LLM","Tools","Agentic Workflows","AI Features"]};let r;try{r=JSON.parse(a.components)}catch{r={google_calendar:!0,gmail:!0,tasks:!0,news:!0}}const n=a.news_topics?a.news_topics.split(",").map(s=>s.trim()).filter(Boolean):["AI","LLM","Tools","Agentic Workflows","AI Features"];return{components:r,newsTopics:n}}async function Ja(e,t,a){var E;const r=t.timezone||"Asia/Kolkata",n=Dn(r),{components:s,newsTopics:o}=await $n(e,t.id),i=[],l=[];s.google_calendar&&(i.push(On(e,t.id,t.pin_hash,a.GOOGLE_CLIENT_ID,a.GOOGLE_CLIENT_SECRET,n)),l.push("googleEvents")),s.gmail&&(i.push(Rn(e,t.id,t.pin_hash,a.GOOGLE_CLIENT_ID,a.GOOGLE_CLIENT_SECRET)),l.push("gmailSummary")),s.tasks&&(i.push(Cn(e,t.id)),l.push("tasks")),s.news&&(i.push(An(o)),l.push("news"));const c=await Promise.all(i),u={};l.forEach((d,y)=>{u[d]=c[y]});const m={unreadCount:0,importantCount:0,topSenders:[],hasUrgent:!1},f={pending:0,dueToday:0,items:[]},v={generatedAt:new Date().toISOString(),targetDate:n.dateStr,calendar:{google:u.googleEvents||[],totalCount:((E=u.googleEvents)==null?void 0:E.length)||0},emails:{gmail:u.gmailSummary||m},tasks:u.tasks||f,news:{items:u.news||[],fetchedAt:new Date().toISOString()},summary:""};v.summary=Ln(v);const g=Nn(v),_=await e.prepare(`
  ...
  `).bind(t.id,JSON.stringify(v)).first(),w=(_==null?void 0:_.id)||0;for(const d of g)await e.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `).bind(w,d.type,d.key,d.text,JSON.stringify(d.metadata),d.sortOrder).run();return{briefingId:w,content:v,items:g}}async function Mn(e,t,a){const r=await e.prepare(`
