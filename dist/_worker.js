...
`);for(const _ of m)if(_.startsWith("data: ")){const v=_.slice(6);if(v==="[DONE]")continue;try{const T=(f=(b=(h=JSON.parse(v).choices)==null?void 0:h[0])==null?void 0:b.delta)==null?void 0:f.content;T&&o.enqueue(new TextEncoder().encode("data: "+JSON.stringify({text:T})+`
...
`||o==="\r"){r.push(n),t.push(r),r=[],n="",s++;continue}n+=o,s++}for((n||r.length)&&(r.push(n),t.push(r));t.length&&t[t.length-1].every(o=>o==="");)t.pop();return t}async function gs(e,t,r,n,a,s,i,o,l,d,c,m){var b,f,_,v,x,T,O,A,B,G,F,j,W,q,R,H;const h=new Y(r);switch(e){case"create_schedule":{const u=new Date;let y;const p=d||"UTC";if(t.minutes_from_now&&typeof t.minutes_from_now=="number"&&t.minutes_from_now>0){y=new Date(u.getTime()+t.minutes_from_now*60*1e3);const D=y.toLocaleString("en-US",{timeZone:p,hour12:!1,year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"}).split(", "),[C,$,I]=(D[0]||"").split("/");t.schedule_value=`${I}-${C}-${$} ${D[1]||"00:00"}`,t.schedule_type="once"}else if(t.schedule_type==="interval"){const E=parseInt(t.schedule_value,10);y=new Date(u.getTime()+E*60*1e3)}else if(t.schedule_type==="daily"&&t.action_type==="reminder"){const E=`${t.name||""} ${t.action_description||""}`.toLowerCase();if(/\bevery\b|\bdaily\b|\beach\b|\bmorning\b|\bevening\b|\bnight\b|\bweekday\b|\bweekend\b|\brecurring\b|\brepeat\b/.test(E)){const[C,$]=t.schedule_value.split(":").map(Number),I=u.toLocaleString("en-US",{timeZone:p}),N=new Date(I),M=new Date(N);M.setHours(C,$,0,0),M<=N&&M.setDate(M.getDate()+1);const K=new Date(M.toLocaleString("en-US",{timeZone:"UTC"})),Z=new Date(M.toLocaleString("en-US",{timeZone:p})),Q=K.getTime()-Z.getTime();y=new Date(M.getTime()+Q)}else{const[C,$]=t.schedule_value.split(":").map(Number),I=u.toLocaleString("en-US",{timeZone:p}),N=new Date(I),M=new Date(N);M.setHours(C,$,0,0),M<=N&&M.setDate(M.getDate()+1);const K=mt=>String(mt).padStart(2,"0"),Z=M.getFullYear(),Q=K(M.getMonth()+1),ee=K(M.getDate());t.schedule_value=`${Z}-${Q}-${ee} ${K(C)}:${K($)}`,t.schedule_type="once";const ne=new Date(M.toLocaleString("en-US",{timeZone:"UTC"})),de=new Date(M.toLocaleString("en-US",{timeZone:p})),Ie=ne.getTime()-de.getTime();y=new Date(M.getTime()+Ie)}}else if(t.schedule_type==="daily"){const[E,D]=t.schedule_value.split(":").map(Number),C=u.toLocaleString("en-US",{timeZone:p}),$=new Date(C),I=new Date($);I.setHours(E,D,0,0),I<=$&&I.setDate(I.getDate()+1);const N=new Date(I.toLocaleString("en-US",{timeZone:"UTC"})),M=new Date(I.toLocaleString("en-US",{timeZone:p})),K=N.getTime()-M.getTime();y=new Date(I.getTime()+K)}else if(t.schedule_type==="weekly"){const[E,D]=t.schedule_value.split(" "),[C,$]=(D||"00:00").split(":").map(Number),N=["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"].findIndex(Ie=>Ie.toLowerCase()===E.toLowerCase()),M=u.toLocaleString("en-US",{timeZone:p}),K=new Date(M),Z=new Date(K);Z.setHours(C,$,0,0);let Q=(N-Z.getDay()+7)%7;Q===0&&Z<=K&&(Q=7),Z.setDate(Z.getDate()+Q);const ee=new Date(Z.toLocaleString("en-US",{timeZone:"UTC"})),ne=new Date(Z.toLocaleString("en-US",{timeZone:p})),de=ee.getTime()-ne.getTime();y=new Date(Z.getTime()+de)}else if(t.schedule_type==="once"){const[E,D]=t.schedule_value.split(" "),[C,$,I]=E.split("-").map(Number),[N,M]=(D||"00:00").split(":").map(Number),K=u.toLocaleString("en-US",{timeZone:p}),Z=new Date(K),Q=new Date(Z);Q.setFullYear(C,$-1,I),Q.setHours(N,M,0,0);const ee=new Date(Q.toLocaleString("en-US",{timeZone:"UTC"})),ne=new Date(Q.toLocaleString("en-US",{timeZone:p})),de=ee.getTime()-ne.getTime();y=new Date(Q.getTime()+de);const Ie=new Date(u.getTime()+120*1e3);if(y.getTime()<u.getTime()+60*1e3){const mt=y.toISOString();y=Ie;const pt=` [Note: The requested time ${t.schedule_value} in ${p} resolved to ${mt} UTC which is in the past. Auto-adjusted to fire in ~2 minutes at ${y.toISOString()}.]`;t._pastTimeWarning=pt}}else y=new Date(u.getTime()+3600*1e3);if(t.action_type==="custom"&&t.schedule_type==="once"){const E=`${t.name||""} ${t.action_description||t.description||""}`.toLowerCase();/\b(check|search|look\s*up|read|fetch|find|verify|track|scan|review|query|pull|get)\b/i.test(E)||(t.action_type="reminder")}if(await r.prepare("SELECT id FROM cron_jobs WHERE user_id = ? AND name = ? AND schedule_type = ? AND schedule_value = ? AND state != 'completed' LIMIT 1").bind(n,t.name,t.schedule_type,t.schedule_value).first()){const E=y.toLocaleString("en-US",{timeZone:p,weekday:"short",year:"numeric",month:"short",day:"numeric",hour:"2-digit",minute:"2-digit",hour12:!0});return`Schedule already exists: "${t.name}" is already set for ${E} (${p}). No duplicate created.`}await r.prepare(`INSERT INTO cron_jobs (user_id, name, description, schedule_type, schedule_value, action_type, action_config, next_run, state)
...
The sheet write has been saved to temporary memory. After reconnecting, tell me "write the pending sheet data" to complete this.`:"")}const p=t.values;let g=t.range;const E=Math.max(...p.map(I=>I.length))+4,D=p.map(I=>{const N=[...I];for(;N.length<E;)N.push("");return N}),C=g.match(/^(.+!)?([A-Z]+)(\d+):([A-Z]+)(\d+)$/i);if(C){const I=C[1]||"",N=C[2],M=C[3],K=C[5],Q=N.toUpperCase().charCodeAt(0)-64+E-1,ee=Q<=26?String.fromCharCode(64+Q):"Z";g=`${I}${N}${M}:${ee}${K}`}const $=await u.sheets.writeRange(t.spreadsheet_id,g,D);try{const I=new Y(r),N=await I.search(n,`Pending sheet write: ${t.spreadsheet_id}`);for(const M of N)M.title.startsWith(`Pending sheet write: ${t.spreadsheet_id}`)&&await I.remove(M.id,n)}catch{}return`Written ${$.updatedCells} cells to ${g}.`}catch(u){return await U(r,n,"google","write_sheet",u.message),`Failed to write sheet: ${u.message}`}}case"append_sheet":{if(!a)return"Authentication context unavailable.";try{const u=new ce(r,n,a,s||"",i||"");if(!(await u.isConnected()).connected){if(t.spreadsheet_id&&t.range&&t.values)try{await new Y(r).store(n,"context",`Pending sheet append: ${t.spreadsheet_id} — ${t.range}`,JSON.stringify({tool:"append_sheet",spreadsheet_id:t.spreadsheet_id,range:t.range,values:t.values}),9,"working")}catch{}return"Google account not connected. Please go to Settings → Keys → Google Workspace and connect your account."+(t.spreadsheet_id&&t.range?`
...
${g.text}`}catch(u){return await U(r,n,"search","read_url",u.message),`Read URL error: ${u.message}`}case"research":{if(!c)return"Research tool requires an LLM provider but none is available.";try{let u;try{const E=await r.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(n,"perplexity_api_key").first();E&&a&&(u=await J(E.encrypted_value,a))}catch{}const y=2e4,p=Sn(t.query,c,{depth:t.depth||"quick",site:t.site,perplexityApiKey:u}),g=new Promise(E=>setTimeout(()=>E(null),y)),w=await Promise.race([p,g]);if(w===null){const{webSearch:E}=await Promise.resolve().then(()=>Qa),D=await E(t.query,{num:5});if(D.error||D.results.length===0)return"Research timed out and fallback search returned no results. Try rephrasing or asking a more specific question.";let C=`Research took too long, but here are the top search results:
...
${p.extracted_text}`;const{file_name:g,file_type:w}=p;let{file_data:S}=p;if(S==="r2"){if(!m)return`File "${g}" is stored in R2 but no storage bucket is configured.`;const E=await m.get(u);if(!E)return`File "${g}" not found in storage. It may have been deleted.`;const D=await E.arrayBuffer();S=Buffer.from(D).toString("base64")}if(w.startsWith("text/"))try{const E=Buffer.from(S,"base64").toString("utf-8");return`Document: ${g}

${E.substring(0,2e4)}`}catch{return`Could not decode text file: ${g}`}if(w==="application/pdf"||w==="application/vnd.openxmlformats-officedocument.wordprocessingml.document"||g.toLowerCase().endsWith(".pdf")||g.toLowerCase().endsWith(".docx")){if(w==="application/vnd.openxmlformats-officedocument.wordprocessingml.document"||g.toLowerCase().endsWith(".docx")){try{const C=await Rn(Buffer.from(S,"base64"));if(C.length>50)return`Document: ${g}

${C.substring(0,2e4)}`}catch{}return`Could not extract text from "${g}". Try uploading to Google Drive and sharing the link — Drive can open and export Word documents directly.`}let E=null,D="claude-haiku-4-5-20251001";for(const C of["llm_slot_1","llm_slot_2","llm_slot_3"])try{const $=await r.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(n,C).first();if($&&a){const I=await J($.encrypted_value,a),N=JSON.parse(I);if(N.provider==="anthropic"){E=N.apiKey,N.model&&(D=N.model);break}}}catch{}if(E)try{const C=y?`Focus specifically on extracting: ${y}`:"Extract and return all readable text content from this document. Preserve structure where relevant.",$=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"x-api-key":E,"anthropic-version":"2023-06-01","anthropic-beta":"pdfs-2024-09-25","content-type":"application/json"},body:JSON.stringify({model:D,max_tokens:4096,messages:[{role:"user",content:[{type:"document",source:{type:"base64",media_type:"application/pdf",data:S}},{type:"text",text:C}]}]})});if($.ok){const N=((F=(G=(await $.json()).content)==null?void 0:G[0])==null?void 0:F.text)||"";return`Document: ${g}

${N}`}else{const I=await $.text();return`Could not parse ${g} via Anthropic API: ${I.substring(0,200)}`}}catch(C){return`Document parsing error for ${g}: ${C.message}`}return"To parse PDF documents, please configure an Anthropic API key in Settings → Keys. No Anthropic key is currently set."}try{const E=Buffer.from(S,"base64").toString("utf-8").substring(0,2e3);return`Document: ${g} (${w})

Content preview:
${E}`}catch{return`Cannot read file: ${g} (${w})`}}case"search_library":{const u=t.query,y=Math.min(typeof t.limit=="number"?t.limit:10,20);if(!u)return"query is required for search_library.";const p=await r.prepare(`
      ...
      `).bind(n,`%${u}%`,`%${u}%`,`%${u}%`,y).all(),g=await r.prepare(`
        ...
        ORDER BY dl.created_at DESC
        LIMIT ?
      `).bind(n,n,`%${u}%`,n,`%${u}%`,`%${u}%`,`%${u}%`,y).all(),w=[...p.results||[],...g.results||[]].slice(0,y);if(w.length===0)return`No documents found matching "${u}" in your library.`;const S=w.map(E=>{const D=(E.summary||E.dl_extracted||"").substring(0,200);return`[id:${E.id}] "${E.name}" (source: ${E.source}, status: ${E.status})
...
`),"Markdown",a,n.id);b.success||console.warn(`[/tasks] Failed to send message: ${b.errors.join(" | ")}`)}catch(i){const o=await te(r,t,"❌ Could not fetch tasks: "+i.message,"Markdown",a,n==null?void 0:n.id);o.success||console.warn(`[/tasks error] Failed to send message: ${o.errors.join(" | ")}`)}return!0}default:return!1}}Dt.post("/webhook",async e=>{let t;try{t=await e.req.json()}catch{return e.json({ok:!0})}const r=e.env.DB,n={GOOGLE_CLIENT_ID:e.env.GOOGLE_CLIENT_ID,GOOGLE_CLIENT_SECRET:e.env.GOOGLE_CLIENT_SECRET,GOOGLE_API_KEY:e.env.GOOGLE_API_KEY,GOOGLE_CSE_ID:e.env.GOOGLE_CSE_ID},a=async()=>{var s,i,o,l,d;try{if(t.callback_query){await Cs(r,t.callback_query);return}const c=t.message;if(!c)return;const m=!!c.text,h=!!c.voice,b=!!c.document,f=!!c.photo,_=!!c.caption;if(!m&&!h&&!b&&!f)return;const v=String(c.chat.id);let x=c.text||"";const T=await r.prepare("SELECT * FROM users WHERE telegram_chat_id = ?").bind(v).first();let O=null;if(T){const R=await r.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(T.id,"telegram_bot_token").first();R&&(O=await J(R.encrypted_value,T.pin_hash))}if(!O){const R=await r.prepare(`SELECT c.encrypted_value, u.pin_hash FROM credentials c
         JOIN users u ON c.user_id = u.id
         WHERE c.service = 'telegram_bot_token' LIMIT 1`).first();R&&(O=await J(R.encrypted_value,R.pin_hash))}if(!O||x.startsWith("/")&&await Is(x,v,O,T,r))return;if(!T){const R=await te(O,v,`⚠️ Your account isn't linked yet.

Your Telegram Chat ID is: \`${v}\`

Go to the web app → Settings → Profile → set your Telegram Chat ID to this value.`,"Markdown",r);R.success||console.warn(`Failed to send unlinked account message: ${R.errors.join(" | ")}`);return}if(c.voice&&O&&T)try{const R=await te(O,v,"🎤 Processing voice note...","Markdown",r,T.id);R.success||console.warn(`[voice start] Failed to send message: ${R.errors.join(" | ")}`);const u=await(await fetch(`https://api.telegram.org/bot${O}/getFile?file_id=${c.voice.file_id}`)).json();if(u.ok&&((s=u.result)!=null&&s.file_path)){const p=await(await fetch(`https://api.telegram.org/file/bot${O}/${u.result.file_path}`)).blob();let g="",w="",S="whisper-1";const E=await e.env.DB.prepare("SELECT service, encrypted_value FROM credentials WHERE user_id = ? AND (service IN ('llm_slot_1', 'llm_slot_2', 'llm_slot_3', 'openai'))").bind(T.id).all();for(const N of E.results){const M=await J(N.encrypted_value,T.pin_hash);if(N.service==="openai"){g="https://api.openai.com/v1/audio/transcriptions",w=M;break}else if(N.service.startsWith("llm_slot_"))try{const K=JSON.parse(M);if(K.provider==="openai"){g="https://api.openai.com/v1/audio/transcriptions",w=K.apiKey;break}else if(K.provider==="groq"){g="https://api.groq.com/openai/v1/audio/transcriptions",w=K.apiKey,S="whisper-large-v3";break}}catch{}}if(!g){const N=await te(O,v,"⚠️ To use voice notes, configure an OpenAI API key in your LLM slots (Settings → Keys).","Markdown",r,T.id);N.success||console.warn(`[voice no stt] Failed to send message: ${N.errors.join(" | ")}`);return}const D=new FormData;D.append("file",p,"voice.ogg"),D.append("model",S),D.append("language","en");const C=await fetch(g,{method:"POST",headers:{Authorization:`Bearer ${w}`},body:D});if(!C.ok){const N=await C.text(),M=await te(O,v,`⚠️ Transcription failed: ${C.status} ${N}`,"Markdown",r,T.id);M.success||console.warn(`[voice transcription error] Failed to send message: ${M.errors.join(" | ")}`);return}x=(await C.json()).text;const I=await te(O,v,`🗣️ *You said:* ${x}`,"Markdown",r,T.id);I.success||console.warn(`[voice transcript echo] Failed to send message: ${I.errors.join(" | ")}`)}}catch(R){const H=await te(O,v,`⚠️ Failed to process voice note: ${R.message}`,"Markdown",r,T==null?void 0:T.id);H.success||console.warn(`[voice processing error] Failed to send message: ${H.errors.join(" | ")}`);return}if((b||f)&&O&&T)try{let R,H="unknown",u="unknown",y=0;if(b)R=c.document.file_id,H=c.document.file_name||"document",u=c.document.mime_type||"unknown",y=c.document.file_size||0;else if(f){const p=c.photo[c.photo.length-1];R=p.file_id,H="photo.jpg",u="image/jpeg",y=p.file_size||0}if(R){const g=await(await fetch(`https://api.telegram.org/bot${O}/getFile?file_id=${R}`)).json();let w="";if(g.ok&&((i=g.result)!=null&&i.file_path)&&(/\.(txt|csv|json|md|xml|html|log|yaml|yml|tsv|ini|cfg|conf|py|js|ts|sh|sql)$/i.test(H)||/^text\/|application\/json|application\/xml|application\/csv/i.test(u))&&y<5e4)try{w=await(await fetch(`https://api.telegram.org/file/bot${O}/${g.result.file_path}`)).text()}catch{}const S=c.caption||"",E=`[Telegram file received: "${H}" (${u}, ${Math.round(y/1024)}KB)]`;w?x=`${S?S+`
...
🕐 ${O}${A}`;await fetch(`https://api.telegram.org/bot${x}/sendMessage`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({chat_id:l.telegram_chat_id,text:B,parse_mode:"Markdown"})})}a.push({user_id:l.id,reminders_sent:_.length})}catch(d){a.push({user_id:l.id,status:"error",error:d.message})}return e.json({executed:a.length,results:a})}catch(n){return e.json({error:n.message},500)}});oe.post("/cron/morning-briefing",async e=>{const t=e.req.header("X-Cron-Secret")||"",r=e.env.CRON_SECRET||"karna-cron-default-v1";if(t!==r)return e.json({error:"Unauthorized"},401);try{const n=await e.env.DB.prepare(`
  ...
  `).bind(t.id,n.toISOString(),a.toISOString()).all(),i=await e.prepare(`
    ...
    LIMIT 10
  `).bind(t.id).all(),o=await Yn(e,t,r);let l=[];try{const h=await e.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = 'google_oauth_tokens'").bind(t.id).first();if(h){const b=await J(h.encrypted_value,t.pin_hash),_=JSON.parse(b).access_token;if(_){const v=n.toISOString(),x=a.toISOString(),T=await fetch(`https://www.googleapis.com/calendar/v3/calendars/primary/events?singleEvents=true&orderBy=startTime&timeMin=${encodeURIComponent(v)}&timeMax=${encodeURIComponent(x)}&maxResults=20`,{headers:{Authorization:`Bearer ${_}`}});T.ok&&(l=((await T.json()).items||[]).map(A=>{var B,G,F,j;return{title:A.summary||"Untitled",startTime:((B=A.start)==null?void 0:B.dateTime)||((G=A.start)==null?void 0:G.date),endTime:((F=A.end)==null?void 0:F.dateTime)||((j=A.end)==null?void 0:j.date)}}))}}}catch{}const d={generatedAt:new Date().toISOString(),type:"morning",todayReminders:(s.results||[]).map(h=>({name:h.name,description:h.description,next_run:h.next_run})),pendingActions:(i.results||[]).map(h=>({title:h.title,priority:h.priority})),emailDigest:o,calendarEvents:l},c=await e.prepare(`
    INSERT INTO briefings (user_id, briefing_type, content_json, channel)
    VALUES (?, 'morning', ?, 'all')
    RETURNING id
