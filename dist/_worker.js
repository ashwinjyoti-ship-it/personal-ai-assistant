...
`),t.trim()}async function za(e,t,a={}){const r=a.maxPages||(a.depth==="thorough"?5:3),n=a.maxResults||(a.depth==="thorough"?8:5),s=await ht(e,{num:n,site:a.site});if(s.error)return{report:"",sources:[],pagesRead:0,error:`Search failed: ${s.error}`};if(s.results.length===0)return{report:`No web results found for "${e}".`,sources:[],pagesRead:0};const i=s.results.slice(0,r).map(async f=>{const v=await Ga(f.link);return{title:f.title,url:f.link,displayLink:f.displayLink,snippet:f.snippet,content:v.text,error:v.error}}),c=(await Promise.all(i)).filter(f=>f.content.length>50);if(c.length===0){const f=s.results.map((y,x)=>`[${x+1}] ${y.title}
${y.snippet}
Source: ${y.link}`).join(`

`);return{report:await zt(e,f,t,"snippets"),sources:s.results.map(y=>({title:y.title,url:y.link})),pagesRead:0}}const u=c.map((f,v)=>`--- SOURCE ${v+1}: ${f.title} (${f.displayLink}) ---
${f.content}
--- END SOURCE ${v+1} ---`).join(`

`);return{report:await zt(e,u,t,"full"),sources:c.map(f=>({title:f.title,url:f.url})),pagesRead:c.length}}async function zt(e,t,a,r){const s=`You are a research analyst. Your job is to synthesize web research into clear, actionable reports.
...
ID: ${p.id}
Start: ${p.start.dateTime||p.start.date}`}catch(d){return await C(a,r,"google","create_event",d.message),`Failed to create event: ${d.message}`}}case"create_doc":{if(!n)return"Authentication context unavailable.";try{const d=new te(a,r,n,s||"",o||"");if(!(await d.isConnected()).connected)return'Google account not connected. Please go to Settings → Keys → Google Workspace and click "Connect Google Account" to sign in with your Google account first.';const p=await d.docs.createDocument(t.title);t.content&&await d.docs.appendText(p.documentId,t.content);let h="";if(t.folder_name)try{const{token:_}=await(await Promise.resolve().then(()=>st)).getGoogleAuth(a,r,n,s||"",o||"");h=`
Folder: "${(await Wt(_,p.documentId,t.folder_name)).folderName}"`}catch(_){h=`
...
`)}catch(d){return await C(a,r,"search","web_search",d.message),`Web search error: ${d.message}`}case"read_url":try{const d=t.url;if(!d||!d.startsWith("http://")&&!d.startsWith("https://"))return"Invalid URL. Please provide a full URL starting with http:// or https://";const g=Math.min(t.max_length||8e3,15e3),{fetchPageContent:p}=await Promise.resolve().then(()=>rn),h=await p(d,g);return h.error?`Failed to read page: ${h.error}`:!h.text||h.text.length<20?`Page at ${d} returned no readable content.`:`Content from ${d} (${h.text.length} chars):

${h.text}`}catch(d){return await C(a,r,"search","read_url",d.message),`Read URL error: ${d.message}`}case"research":{if(!u)return"Research tool requires an LLM provider but none is available.";try{const g=za(t.query,u,{depth:t.depth||"quick",site:t.site}),p=new Promise(T=>setTimeout(()=>T(null),45e3)),h=await Promise.race([g,p]);if(h===null){const{webSearch:T}=await Promise.resolve().then(()=>Zr),O=await T(t.query,{num:5});if(O.error||O.results.length===0)return"Research timed out and fallback search returned no results. Try rephrasing or asking a more specific question.";let k=`Research took too long, but here are the top search results:
...
`;for(const c of s)o+=`- ${c.name} (${c.type}, ${Math.round(c.size/1024)}KB, file_id: ${c.file_id})`,c.text_preview&&(o+=`
  Preview: ${c.text_preview.substring(0,300)}...`),o+=`
`;o+="Use parse_document tool to read full file contents. Use drive_upload to upload to Google Drive.]"}let i=n;if(!i){const c=await e.env.DB.prepare("INSERT INTO threads (user_id, title) VALUES (?, ?) RETURNING *").bind(t.id,a.trim().substring(0,60)+(a.trim().length>60?"...":"")).first();i=c==null?void 0:c.id}const l={userId:t.id,username:t.username,channel:r,text:a.trim()+o,sessionId:e.get("sessionId"),timestamp:new Date().toISOString(),metadata:{thread_id:i}};try{const{provider:c,rotation:u}=await tt(e.env.DB,t.id,t.pin_hash),m=new ReadableStream({async start(f){const v=new TextEncoder;try{const y=fn(l,e.env.DB,c,t,u,{GOOGLE_CLIENT_ID:e.env.GOOGLE_CLIENT_ID,GOOGLE_CLIENT_SECRET:e.env.GOOGLE_CLIENT_SECRET,GOOGLE_API_KEY:e.env.GOOGLE_API_KEY,GOOGLE_CSE_ID:e.env.GOOGLE_CSE_ID});for await(const x of y)x.data.threadId||(x.data.threadId=i),f.enqueue(v.encode(Yt(x)));i&&await e.env.DB.prepare("UPDATE threads SET message_count = message_count + 2, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(i).run(),f.close()}catch(y){const x={type:"error",data:{error:y.message||"An error occurred",threadId:i}};f.enqueue(v.encode(Yt(x))),f.close()}}});return new Response(m,{headers:{"Content-Type":"text/event-stream","Cache-Control":"no-cache",Connection:"keep-alive","X-Thread-Id":String(i||"")}})}catch(c){console.error("Stream setup error:",c);const u=c.message||"";return u.includes("No LLM provider configured")?e.json({error:"No AI provider configured. Please add at least one API key in Settings → Keys.",type:"no_provider",thread_id:i},400):u.includes("limit reached")?e.json({error:u,type:"cost_limit",thread_id:i},429):e.json({error:"Something went wrong setting up the stream.",details:u,thread_id:i},500)}});B.get("/threads/:id/messages",async e=>{var s;const t=e.get("user"),a=parseInt(e.req.param("id")),r=parseInt(e.req.query("limit")||"50"),n=await e.env.DB.prepare(`SELECT id, role, content, channel, created_at FROM conversations 
...
`+y;return await e.env.DB.prepare("INSERT INTO notifications (user_id, type, title, body, source, is_read) VALUES (?, ?, ?, ?, ?, 0)").bind(n.user_id,"reminder",i,y,"cron:"+n.id).run(),u&&await e.env.DB.prepare("INSERT INTO conversations (user_id, channel, role, content, metadata) VALUES (?, ?, ?, ?, ?)").bind(n.user_id,"system","assistant",x,JSON.stringify({type:"cron",job_id:n.id})).run(),n.telegram_chat_id&&await bn(e.env.DB,n.user_id,n.telegram_chat_id,x),e.json({job_id:r,status:"completed",response_length:c.length})});async function Ya(e){var r;const t=(r=e.req.header("Authorization"))==null?void 0:r.replace("Bearer ","");if(!t)return null;const a=await e.env.DB.prepare("SELECT user_id FROM sessions WHERE id = ? AND expires_at > datetime('now')").bind(t).first();return(a==null?void 0:a.user_id)||null}de.get("/health/tools",async e=>{var a;const t=await Ya(e);if(!t)return e.json({error:"Not authenticated"},401);try{const r=await e.env.DB.prepare(`SELECT tool_name,
       ...
       ORDER BY calls DESC`).bind(t).all();return e.json({period:"last_24h",tool_stats:r.results,enforcement:{triggers:n.results,retry_results:((a=s.results)==null?void 0:a[0])||{total_retries:0,successful_retries:0}},cron:{executions:o.results,warnings:i.results},providers:l.results})}catch(r){return e.json({error:r.message||"Failed to fetch metrics"},500)}});de.get("/health/tools/recent",async e=>{const t=await Ya(e);if(!t)return e.json({error:"Not authenticated"},401);try{const a=await e.env.DB.prepare(`SELECT id, agent_type, provider_name, tool_name, tool_args, tool_result,
              success, error_message, latency_ms, was_enforcement_retry, channel, created_at
       FROM tool_execution_log
       WHERE user_id = ?
       ORDER BY id DESC LIMIT 50`).bind(t).all();return e.json({logs:a.results})}catch(a){return e.json({error:a.message},500)}});function wn(e,t,a){return a==="reminder"?`[Scheduled Reminder] "${e}": ${t||"Time for your reminder."}. Respond with a short, clean plain-text summary.`:a==="check_mail"?`[Autonomous Scheduled Task] Execute this task NOW using tools — do NOT just describe what you'd do.
Task: "${e}"
...
Task: "${e}"
Instructions: ${t}
Execute the instructions above by calling the appropriate tools (web_search, gmail_search, read_sheet, etc.) and present the results as a concise plain-text summary.`:`[Scheduled task "${e}"]: ${t||"Execute this scheduled task."}. Respond with a short, clean plain-text summary.`}function _n(e,t,a,r){return{userId:e,username:t,channel:"telegram",text:a,sessionId:`telegram-${r}`,timestamp:new Date().toISOString()}}function xn(e,t){return e.replace(/\*\*(.*?)\*\*/g,"*$1*").replace(/#{1,3}\s/g,"*").replace(/```(\w*)\n([\s\S]*?)```/g,"```$2```")}const at=new ve,En=4e3;async function G(e,t,a,r="Markdown"){var s,o;const n=kn(a,En);for(const i of n)try{const l=await fetch(`https://api.telegram.org/bot${e}/sendMessage`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({chat_id:t,text:i,parse_mode:r,disable_web_page_preview:!1})});if(!l.ok){const c=await l.json().catch(()=>null);((s=c==null?void 0:c.description)!=null&&s.includes("parse")||(o=c==null?void 0:c.description)!=null&&o.includes("entities"))&&await fetch(`https://api.telegram.org/bot${e}/sendMessage`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({chat_id:t,text:i})})}}catch{}}async function Tn(e,t){try{await fetch(`https://api.telegram.org/bot${e}/sendChatAction`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({chat_id:t,action:"typing"})})}catch{}}function kn(e,t){if(e.length<=t)return[e];const a=[];let r=e;for(;r.length>0;){if(r.length<=t){a.push(r);break}let n=r.lastIndexOf(`
`,t);n<t*.3&&(n=r.lastIndexOf(" ",t)),n<t*.3&&(n=t),a.push(r.substring(0,n)),r=r.substring(n).trimStart()}return a}async function Sn(e,t,a,r,n){switch(e.split("@")[0].toLowerCase()){case"/start":{const o=(r==null?void 0:r.name)||"there",i=(r==null?void 0:r.assistant_name)||"Karna",l=`👋 *Hello, ${o}!*
...
`)}function Nn(e){const t=[];let a=0;for(const r of e.calendar.google)t.push({type:"calendar",key:r.id,text:`${r.title} - ${new Date(r.startTime).toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"})}`,metadata:{event:r},sortOrder:a++});e.emails.gmail.unreadCount>0&&t.push({type:"email",key:"gmail-unread",text:`Review ${e.emails.gmail.unreadCount} unread Gmail messages`,metadata:{source:"gmail",count:e.emails.gmail.unreadCount},sortOrder:a++});for(const r of e.tasks.items)t.push({type:"task",key:`task-${r}`,text:r,metadata:{},sortOrder:a++});for(const r of e.news.items)t.push({type:"news",key:`news-${r.url}`,text:`📰 ${r.title}`,metadata:{url:r.url,source:r.source},sortOrder:a++});return t}async function $n(e,t){const a=await e.prepare("SELECT * FROM briefing_preferences WHERE user_id = ?").bind(t).first();if(!a)return{components:{google_calendar:!0,gmail:!0,tasks:!0,news:!0},newsTopics:["AI","LLM","Tools","Agentic Workflows","AI Features"]};let r;try{r=JSON.parse(a.components)}catch{r={google_calendar:!0,gmail:!0,tasks:!0,news:!0}}const n=a.news_topics?a.news_topics.split(",").map(s=>s.trim()).filter(Boolean):["AI","LLM","Tools","Agentic Workflows","AI Features"];return{components:r,newsTopics:n}}async function Ja(e,t,a){var E;const r=t.timezone||"Asia/Kolkata",n=Dn(r),{components:s,newsTopics:o}=await $n(e,t.id),i=[],l=[];s.google_calendar&&(i.push(On(e,t.id,t.pin_hash,a.GOOGLE_CLIENT_ID,a.GOOGLE_CLIENT_SECRET,n)),l.push("googleEvents")),s.gmail&&(i.push(Rn(e,t.id,t.pin_hash,a.GOOGLE_CLIENT_ID,a.GOOGLE_CLIENT_SECRET)),l.push("gmailSummary")),s.tasks&&(i.push(Cn(e,t.id)),l.push("tasks")),s.news&&(i.push(An(o)),l.push("news"));const c=await Promise.all(i),u={};l.forEach((d,g)=>{u[d]=c[g]});const m={unreadCount:0,importantCount:0,topSenders:[],hasUrgent:!1},f={pending:0,dueToday:0,items:[]},v={generatedAt:new Date().toISOString(),targetDate:n.dateStr,calendar:{google:u.googleEvents||[],totalCount:((E=u.googleEvents)==null?void 0:E.length)||0},emails:{gmail:u.gmailSummary||m},tasks:u.tasks||f,news:{items:u.news||[],fetchedAt:new Date().toISOString()},summary:""};v.summary=Ln(v);const y=Nn(v),x=await e.prepare(`
  ...
  `).bind(t.id,JSON.stringify(v)).first(),w=(x==null?void 0:x.id)||0;for(const d of y)await e.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `).bind(w,d.type,d.key,d.text,JSON.stringify(d.metadata),d.sortOrder).run();return{briefingId:w,content:v,items:y}}async function Mn(e,t,a){const r=await e.prepare(`
  ...
  `).bind(o,o,r,a).run(),{checked:o===1}}async function jn(e,t,a=10){return((await e.prepare(`
    SELECT b.*, 
