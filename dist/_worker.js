...
`);for(const _ of g)if(_.startsWith("data: ")){const w=_.slice(6);if(w==="[DONE]")continue;try{const T=JSON.parse(w);T.type==="content_block_delta"&&((f=T.delta)!=null&&f.text)&&c.enqueue(new TextEncoder().encode("data: "+JSON.stringify({text:T.delta.text})+`
...
`)}function _i(e,t,n){if(e.length===0)return`No purchase-related emails found for "${t}" (query: ${n}).`;const s=[...e].sort((i,o)=>un(o,t)-un(i,t)),r=s[0];if(un(r,t)>0){const i=`**Purchase email for "${t}"**
...
- **HARD RULE — no false confirmations**: You have ZERO tool access. You CANNOT set reminders, create schedules, send emails, read sheets, or perform any action. NEVER output phrases like "Reminder set for...", "I've scheduled...", "Done ✅", "Task created", or any language implying an action was completed. If the user wants an action, say: "I can do that — just send your message and I'll take care of it." Do NOT simulate the outcome.`;default:return""}}const Li=Object.freeze(Object.defineProperty({__proto__:null,buildPurchaseGmailQuery:On,buildSubAgentPrompt:yr,classifyIntentFast:Rn,detectDeterministicOp:Nn,detectTierTwoOp:In,extractPurchaseProduct:fr},Symbol.toStringTag,{value:"Module"})),Mi=new Set(["create_skill","list_skills","store_memory","search_memory","delete_memory","update_memory","get_schedules","delete_schedule","create_schedule","toggle_schedule","gmail_unread_count"]),$i=3,Pi=3,vr=5,wr=.4,_r=5;async function br(e,t,n,s,r,a,i=!0){try{const o=r.filter(f=>!Mi.has(f));if(o.length<$i)return;const c=[...[...new Set(o)]].sort().join(","),d=await e.prepare(`INSERT INTO skill_patterns (user_id, tool_signature, user_message_sample, tool_sequence, turn_count, succeeded)
     ...
     )`).bind(t.id,s,n).first(),a=(r==null?void 0:r.avg_success)??1,i=(r==null?void 0:r.total)??0,o=a<wr&&i>=_r;if(await e.prepare(`UPDATE user_skills
...
`}catch{return""}}async function ji(e,t,n){var i;let s=0,r=0,a=0;try{const o=await e.prepare(`SELECT us.id, us.user_id, us.name, us.instructions, us.refinement_count,
       ...
       WHERE us.user_id = ? AND us.is_auto = 1 AND us.enabled = 1
         AND us.confidence_score < ? AND us.usage_count >= ?`).bind(n,wr,_r).all();for(const l of o.results??[]){s++;const c=await e.prepare(`SELECT user_message_sample, tool_sequence
         FROM skill_patterns
...
REWRITTEN_INSTRUCTIONS: <revised instructions>`}];try{const _=(((i=(await t.chat(p,{tools:[]})).content)==null?void 0:i.trim())??"").match(/^REWRITTEN_INSTRUCTIONS:\s*([\s\S]+)$/m);_&&_[1].trim()&&(await e.prepare(`UPDATE user_skills
             SET instructions = ?, refinement_count = refinement_count + 1, updated_at = CURRENT_TIMESTAMP
             WHERE id = ?`).bind(_[1].trim(),l.id).run(),r++)}catch{await e.prepare("UPDATE user_skills SET enabled = 0, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(l.id).run(),a++}}}catch{}return{reviewed:s,rewritten:r,disabled:a}}async function Ui(e,t,n,s,r){var R;const i=((await e.prepare(`SELECT user_message_sample, tool_sequence
     FROM skill_patterns
...
[...truncated to fit token budget]`}const kr=6e3,_n="The user is following up on prior research in this thread. Answer from the injected research context first. If the follow-up requires new or updated information, call the research tool again with a query that includes the original topic.";function An(e){try{const t=JSON.parse(e||"{}");return typeof t=="object"&&t!==null?t:{}}catch{return{}}}function xr(e,t){const n=[...new Set(e)],s={};return n.length>0&&(s.tools=n),t&&(s.research_query=t.query.substring(0,200),s.research_report=t.report.substring(0,kr)),JSON.stringify(s)}function Ln(e){const t=[];for(const n of e)if(!(n.role!=="user"&&n.role!=="assistant")){if(n.role==="assistant"){const s=An(n.metadata);s.research_report&&t.push({role:"user",content:`[Tool Result for research]: ${s.research_report}`})}t.push({role:n.role,content:n.content})}return t}function Dr(e){return e.slice(-6).map(t=>{var n;return t.role==="assistant"&&(n=An(t.metadata).tools)!=null&&n.includes("research")?`[TOOLS_USED: research] ${t.content}`:t.content}).join(`
`)}function Mn(e){var t;for(let n=e.length-1;n>=0;n--){const s=e[n];if(s.role==="assistant")return((t=An(s.metadata).tools)==null?void 0:t.includes("research"))??!1}return!1}function Rr(e,t,n,s){return e!=="research"||/^(Research failed|Research error|Research timed out|\[Tool Error)/i.test(n)?s:{query:String(t.query||""),report:n.substring(0,kr)}}function Or(e,t){if(!t)return;const n=e[e.length-1];(n==null?void 0:n.role)!=="user"||typeof n.content!="string"||n.content.startsWith(_n)||(e[e.length-1]={role:"user",content:`${_n}
...
`)}case"toggle_schedule":{const m=t.enabled?1:0,y=m?"active":"paused";return await n.prepare("UPDATE cron_jobs SET enabled = ?, state = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?").bind(m,y,t.job_id,s).run(),`Schedule ${t.job_id} ${m?"enabled (active)":"paused"}.`}case"update_schedule_state":{const m=["created","active","reminding","paused","completed"],y=t.state;if(!m.includes(y))return`Invalid state "${y}". Valid states: ${m.join(", ")}`;const h=y==="completed"||y==="paused"?0:1;return await n.prepare("UPDATE cron_jobs SET state = ?, enabled = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?").bind(y,h,t.job_id,s).run(),`Schedule ${t.job_id} state updated to "${y}".`}case"update_schedule":{const m=t.job_id,y=c||"UTC",h=new Date,v=["updated_at = CURRENT_TIMESTAMP"],b=[];t.name&&(v.push("name = ?"),b.push(t.name)),t.description&&(v.push("description = ?"),b.push(t.description));let E=null,x=t.schedule_type,D=t.schedule_value;if(t.minutes_from_now&&typeof t.minutes_from_now=="number"&&t.minutes_from_now>0){E=new Date(h.getTime()+t.minutes_from_now*60*1e3);const N=E.toLocaleString("en-US",{timeZone:y,hour12:!1,year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"}).split(", "),[I,P,j]=(N[0]||"").split("/");D=`${j}-${I}-${P} ${N[1]||"00:00"}`,x="once"}else if(x&&D){if(x==="interval")E=new Date(h.getTime()+parseInt(D,10)*60*1e3);else if(x==="daily"){const[$,N]=D.split(":").map(Number),I=new Date(h.toLocaleString("en-US",{timeZone:y})),P=new Date(I);P.setHours($,N,0,0),P<=I&&P.setDate(P.getDate()+1);const j=new Date(P.toLocaleString("en-US",{timeZone:"UTC"})).getTime()-new Date(P.toLocaleString("en-US",{timeZone:y})).getTime();E=new Date(P.getTime()+j)}else if(x==="weekly"){const[$,N]=D.split(" "),[I,P]=(N||"00:00").split(":").map(Number),G=["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"].findIndex(fe=>fe.toLowerCase()===$.toLowerCase()),X=new Date(h.toLocaleString("en-US",{timeZone:y})),V=new Date(X);V.setHours(I,P,0,0);let oe=(G-V.getDay()+7)%7;oe===0&&V<=X&&(oe=7),V.setDate(V.getDate()+oe);const ie=new Date(V.toLocaleString("en-US",{timeZone:"UTC"})).getTime()-new Date(V.toLocaleString("en-US",{timeZone:y})).getTime();E=new Date(V.getTime()+ie)}else if(x==="once"){const[$,N]=D.split(" "),[I,P,j]=$.split("-").map(Number),[G,X]=(N||"00:00").split(":").map(Number),V=new Date(h.toLocaleString("en-US",{timeZone:y})),oe=new Date(V);oe.setFullYear(I,P-1,j),oe.setHours(G,X,0,0);const ie=new Date(oe.toLocaleString("en-US",{timeZone:"UTC"})).getTime()-new Date(oe.toLocaleString("en-US",{timeZone:y})).getTime();E=new Date(oe.getTime()+ie),E.getTime()<h.getTime()+60*1e3&&(E=new Date(h.getTime()+120*1e3))}}if(x&&(v.push("schedule_type = ?"),b.push(x)),D&&(v.push("schedule_value = ?"),b.push(D)),E&&(v.push("next_run = ?"),b.push(E.toISOString())),v.length===1)return"No changes provided. Specify name, description, or schedule fields to update.";b.push(m,s),await n.prepare(`UPDATE cron_jobs SET ${v.join(", ")} WHERE id = ? AND user_id = ?`).bind(...b).run();const O=E?E.toLocaleString("en-US",{timeZone:y,weekday:"short",year:"numeric",month:"short",day:"numeric",hour:"2-digit",minute:"2-digit",hour12:!0}):null;return`Schedule ${m} updated.${O?` New fire time: ${O} (${y}).`:""} IMPORTANT: Use this exact time "${O}" when confirming to the user.`}case"delete_schedule":return await n.prepare("DELETE FROM cron_jobs WHERE id = ? AND user_id = ?").bind(t.job_id,s).run(),`Schedule ${t.job_id} deleted.`;case"store_memory":{const m=t.importance||5,y=t.type==="task"?"preference":t.type,h=m>=7?"working":"long_term";return await _.store(s,y,t.title,t.content,m,h),`Stored in ${h==="working"?"working":"long-term"} memory: [${y}] ${t.title} (importance: ${m})`}case"search_memory":{const m=await _.search(s,t.query);return m.length===0?"No matching memories found.":m.map(y=>`[id:${y.id}] [${y.tier||"long_term"}] [${y.type}] **${y.title}**: ${y.content}`).join(`
`)}case"delete_memory":return await _.remove(t.id,s),`Memory entry ${t.id} deleted.`;case"update_memory":return await _.update(t.id,s,t.content),`Memory entry ${t.id} updated.`;case"get_system_status":{const m=await n.prepare("SELECT COUNT(*) as cnt FROM cron_jobs WHERE user_id = ? AND enabled = 1").bind(s).first(),y=await n.prepare("SELECT COUNT(*) as cnt FROM memory WHERE user_id = ?").bind(s).first(),h=await n.prepare("SELECT COUNT(*) as cnt FROM memory WHERE user_id = ? AND tier = 'working'").bind(s).first(),v=await n.prepare("SELECT COUNT(*) as cnt FROM conversations WHERE user_id = ?").bind(s).first(),b=await n.prepare("SELECT COUNT(*) as cnt FROM error_log WHERE (user_id = ? OR user_id IS NULL) AND acknowledged = 0").bind(s).first();return`System Status:
...
- Unread errors: ${(b==null?void 0:b.cnt)||0}`}case"read_sheet":{if(!r)return"Authentication context unavailable.";try{const m=new ve(n,s,r,a||"",i||""),y=t.spreadsheet_id;let h=t.range;const v=await m.sheets.getMetadata(y),b=v.sheets;h.includes("!")||(h=`${b[0]}!${h}`);let E;try{E=await m.sheets.readRange(y,h)}catch(D){if((w=D.message)!=null&&w.includes("Unable to parse range")||(T=D.message)!=null&&T.includes("400")){const O=h.includes("!")?h.split("!")[1]:h;h=`${b[0]}!${O}`,E=await m.sheets.readRange(y,h)}else throw D}let x=`[Spreadsheet: "${v.title}" | Reading tab: "${h.split("!")[0]}" | All tabs in this spreadsheet: ${b.map(D=>`"${D}"`).join(", ")}]
`;return b.length>1&&(x+=`[To read a different tab, call read_sheet again with range like "${b[1]}!A1:Z500"]
`),E.length===0?x+"No data found in the specified range.":x+E.map(D=>D.join("	| ")).join(`
...
The sheet write has been saved to temporary memory. After reconnecting, tell me "write the pending sheet data" to complete this.`:"")}const h=t.values;let v=t.range;const x=Math.max(...h.map(N=>N.length))+4,D=h.map(N=>{const I=[...N];for(;I.length<x;)I.push("");return I}),O=v.match(/^(.+!)?([A-Z]+)(\d+):([A-Z]+)(\d+)$/i);if(O){const N=O[1]||"",I=O[2],P=O[3],j=O[5],X=I.toUpperCase().charCodeAt(0)-64+x-1,V=X<=26?String.fromCharCode(64+X):"Z";v=`${N}${I}${P}:${V}${j}`}const $=await m.sheets.writeRange(t.spreadsheet_id,v,D);try{const N=new Q(n),I=await N.search(s,`Pending sheet write: ${t.spreadsheet_id}`);for(const P of I)P.title.startsWith(`Pending sheet write: ${t.spreadsheet_id}`)&&await N.remove(P.id,s)}catch{}return`Written ${$.updatedCells} cells to ${v}.`}catch(m){return await H(n,s,"google","write_sheet",m.message),`Failed to write sheet: ${m.message}`}}case"append_sheet":{if(!r)return"Authentication context unavailable.";try{const m=new ve(n,s,r,a||"",i||"");if(!(await m.isConnected()).connected){if(t.spreadsheet_id&&t.range&&t.values)try{await new Q(n).store(s,"context",`Pending sheet append: ${t.spreadsheet_id} — ${t.range}`,JSON.stringify({tool:"append_sheet",spreadsheet_id:t.spreadsheet_id,range:t.range,values:t.values}),9,"working")}catch{}return"Google account not connected. Please go to Settings → Keys → Google Workspace and connect your account."+(t.spreadsheet_id&&t.range?`

The append data has been saved to temporary memory. After reconnecting, tell me "append the pending sheet data" to complete this.`:"")}const h=await m.sheets.appendRows(t.spreadsheet_id,t.range,t.values);try{const v=new Q(n),b=await v.search(s,`Pending sheet append: ${t.spreadsheet_id}`);for(const E of b)E.title.startsWith(`Pending sheet append: ${t.spreadsheet_id}`)&&await v.remove(E.id,s)}catch{}return`Appended ${h.updatedCells} cells to ${t.range}.`}catch(m){return await H(n,s,"google","append_sheet",m.message),`Failed to append to sheet: ${m.message}`}}case"create_sheet":{if(!r)return"Authentication context unavailable.";try{const m=new ve(n,s,r,a||"",i||"");if(!(await m.isConnected()).connected){if(t.title)try{await new Q(n).store(s,"context",`Pending spreadsheet create: "${t.title}"`,JSON.stringify({tool:"create_sheet",title:t.title,sheet_names:t.sheet_names??null,folder_name:t.folder_name??null}),9,"working")}catch{}return'Google account not connected. Please go to Settings → Keys → Google Workspace and click "Connect Google Account" to sign in with your Google account first.'+(t.title?`

The spreadsheet request has been saved to temporary memory. After reconnecting, tell me "create the pending spreadsheet" and I'll complete this automatically.`:"")}const h=await m.sheets.createSpreadsheet(t.title,t.sheet_names);let v="";if(t.folder_name)try{const{token:b}=await(await Promise.resolve().then(()=>Xe)).getGoogleAuth(n,s,r,a||"",i||"");v=`
Folder: "${(await pn(b,h.spreadsheetId,t.folder_name)).folderName}"`}catch(b){v=`
...
ID: ${h.spreadsheetId}
URL: ${h.url}`}catch(m){return await H(n,s,"google","create_sheet",m.message),`Failed to create spreadsheet: ${m.message}`}}case"list_calendar_events":{if(!r)return"Authentication context unavailable.";try{const m=new ve(n,s,r,a||"",i||""),y=t.calendar_id||"primary",h=t.days_ahead||7,v=new Date,b=new Date(v.getTime()+h*24*60*60*1e3),E=await m.calendar.listEvents(y,{timeMin:v.toISOString(),timeMax:b.toISOString(),query:t.query});return E.length===0?`No events found in the next ${h} days.`:E.map(x=>{var I;const D=x.start.dateTime||x.start.date||"TBD",O=x.end.dateTime||x.end.date||"",$=x.location?` 📍 ${x.location}`:"",N=((I=x.attendees)==null?void 0:I.map(P=>P.email).join(", "))||"";return`• ${x.summary} — ${D} to ${O}${$}${N?`
  Attendees: ${N}`:""}`}).join(`
...
Your content has been saved to temporary memory. After reconnecting, tell me "save the pending document" and I'll complete this automatically.`:"")}let h;try{h=await m.docs.createDocument(t.title)}catch(b){return await H(n,s,"google","create_doc",b.message),`Failed to create document: ${b.message}`}if(t.content){const b=t.content,E=async()=>{b.length>12e3?await m.docs.appendText(h.documentId,b):await m.docs.appendFormattedContent(h.documentId,b)};try{await E()}catch(x){try{await m.docs.appendText(h.documentId,b)}catch(D){return await H(n,s,"google","create_doc_append",D.message),`Document created but content could not be written (${D.message}).
ID: ${h.documentId}
URL: ${h.url}

Use append_to_doc with the document ID above to add content.`}await H(n,s,"google","create_doc_append_fallback",`Formatted append failed, used plain text: ${x.message}`)}}let v="";if(t.folder_name)try{const{token:b}=await(await Promise.resolve().then(()=>Xe)).getGoogleAuth(n,s,r,a||"",i||"");v=`
Folder: "${(await pn(b,h.documentId,t.folder_name)).folderName}"`}catch(b){v=`
(Note: document saved to Drive root — could not place in folder "${t.folder_name}": ${b.message})`}try{await new Q(n).store(s,"context",`Document: ${t.title}`,`Document ID: ${h.documentId} | URL: ${h.url}`,6,"working")}catch{}try{const b=t.content;await n.prepare(`INSERT OR IGNORE INTO document_library (user_id, source, drive_file_id, name, summary, extracted_text, status)
           VALUES (?, 'drive', ?, ?, ?, ?, 'parsed')`).bind(s,h.documentId,t.title,b?b.substring(0,500):null,b?b.substring(0,5e4):null).run()}catch{}try{const b=new Q(n),E=await b.search(s,`Pending Google Doc save: "${t.title}"`);for(const x of E)x.title.startsWith(`Pending Google Doc save: "${t.title}"`)&&await b.remove(x.id,s)}catch{}return`Document created: "${t.title}"${v}
ID: ${h.documentId}
...
${h}`}catch(m){return await H(n,s,"gmail","read",m.message),`Gmail read error: ${m.message}`}}case"gmail_search":{if(!r)return"Authentication context unavailable.";try{const m=Xi(t);if(!m)return"Gmail search requires a non-empty query (e.g. from:sender@example.com subject:invoice). Use Gmail search syntax.";const y=typeof t.product_hint=="string"?t.product_hint.trim():"",h=Math.min(Math.max(t.max_results||10,1),20),v=new De(n,s,r,a||"",i||"");let b=await v.search(m,h);if(b.length===0&&y){const E=On(y).replace("180d","365d");b=await v.search(E,h)}return b.length===0?`No results for: ${m}`:y?_i(b,y,m):b.map((E,x)=>`${E.isUnread?"● ":"  "}${x+1}. **${E.subject}**
   From: ${E.from}
   Date: ${E.date}
   ${E.snippet}
   [id: ${E.id}]`).join(`

`)}catch(m){await H(n,s,"gmail","search",m.message);const y=String((m==null?void 0:m.message)||m);return/403|access denied|insufficient|permission/i.test(y)?`${y} Go to Settings → Keys → Google Workspace and reconnect your account.`:`Gmail search error: ${y}`}}case"gmail_send":{if(!r)return"Authentication context unavailable.";try{const m=new De(n,s,r,a||"",i||"");if(!(await new ve(n,s,r,a||"",i||"").isConnected()).connected){if(t.to&&t.subject&&t.body){try{await new Q(n).store(s,"context",`Pending email: "${t.subject}"`,JSON.stringify({tool:"gmail_send",to:t.to,subject:t.subject,body:t.body,cc:t.cc??null}),9,"working")}catch{}try{await n.prepare(`INSERT OR IGNORE INTO action_items (user_id, type, title, body, priority, source, source_id, action_payload)
...
Use read_library_file with the id to get the full document text.`}}catch{}const h=await n.prepare(`
      ...
      `).bind(s,`%${m}%`,`%${m}%`,`%${m}%`,y).all(),v=await n.prepare(`
        ...
        ORDER BY dl.created_at DESC
        LIMIT ?
      `).bind(s,s,`%${m}%`,s,`%${m}%`,`%${m}%`,`%${m}%`,y).all(),b=[...h.results||[],...v.results||[]].slice(0,y);if(b.length===0)return`No documents found matching "${m}" in your library.`;const E=b.map(x=>{const D=(x.summary||x.dl_extracted||"").substring(0,200);return`[id:${x.id}] "${x.name}" (source: ${x.source}, status: ${x.status})
...
${v.substring(0,2e4)}`:`Document "${h.name}" has no extracted text yet. Ask Karna to parse it with parse_document(file_id="${h.file_id}") to extract the text first.`}case"create_skill":{const m=(W=t.name)==null?void 0:W.trim(),y=(te=t.description)==null?void 0:te.trim(),h=(le=t.instructions)==null?void 0:le.trim();if(!m||!y||!h)return"create_skill requires name, description, and instructions.";let v=m.toLowerCase().replace(/[^a-z0-9\s_-]/g,"").replace(/\s+/g,"_").replace(/_+/g,"_").substring(0,50).replace(/^_|_$/g,"");v||(v=`skill_${Date.now()}`);const b=await n.prepare("SELECT slug FROM user_skills WHERE user_id = ? AND slug LIKE ?").bind(s,`${v}%`).all();(ne=b.results)!=null&&ne.some(O=>O.slug===v)&&(v=`${v}_${(((me=b.results)==null?void 0:me.length)||0)+1}`);const E=JSON.stringify(t.parameters||{}),x=JSON.stringify(t.required_tools||[]),D=JSON.stringify(t.examples||[]);return await n.prepare(`INSERT INTO user_skills (user_id, name, slug, description, instructions, parameters, required_tools, examples)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).bind(s,m,v,y,h,E,x,D).run(),`Skill created: **${m}** (invoke as: "${v}")

You can now ask me to run "${m}" at any time. The skill will appear in Settings → Skills. You can also say "run my ${m} skill" to execute it.`}case"list_skills":{const y=t.include_disabled===!0?"SELECT id, name, slug, description, enabled, usage_count, last_used_at FROM user_skills WHERE user_id = ? ORDER BY created_at DESC":"SELECT id, name, slug, description, enabled, usage_count, last_used_at FROM user_skills WHERE user_id = ? AND enabled = 1 ORDER BY created_at DESC",v=(await n.prepare(y).bind(s).all()).results||[];if(v.length===0)return`You haven't created any custom skills yet. Ask me to create one: "Create a skill that..."`;const b=v.map(E=>`• **${E.name}** (${E.slug}): ${E.description} [used ${E.usage_count} times${E.enabled?"":" — disabled"}]`).join(`
`);return`Your custom skills (${v.length}):

${b}`}default:{const m=e,y=await n.prepare("SELECT id, name, description, instructions, required_tools FROM user_skills WHERE user_id = ? AND slug = ? AND enabled = 1").bind(s,m).first();if(y){await n.prepare("UPDATE user_skills SET usage_count = usage_count + 1, last_used_at = CURRENT_TIMESTAMP WHERE id = ?").bind(y.id).run();const h=(()=>{try{return JSON.parse(y.required_tools).join(", ")}catch{return""}})(),v=Object.keys(t).length>0?`
...
`)){const p=u.trim().split("|");if(p.length<4)continue;const[g,f,_,w]=p,T=["fact","preference","context","decision","summary","task"].find(R=>R===g.trim().toLowerCase());if(!T||!(f!=null&&f.trim())||!(_!=null&&_.trim()))continue;const S=Math.min(10,Math.max(1,parseInt(w)||5));await s.store(n.id,T,f.trim(),_.trim(),S,"long_term")}}const as={"claude-sonnet-4-6":1e6,"claude-haiku-4-5":2e5,"gpt-4o":128e3,"gpt-4o-mini":128e3,"grok-3-mini":131072,"grok-3":131072,"deepseek-chat":64e3,"gemini-2.0-flash":1e6,default:32e3};function oo(e){for(const[t,n]of Object.entries(as))if(e.toLowerCase().includes(t.toLowerCase()))return n;return as.default}function lo(e,t,n,s){const r=oo(s),a=Math.floor(r*.75),i=[];let o=0,l=!1;const c=mn(e);i.push({role:"system",content:e}),o+=c;const d=mn(n);o+=d;const u=a-o,p=[];let g=0;for(let f=t.length-1;f>=0;f--){const _=t[f],w=mn(_.content);if(g+w<=u)p.unshift({role:_.role,content:_.content}),g+=w;else{l=!0;break}}return i.push(...p),i.push({role:"user",content:n}),o+=g,{maxTokens:r,usedTokens:o,messages:i,wasTruncated:l}}async function*co(e,t,n,s,r,a){var ne,me,m;const i=new Q(t),o=(ne=e.metadata)==null?void 0:ne.thread_id,l=Date.now();yield{type:"thinking",data:{threadId:o,provider:n.name}};const[c,d,u]=await Promise.all([i.buildContext(s.id),Bn(t,s.id),Er(t,s.id)]),p=await i.getRecentConversations(s.id,30,o);await Lr(i,p,s.id,e.channel,o);const g=Nr(s,c,e.channel,d,u),f=Mn(p),_=$n([...Ln(p)]);let w=e.text;f&&(w=`${_n}

${e.text}`);const T=lo(g,_.map(y=>({role:y.role,content:y.content})),w,n.name);await i.storeMessage(s.id,e.channel,"user",e.text,"{}",o);const S=await Pn(t,s.id),R=10;let C="",M=0;const A=[...T.messages],U=[];let q,Z=0,se=0;const L={hasActiveTask:!1,persistSession:!1,threadId:o,channel:e.channel},z=[/\bmy (address|phone|email|budget|preferences?|settings?|rules?|sheet|doc|folder|password|login|account)\b/i,/\b(do you remember|last time|what did i|you said|i told you|my usual|like before|same as last|remind me what|what was the|previously)\b/i],J=(c.match(/^- /gm)||[]).length;if(z.some(y=>y.test(e.text))||J<3||f)try{const y=await i.searchLongTerm(s.id,e.text,5);if(y.length>0){const h=y.map(v=>`- [${v.type}] ${v.title}: ${v.content}`).join(`
...
${h}]`})}}catch{}Mr(A);const te=()=>xr(U,q);for(let y=0;y<R;y++){Z=y+1;try{y>0&&(yield{type:"thinking",data:{threadId:o}},Ar(A));const h=await n.chat(A,{tools:S});if(h.usage&&(M+=h.usage.promptTokens+h.usage.completionTokens),h.toolCalls&&h.toolCalls.length>0){const E=((me=h.content)==null?void 0:me.trim())??"";E&&E.length<=150&&!/^\[calling:/i.test(E)&&(yield{type:"chunk",data:{text:h.content,threadId:o}});const x=h.content||"(tools executed)";A.push({role:"assistant",content:x});const D=[];for(const O of h.toolCalls){yield{type:"tool_start",data:{tool:O.name,toolArgs:O.arguments,threadId:o}},U.push(O.name);try{const $=(G,X)=>At(G,X,t,s.id,{agentType:"full",providerName:n.name,channel:e.channel},s.pin_hash,a==null?void 0:a.GOOGLE_CLIENT_ID,a==null?void 0:a.GOOGLE_CLIENT_SECRET,a==null?void 0:a.GOOGLE_API_KEY,a==null?void 0:a.GOOGLE_CSE_ID,s.timezone,n,a==null?void 0:a.DOCUMENTS_BUCKET,{ai:a==null?void 0:a.AI,vectorize:a==null?void 0:a.VECTORIZE},L);let N;if(O.name==="browser_task"||O.name==="browser_task_status"){if(O.name==="browser_task"){const ie=O.arguments.site_name;yield{type:"browser_ack",data:{message:ie?`Starting now — opening ${ie} in a browser. I'll notify you when done.`:"Starting now — running browser task. I'll notify you when done.",startedAt:new Date().toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit",timeZone:s.timezone||"UTC"}),threadId:o}}}const X=["Still working — browser launched, navigating to site...","Still working — page loaded, scanning for content...","Still working — reading and extracting results...","Still working — almost there, finalising output...","Taking a bit longer — site may require extra steps...","Still running — browser is working through the page...","Continuing — extracting and processing data...","Still going — complex task, nearly there...","Almost done — wrapping up the browser session...","Still running — holding on a little longer...","Browser is still active — this one is taking time...","Patience — still working through the task...","Still running — will have a result for you shortly..."],V=$(O.name,O.arguments);let oe=0;e:for(;;){const ie=await Promise.race([V.then(fe=>({done:!0,r:fe})),new Promise(fe=>setTimeout(()=>fe({done:!1}),15e3))]);if(ie.done){N=ie.r;break e}O.name==="browser_task"?yield{type:"browser_progress",data:{message:X[Math.min(oe,X.length-1)],elapsed_s:(oe+1)*15,threadId:o}}:yield{type:"thinking",data:{threadId:o}},oe++}if(O.name==="browser_task"){const ie=N.match(/^\[BROWSER_TIMEOUT:([^\]]+)\]/);if(ie){yield{type:"browser_progress",data:{message:"Task still running — checking final status...",threadId:o}};const fe=$("browser_task_status",{task_id:ie[1]});e:for(;;){const Ie=await Promise.race([fe.then(Te=>({done:!0,r:Te})),new Promise(Te=>setTimeout(()=>Te({done:!1}),15e3))]);if(Ie.done){N=Ie.r;break e}yield{type:"thinking",data:{threadId:o}}}if(!N.startsWith("[still-running]")&&!N.startsWith("[NO-OUTPUT]")&&!N.startsWith("Browser"))try{await t.prepare("DELETE FROM pending_browser_tasks WHERE user_id = ? AND task_id = ? AND notified = 0").bind(s.id,ie[1]).run()}catch{}}}}else N=await $(O.name,O.arguments);let I=N;(O.name==="browser_task"||O.name==="browser_task_status")&&(/^\[BROWSER_TIMEOUT:/.test(I)?I="Task timed out — still running in background.":/^\[NO-OUTPUT\]/.test(I)?I="Browser task finished but returned no content.":/^\[still-running\]/.test(I)?I="Still running — will notify when done.":I=I.replace(/\s*\|\s*Operator hint:.*$/s,"")),yield{type:"tool_end",data:{tool:O.name,toolResult:I.substring(0,500)+(I.length>500?"...":""),threadId:o}};const P=["parse_document","drive_read_file","read_library_file"].includes(O.name)?2e4:8e3,j=N.length>P?N.substring(0,P)+`
[...result truncated to prevent token limit — full content was extracted]`:N;q=Rr(O.name,O.arguments,N,q),D.push(`[Tool Result for ${O.name}]: ${j}`)}catch($){se++,await H(t,s.id,"tool",O.name,$.message||"Tool execution failed"),yield{type:"tool_end",data:{tool:O.name,toolResult:`Error: ${$.message||"Execution failed"}`,threadId:o}},D.push(`[Tool Error for ${O.name}]: ${$.message||"Execution failed"}`)}}A.push({role:"user",content:D.join(`

`)});continue}C=h.content;const v=Ct(C);await i.storeMessage(s.id,e.channel,"assistant",v,te(),o);const b=50;for(let E=0;E<v.length;E+=b)yield{type:"chunk",data:{text:v.substring(E,E+b),threadId:o}},E+b<v.length&&await new Promise(D=>setTimeout(D,10));break}catch(h){if(r){const E=h.message||"",x=E.includes("401")||E.includes("403")||E.includes("authentication")||E.includes("credit balance"),D=E.includes("429"),O=x?1440:D?10:5;await r.recordError(n.name,E,O)}await H(t,s.id,"llm","provider_error",h.message||"Unknown LLM error",{provider:n.name,turn:y});const v=h.message||"An error occurred",b=v.includes("429")||v.toLowerCase().includes("rate limit")||v.toLowerCase().includes("too many requests")?"Rate limit reached across all configured providers. Add a second AI provider in Settings → API Keys (Gemini and DeepSeek offer free tiers) to enable automatic failover and avoid this.":v;try{await i.storeMessage(s.id,e.channel,"assistant",`⚠️ ${b}`,"{}",o)}catch{}yield{type:"error",data:{error:b,threadId:o}};return}}if(C=(C==null?void 0:C.trim())??"",!C)try{A.push({role:"user",content:"You have used all available research steps. Please now give a final answer based on the information gathered above and your own training knowledge. Be clear about what you found vs. what comes from your general knowledge."}),C=(await n.chat(A,{tools:[]})).content||"I reached the maximum number of research steps without a conclusive result. Please try rephrasing your question or ask me to answer directly from my knowledge.";const h=Ct(C);await i.storeMessage(s.id,e.channel,"assistant",h,te(),o);const v=50;for(let b=0;b<h.length;b+=v)yield{type:"chunk",data:{text:h.substring(b,b+v),threadId:o}},b+v<h.length&&await new Promise(E=>setTimeout(E,10))}catch{C="I reached the maximum number of research steps. Please try rephrasing your question or ask me to answer directly from my knowledge.",await i.storeMessage(s.id,e.channel,"assistant",C,te(),o).catch(()=>{}),yield{type:"chunk",data:{text:C,threadId:o}}}if(r&&M>0)try{await r.recordUsage(n.name,M)}catch{}try{await t.prepare("INSERT INTO llm_calls (user_id, provider_name, agent_type, tokens_used, latency_ms, success, channel) VALUES (?, ?, ?, ?, ?, ?, ?)").bind(s.id,n.name,"full",M,Date.now()-l,1,e.channel).run()}catch{}const le=[{claimPattern:/\b(reminder set|set a reminder|i.ve set|i've set|scheduled for|reminder.*\d{1,2}:\d{2}|reminder.*(am|pm)\b|updated.*reminder|reminder.*updated|now set for|reminder now|set for.*\d{1,2}:\d{2}|set for.*(am|pm)\b)\b/i,requiredTools:["create_schedule","update_schedule"],enforcementMsg:"[SYSTEM ENFORCEMENT] You claimed a reminder was created or updated but neither create_schedule nor update_schedule was called. You MUST call the appropriate tool NOW. Do not respond with text only.",logType:"schedule_hallucination"},{claimPattern:/\b(email\s+(sent|delivered)|sent\s+(the\s+)?(email|message)|i.ve\s+(sent|emailed|mailed)|message\s+sent)\b/i,requiredTools:["gmail_send"],enforcementMsg:"[SYSTEM ENFORCEMENT] You claimed an email was sent but gmail_send was never called. You MUST call gmail_send NOW or clarify that only a draft was prepared.",logType:"email_hallucination"},{claimPattern:/\b(i.ve\s+(remembered|stored|saved|noted)|stored\s+(that|this|it)|saved\s+(that|this|it)\s+to\s+memory|i.ll\s+remember\s+that|noted\s+(that|this|it))\b/i,requiredTools:["store_memory"],enforcementMsg:"[SYSTEM ENFORCEMENT] You claimed to have stored something in memory but store_memory was never called. You MUST call store_memory NOW with the relevant content.",logType:"memory_hallucination"},{claimPattern:/\b(added\s+(row|entry|item|data)|appended\s+(to|into)\s+(the\s+)?(sheet|spreadsheet)|updated\s+(the\s+)?(sheet|row|cell|entry)|added\s+(to|in)\s+(your\s+|the\s+)(sheet|spreadsheet)|saved\s+(your\s+|the\s+)?(data|entry|note|info)\s+(to|in)\s+(the\s+)?(sheet|spreadsheet)|data\s+(is\s+)?(now\s+)?(in|on)\s+(your\s+|the\s+)?(sheet|spreadsheet)|(entry|note|data|row)\s+(has\s+been\s+|is\s+)(added|saved|recorded|entered|written))\b/i,requiredTools:["append_sheet","write_sheet"],enforcementMsg:"[SYSTEM ENFORCEMENT] You claimed to have written to a sheet but append_sheet/write_sheet was never called. You MUST call the appropriate tool NOW.",logType:"sheet_hallucination"},{claimPattern:/\b(created\s+(the\s+)?(event|meeting|appointment)|added\s+(to\s+)?(your\s+)?calendar|(calendar\s+)?event\s+(created|added|scheduled))\b/i,requiredTools:["create_calendar_event"],enforcementMsg:"[SYSTEM ENFORCEMENT] You claimed to have created a calendar event but create_calendar_event was never called. You MUST call create_calendar_event NOW.",logType:"calendar_hallucination"},{claimPattern:/\b(moved?\s+to\s+trash|(file|document|doc|spreadsheet)\s+(has\s+been\s+)?(deleted|trashed|moved\s+to\s+trash)|(deleted|trashed)\s+(the|your|that)\s+(file|document|doc|spreadsheet)|i.ve\s+(deleted|trashed)\s+(the|your|that)\s+(file|document|doc))\b/i,requiredTools:["drive_delete_file"],enforcementMsg:"[SYSTEM ENFORCEMENT] You claimed a Drive file was deleted or moved to trash but drive_delete_file was never called. You MUST call drive_delete_file NOW with the file URL or ID.",logType:"drive_delete_hallucination"},{claimPattern:/\b(moved?\s+(the\s+)?(file|document|doc|spreadsheet)\s+to\b|renamed\s+(the\s+)?(file|document|doc)|(file|document|doc)\s+(has\s+been\s+)?(moved|renamed)|i.ve\s+(moved|renamed)\s+(the\s+)?(file|document|doc))\b/i,requiredTools:["drive_organise"],enforcementMsg:"[SYSTEM ENFORCEMENT] You claimed to have moved or renamed a Drive file but drive_organise was never called. You MUST call drive_organise NOW.",logType:"drive_organise_hallucination"},{claimPattern:/\b(created\s+(a\s+)?(new\s+)?(google\s+)?doc(ument)?|doc(ument)?\s+(has\s+been\s+|is\s+)(created|ready|live)|i.ve\s+created\s+(a\s+)?(new\s+)?(google\s+)?doc|(saved|stored)\s+(it\s+|the\s+essay\s+|the\s+article\s+)?(to|in|on)\s+(your\s+)?(google\s+)?drive|essay\s+(is\s+)?(saved|stored)\s+(in|on|to)\s+(drive|google\s+docs?))\b/i,requiredTools:["create_doc"],enforcementMsg:"[SYSTEM ENFORCEMENT] You claimed to have created or saved a Google Document but create_doc was never called. You MUST call create_doc NOW with the full content in the content parameter.",logType:"create_doc_hallucination"},{claimPattern:/\b(appended\s+(to|into)\s+(the\s+|your\s+)?(document|google\s+doc|doc|notes?)|(added|saved)\s+(the\s+)?(content|text|data|note|entry)\s+(to|in)\s+(the\s+|your\s+)?(document|doc|notes?)|(added|saved)\s+(to|in)\s+(your\s+|the\s+)?(quick\s+)?notes?|i.ve\s+(appended|added)\s+(to\s+)?(the\s+|your\s+)?(document|doc|notes?))\b/i,requiredTools:["append_to_doc"],enforcementMsg:"[SYSTEM ENFORCEMENT] You claimed to have appended content to a Google Document but append_to_doc was never called. You MUST call append_to_doc NOW.",logType:"append_doc_hallucination"},{claimPattern:/\b(created\s+(a\s+)?(new\s+)?(google\s+)?(spreadsheet|sheet)|spreadsheet\s+(has\s+been\s+|is\s+)(created|ready|live)|i.ve\s+created\s+(a\s+)?(new\s+)?(google\s+)?(spreadsheet|sheet))\b/i,requiredTools:["create_sheet"],enforcementMsg:"[SYSTEM ENFORCEMENT] You claimed to have created a spreadsheet but create_sheet was never called. You MUST call create_sheet NOW.",logType:"create_sheet_hallucination"},{claimPattern:/\b(drafted\s+(an?\s+)?(email|message)|created\s+(a\s+)?draft(\s+email|\s+message)?|draft\s+(is\s+)?(ready|saved|created)|i.ve\s+drafted\s+(the\s+|an?\s+)?(email|message))\b/i,requiredTools:["gmail_draft"],enforcementMsg:"[SYSTEM ENFORCEMENT] You claimed to have drafted an email but gmail_draft was never called. You MUST call gmail_draft NOW.",logType:"draft_hallucination"},{claimPattern:/\b(archived\s+(the\s+|that\s+)?(email|message|thread)|marked\s+(it\s*|the\s+email\s*|that\s+email\s*)?(as\s+)?(read|unread|starred|important)|i.ve\s+archived\s+(the\s+|that\s+)?(email|message))\b/i,requiredTools:["gmail_modify"],enforcementMsg:"[SYSTEM ENFORCEMENT] You claimed to have archived or marked an email but gmail_modify was never called. You MUST call gmail_modify NOW.",logType:"gmail_modify_hallucination"},{claimPattern:/\b(deleted\s+(the\s+)?(reminder|schedule|task)|removed\s+(the\s+)?(reminder|schedule|task)|(reminder|schedule|task)\s+(has\s+been\s+)?(deleted|removed|cancelled)|i.ve\s+(deleted|removed)\s+(the\s+)?(reminder|task))\b/i,requiredTools:["delete_schedule"],enforcementMsg:"[SYSTEM ENFORCEMENT] You claimed to have deleted a reminder or schedule but delete_schedule was never called. You MUST call delete_schedule NOW.",logType:"delete_schedule_hallucination"},{claimPattern:/\b(disabled\s+(the\s+)?(reminder|schedule|task)|paused\s+(the\s+)?(reminder|task)|turned\s+off\s+(the\s+)?(reminder|schedule|task)|re-?enabled\s+(the\s+)?(reminder|task))\b/i,requiredTools:["toggle_schedule"],enforcementMsg:"[SYSTEM ENFORCEMENT] You claimed to have enabled or disabled a reminder but toggle_schedule was never called. You MUST call toggle_schedule NOW with the job ID and enabled state.",logType:"toggle_schedule_hallucination"},{claimPattern:/\b(deleted?\s+(row|entry|line)\s+\d+|(row|entry)\s+\d+\s+(deleted?|removed?)|(duplicate\s+)?(row|entry)\s+(deleted?|removed?)|i.ve\s+(deleted?|removed?)\s+(the\s+)?(duplicate\s+)?(row|entry))\b/i,requiredTools:["delete_sheet_row"],enforcementMsg:"[SYSTEM ENFORCEMENT] You claimed to have deleted a sheet row but delete_sheet_row was never called. You MUST call delete_sheet_row NOW with the correct row number.",logType:"delete_sheet_row_hallucination"},{claimPattern:/\b(removed?\s+(the\s+)?(duplicate\s+)?(text|entry|line|content)\s+from\s+(the\s+)?(document|doc)|deleted?\s+(the\s+)?(duplicate\s+)?(text|entry|line)\s+from\s+(the\s+)?(document|doc)|i.ve\s+(removed?|deleted?)\s+(the\s+)?(duplicate|text|entry)\s+from\s+(the\s+)?doc)\b/i,requiredTools:["delete_doc_content"],enforcementMsg:"[SYSTEM ENFORCEMENT] You claimed to have deleted content from a Google Document but delete_doc_content was never called. You MUST call delete_doc_content NOW.",logType:"delete_doc_content_hallucination"}];for(const y of le){const h=y.claimPattern.test(C),v=y.requiredTools.some(b=>U.includes(b));if(h&&!v){try{await H(t,s.id,"llm",y.logType,"LLM claimed action without tool call (streaming)",{response:C.substring(0,200)}),A.push({role:"assistant",content:C}),A.push({role:"user",content:y.enforcementMsg});const b=await n.chat(A,{tools:S.filter(E=>y.requiredTools.includes(E.name)),temperature:0});if((m=b.toolCalls)!=null&&m.length){for(const x of b.toolCalls){const D=await At(x.name,x.arguments,t,s.id,{agentType:"full",providerName:n.name,channel:e.channel},s.pin_hash,a==null?void 0:a.GOOGLE_CLIENT_ID,a==null?void 0:a.GOOGLE_CLIENT_SECRET,a==null?void 0:a.GOOGLE_API_KEY,a==null?void 0:a.GOOGLE_CSE_ID,s.timezone,n,a==null?void 0:a.DOCUMENTS_BUCKET,{ai:a==null?void 0:a.AI,vectorize:a==null?void 0:a.VECTORIZE});U.push(x.name),A.push({role:"assistant",content:"",toolCalls:b.toolCalls}),A.push({role:"user",content:D})}const E=await n.chat(A,{tools:[]});E.content&&(C=E.content)}else C="I need to complete that action — could you confirm the details so I can actually do it?"}catch{}break}}U.length>=3&&Promise.race([br(t,n,s,e.text,U,Z,se===0),new Promise(y=>setTimeout(y,6e3))]).catch(()=>{}),L.sessionId&&L.apiKey&&!L.hasActiveTask&&!L.persistSession&&nn(L.sessionId,L.apiKey).catch(()=>{}),yield{type:"done",data:{threadId:o,provider:n.name,tokenCount:M}}}async function bn(e,t,n,s,r,a,i,o){await a.storeMessage(r.id,t.channel,"user",t.text,"{}",o);const l=await At(e.tool,e.args,n,r.id,{agentType:"direct",channel:t.channel},r.pin_hash,i==null?void 0:i.GOOGLE_CLIENT_ID,i==null?void 0:i.GOOGLE_CLIENT_SECRET,i==null?void 0:i.GOOGLE_API_KEY,i==null?void 0:i.GOOGLE_CSE_ID,r.timezone,s,i==null?void 0:i.DOCUMENTS_BUCKET,{ai:i==null?void 0:i.AI,vectorize:i==null?void 0:i.VECTORIZE}),c=`[TOOLS_USED: ${e.tool}] ${l}`.replace(/^\[TOOLS_USED: [^\]]*\]\s*/i,"");return await a.storeMessage(r.id,t.channel,"assistant",c,"{}",o),l}async function jn(e,t,n,s,r,a){var _;const i=new Q(t),o=(_=e.metadata)==null?void 0:_.thread_id,l=await i.buildContext(s.id),c=await i.getRecentConversations(s.id,6,o),d=Rn(e.text,l,Dr(c));if(d.agent==="conversation")return $r(e,t,n,s,l,r,o);const u=Nn(e.text);if(u)return bn(u,e,t,n,s,i,a,o);const p=(await i.getRecentConversations(s.id,10,o)).map(w=>w.content).join(`
`),g=In(e.text,p);if(g)return bn(g,e,t,n,s,i,a,o);const f=d.confidence>=.85;if(e.channel==="telegram"){const w=await Pn(t,s.id);return rs(e,t,n,s,r,a,{maxTurns:10,tools:w,forceToolUseOnFirstTurn:f})}return rs(e,t,n,s,r,a,{forceToolUseOnFirstTurn:f})}async function $r(e,t,n,s,r,a,i){const o=new Q(t),l=Date.now(),c=Ir(s.timezone),d=await Bn(t,s.id),u=d?`## Your Standing Instructions (User-Set — Do Not Duplicate in Memory)
These rules were explicitly configured by the user in Settings. Do not store copies in memory or contradict them.
${d}

${r}`:r,p=yr("conversation",s,u,s.timezone,c,e.channel),g=(await o.getRecentConversations(s.id,30,i)).filter(R=>!R.content.startsWith("[Autonomous Scheduled Task]")&&!R.content.startsWith("[Scheduled Reminder]")),f=Mn(g),_=$n([{role:"system",content:p},...Ln(g),{role:"user",content:e.text}]);Or(_,f),await o.storeMessage(s.id,e.channel,"user",e.text,"{}",i);let w=0,T="";try{const R=await n.chat(_,{temperature:.8});R.usage&&(w=R.usage.promptTokens+R.usage.completionTokens),T=R.content}catch(R){if(a){const C=R.message||"",M=C.includes("401")||C.includes("403")||C.includes("authentication")||C.includes("credit balance"),A=C.includes("429"),U=M?1440:A?10:5;await a.recordError(n.name,C,U)}throw await H(t,s.id,"llm","conversation_error",R.message,{provider:n.name}),R}if(a&&w>0)try{await a.recordUsage(n.name,w)}catch{}try{await t.prepare("INSERT INTO llm_calls (user_id, provider_name, agent_type, tokens_used, latency_ms, success, channel) VALUES (?, ?, ?, ?, ?, ?, ?)").bind(s.id,n.name,"conversation",w,Date.now()-l,1,e.channel).run()}catch{}const S=Ct(T);return await o.storeMessage(s.id,e.channel,"assistant",S,"{}",i),S}async function*is(e,t,n,s,r,a,i,o){yield{type:"tool_start",data:{tool:e.tool,toolArgs:e.args,threadId:o}};const l=await bn(e,t,n,s,r,a,i,o);yield{type:"tool_end",data:{tool:e.tool,toolResult:l.substring(0,500)+(l.length>500?"...":""),threadId:o}};const c=Ct(l),d=50;for(let u=0;u<c.length;u+=d)yield{type:"chunk",data:{text:c.substring(u,u+d),threadId:o}},u+d<c.length&&await new Promise(p=>setTimeout(p,10));yield{type:"done",data:{threadId:o,provider:s.name,tokenCount:0}}}async function*uo(e,t,n,s,r,a){var u;const i=new Q(t),o=(u=e.metadata)==null?void 0:u.thread_id,l=await i.buildContext(s.id),c=await i.getRecentConversations(s.id,6,o),d=Rn(e.text,l,Dr(c));if(yield{type:"thinking",data:{threadId:o,provider:n.name}},d.agent!=="conversation"){const p=Nn(e.text);if(p){yield*is(p,e,t,n,s,i,a,o);return}const g=(await i.getRecentConversations(s.id,10,o)).map(_=>_.content).join(`
`),f=In(e.text,g);if(f){yield*is(f,e,t,n,s,i,a,o);return}yield*co(e,t,n,s,r,a);return}try{const p=await $r(e,t,n,s,l,r,o),g=50;for(let f=0;f<p.length;f+=g)yield{type:"chunk",data:{text:p.substring(f,f+g),threadId:o}},f+g<p.length&&await new Promise(_=>setTimeout(_,10))}catch(p){yield{type:"error",data:{error:p.message||"An error occurred",threadId:o}};return}yield{type:"done",data:{threadId:o,provider:n.name,tokenCount:0}}}const ce=new be;async function mo(e,t){var r;const n=(r=e.req.header("Authorization"))==null?void 0:r.replace("Bearer ","");if(!n)return e.json({error:"Authentication required"},401);const s=await e.env.DB.prepare(`SELECT s.*, u.* FROM sessions s JOIN users u ON s.user_id = u.id 
...
`;for(const c of a)i+=`- ${c.name} (${c.type}, ${Math.round(c.size/1024)}KB, file_id: ${c.file_id})`,c.text_preview&&(i+=`
  Preview: ${c.text_preview.substring(0,300)}...`),i+=`
`;i+="Use parse_document tool to read full file contents. Use drive_upload to upload to Google Drive.]"}let o=r;if(!o){const c=await e.env.DB.prepare("INSERT INTO threads (user_id, title) VALUES (?, ?) RETURNING *").bind(t.id,n.trim().substring(0,60)+(n.trim().length>60?"...":"")).first();o=c==null?void 0:c.id}const l={userId:t.id,username:t.username,channel:s,text:n.trim()+i,sessionId:e.get("sessionId"),timestamp:new Date().toISOString(),metadata:{thread_id:o}};try{const{provider:c,rotation:d}=await Je(e.env.DB,t.id,t.pin_hash),u=new ReadableStream({async start(p){const g=new TextEncoder;try{const f=uo(l,e.env.DB,c,t,d,{GOOGLE_CLIENT_ID:e.env.GOOGLE_CLIENT_ID,GOOGLE_CLIENT_SECRET:e.env.GOOGLE_CLIENT_SECRET,GOOGLE_API_KEY:e.env.GOOGLE_API_KEY,GOOGLE_CSE_ID:e.env.GOOGLE_CSE_ID,DOCUMENTS_BUCKET:e.env.DOCUMENTS_BUCKET});for await(const _ of f)_.data.threadId||(_.data.threadId=o),p.enqueue(g.encode(os(_)));o&&await e.env.DB.prepare("UPDATE threads SET message_count = message_count + 2, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(o).run(),p.close()}catch(f){const _={type:"error",data:{error:f.message||"An error occurred",threadId:o}};p.enqueue(g.encode(os(_))),p.close()}}});return new Response(u,{headers:{"Content-Type":"text/event-stream","Cache-Control":"no-cache",Connection:"keep-alive","X-Thread-Id":String(o||"")}})}catch(c){console.error("Stream setup error:",c);const d=c.message||"";return d.includes("No LLM provider configured")?e.json({error:"No AI provider configured. Please add at least one API key in Settings → Keys.",type:"no_provider",thread_id:o},400):d.includes("429")||d.includes("limit reached")||d.includes("rate limit")||d.includes("Too Many Requests")?e.json({error:"Rate limit reached across all configured providers. Add a second AI provider in Settings → API Keys (Gemini and DeepSeek offer free tiers) to enable automatic failover and avoid this.",type:"rate_limit",thread_id:o},429):e.json({error:"Something went wrong setting up the stream.",details:d,thread_id:o},500)}});ce.get("/threads/:id/messages",async e=>{var a;const t=e.get("user"),n=parseInt(e.req.param("id")),s=parseInt(e.req.query("limit")||"50"),r=await e.env.DB.prepare(`SELECT id, role, content, channel, created_at FROM conversations 
...
`];for(const f of o){let _="";if(f.due_date){const w=f.due_date.slice(0,10);w<c?_=" ⚠️ _overdue_":w===c?_=" 🔴 _due today_":w===u?_=" 🟡 _due tomorrow_":_=` _${new Date(f.due_date).toLocaleDateString("en-GB",{day:"numeric",month:"short"})}_`}p.push(`☐ ${f.title}${_}`)}p.push(`
