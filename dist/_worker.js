...
Note: Always use this date/time as the current time. Do NOT guess or use UTC.`}async function qt(e,t,a){var c;const n=await(await fetch(`https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(`name='${a.replace(/'/g,"\\'")}' and mimeType='application/vnd.google-apps.folder' and trashed=false`)}&fields=files(id,name)`,{headers:{Authorization:`Bearer ${e}`}})).json();let s;((c=n.files)==null?void 0:c.length)>0?s=n.files[0].id:s=(await(await fetch("https://www.googleapis.com/drive/v3/files",{method:"POST",headers:{Authorization:`Bearer ${e}`,"Content-Type":"application/json"},body:JSON.stringify({name:a,mimeType:"application/vnd.google-apps.folder"})})).json()).id;const l=((await(await fetch(`https://www.googleapis.com/drive/v3/files/${t}?fields=parents`,{headers:{Authorization:`Bearer ${e}`}})).json()).parents||[]).join(",");return await fetch(`https://www.googleapis.com/drive/v3/files/${t}?addParents=${s}&removeParents=${l}`,{method:"PATCH",headers:{Authorization:`Bearer ${e}`,"Content-Type":"application/json"},body:JSON.stringify({})}),{folderId:s,folderName:a}}function yt(e){try{const t=new Date;return new Intl.DateTimeFormat("en-US",{timeZone:e,weekday:"long",year:"numeric",month:"long",day:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:!0}).format(t)}catch{return new Date().toISOString()}}function dn(e){const t=e.replace(/(\d+)\s*[.,;!]+\s*(minutes?|mins?|hours?|hrs?|h|days?|seconds?|secs?)/gi,"$1 $2"),a=t.toLowerCase().trim(),r=a.match(/\bin\s+(\d+)\s*(minutes?|mins?|hours?|hrs?|h|days?)\b/i);if(r){let o=parseInt(r[1],10);const i=r[2].toLowerCase();i.startsWith("h")&&(o*=60),i.startsWith("d")&&(o*=1440);const l=t.match(/(?:to|about|that)\s+(.+?)\.?$/i);let c;l?c=l[1].trim():c=t.replace(/^(remind|alert|notify|tell|ping|nudge|buzz)\s*(me)?\s*/i,"").replace(/in\s+\d+\s*\.?\s*(minutes?|mins?|hours?|hrs?|h|days?)\s*/i,"").replace(/^[,.\s]+|[,.\s]+$/g,"").trim()||"Reminder";const u=c.length>50?c.substring(0,47)+"...":c;return{args:{name:u.charAt(0).toUpperCase()+u.slice(1),description:c,schedule_type:"once",minutes_from_now:o,action_type:"reminder",action_description:c,schedule_value:""}}}const n=a.match(/(?:at|by)\s+(\d{1,2})[:.]?(\d{2})?\s*(am|pm|a\.m\.|p\.m\.)?/i);if(n){let o=parseInt(n[1],10);const i=n[2]?parseInt(n[2],10):0,l=(n[3]||"").replace(/\./g,"").toLowerCase();l==="pm"&&o<12&&(o+=12),l==="am"&&o===12&&(o=0);const c=t.match(/(?:to|about|that)\s+(.+?)\.?$/i);let u;c?u=c[1].trim():u=t.replace(/^(remind|alert|notify|tell|ping|nudge|buzz)\s*(me)?\s*/i,"").replace(/(?:at|by)\s+\d{1,2}[:.]?\d{0,2}\s*(am|pm|a\.m\.|p\.m\.)?\s*/i,"").replace(/^[,.\s]+|[,.\s]+$/g,"").trim()||"Reminder";const m=u.length>50?u.substring(0,47)+"...":u,f=new Date,v=f.getFullYear(),y=String(f.getMonth()+1).padStart(2,"0"),x=String(f.getDate()).padStart(2,"0"),w=`${v}-${y}-${x} ${String(o).padStart(2,"0")}:${String(i).padStart(2,"0")}`;return{args:{name:m.charAt(0).toUpperCase()+m.slice(1),description:u,schedule_type:"once",schedule_value:w,action_type:"reminder",action_description:u}}}const s=a.match(/^(remind|alert|notify)\s+me\s+(?:to|about|that)\s+(.+?)\.?$/i);if(s){const o=s[2].trim(),i=o.length>50?o.substring(0,47)+"...":o;return{args:{name:i.charAt(0).toUpperCase()+i.slice(1),description:o,schedule_type:"once",minutes_from_now:5,action_type:"reminder",action_description:o,schedule_value:""}}}return null}async function Ne(e,t,a,r,n,s,o,i,l,c,u,m){const f=Date.now();let v=!0,y="",x="";try{return x=await un(e,t,a,r,s,o,i,l,c,u,m),x}catch(w){throw v=!1,y=w.message||"Unknown error",w}finally{const w=Date.now()-f;try{await a.prepare(`INSERT INTO tool_execution_log (user_id, agent_type, provider_name, tool_name, tool_args, tool_result, success, error_message, latency_ms, was_enforcement_retry, channel)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(r,n.agentType||null,n.providerName||null,e,JSON.stringify(t).substring(0,2e3),(v?x:"").substring(0,500),v?1:0,y||null,w,n.isEnforcementRetry?1:0,n.channel||"web").run()}catch{}}}async function un(e,t,a,r,n,s,o,i,l,c,u){var f,v,y,x,w,E;const m=new J(a);switch(e){case"create_schedule":{const d=new Date;let g;const p=c||"UTC";if(t.minutes_from_now&&typeof t.minutes_from_now=="number"&&t.minutes_from_now>0){g=new Date(d.getTime()+t.minutes_from_now*60*1e3);const D=g.toLocaleString("en-US",{timeZone:p,hour12:!1,year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"}).split(", "),[k,I,O]=(D[0]||"").split("/");t.schedule_value=`${O}-${k}-${I} ${D[1]||"00:00"}`,t.schedule_type="once"}else if(t.schedule_type==="interval"){const T=parseInt(t.schedule_value,10);g=new Date(d.getTime()+T*60*1e3)}else if(t.schedule_type==="daily"){const[T,D]=t.schedule_value.split(":").map(Number),k=d.toLocaleString("en-US",{timeZone:p}),I=new Date(k),O=new Date(I);O.setHours(T,D,0,0),O<=I&&O.setDate(O.getDate()+1);const L=new Date(O.toLocaleString("en-US",{timeZone:"UTC"})),$=new Date(O.toLocaleString("en-US",{timeZone:p})),z=L.getTime()-$.getTime();g=new Date(O.getTime()+z)}else if(t.schedule_type==="weekly"){const[T,D]=t.schedule_value.split(" "),[k,I]=(D||"00:00").split(":").map(Number),L=["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"].findIndex(bt=>bt.toLowerCase()===T.toLowerCase()),$=d.toLocaleString("en-US",{timeZone:p}),z=new Date($),K=new Date(z);K.setHours(k,I,0,0);let W=(L-K.getDay()+7)%7;W===0&&K<=z&&(W=7),K.setDate(K.getDate()+W);const Oe=new Date(K.toLocaleString("en-US",{timeZone:"UTC"})),ue=new Date(K.toLocaleString("en-US",{timeZone:p})),vt=Oe.getTime()-ue.getTime();g=new Date(K.getTime()+vt)}else if(t.schedule_type==="once"){const[T,D]=t.schedule_value.split(" "),[k,I,O]=T.split("-").map(Number),[L,$]=(D||"00:00").split(":").map(Number),z=d.toLocaleString("en-US",{timeZone:p}),K=new Date(z),W=new Date(K);W.setFullYear(k,I-1,O),W.setHours(L,$,0,0);const Oe=new Date(W.toLocaleString("en-US",{timeZone:"UTC"})),ue=new Date(W.toLocaleString("en-US",{timeZone:p})),vt=Oe.getTime()-ue.getTime();g=new Date(W.getTime()+vt);const bt=new Date(d.getTime()+120*1e3);if(g.getTime()<d.getTime()+60*1e3){const Qa=g.toISOString();g=bt;const er=` [Note: The requested time ${t.schedule_value} in ${p} resolved to ${Qa} UTC which is in the past. Auto-adjusted to fire in ~2 minutes at ${g.toISOString()}.]`;t._pastTimeWarning=er}}else g=new Date(d.getTime()+3600*1e3);await a.prepare(`INSERT INTO cron_jobs (user_id, name, description, schedule_type, schedule_value, action_type, action_config, next_run, state)
...
- Unread errors: ${(_==null?void 0:_.cnt)||0}`}case"read_sheet":{if(!n)return"Authentication context unavailable.";try{const d=new te(a,r,n,s||"",o||""),g=t.spreadsheet_id;let p=t.range;const h=await d.sheets.getMetadata(g),_=h.sheets;p.includes("!")||(p=`${_[0]}!${p}`);let T;try{T=await d.sheets.readRange(g,p)}catch(k){if((f=k.message)!=null&&f.includes("Unable to parse range")||(v=k.message)!=null&&v.includes("400")){const I=p.includes("!")?p.split("!")[1]:p;p=`${_[0]}!${I}`,T=await d.sheets.readRange(g,p)}else throw k}let D=`[Spreadsheet: "${h.title}" | Reading tab: "${p.split("!")[0]}" | All tabs in this spreadsheet: ${_.map(k=>`"${k}"`).join(", ")}]
`;return _.length>1&&(D+=`[To read a different tab, call read_sheet again with range like "${_[1]}!A1:Z500"]
`),T.length===0?D+"No data found in the specified range.":D+T.map(k=>k.join("	| ")).join(`
...
ID: ${p.spreadsheetId}
URL: ${p.url}`}catch(d){return await R(a,r,"google","create_sheet",d.message),`Failed to create spreadsheet: ${d.message}`}}case"list_calendar_events":{if(!n)return"Authentication context unavailable.";try{const d=new te(a,r,n,s||"",o||""),g=t.calendar_id||"primary",p=t.days_ahead||7,h=new Date,_=new Date(h.getTime()+p*24*60*60*1e3),T=await d.calendar.listEvents(g,{timeMin:h.toISOString(),timeMax:_.toISOString(),query:t.query});return T.length===0?`No events found in the next ${p} days.`:T.map(D=>{var $;const k=D.start.dateTime||D.start.date||"TBD",I=D.end.dateTime||D.end.date||"",O=D.location?` 📍 ${D.location}`:"",L=(($=D.attendees)==null?void 0:$.map(z=>z.email).join(", "))||"";return`• ${D.summary} — ${k} to ${I}${O}${L?`
  Attendees: ${L}`:""}`}).join(`
...
ID: ${p.documentId}
URL: ${p.url}`}catch(d){return await R(a,r,"google","create_doc",d.message),`Failed to create document: ${d.message}`}}case"read_doc":{if(!n)return"Authentication context unavailable.";try{const g=await new te(a,r,n,s||"",o||"").docs.readDocument(t.document_id);return`Document: "${g.title}"

${g.content}`}catch(d){return await R(a,r,"google","read_doc",d.message),`Failed to read document: ${d.message}`}}case"append_to_doc":{if(!n)return"Authentication context unavailable.";try{const d=new te(a,r,n,s||"",o||"");if(!(await d.isConnected()).connected)return'Google account not connected. Please go to Settings → Keys → Google Workspace and click "Connect Google Account" first.';await d.docs.appendText(t.document_id,t.content);let p=t.document_id;try{p=(await d.docs.readDocument(t.document_id)).title}catch{}return`Content appended to "${p}".
...
`,_+=h.sources.map((T,D)=>`[${D+1}] ${T.title}
    ${T.url}`).join(`
`)),_}catch(d){return await R(a,r,"research","research",d.message),`Research error: ${d.message}`}}case"search_places":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured. Add it in Settings → Keys → Google API Key.";const g=await M(d.encrypted_value,n),p=await $a(g,t.query,{type:t.type});return p.error?`Places search failed: ${p.error}`:p.results.length===0?`No places found for "${t.query}".`:p.results.map((h,_)=>{const T=h.rating?` ★${h.rating} (${h.userRatingsTotal||0} reviews)`:"",D=h.openNow!==void 0?h.openNow?" · Open now":" · Closed":"",k=h.googleMapsUri?`
   ${h.googleMapsUri}`:"";return`${_+1}. **${h.name}**${T}${D}
   ${h.address}${k}
...
`)}catch(d){return await R(a,r,"google_api","geocode",d.message),`Geocoding error: ${d.message}`}}default:return`Unknown tool: ${e}`}}async function Kt(e,t,a,r,n,s){var E;const o=new J(t),i=(E=e.metadata)==null?void 0:E.thread_id,l=await o.buildContext(r.id),c=await o.getRecentConversations(r.id,25,i),m=[{role:"system",content:Ya(r,l)},...c.map(d=>({role:d.role,content:d.content})),{role:"user",content:e.text}];await o.storeMessage(r.id,e.channel,"user",e.text,"{}",i);const f=10;let v="",y=0;const x=[];for(let d=0;d<f;d++)try{const g=await a.chat(m,{tools:ft});if(g.usage&&(y+=g.usage.promptTokens+g.usage.completionTokens),g.toolCalls&&g.toolCalls.length>0){g.content&&m.push({role:"assistant",content:g.content});for(const p of g.toolCalls){x.push(p.name);try{const h=await Ne(p.name,p.arguments,t,r.id,{agentType:"full",providerName:a.name,channel:e.channel},r.pin_hash,s==null?void 0:s.GOOGLE_CLIENT_ID,s==null?void 0:s.GOOGLE_CLIENT_SECRET,s==null?void 0:s.GOOGLE_API_KEY,s==null?void 0:s.GOOGLE_CSE_ID,r.timezone,a);m.push({role:"user",content:`[Tool Result for ${p.name}]: ${h}`})}catch(h){await R(t,r.id,"tool",p.name,h.message||"Tool execution failed"),m.push({role:"user",content:`[Tool Error for ${p.name}]: ${h.message||"Execution failed"}`})}}continue}v=g.content;break}catch(g){if(n){const p=g.message||"",h=p.includes("401")||p.includes("403")||p.includes("authentication")||p.includes("credit balance"),_=p.includes("429"),T=h?1440:_?10:5;await n.recordError(a.name,p,T)}throw await R(t,r.id,"llm","provider_error",g.message||"Unknown LLM error",{provider:a.name,turn:d}),g}if(n&&y>0)try{await n.recordUsage(a.name,y)}catch{}const w=x.length>0?`[TOOLS_USED: ${[...new Set(x)].join(", ")}] `:"";return await o.storeMessage(r.id,e.channel,"assistant",w+v,"{}",i),await o.compactHistory(r.id,30),v}const Yt={"claude-sonnet-4-20250514":2e5,"claude-haiku-4-20250514":2e5,"gpt-4o":128e3,"gpt-4o-mini":128e3,"grok-3-mini":131072,"grok-3":131072,"deepseek-chat":64e3,"gemini-2.0-flash":1e6,default:32e3};function pn(e){for(const[t,a]of Object.entries(Yt))if(e.toLowerCase().includes(t.toLowerCase()))return a;return Yt.default}function mn(e,t,a,r){const n=pn(r),s=Math.floor(n*.75),o=[];let i=0,l=!1;const c=kt(e);o.push({role:"system",content:e}),i+=c;const u=kt(a);i+=u;const m=s-i,f=[];let v=0;for(let y=t.length-1;y>=0;y--){const x=t[y],w=kt(x.content);if(v+w<=m)f.unshift({role:x.role,content:x.content}),v+=w;else{l=!0;break}}return o.push(...f),o.push({role:"user",content:a}),i+=v,{maxTokens:n,usedTokens:i,messages:o,wasTruncated:l}}async function*hn(e,t,a,r,n,s){var w;const o=new J(t),i=(w=e.metadata)==null?void 0:w.thread_id;yield{type:"thinking",data:{threadId:i,provider:a.name}};const l=await o.buildContext(r.id),c=await o.getRecentConversations(r.id,20,i),u=Ya(r,l),m=mn(u,c,e.text,a.name);await o.storeMessage(r.id,e.channel,"user",e.text,"{}",i);const f=10;let v="",y=0;const x=[...m.messages];for(let E=0;E<f;E++)try{E>0&&(yield{type:"thinking",data:{threadId:i}});const d=await a.chat(x,{tools:ft});if(d.usage&&(y+=d.usage.promptTokens+d.usage.completionTokens),d.toolCalls&&d.toolCalls.length>0){d.content&&(yield{type:"chunk",data:{text:d.content,threadId:i}},x.push({role:"assistant",content:d.content}));for(const p of d.toolCalls){yield{type:"tool_start",data:{tool:p.name,toolArgs:p.arguments,threadId:i}};try{const h=await Ne(p.name,p.arguments,t,r.id,{agentType:"full",providerName:a.name,channel:e.channel},r.pin_hash,s==null?void 0:s.GOOGLE_CLIENT_ID,s==null?void 0:s.GOOGLE_CLIENT_SECRET,s==null?void 0:s.GOOGLE_API_KEY,s==null?void 0:s.GOOGLE_CSE_ID,r.timezone,a);yield{type:"tool_end",data:{tool:p.name,toolResult:h.substring(0,500)+(h.length>500?"...":""),threadId:i}},x.push({role:"user",content:`[Tool Result for ${p.name}]: ${h}`})}catch(h){await R(t,r.id,"tool",p.name,h.message||"Tool execution failed"),yield{type:"tool_end",data:{tool:p.name,toolResult:`Error: ${h.message||"Execution failed"}`,threadId:i}},x.push({role:"user",content:`[Tool Error for ${p.name}]: ${h.message||"Execution failed"}`})}}continue}v=d.content;const g=50;for(let p=0;p<v.length;p+=g)yield{type:"chunk",data:{text:v.substring(p,p+g),threadId:i}},p+g<v.length&&await new Promise(_=>setTimeout(_,10));break}catch(d){if(n){const g=d.message||"",p=g.includes("401")||g.includes("403")||g.includes("authentication")||g.includes("credit balance"),h=g.includes("429"),_=p?1440:h?10:5;await n.recordError(a.name,g,_)}await R(t,r.id,"llm","provider_error",d.message||"Unknown LLM error",{provider:a.name,turn:E}),yield{type:"error",data:{error:d.message||"An error occurred",threadId:i}};return}if(n&&y>0)try{await n.recordUsage(a.name,y)}catch{}await o.storeMessage(r.id,e.channel,"assistant",v,"{}",i),await o.compactHistory(r.id,30),yield{type:"done",data:{threadId:i,provider:a.name,tokenCount:y}}}async function Bt(e,t,a,r,n,s){var u;const o=new J(t),i=(u=e.metadata)==null?void 0:u.thread_id,l=await o.buildContext(r.id),c=Wa(e.text,l);if(c.agent==="conversation")return fn(e,t,a,r,l,n,i);if(c.agent==="multi"||c.confidence<.5)return Kt(e,t,a,r,n,s);try{return await gn(c.agent,e,t,a,r,l,n,s,i)}catch(m){return await R(t,r.id,"router","subagent_fallback",`${c.agent} failed: ${m.message}`,{route:c}),Kt(e,t,a,r,n,s)}}async function gn(e,t,a,r,n,s,o,i,l){const c=new J(a),u=yt(n.timezone),m=ut(e,n,s,n.timezone,u),f=qa(e,ft),v=(await c.getRecentConversations(n.id,25,l)).map(k=>({...k,content:k.content.replace(/^\[TOOLS_USED: [^\]]+\]\s*/i,"")})),y=[{role:"system",content:m},...v.map(k=>({role:k.role,content:k.content})),{role:"user",content:t.text}];await c.storeMessage(n.id,t.channel,"user",t.text,"{}",l);const x=10;let w="",E=0,d="",g=!1;const p=[];for(let k=0;k<x;k++)try{const I=await r.chat(y,{tools:f.length>0?f:void 0});if(I.usage&&(E+=I.usage.promptTokens+I.usage.completionTokens),I.toolCalls&&I.toolCalls.length>0){g=!0,I.content&&(d=I.content,y.push({role:"assistant",content:I.content}));for(const O of I.toolCalls){p.push(O.name);try{const L=await Ne(O.name,O.arguments,a,n.id,{agentType:e,providerName:r.name,channel:t.channel},n.pin_hash,i==null?void 0:i.GOOGLE_CLIENT_ID,i==null?void 0:i.GOOGLE_CLIENT_SECRET,i==null?void 0:i.GOOGLE_API_KEY,i==null?void 0:i.GOOGLE_CSE_ID,n.timezone,r);y.push({role:"user",content:`[Tool Result for ${O.name}]: ${L}`})}catch(L){await R(a,n.id,"tool",O.name,L.message||"Tool execution failed"),y.push({role:"user",content:`[Tool Error for ${O.name}]: ${L.message||"Execution failed"}`})}}continue}w=I.content,!w&&d&&(w=d);break}catch(I){if(o){const O=I.message||"",L=O.includes("401")||O.includes("403")||O.includes("authentication")||O.includes("credit balance"),$=O.includes("429"),z=L?1440:$?10:5;await o.recordError(r.name,O,z)}throw await R(a,n.id,"llm","subagent_error",I.message||"Unknown error",{agent:e,provider:r.name,turn:k}),I}const h=["scheduler","workspace","research"],_=e==="scheduler"&&!p.includes("create_schedule")&&!p.includes("list_schedules")&&!p.includes("toggle_schedule")&&!p.includes("update_schedule_state")&&!p.includes("delete_schedule")&&/\b(remind|in\s+\d+|at\s+\d{1,2}[:.]\d{0,2}|timer|alarm|schedule|tell\s+me\s+in|notify|alert|ping)\b/i.test(t.text);if(!g&&h.includes(e)&&f.length>0||_){try{await a.prepare(`INSERT INTO tool_execution_log (user_id, agent_type, provider_name, tool_name, tool_args, tool_result, success, error_message, latency_ms, was_enforcement_retry, channel)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(n.id,e,r.name,"__enforcement_trigger",JSON.stringify({userMessage:t.text.substring(0,200)}),w.substring(0,200),0,"LLM narrated without calling tools",0,0,t.channel||"web").run()}catch{}try{y.push({role:"assistant",content:w}),y.push({role:"user",content:e==="scheduler"&&_?`[SYSTEM OVERRIDE] You called ${p.join(", ")||"no tools"} but did NOT call create_schedule. The user wants a reminder or schedule created. Call create_schedule NOW with the correct parameters. Do NOT respond with text saying a reminder is set — actually create it.`:`[SYSTEM OVERRIDE] You responded with text but did NOT call any tool. This is a ${e} request — you MUST use your tools. Do NOT repeat your text response. Call the appropriate tool NOW (e.g., ${f.slice(0,3).map(I=>I.name).join(", ")}). The user is waiting for an actual action, not a description of what you would do.`});const k=await r.chat(y,{tools:f});if(k.usage&&(E+=k.usage.promptTokens+k.usage.completionTokens),k.toolCalls&&k.toolCalls.length>0){g=!0;for(const O of k.toolCalls){p.push(O.name);try{const L=await Ne(O.name,O.arguments,a,n.id,{agentType:e,providerName:r.name,channel:t.channel,isEnforcementRetry:!0},n.pin_hash,i==null?void 0:i.GOOGLE_CLIENT_ID,i==null?void 0:i.GOOGLE_CLIENT_SECRET,i==null?void 0:i.GOOGLE_API_KEY,i==null?void 0:i.GOOGLE_CSE_ID,n.timezone,r);y.push({role:"user",content:`[Tool Result for ${O.name}]: ${L}`})}catch(L){y.push({role:"user",content:`[Tool Error for ${O.name}]: ${L.message||"Execution failed"}`})}}const I=await r.chat(y,{tools:f});I.content&&(w=I.content),I.usage&&(E+=I.usage.promptTokens+I.usage.completionTokens)}if(!p.includes("create_schedule")&&e==="scheduler")try{const I=dn(t.text);if(I){const O=await Ne("create_schedule",I.args,a,n.id,{agentType:"scheduler",providerName:"programmatic_fallback",channel:t.channel,isEnforcementRetry:!0},n.pin_hash,i==null?void 0:i.GOOGLE_CLIENT_ID,i==null?void 0:i.GOOGLE_CLIENT_SECRET,i==null?void 0:i.GOOGLE_API_KEY,i==null?void 0:i.GOOGLE_CSE_ID,n.timezone,r);g=!0,p.push("create_schedule"),w=O}}catch(I){await R(a,n.id,"tool_enforcement","scheduler_fallback",`Programmatic scheduler fallback failed: ${I.message||"Unknown"}`)}}catch(k){await R(a,n.id,"tool_enforcement",e,`Tool enforcement retry failed: ${k.message||"Unknown"}`,{originalResponse:w.substring(0,200)})}}if(o&&E>0)try{await o.recordUsage(r.name,E)}catch{}const D=p.length>0?`[TOOLS_USED: ${[...new Set(p)].join(", ")}] `:"";return await c.storeMessage(n.id,t.channel,"assistant",D+w,"{}",l),await c.compactHistory(n.id,30),w}async function fn(e,t,a,r,n,s,o){const i=new J(t),l=yt(r.timezone),c=ut("conversation",r,n,r.timezone,l),u=await i.getRecentConversations(r.id,25,o),m=[{role:"system",content:c},...u.map(y=>({role:y.role,content:y.content})),{role:"user",content:e.text}];await i.storeMessage(r.id,e.channel,"user",e.text,"{}",o);let f=0,v="";try{const y=await a.chat(m,{temperature:.8});y.usage&&(f=y.usage.promptTokens+y.usage.completionTokens),v=y.content}catch(y){if(s){const x=y.message||"",w=x.includes("401")||x.includes("403")||x.includes("authentication")||x.includes("credit balance"),E=x.includes("429"),d=w?1440:E?10:5;await s.recordError(a.name,x,d)}throw await R(t,r.id,"llm","conversation_error",y.message,{provider:a.name}),y}if(s&&f>0)try{await s.recordUsage(a.name,f)}catch{}return await i.storeMessage(r.id,e.channel,"assistant",v,"{}",o),await i.compactHistory(r.id,30),v}async function*yn(e,t,a,r,n,s){var d;const o=new J(t),i=(d=e.metadata)==null?void 0:d.thread_id,l=await o.buildContext(r.id),c=Wa(e.text,l);if(yield{type:"thinking",data:{threadId:i,provider:a.name}},c.agent==="multi"||c.confidence<.5){yield*hn(e,t,a,r,n,s);return}const u=yt(r.timezone),m=c.agent==="conversation"?ut("conversation",r,l,r.timezone,u):ut(c.agent,r,l,r.timezone,u),f=qa(c.agent,ft),v=await o.getRecentConversations(r.id,25,i),y=[{role:"system",content:m},...v.map(g=>({role:g.role,content:g.content})),{role:"user",content:e.text}];await o.storeMessage(r.id,e.channel,"user",e.text,"{}",i);const x=10;let w="",E=0;for(let g=0;g<x;g++)try{g>0&&(yield{type:"thinking",data:{threadId:i}});const p=await a.chat(y,{tools:f.length>0?f:void 0});if(p.usage&&(E+=p.usage.promptTokens+p.usage.completionTokens),p.toolCalls&&p.toolCalls.length>0){p.content&&(yield{type:"chunk",data:{text:p.content,threadId:i}},y.push({role:"assistant",content:p.content}));for(const _ of p.toolCalls){yield{type:"tool_start",data:{tool:_.name,toolArgs:_.arguments,threadId:i}};try{const T=await Ne(_.name,_.arguments,t,r.id,{agentType:c.agent,providerName:a.name,channel:e.channel},r.pin_hash,s==null?void 0:s.GOOGLE_CLIENT_ID,s==null?void 0:s.GOOGLE_CLIENT_SECRET,s==null?void 0:s.GOOGLE_API_KEY,s==null?void 0:s.GOOGLE_CSE_ID,r.timezone,a);yield{type:"tool_end",data:{tool:_.name,toolResult:T.substring(0,500)+(T.length>500?"...":""),threadId:i}},y.push({role:"user",content:`[Tool Result for ${_.name}]: ${T}`})}catch(T){await R(t,r.id,"tool",_.name,T.message||"Tool execution failed"),yield{type:"tool_end",data:{tool:_.name,toolResult:`Error: ${T.message||"Execution failed"}`,threadId:i}},y.push({role:"user",content:`[Tool Error for ${_.name}]: ${T.message||"Execution failed"}`})}}continue}w=p.content;const h=50;for(let _=0;_<w.length;_+=h)yield{type:"chunk",data:{text:w.substring(_,_+h),threadId:i}},_+h<w.length&&await new Promise(D=>setTimeout(D,10));break}catch(p){if(n){const h=p.message||"",_=h.includes("401")||h.includes("403")||h.includes("authentication")||h.includes("credit balance"),T=h.includes("429"),D=_?1440:T?10:5;await n.recordError(a.name,h,D)}await R(t,r.id,"llm","subagent_stream_error",p.message||"Unknown error",{agent:c.agent,provider:a.name,turn:g}),yield{type:"error",data:{error:p.message||"An error occurred",threadId:i}};return}if(n&&E>0)try{await n.recordUsage(a.name,E)}catch{}await o.storeMessage(r.id,e.channel,"assistant",w,"{}",i),await o.compactHistory(r.id,30),yield{type:"done",data:{threadId:i,provider:a.name,tokenCount:E}}}const B=new ve;async function vn(e,t){var n;const a=(n=e.req.header("Authorization"))==null?void 0:n.replace("Bearer ","");if(!a)return e.json({error:"Authentication required"},401);const r=await e.env.DB.prepare(`SELECT s.*, u.* FROM sessions s JOIN users u ON s.user_id = u.id 
...
Go to the web app → Settings → Profile → set your Telegram Chat ID to this value.`),e.json({ok:!0});if(i.voice&&w&&x)try{await G(w,v,"🎤 Processing voice note...");const _=await(await fetch(`https://api.telegram.org/bot${w}/getFile?file_id=${i.voice.file_id}`)).json();if(_.ok&&((t=_.result)!=null&&t.file_path)){const D=await(await fetch(`https://api.telegram.org/file/bot${w}/${_.result.file_path}`)).blob();let k="",I="",O="whisper-1";const L=await e.env.DB.prepare("SELECT service, encrypted_value FROM credentials WHERE user_id = ? AND (service IN ('llm_slot_1', 'llm_slot_2', 'llm_slot_3', 'openai'))").bind(x.id).all();for(const W of L.results){const Oe=await M(W.encrypted_value,x.pin_hash);if(W.service==="openai"){k="https://api.openai.com/v1/audio/transcriptions",I=Oe;break}else if(W.service.startsWith("llm_slot_"))try{const ue=JSON.parse(Oe);if(ue.provider==="openai"){k="https://api.openai.com/v1/audio/transcriptions",I=ue.apiKey;break}else if(ue.provider==="groq"){k="https://api.groq.com/openai/v1/audio/transcriptions",I=ue.apiKey,O="whisper-large-v3";break}}catch{}}if(!k)return await G(w,v,"⚠️ To use voice notes, configure an OpenAI API key in your LLM slots (Settings → Keys)."),e.json({ok:!0});const $=new FormData;$.append("file",D,"voice.ogg"),$.append("model",O),$.append("language","en");const z=await fetch(k,{method:"POST",headers:{Authorization:`Bearer ${I}`},body:$});if(!z.ok){const W=await z.text();return await G(w,v,`⚠️ Transcription failed: ${z.status} ${W}`),e.json({ok:!0})}y=(await z.json()).text,await G(w,v,`🗣️ *You said:* ${y}`)}}catch(h){return await G(w,v,`⚠️ Failed to process voice note: ${h.message}`),e.json({ok:!0})}if((u||m)&&w&&x)try{let h,_="unknown",T="unknown",D=0;if(u)h=i.document.file_id,_=i.document.file_name||"document",T=i.document.mime_type||"unknown",D=i.document.file_size||0;else if(m){const k=i.photo[i.photo.length-1];h=k.file_id,_="photo.jpg",T="image/jpeg",D=k.file_size||0}if(h){const I=await(await fetch(`https://api.telegram.org/bot${w}/getFile?file_id=${h}`)).json();let O="";if(I.ok&&((a=I.result)!=null&&a.file_path)&&(/\.(txt|csv|json|md|xml|html|log|yaml|yml|tsv|ini|cfg|conf|py|js|ts|sh|sql)$/i.test(_)||/^text\/|application\/json|application\/xml|application\/csv/i.test(T))&&D<5e4)try{O=await(await fetch(`https://api.telegram.org/file/bot${w}/${I.result.file_path}`)).text()}catch{}const L=i.caption||"",$=`[Telegram file received: "${_}" (${T}, ${Math.round(D/1024)}KB)]`;O?y=`${L?L+`

`:""}${$}
File contents:
${O.substring(0,8e3)}${O.length>8e3?`
[...truncated]`:""}`:y=`${L?L+`
...
${a}`,parse_mode:"Markdown",reply_markup:{inline_keyboard:r.map(i=>i.map(l=>({...l,callback_data:`${l.callback_data}:${n}`})))}})}),await e.prepare("UPDATE briefings SET delivered_telegram = 1 WHERE id = ?").bind(n).run()}catch(s){console.error("Telegram briefing error:",s.message)}}V.delete("/briefings/:id",Xa,async e=>{const t=e.get("user").id,a=e.req.param("id");return await e.env.DB.prepare("DELETE FROM briefings WHERE id = ? AND user_id = ?").bind(a,t).run(),e.json({success:!0})});const ee=new ve;ee.use("/api/*",Br());ee.route("/api/auth",be);ee.route("/api/chat",B);ee.route("/api/settings",j);ee.route("/api/system",de);ee.route("/api/telegram",rt);ee.route("/api/proactive",V);ee.get("/auth/google/callback",async e=>{const t=new URL(e.req.url),a=t.searchParams.get("code"),r=t.searchParams.get("state"),n=t.searchParams.get("error");if(n)return e.html(Ce(!1,`Google denied access: ${n}`));if(!a||!r)return e.html(Ce(!1,"Missing authorization code or state parameter."));try{const o=JSON.parse(atob(r)).sessionId;if(!o)return e.html(Ce(!1,"Invalid state parameter — missing session."));const i=await e.env.DB.prepare(`SELECT s.*, u.* FROM sessions s JOIN users u ON s.user_id = u.id 
       WHERE s.id = ? AND s.expires_at > datetime('now')`).bind(o).first();if(!i)return e.html(Ce(!1,"Session expired. Please log in again and retry."));const l=i.user_id,c=i.pin_hash,u=`${t.protocol}//${t.host}/auth/google/callback`,m=await Ra(e.env.DB,l,c,a,u,e.env.GOOGLE_CLIENT_ID,e.env.GOOGLE_CLIENT_SECRET);return e.html(Ce(!0,`Connected as ${m.email}`,m.email))}catch(s){return e.html(Ce(!1,`OAuth failed: ${s.message}`))}});ee.get("/",e=>(e.header("Cache-Control","no-cache, no-store, must-revalidate"),e.header("Pragma","no-cache"),e.header("Expires","0"),e.html(ba())));ee.get("*",e=>e.req.path.startsWith("/api/")?e.json({error:"Not found"},404):(e.header("Cache-Control","no-cache, no-store, must-revalidate"),e.header("Pragma","no-cache"),e.header("Expires","0"),e.html(ba())));function Ce(e,t,a){return`<!DOCTYPE html>
