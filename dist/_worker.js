...
`),t.trim()}async function Ha(e,t,a={}){const r=a.maxPages||(a.depth==="thorough"?5:3),n=a.maxResults||(a.depth==="thorough"?8:5),s=await mt(e,{num:n,site:a.site});if(s.error)return{report:"",sources:[],pagesRead:0,error:`Search failed: ${s.error}`};if(s.results.length===0)return{report:`No web results found for "${e}".`,sources:[],pagesRead:0};const i=s.results.slice(0,r).map(async g=>{const v=await Ga(g.link);return{title:g.title,url:g.link,displayLink:g.displayLink,snippet:g.snippet,content:v.text,error:v.error}}),c=(await Promise.all(i)).filter(g=>g.content.length>50);if(c.length===0){const g=s.results.map((y,x)=>`[${x+1}] ${y.title}
${y.snippet}
Source: ${y.link}`).join(`

`);return{report:await zt(e,g,t,"snippets"),sources:s.results.map(y=>({title:y.title,url:y.link})),pagesRead:0}}const u=c.map((g,v)=>`--- SOURCE ${v+1}: ${g.title} (${g.displayLink}) ---
${g.content}
--- END SOURCE ${v+1} ---`).join(`
...
Note: Always use this date/time as the current time. Do NOT guess or use UTC.`}async function Wt(e,t,a){var c;const n=await(await fetch(`https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(`name='${a.replace(/'/g,"\\'")}' and mimeType='application/vnd.google-apps.folder' and trashed=false`)}&fields=files(id,name)`,{headers:{Authorization:`Bearer ${e}`}})).json();let s;((c=n.files)==null?void 0:c.length)>0?s=n.files[0].id:s=(await(await fetch("https://www.googleapis.com/drive/v3/files",{method:"POST",headers:{Authorization:`Bearer ${e}`,"Content-Type":"application/json"},body:JSON.stringify({name:a,mimeType:"application/vnd.google-apps.folder"})})).json()).id;const l=((await(await fetch(`https://www.googleapis.com/drive/v3/files/${t}?fields=parents`,{headers:{Authorization:`Bearer ${e}`}})).json()).parents||[]).join(",");return await fetch(`https://www.googleapis.com/drive/v3/files/${t}?addParents=${s}&removeParents=${l}`,{method:"PATCH",headers:{Authorization:`Bearer ${e}`,"Content-Type":"application/json"},body:JSON.stringify({})}),{folderId:s,folderName:a}}function gt(e){try{const t=new Date;return new Intl.DateTimeFormat("en-US",{timeZone:e,weekday:"long",year:"numeric",month:"long",day:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:!0}).format(t)}catch{return new Date().toISOString()}}async function ft(e,t,a,r,n,s,o,i,l,c,u){var g,v,y,x,_,E;const h=new J(a);switch(e){case"create_schedule":{const d=new Date;let f;const p=c||"UTC";if(t.schedule_type==="interval"){const w=parseInt(t.schedule_value,10);f=new Date(d.getTime()+w*60*1e3)}else if(t.schedule_type==="daily"){const[w,T]=t.schedule_value.split(":").map(Number),S=d.toLocaleString("en-US",{timeZone:p}),I=new Date(S),R=new Date(I);R.setHours(w,T,0,0),R<=I&&R.setDate(R.getDate()+1);const A=new Date(R.toLocaleString("en-US",{timeZone:"UTC"})),N=new Date(R.toLocaleString("en-US",{timeZone:p})),$=A.getTime()-N.getTime();f=new Date(R.getTime()+$)}else if(t.schedule_type==="weekly"){const[w,T]=t.schedule_value.split(" "),[S,I]=(T||"00:00").split(":").map(Number),A=["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"].findIndex(yt=>yt.toLowerCase()===w.toLowerCase()),N=d.toLocaleString("en-US",{timeZone:p}),$=new Date(N),G=new Date($);G.setHours(S,I,0,0);let q=(A-G.getDay()+7)%7;q===0&&G<=$&&(q=7),G.setDate(G.getDate()+q);const ae=new Date(G.toLocaleString("en-US",{timeZone:"UTC"})),Ie=new Date(G.toLocaleString("en-US",{timeZone:p})),de=ae.getTime()-Ie.getTime();f=new Date(G.getTime()+de)}else if(t.schedule_type==="once"){const[w,T]=t.schedule_value.split(" "),[S,I,R]=w.split("-").map(Number),[A,N]=(T||"00:00").split(":").map(Number),$=d.toLocaleString("en-US",{timeZone:p}),G=new Date($),q=new Date(G);q.setFullYear(S,I-1,R),q.setHours(A,N,0,0);const ae=new Date(q.toLocaleString("en-US",{timeZone:"UTC"})),Ie=new Date(q.toLocaleString("en-US",{timeZone:p})),de=ae.getTime()-Ie.getTime();f=new Date(q.getTime()+de);const yt=new Date(d.getTime()+120*1e3);if(f.getTime()<d.getTime()+60*1e3){const Va=f.toISOString();f=yt;const Xa=` [Note: The requested time ${t.schedule_value} in ${p} resolved to ${Va} UTC which is in the past. Auto-adjusted to fire in ~2 minutes at ${f.toISOString()}.]`;t._pastTimeWarning=Xa}}else f=new Date(d.getTime()+3600*1e3);await a.prepare(`INSERT INTO cron_jobs (user_id, name, description, schedule_type, schedule_value, action_type, action_config, next_run, state)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'active')`).bind(r,t.name,t.description||t.action_description||"",t.schedule_type,t.schedule_value,t.action_type,JSON.stringify({description:t.action_description||t.description||""}),f.toISOString()).run();const m=t._pastTimeWarning||"";return`Schedule created: "${t.name}" — ${t.schedule_type} at ${t.schedule_value}. State: active. Next run: ${f.toISOString()}${m}`}case"list_schedules":{const f=(await a.prepare("SELECT * FROM cron_jobs WHERE user_id = ? ORDER BY enabled DESC, next_run ASC").bind(r).all()).results||[];return f.length===0?"No scheduled tasks found.":f.map(p=>`[ID:${p.id}] ${p.enabled?"▶":"⏸"} "${p.name}" — [${p.schedule_type}] ${p.schedule_value} — ${p.action_type} — state: ${p.state||"active"} — next: ${p.next_run||"N/A"}`).join(`
`)}case"toggle_schedule":{const d=t.enabled?1:0,f=d?"active":"paused";return await a.prepare("UPDATE cron_jobs SET enabled = ?, state = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?").bind(d,f,t.job_id,r).run(),`Schedule ${t.job_id} ${d?"enabled (active)":"paused"}.`}case"update_schedule_state":{const d=["created","active","reminding","paused","completed"],f=t.state;if(!d.includes(f))return`Invalid state "${f}". Valid states: ${d.join(", ")}`;const p=f==="completed"||f==="paused"?0:1;return await a.prepare("UPDATE cron_jobs SET state = ?, enabled = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?").bind(f,p,t.job_id,r).run(),`Schedule ${t.job_id} state updated to "${f}".`}case"delete_schedule":return await a.prepare("DELETE FROM cron_jobs WHERE id = ? AND user_id = ?").bind(t.job_id,r).run(),`Schedule ${t.job_id} deleted.`;case"store_memory":{const d=t.importance||5,f=d>=7?"working":"long_term";return await h.store(r,t.type,t.title,t.content,d,f),`Stored in ${f==="working"?"working":"long-term"} memory: [${t.type}] ${t.title} (importance: ${d})`}case"search_memory":{const d=await h.search(r,t.query);return d.length===0?"No matching memories found.":d.map(f=>`[${f.tier||"long_term"}] [${f.type}] **${f.title}**: ${f.content}`).join(`
`)}case"get_system_status":{const d=await a.prepare("SELECT COUNT(*) as cnt FROM cron_jobs WHERE user_id = ? AND enabled = 1").bind(r).first(),f=await a.prepare("SELECT COUNT(*) as cnt FROM memory WHERE user_id = ?").bind(r).first(),p=await a.prepare("SELECT COUNT(*) as cnt FROM memory WHERE user_id = ? AND tier = 'working'").bind(r).first(),m=await a.prepare("SELECT COUNT(*) as cnt FROM conversations WHERE user_id = ?").bind(r).first(),w=await a.prepare("SELECT COUNT(*) as cnt FROM error_log WHERE (user_id = ? OR user_id IS NULL) AND acknowledged = 0").bind(r).first();return`System Status:
...
- Unread errors: ${(w==null?void 0:w.cnt)||0}`}case"read_sheet":{if(!n)return"Authentication context unavailable.";try{const d=new ee(a,r,n,s||"",o||""),f=t.spreadsheet_id;let p=t.range;const m=await d.sheets.getMetadata(f),w=m.sheets;p.includes("!")||(p=`${w[0]}!${p}`);let T;try{T=await d.sheets.readRange(f,p)}catch(I){if((g=I.message)!=null&&g.includes("Unable to parse range")||(v=I.message)!=null&&v.includes("400")){const R=p.includes("!")?p.split("!")[1]:p;p=`${w[0]}!${R}`,T=await d.sheets.readRange(f,p)}else throw I}let S=`[Spreadsheet: "${m.title}" | Reading tab: "${p.split("!")[0]}" | All tabs in this spreadsheet: ${w.map(I=>`"${I}"`).join(", ")}]
...
ID: ${p.spreadsheetId}
URL: ${p.url}`}catch(d){return await O(a,r,"google","create_sheet",d.message),`Failed to create spreadsheet: ${d.message}`}}case"list_calendar_events":{if(!n)return"Authentication context unavailable.";try{const d=new ee(a,r,n,s||"",o||""),f=t.calendar_id||"primary",p=t.days_ahead||7,m=new Date,w=new Date(m.getTime()+p*24*60*60*1e3),T=await d.calendar.listEvents(f,{timeMin:m.toISOString(),timeMax:w.toISOString(),query:t.query});return T.length===0?`No events found in the next ${p} days.`:T.map(S=>{var $;const I=S.start.dateTime||S.start.date||"TBD",R=S.end.dateTime||S.end.date||"",A=S.location?` 📍 ${S.location}`:"",N=(($=S.attendees)==null?void 0:$.map(G=>G.email).join(", "))||"";return`• ${S.summary} — ${I} to ${R}${A}${N?`
  Attendees: ${N}`:""}`}).join(`
...
`)}catch(d){return await O(a,r,"search","web_search",d.message),`Web search error: ${d.message}`}case"read_url":try{const d=t.url;if(!d||!d.startsWith("http://")&&!d.startsWith("https://"))return"Invalid URL. Please provide a full URL starting with http:// or https://";const f=Math.min(t.max_length||8e3,15e3),{fetchPageContent:p}=await Promise.resolve().then(()=>tn),m=await p(d,f);return m.error?`Failed to read page: ${m.error}`:!m.text||m.text.length<20?`Page at ${d} returned no readable content.`:`Content from ${d} (${m.text.length} chars):

${m.text}`}catch(d){return await O(a,r,"search","read_url",d.message),`Read URL error: ${d.message}`}case"research":{if(!u)return"Research tool requires an LLM provider but none is available.";try{const f=Ha(t.query,u,{depth:t.depth||"quick",site:t.site}),p=new Promise(T=>setTimeout(()=>T(null),45e3)),m=await Promise.race([f,p]);if(m===null){const{webSearch:T}=await Promise.resolve().then(()=>Vr),S=await T(t.query,{num:5});if(S.error||S.results.length===0)return"Research timed out and fallback search returned no results. Try rephrasing or asking a more specific question.";let I=`Research took too long, but here are the top search results:
...
`)}catch(d){return await O(a,r,"google_api","search_places",d.message),`Places search error: ${d.message}`}}case"get_place_details":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const f=await M(d.encrypted_value,n),p=await Na(f,t.place_id);if(p.error)return`Details lookup failed: ${p.error}`;if(!p.details)return"No details found.";const m=p.details;let w=`**${m.name}**
...
— ${T.author} (★${T.rating}, ${T.time}): "${T.text}"`}return w}catch(d){return await O(a,r,"google_api","place_details",d.message),`Place details error: ${d.message}`}}case"get_directions":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const f=await M(d.encrypted_value,n),p=await $a(f,t.origin,t.destination,{mode:t.mode||"driving"});if(p.error)return`Directions failed: ${p.error}`;if(!p.route)return"No route found.";const m=p.route;let w=`**${m.startAddress}** → **${m.endAddress}**
`;return w+=`📏 ${m.distance} · ⏱️ ${m.duration}`,m.durationInTraffic&&(w+=` (with traffic: ${m.durationInTraffic})`),w+=`
via ${m.summary}`,w+=`

Steps:`,m.steps.forEach((T,S)=>{w+=`
${S+1}. ${T.instruction} (${T.distance}, ${T.duration})`}),w}catch(d){return await O(a,r,"google_api","directions",d.message),`Directions error: ${d.message}`}}case"get_travel_time":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const f=await M(d.encrypted_value,n),p=await Pa(f,t.origin,t.destination,t.mode||"driving");if(p.error)return`Travel time lookup failed: ${p.error}`;let m=`${t.origin} → ${t.destination}: ${p.distance}, ${p.duration}`;return p.durationInTraffic&&(m+=` (with traffic: ${p.durationInTraffic})`),m}catch(d){return await O(a,r,"google_api","travel_time",d.message),`Travel time error: ${d.message}`}}case"translate_text":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const f=await M(d.encrypted_value,n),p=await Ma(f,t.text,t.target_language,t.source_language);return p.error?`Translation failed: ${p.error}`:`[${p.detectedSourceLang||t.source_language||"auto"} → ${t.target_language}]

${p.translatedText}`}catch(d){return await O(a,r,"google_api","translate",d.message),`Translation error: ${d.message}`}}case"search_youtube":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const f=await M(d.encrypted_value,n),p=await ja(f,t.query,{maxResults:t.max_results||5,order:t.order||"relevance"});return p.error?`YouTube search failed: ${p.error}`:p.results.length===0?`No YouTube results for "${t.query}".`:p.results.map((m,w)=>{var T;return`${w+1}. **${m.title}**
...
`)}catch(d){return await O(a,r,"google_api","geocode",d.message),`Geocoding error: ${d.message}`}}default:return`Unknown tool: ${e}`}}async function Ot(e,t,a,r,n,s){var x;const o=new J(t),i=(x=e.metadata)==null?void 0:x.thread_id,l=await o.buildContext(r.id),c=await o.getRecentConversations(r.id,25,i),h=[{role:"system",content:qa(r,l)},...c.map(_=>({role:_.role,content:_.content})),{role:"user",content:e.text}];await o.storeMessage(r.id,e.channel,"user",e.text,"{}",i);const g=10;let v="",y=0;for(let _=0;_<g;_++)try{const E=await a.chat(h,{tools:ht});if(E.usage&&(y+=E.usage.promptTokens+E.usage.completionTokens),E.toolCalls&&E.toolCalls.length>0){E.content&&h.push({role:"assistant",content:E.content});for(const d of E.toolCalls)try{const f=await ft(d.name,d.arguments,t,r.id,r.pin_hash,s==null?void 0:s.GOOGLE_CLIENT_ID,s==null?void 0:s.GOOGLE_CLIENT_SECRET,s==null?void 0:s.GOOGLE_API_KEY,s==null?void 0:s.GOOGLE_CSE_ID,r.timezone,a);h.push({role:"user",content:`[Tool Result for ${d.name}]: ${f}`})}catch(f){await O(t,r.id,"tool",d.name,f.message||"Tool execution failed"),h.push({role:"user",content:`[Tool Error for ${d.name}]: ${f.message||"Execution failed"}`})}continue}v=E.content;break}catch(E){if(n){const d=E.message||"",f=d.includes("401")||d.includes("403")||d.includes("authentication")||d.includes("credit balance"),p=d.includes("429"),m=f?1440:p?10:5;await n.recordError(a.name,d,m)}throw await O(t,r.id,"llm","provider_error",E.message||"Unknown LLM error",{provider:a.name,turn:_}),E}if(n&&y>0)try{await n.recordUsage(a.name,y)}catch{}return await o.storeMessage(r.id,e.channel,"assistant",v,"{}",i),await o.compactHistory(r.id,30),v}const qt={"claude-sonnet-4-20250514":2e5,"claude-haiku-4-20250514":2e5,"gpt-4o":128e3,"gpt-4o-mini":128e3,"grok-3-mini":131072,"grok-3":131072,"deepseek-chat":64e3,"gemini-2.0-flash":1e6,default:32e3};function on(e){for(const[t,a]of Object.entries(qt))if(e.toLowerCase().includes(t.toLowerCase()))return a;return qt.default}function ln(e,t,a,r){const n=on(r),s=Math.floor(n*.75),o=[];let i=0,l=!1;const c=Et(e);o.push({role:"system",content:e}),i+=c;const u=Et(a);i+=u;const h=s-i,g=[];let v=0;for(let y=t.length-1;y>=0;y--){const x=t[y],_=Et(x.content);if(v+_<=h)g.unshift({role:x.role,content:x.content}),v+=_;else{l=!0;break}}return o.push(...g),o.push({role:"user",content:a}),i+=v,{maxTokens:n,usedTokens:i,messages:o,wasTruncated:l}}async function*cn(e,t,a,r,n,s){var _;const o=new J(t),i=(_=e.metadata)==null?void 0:_.thread_id;yield{type:"thinking",data:{threadId:i,provider:a.name}};const l=await o.buildContext(r.id),c=await o.getRecentConversations(r.id,20,i),u=qa(r,l),h=ln(u,c,e.text,a.name);await o.storeMessage(r.id,e.channel,"user",e.text,"{}",i);const g=10;let v="",y=0;const x=[...h.messages];for(let E=0;E<g;E++)try{E>0&&(yield{type:"thinking",data:{threadId:i}});const d=await a.chat(x,{tools:ht});if(d.usage&&(y+=d.usage.promptTokens+d.usage.completionTokens),d.toolCalls&&d.toolCalls.length>0){d.content&&(yield{type:"chunk",data:{text:d.content,threadId:i}},x.push({role:"assistant",content:d.content}));for(const p of d.toolCalls){yield{type:"tool_start",data:{tool:p.name,toolArgs:p.arguments,threadId:i}};try{const m=await ft(p.name,p.arguments,t,r.id,r.pin_hash,s==null?void 0:s.GOOGLE_CLIENT_ID,s==null?void 0:s.GOOGLE_CLIENT_SECRET,s==null?void 0:s.GOOGLE_API_KEY,s==null?void 0:s.GOOGLE_CSE_ID,r.timezone,a);yield{type:"tool_end",data:{tool:p.name,toolResult:m.substring(0,500)+(m.length>500?"...":""),threadId:i}},x.push({role:"user",content:`[Tool Result for ${p.name}]: ${m}`})}catch(m){await O(t,r.id,"tool",p.name,m.message||"Tool execution failed"),yield{type:"tool_end",data:{tool:p.name,toolResult:`Error: ${m.message||"Execution failed"}`,threadId:i}},x.push({role:"user",content:`[Tool Error for ${p.name}]: ${m.message||"Execution failed"}`})}}continue}v=d.content;const f=50;for(let p=0;p<v.length;p+=f)yield{type:"chunk",data:{text:v.substring(p,p+f),threadId:i}},p+f<v.length&&await new Promise(w=>setTimeout(w,10));break}catch(d){if(n){const f=d.message||"",p=f.includes("401")||f.includes("403")||f.includes("authentication")||f.includes("credit balance"),m=f.includes("429"),w=p?1440:m?10:5;await n.recordError(a.name,f,w)}await O(t,r.id,"llm","provider_error",d.message||"Unknown LLM error",{provider:a.name,turn:E}),yield{type:"error",data:{error:d.message||"An error occurred",threadId:i}};return}if(n&&y>0)try{await n.recordUsage(a.name,y)}catch{}await o.storeMessage(r.id,e.channel,"assistant",v,"{}",i),await o.compactHistory(r.id,30),yield{type:"done",data:{threadId:i,provider:a.name,tokenCount:y}}}async function Mt(e,t,a,r,n,s){var u;const o=new J(t),i=(u=e.metadata)==null?void 0:u.thread_id,l=await o.buildContext(r.id),c=za(e.text,l);if(c.agent==="conversation")return un(e,t,a,r,l,n,i);if(c.agent==="multi"||c.confidence<.5)return Ot(e,t,a,r,n,s);try{return await dn(c.agent,e,t,a,r,l,n,s,i)}catch(h){return await O(t,r.id,"router","subagent_fallback",`${c.agent} failed: ${h.message}`,{route:c}),Ot(e,t,a,r,n,s)}}async function dn(e,t,a,r,n,s,o,i,l){const c=new J(a),u=gt(n.timezone),h=ct(e,n,s,n.timezone,u),g=Fa(e,ht),v=await c.getRecentConversations(n.id,25,l),y=[{role:"system",content:h},...v.map(f=>({role:f.role,content:f.content})),{role:"user",content:t.text}];await c.storeMessage(n.id,t.channel,"user",t.text,"{}",l);const x=10;let _="",E=0,d="";for(let f=0;f<x;f++)try{const p=await r.chat(y,{tools:g.length>0?g:void 0});if(p.usage&&(E+=p.usage.promptTokens+p.usage.completionTokens),p.toolCalls&&p.toolCalls.length>0){p.content&&(d=p.content,y.push({role:"assistant",content:p.content}));for(const m of p.toolCalls)try{const w=await ft(m.name,m.arguments,a,n.id,n.pin_hash,i==null?void 0:i.GOOGLE_CLIENT_ID,i==null?void 0:i.GOOGLE_CLIENT_SECRET,i==null?void 0:i.GOOGLE_API_KEY,i==null?void 0:i.GOOGLE_CSE_ID,n.timezone,r);y.push({role:"user",content:`[Tool Result for ${m.name}]: ${w}`})}catch(w){await O(a,n.id,"tool",m.name,w.message||"Tool execution failed"),y.push({role:"user",content:`[Tool Error for ${m.name}]: ${w.message||"Execution failed"}`})}continue}_=p.content,!_&&d&&(_=d);break}catch(p){if(o){const m=p.message||"",w=m.includes("401")||m.includes("403")||m.includes("authentication")||m.includes("credit balance"),T=m.includes("429"),S=w?1440:T?10:5;await o.recordError(r.name,m,S)}throw await O(a,n.id,"llm","subagent_error",p.message||"Unknown error",{agent:e,provider:r.name,turn:f}),p}if(o&&E>0)try{await o.recordUsage(r.name,E)}catch{}return await c.storeMessage(n.id,t.channel,"assistant",_,"{}",l),await c.compactHistory(n.id,30),_}async function un(e,t,a,r,n,s,o){const i=new J(t),l=gt(r.timezone),c=ct("conversation",r,n,r.timezone,l),u=await i.getRecentConversations(r.id,25,o),h=[{role:"system",content:c},...u.map(y=>({role:y.role,content:y.content})),{role:"user",content:e.text}];await i.storeMessage(r.id,e.channel,"user",e.text,"{}",o);let g=0,v="";try{const y=await a.chat(h,{temperature:.8});y.usage&&(g=y.usage.promptTokens+y.usage.completionTokens),v=y.content}catch(y){if(s){const x=y.message||"",_=x.includes("401")||x.includes("403")||x.includes("authentication")||x.includes("credit balance"),E=x.includes("429"),d=_?1440:E?10:5;await s.recordError(a.name,x,d)}throw await O(t,r.id,"llm","conversation_error",y.message,{provider:a.name}),y}if(s&&g>0)try{await s.recordUsage(a.name,g)}catch{}return await i.storeMessage(r.id,e.channel,"assistant",v,"{}",o),await i.compactHistory(r.id,30),v}async function*pn(e,t,a,r,n,s){var d;const o=new J(t),i=(d=e.metadata)==null?void 0:d.thread_id,l=await o.buildContext(r.id),c=za(e.text,l);if(yield{type:"thinking",data:{threadId:i,provider:a.name}},c.agent==="multi"||c.confidence<.5){yield*cn(e,t,a,r,n,s);return}const u=gt(r.timezone),h=c.agent==="conversation"?ct("conversation",r,l,r.timezone,u):ct(c.agent,r,l,r.timezone,u),g=Fa(c.agent,ht),v=await o.getRecentConversations(r.id,25,i),y=[{role:"system",content:h},...v.map(f=>({role:f.role,content:f.content})),{role:"user",content:e.text}];await o.storeMessage(r.id,e.channel,"user",e.text,"{}",i);const x=10;let _="",E=0;for(let f=0;f<x;f++)try{f>0&&(yield{type:"thinking",data:{threadId:i}});const p=await a.chat(y,{tools:g.length>0?g:void 0});if(p.usage&&(E+=p.usage.promptTokens+p.usage.completionTokens),p.toolCalls&&p.toolCalls.length>0){p.content&&(yield{type:"chunk",data:{text:p.content,threadId:i}},y.push({role:"assistant",content:p.content}));for(const w of p.toolCalls){yield{type:"tool_start",data:{tool:w.name,toolArgs:w.arguments,threadId:i}};try{const T=await ft(w.name,w.arguments,t,r.id,r.pin_hash,s==null?void 0:s.GOOGLE_CLIENT_ID,s==null?void 0:s.GOOGLE_CLIENT_SECRET,s==null?void 0:s.GOOGLE_API_KEY,s==null?void 0:s.GOOGLE_CSE_ID,r.timezone,a);yield{type:"tool_end",data:{tool:w.name,toolResult:T.substring(0,500)+(T.length>500?"...":""),threadId:i}},y.push({role:"user",content:`[Tool Result for ${w.name}]: ${T}`})}catch(T){await O(t,r.id,"tool",w.name,T.message||"Tool execution failed"),yield{type:"tool_end",data:{tool:w.name,toolResult:`Error: ${T.message||"Execution failed"}`,threadId:i}},y.push({role:"user",content:`[Tool Error for ${w.name}]: ${T.message||"Execution failed"}`})}}continue}_=p.content;const m=50;for(let w=0;w<_.length;w+=m)yield{type:"chunk",data:{text:_.substring(w,w+m),threadId:i}},w+m<_.length&&await new Promise(S=>setTimeout(S,10));break}catch(p){if(n){const m=p.message||"",w=m.includes("401")||m.includes("403")||m.includes("authentication")||m.includes("credit balance"),T=m.includes("429"),S=w?1440:T?10:5;await n.recordError(a.name,m,S)}await O(t,r.id,"llm","subagent_stream_error",p.message||"Unknown error",{agent:c.agent,provider:a.name,turn:f}),yield{type:"error",data:{error:p.message||"An error occurred",threadId:i}};return}if(n&&E>0)try{await n.recordUsage(a.name,E)}catch{}await o.storeMessage(r.id,e.channel,"assistant",_,"{}",i),await o.compactHistory(r.id,30),yield{type:"done",data:{threadId:i,provider:a.name,tokenCount:E}}}const B=new ye;async function mn(e,t){var n;const a=(n=e.req.header("Authorization"))==null?void 0:n.replace("Bearer ","");if(!a)return e.json({error:"Authentication required"},401);const r=await e.env.DB.prepare(`SELECT s.*, u.* FROM sessions s JOIN users u ON s.user_id = u.id 
...
`;for(const c of s)o+=`- ${c.name} (${c.type}, ${Math.round(c.size/1024)}KB, file_id: ${c.file_id})`,c.text_preview&&(o+=`
  Preview: ${c.text_preview.substring(0,300)}...`),o+=`
`;o+="Use parse_document tool to read full file contents. Use drive_upload to upload to Google Drive.]"}let i=n;if(!i){const c=await e.env.DB.prepare("INSERT INTO threads (user_id, title) VALUES (?, ?) RETURNING *").bind(t.id,a.trim().substring(0,60)+(a.trim().length>60?"...":"")).first();i=c==null?void 0:c.id}const l={userId:t.id,username:t.username,channel:r,text:a.trim()+o,sessionId:e.get("sessionId"),timestamp:new Date().toISOString(),metadata:{thread_id:i}};try{const{provider:c,rotation:u}=await et(e.env.DB,t.id,t.pin_hash),h=new ReadableStream({async start(g){const v=new TextEncoder;try{const y=pn(l,e.env.DB,c,t,u,{GOOGLE_CLIENT_ID:e.env.GOOGLE_CLIENT_ID,GOOGLE_CLIENT_SECRET:e.env.GOOGLE_CLIENT_SECRET,GOOGLE_API_KEY:e.env.GOOGLE_API_KEY,GOOGLE_CSE_ID:e.env.GOOGLE_CSE_ID});for await(const x of y)x.data.threadId||(x.data.threadId=i),g.enqueue(v.encode(Kt(x)));i&&await e.env.DB.prepare("UPDATE threads SET message_count = message_count + 2, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(i).run(),g.close()}catch(y){const x={type:"error",data:{error:y.message||"An error occurred",threadId:i}};g.enqueue(v.encode(Kt(x))),g.close()}}});return new Response(h,{headers:{"Content-Type":"text/event-stream","Cache-Control":"no-cache",Connection:"keep-alive","X-Thread-Id":String(i||"")}})}catch(c){console.error("Stream setup error:",c);const u=c.message||"";return u.includes("No LLM provider configured")?e.json({error:"No AI provider configured. Please add at least one API key in Settings → Keys.",type:"no_provider",thread_id:i},400):u.includes("limit reached")?e.json({error:u,type:"cost_limit",thread_id:i},429):e.json({error:"Something went wrong setting up the stream.",details:u,thread_id:i},500)}});B.get("/threads/:id/messages",async e=>{var s;const t=e.get("user"),a=parseInt(e.req.param("id")),r=parseInt(e.req.query("limit")||"50"),n=await e.env.DB.prepare(`SELECT id, role, content, channel, created_at FROM conversations 
