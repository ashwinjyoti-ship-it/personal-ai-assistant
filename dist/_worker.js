...
ID: ${m.spreadsheetId}
URL: ${m.url}`}catch(u){return await B(a,r,"google","create_sheet",u.message),`Failed to create spreadsheet: ${u.message}`}}case"list_calendar_events":{if(!s)return"Authentication context unavailable.";try{const u=new ue(a,r,s,n||"",i||""),y=t.calendar_id||"primary",m=t.days_ahead||7,f=new Date,E=new Date(f.getTime()+m*24*60*60*1e3),k=await u.calendar.listEvents(y,{timeMin:f.toISOString(),timeMax:E.toISOString(),query:t.query});return k.length===0?`No events found in the next ${m} days.`:k.map(x=>{var A;const I=x.start.dateTime||x.start.date||"TBD",N=x.end.dateTime||x.end.date||"",R=x.location?` 📍 ${x.location}`:"",O=((A=x.attendees)==null?void 0:A.map(P=>P.email).join(", "))||"";return`• ${x.summary} — ${I} to ${N}${R}${O?`
  Attendees: ${O}`:""}`}).join(`
...
`;return N+=I.results.map((R,O)=>`${O+1}. **${R.title}**
   ${R.snippet}
   ${R.link}`).join(`
...
${x.substring(0,2e4)}`}catch{return`Could not decode text file: ${f}`}if(E==="application/pdf"||E==="application/vnd.openxmlformats-officedocument.wordprocessingml.document"||f.toLowerCase().endsWith(".pdf")||f.toLowerCase().endsWith(".docx")){let x=null,I="claude-haiku-4-5-20251001";for(const N of["llm_slot_1","llm_slot_2","llm_slot_3"])try{const R=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,N).first();if(R&&s){const O=await z(R.encrypted_value,s),A=JSON.parse(O);if(A.provider==="anthropic"){x=A.apiKey,A.model&&(I=A.model);break}}}catch{}if(x)try{const N=y?`Focus specifically on extracting: ${y}`:"Extract and return all readable text content from this document. Preserve structure where relevant.",O=E==="application/pdf"||f.toLowerCase().endsWith(".pdf")?"application/pdf":"application/vnd.openxmlformats-officedocument.wordprocessingml.document",A=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"x-api-key":x,"anthropic-version":"2023-06-01","anthropic-beta":"pdfs-2024-09-25","content-type":"application/json"},body:JSON.stringify({model:I,max_tokens:4096,messages:[{role:"user",content:[{type:"document",source:{type:"base64",media_type:O,data:k}},{type:"text",text:N}]}]})});if(A.ok){const K=((D=(_=(await A.json()).content)==null?void 0:_[0])==null?void 0:D.text)||"";return`Document: ${f}

${K}`}else{const P=await A.text();return`Could not parse ${f} via Anthropic API: ${P.substring(0,200)}`}}catch(N){return`Document parsing error for ${f}: ${N.message}`}if(f.toLowerCase().endsWith(".docx"))try{const O=Buffer.from(k,"base64").toString("utf-8").replace(/<[^>]+>/g," ").replace(/\s+/g," ").trim().substring(0,1e4);if(O.length>100)return`Document: ${f} (rough text extraction)

${O}`}catch{}return"To parse PDF/Word documents, please configure an Anthropic API key in Settings → Keys. No Anthropic key is currently set."}try{const x=Buffer.from(k,"base64").toString("utf-8").substring(0,2e3);return`Document: ${f} (${E})
