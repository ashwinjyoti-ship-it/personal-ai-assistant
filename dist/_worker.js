...
Content preview:
${S}`}catch{return`Cannot read file: ${h} (${b})`}}case"create_skill":{const u=(B=e.name)==null?void 0:B.trim(),y=(G=e.description)==null?void 0:G.trim(),p=(q=e.instructions)==null?void 0:q.trim();if(!u||!y||!p)return"create_skill requires name, description, and instructions.";let h=u.toLowerCase().replace(/[^a-z0-9\s_-]/g,"").replace(/\s+/g,"_").replace(/_+/g,"_").substring(0,50).replace(/^_|_$/g,"");h||(h=`skill_${Date.now()}`);const b=await a.prepare("SELECT slug FROM user_skills WHERE user_id = ? AND slug LIKE ?").bind(r,`${h}%`).all();(O=b.results)!=null&&O.some(R=>R.slug===h)&&(h=`${h}_${(((L=b.results)==null?void 0:L.length)||0)+1}`);const k=JSON.stringify(e.parameters||{}),S=JSON.stringify(e.required_tools||[]),D=JSON.stringify(e.examples||[]);return await a.prepare(`INSERT INTO user_skills (user_id, name, slug, description, instructions, parameters, required_tools, examples)
...
Execute the steps above using your available tools. When complete, provide a clear summary of what was done and any results.`}return`Unknown tool: ${t}`}}}async function Dr(t,e,a,r,s){if(e.length>0&&e[e.length-1].role==="user"){const n="(Previous request did not complete. Please try again.)";await t.storeMessage(a,r,"assistant",n,"{}",s),e.push({id:-1,user_id:a,channel:r,role:"assistant",content:n,metadata:"{}",token_estimate:n.length,created_at:new Date().toISOString()})}}function Or(t){for(let e=t.length-1;e>=0;e--)if(t[e].role==="assistant"){const a=typeof t[e].content=="string"?t[e].content:"";a.length<300&&/^(now let me|let me |i'll |i will |i'm going to|let's |to do this)/i.test(a.trim())&&(t[e]={...t[e],content:"(My previous response was cut off before completing. Starting fresh.)"});break}}async function fa(t,e,a,r,s,n,i){var q,O,L,u,y;const o=new Q(e),l=(q=t.metadata)==null?void 0:q.thread_id,c=Date.now(),[d,m]=await Promise.all([o.buildContext(r.id),aa(e,r.id)]),g=await o.getRecentConversations(r.id,30,l);await Dr(o,g,r.id,t.channel,l);const w=xr(r,d,t.channel,m),f=Tr([{role:"system",content:w},...g.map(p=>({role:p.role,content:p.content})),{role:"user",content:t.text}]);Or(f);const T=[/\bmy (address|phone|email|budget|preferences?|settings?|rules?|sheet|doc|folder|password|login|account)\b/i,/\b(do you remember|last time|what did i|you said|i told you|my usual|like before|same as last|remind me what|what was the|previously)\b/i],v=(d.match(/^- /gm)||[]).length;if(T.some(p=>p.test(t.text))||v<3)try{const p=await o.searchLongTerm(r.id,t.text,5);if(p.length>0){const h=p.map(b=>`- [${b.type}] ${b.title}: ${b.content}`).join(`
...
`),"Markdown",s,r.id);w.success||console.warn(`[/tasks] Failed to send message: ${w.errors.join(" | ")}`)}catch(i){const o=await te(a,e,"❌ Could not fetch tasks: "+i.message,"Markdown",s,r==null?void 0:r.id);o.success||console.warn(`[/tasks error] Failed to send message: ${o.errors.join(" | ")}`)}return!0}default:return!1}}_t.post("/webhook",async t=>{let e;try{e=await t.req.json()}catch{return t.json({ok:!0})}const a=t.env.DB,r={GOOGLE_CLIENT_ID:t.env.GOOGLE_CLIENT_ID,GOOGLE_CLIENT_SECRET:t.env.GOOGLE_CLIENT_SECRET,GOOGLE_API_KEY:t.env.GOOGLE_API_KEY,GOOGLE_CSE_ID:t.env.GOOGLE_CSE_ID},s=async()=>{var n,i,o,l,c;try{if(e.callback_query){await yn(a,e.callback_query);return}const d=e.message;if(!d)return;const m=!!d.text,g=!!d.voice,w=!!d.document,f=!!d.photo,T=!!d.caption;if(!m&&!g&&!w&&!f)return;const v=String(d.chat.id);let x=d.text||"";const E=await a.prepare("SELECT * FROM users WHERE telegram_chat_id = ?").bind(v).first();let I=null;if(E){const O=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(E.id,"telegram_bot_token").first();O&&(I=await J(O.encrypted_value,E.pin_hash))}if(!I){const O=await a.prepare(`SELECT c.encrypted_value, u.pin_hash FROM credentials c
         JOIN users u ON c.user_id = u.id
         WHERE c.service = 'telegram_bot_token' LIMIT 1`).first();O&&(I=await J(O.encrypted_value,O.pin_hash))}if(!I||x.startsWith("/")&&await fn(x,v,I,E,a))return;if(!E){const O=await te(I,v,`⚠️ Your account isn't linked yet.

Your Telegram Chat ID is: \`${v}\`

Go to the web app → Settings → Profile → set your Telegram Chat ID to this value.`,"Markdown",a);O.success||console.warn(`Failed to send unlinked account message: ${O.errors.join(" | ")}`);return}if(d.voice&&I&&E)try{const O=await te(I,v,"🎤 Processing voice note...","Markdown",a,E.id);O.success||console.warn(`[voice start] Failed to send message: ${O.errors.join(" | ")}`);const u=await(await fetch(`https://api.telegram.org/bot${I}/getFile?file_id=${d.voice.file_id}`)).json();if(u.ok&&((n=u.result)!=null&&n.file_path)){const p=await(await fetch(`https://api.telegram.org/file/bot${I}/${u.result.file_path}`)).blob();let h="",b="",k="whisper-1";const S=await t.env.DB.prepare("SELECT service, encrypted_value FROM credentials WHERE user_id = ? AND (service IN ('llm_slot_1', 'llm_slot_2', 'llm_slot_3', 'openai'))").bind(E.id).all();for(const M of S.results){const z=await J(M.encrypted_value,E.pin_hash);if(M.service==="openai"){h="https://api.openai.com/v1/audio/transcriptions",b=z;break}else if(M.service.startsWith("llm_slot_"))try{const Y=JSON.parse(z);if(Y.provider==="openai"){h="https://api.openai.com/v1/audio/transcriptions",b=Y.apiKey;break}else if(Y.provider==="groq"){h="https://api.groq.com/openai/v1/audio/transcriptions",b=Y.apiKey,k="whisper-large-v3";break}}catch{}}if(!h){const M=await te(I,v,"⚠️ To use voice notes, configure an OpenAI API key in your LLM slots (Settings → Keys).","Markdown",a,E.id);M.success||console.warn(`[voice no stt] Failed to send message: ${M.errors.join(" | ")}`);return}const D=new FormData;D.append("file",p,"voice.ogg"),D.append("model",k),D.append("language","en");const R=await fetch(h,{method:"POST",headers:{Authorization:`Bearer ${b}`},body:D});if(!R.ok){const M=await R.text(),z=await te(I,v,`⚠️ Transcription failed: ${R.status} ${M}`,"Markdown",a,E.id);z.success||console.warn(`[voice transcription error] Failed to send message: ${z.errors.join(" | ")}`);return}x=(await R.json()).text;const A=await te(I,v,`🗣️ *You said:* ${x}`,"Markdown",a,E.id);A.success||console.warn(`[voice transcript echo] Failed to send message: ${A.errors.join(" | ")}`)}}catch(O){const L=await te(I,v,`⚠️ Failed to process voice note: ${O.message}`,"Markdown",a,E==null?void 0:E.id);L.success||console.warn(`[voice processing error] Failed to send message: ${L.errors.join(" | ")}`);return}if((w||f)&&I&&E)try{let O,L="unknown",u="unknown",y=0;if(w)O=d.document.file_id,L=d.document.file_name||"document",u=d.document.mime_type||"unknown",y=d.document.file_size||0;else if(f){const p=d.photo[d.photo.length-1];O=p.file_id,L="photo.jpg",u="image/jpeg",y=p.file_size||0}if(O){const h=await(await fetch(`https://api.telegram.org/bot${I}/getFile?file_id=${O}`)).json();let b="";if(h.ok&&((i=h.result)!=null&&i.file_path)&&(/\.(txt|csv|json|md|xml|html|log|yaml|yml|tsv|ini|cfg|conf|py|js|ts|sh|sql)$/i.test(L)||/^text\/|application\/json|application\/xml|application\/csv/i.test(u))&&y<5e4)try{b=await(await fetch(`https://api.telegram.org/file/bot${I}/${h.result.file_path}`)).text()}catch{}const k=d.caption||"",S=`[Telegram file received: "${L}" (${u}, ${Math.round(y/1024)}KB)]`;b?x=`${k?k+`
...
`:""}${S}
Note: This file type cannot be read directly. I can see it was sent but cannot extract the content. For PDFs, images, or Office docs — suggest uploading to Google Drive via the web app where I can process them.`}}catch(O){if(T&&d.caption)x=d.caption;else{const L=await te(I,v,`⚠️ Received your file but couldn't process it: ${O.message}`,"Markdown",a,E==null?void 0:E.id);L.success||console.warn(`[file processing error] Failed to send message: ${L.errors.join(" | ")}`);return}}if(!x)return;hn(I,v).catch(()=>{});let N=await a.prepare("SELECT id FROM threads WHERE user_id = ? AND channel = 'telegram' AND is_archived = 0 ORDER BY updated_at DESC LIMIT 1").bind(E.id).first();N?await a.prepare("UPDATE threads SET updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(N.id).run():N={id:(await a.prepare("INSERT INTO threads (user_id, title, channel) VALUES (?, 'Telegram', 'telegram')").bind(E.id).run()).meta.last_row_id};const P=un(E.id,E.username,x,v);P.metadata={thread_id:N.id};let W,F;try{const O=await bt(a,E.id,E.pin_hash);W=O.provider,F=O.rotation}catch(O){console.error("Telegram provider setup error:",O);const L=(o=O.message)!=null&&o.includes("No LLM provider")?`⚠️ No AI provider configured yet.

Go to the web app → Settings → Keys → add at least one API key (DeepSeek, Grok, Abacus AI, etc.).`:(l=O.message)!=null&&l.includes("Daily usage limit")?"⚠️ Daily usage limit reached. Your limit resets at midnight.":`⚠️ AI provider error: ${O.message||"Unknown error"}`,u=await te(I,v,L,"Markdown",a,E.id);u.success||console.warn(`[provider error] Failed to send message: ${u.errors.join(" | ")}`);return}const{classifyIntentFast:B}=await Promise.resolve().then(()=>Vs);if(B(x).agent==="multi"){const O=await te(I,v,"🔍 On it…","Markdown",a,E.id);O.success||console.warn(`[ack] Failed to send: ${O.errors.join(" | ")}`)}const G=9e4;let q=!1;try{const O=await Promise.race([ra(P,a,W,E,F,r),new Promise((y,p)=>setTimeout(()=>p(new Error("TELEGRAM_TIMEOUT")),G))]),L=mn(O,"telegram"),u=await te(I,v,L||"(empty response)","Markdown",a,E.id);if(q=u.success,!u.success){console.error(`[CRITICAL] Agent response failed to send to Telegram for user ${E.id}:`,u.errors);try{const{logError:y}=await Promise.resolve().then(()=>Ve);await y(a,E.id,"telegram","response_send_failed",`Failed to deliver response: ${u.errors.join(" | ")}`)}catch{}}}catch(O){console.error("Telegram agent error:",O);const L=O.message==="TELEGRAM_TIMEOUT",u=L?`⏱️ This took longer than Telegram allows (25s limit).

For long essays, please use the web app — it handles long generation without time limits.`:(c=O.message)!=null&&c.includes("API error")?`⚠️ AI provider returned an error. The provider (${W.name}) may be temporarily unavailable. Your message was saved — try again shortly.`:`⚠️ Something went wrong processing your message. Error: ${(O.message||"Unknown").substring(0,200)}`,y=await te(I,v,u,"Markdown",a,E.id);q=y.success,y.success||console.error(`[CRITICAL] Error message failed to send to Telegram for user ${E.id}:`,y.errors);try{const{logError:p}=await Promise.resolve().then(()=>Ve);await p(a,E.id,"telegram",L?"timeout":"agent_error",O.message||"Agent error",{provider:W.name})}catch{}}}catch(d){console.error("Telegram webhook error:",d);try{const{logError:m}=await Promise.resolve().then(()=>Ve);await m(a,null,"telegram","webhook_error",d.message||"Unknown telegram error")}catch{}}};return t.executionCtx.waitUntil(s()),t.json({ok:!0})});_t.post("/setup-webhook",async t=>{var l;const e=(l=t.req.header("Authorization"))==null?void 0:l.replace("Bearer ","");if(!e)return t.json({error:"Auth required"},401);const a=await t.env.DB.prepare(`SELECT s.user_id, u.pin_hash FROM sessions s JOIN users u ON s.user_id = u.id
...
`)}function kn(t){const e=[];let a=0;for(const r of t.calendar.google)e.push({type:"calendar",key:r.id,text:`${r.title} - ${new Date(r.startTime).toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"})}`,metadata:{event:r},sortOrder:a++});t.emails.gmail.unreadCount>0&&e.push({type:"email",key:"gmail-unread",text:`Review ${t.emails.gmail.unreadCount} unread Gmail messages`,metadata:{source:"gmail",count:t.emails.gmail.unreadCount},sortOrder:a++});for(const r of t.tasks.items)e.push({type:"task",key:`task-${r}`,text:r,metadata:{},sortOrder:a++});for(const r of t.news.items)e.push({type:"news",key:`news-${r.url}`,text:`📰 ${r.title}`,metadata:{url:r.url,source:r.source},sortOrder:a++});return e}async function Sn(t,e){const a=await t.prepare("SELECT * FROM briefing_preferences WHERE user_id = ?").bind(e).first();if(!a)return{components:{google_calendar:!0,gmail:!0,tasks:!0,news:!0},newsTopics:["AI","LLM","Tools","Agentic Workflows","AI Features"]};let r;try{const n=JSON.parse(a.components);r={google_calendar:n.google_calendar!==!1,gmail:n.gmail!==!1,tasks:n.tasks!==!1,news:n.news!==!1}}catch{r={google_calendar:!0,gmail:!0,tasks:!0,news:!0}}const s=a.news_topics?a.news_topics.split(",").map(n=>n.trim()).filter(Boolean):["AI","LLM","Tools","Agentic Workflows","AI Features"];return{components:r,newsTopics:s}}async function Rr(t,e,a){var E,I;const r=e.timezone||"Asia/Kolkata",s=vn(r),{components:n,newsTopics:i}=await Sn(t,e.id),o=[],l=[];n.google_calendar&&(o.push(wn(t,e.id,e.pin_hash,a.GOOGLE_CLIENT_ID,a.GOOGLE_CLIENT_SECRET,s)),l.push("googleEvents")),n.gmail&&(o.push(bn(t,e.id,e.pin_hash,a.GOOGLE_CLIENT_ID,a.GOOGLE_CLIENT_SECRET)),l.push("gmailSummary")),n.tasks&&(o.push(_n(t,e.id)),l.push("tasks")),n.news&&(o.push(Tn(i,t,e.id)),l.push("news"));const c=await Promise.all(o),d={};l.forEach((N,P)=>{d[N]=c[P]});const m={unreadCount:0,importantCount:0,topSenders:[],hasUrgent:!1},g={pending:0,dueToday:0,items:[]},w={generatedAt:new Date().toISOString(),targetDate:s.dateStr,calendar:{google:d.googleEvents||[],totalCount:((E=d.googleEvents)==null?void 0:E.length)||0},emails:{gmail:d.gmailSummary||m},tasks:d.tasks||g,news:{items:d.news||[],fetchedAt:new Date().toISOString()},summary:""},f=((I=await t.prepare("SELECT briefing_time FROM briefing_preferences WHERE user_id = ?").bind(e.id).first())==null?void 0:I.briefing_time)||"20:00";w.summary=xn(w,f);const T=kn(w),v=await t.prepare(`
  ...
  `).bind(a).all();return{briefing:{...r,content:JSON.parse(r.content_json||"{}")},items:s.results||[]}}async function On(t,e,a,r){if(!await t.prepare(`
  ...
  `).bind(i,i,r,a).run(),{checked:i===1}}async function In(t,e,a=10){return((await t.prepare(`
    SELECT b.*, 
