...
`);for(const T of v)if(T.startsWith("data: ")){const E=T.slice(6);if(E==="[DONE]")continue;try{const w=JSON.parse(E);w.type==="content_block_delta"&&((y=w.delta)!=null&&y.text)&&c.enqueue(new TextEncoder().encode("data: "+JSON.stringify({text:w.delta.text})+`
...
`).slice(-8).some(n=>/\[TOOLS_USED:/i.test(n)||/\b(reminder|reminders|schedule|scheduled|cron|alarm|next_run|set for|going off|remind)\b/i.test(n))?{agent:"multi",confidence:.8,reasoning:"Active tool session follow-up — full agent"}:t&&/spreadsheet|sheet|google\s*sheet/i.test(t)&&/\b(event|crew|venue|program|budget|expense|data|who|what|when|list|show)\b/i.test(e)?{agent:"multi",confidence:.85,reasoning:"Memory context — full agent"}:{agent:"conversation",confidence:.8,reasoning:"No tool-triggering keywords — general conversation"}}function yn(e,t,a,r,n,s){const i=t.assistant_name||"Karna",o=t.personality_prompt?`
## Personality
...
- Unread errors: ${(_==null?void 0:_.cnt)||0}`}case"read_sheet":{if(!n)return"Authentication context unavailable.";try{const u=new se(a,r,n,s||"",i||""),h=t.spreadsheet_id;let p=t.range;const f=await u.sheets.getMetadata(h),_=f.sheets;p.includes("!")||(p=`${_[0]}!${p}`);let x;try{x=await u.sheets.readRange(h,p)}catch(I){if((g=I.message)!=null&&g.includes("Unable to parse range")||(v=I.message)!=null&&v.includes("400")){const k=p.includes("!")?p.split("!")[1]:p;p=`${_[0]}!${k}`,x=await u.sheets.readRange(h,p)}else throw I}let S=`[Spreadsheet: "${f.title}" | Reading tab: "${p.split("!")[0]}" | All tabs in this spreadsheet: ${_.map(I=>`"${I}"`).join(", ")}]
...
ID: ${p.id}
Start: ${p.start.dateTime||p.start.date}`}catch(u){return await M(a,r,"google","create_event",u.message),`Failed to create event: ${u.message}`}}case"create_doc":{if(!n)return"Authentication context unavailable.";try{const u=new se(a,r,n,s||"",i||"");if(!(await u.isConnected()).connected)return'Google account not connected. Please go to Settings → Keys → Google Workspace and click "Connect Google Account" to sign in with your Google account first.';const p=await u.docs.createDocument(t.title);t.content&&await u.docs.appendText(p.documentId,t.content);let f="";if(t.folder_name)try{const{token:_}=await(await Promise.resolve().then(()=>ut)).getGoogleAuth(a,r,n,s||"",i||"");f=`
Folder: "${(await Zt(_,p.documentId,t.folder_name)).folderName}"`}catch(_){f=`
(Could not move to folder "${t.folder_name}": ${_.message})`}try{await new ae(a).store(r,"context",`Document: ${t.title}`,`Document ID: ${p.documentId} | URL: ${p.url}`,6,"working")}catch{}return`Document created: "${t.title}"${f}
ID: ${p.documentId}
...
`),I}if(f.error)return`Research failed: ${f.error}`;let _=f.report;return f.sources.length>0&&(_+=`

---
**Sources** (`+f.pagesRead+` pages read):
`,_+=f.sources.map((x,S)=>`[${S+1}] ${x.title}
    ${x.url}`).join(`
`)),_}catch(u){return await M(a,r,"research","research",u.message),`Research error: ${u.message}`}}case"search_places":{if(!n)return"Authentication context unavailable.";try{const u=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!u)return"Google API Key not configured. Add it in Settings → Keys → Google API Key.";const h=await F(u.encrypted_value,n),p=await Ga(h,t.query,{type:t.type});return p.error?`Places search failed: ${p.error}`:p.results.length===0?`No places found for "${t.query}".`:p.results.map((f,_)=>{const x=f.rating?` ★${f.rating} (${f.userRatingsTotal||0} reviews)`:"",S=f.openNow!==void 0?f.openNow?" · Open now":" · Closed":"",I=f.googleMapsUri?`
   ${f.googleMapsUri}`:"";return`${_+1}. **${f.name}**${x}${S}
   ${f.address}${I}
   [place_id: ${f.placeId}]`}).join(`

`)}catch(u){return await M(a,r,"google_api","search_places",u.message),`Places search error: ${u.message}`}}case"get_place_details":{if(!n)return"Authentication context unavailable.";try{const u=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!u)return"Google API Key not configured.";const h=await F(u.encrypted_value,n),p=await Fa(h,t.place_id);if(p.error)return`Details lookup failed: ${p.error}`;if(!p.details)return"No details found.";const f=p.details;let _=`**${f.name}**
...
`:""}${U}
Note: This file type cannot be read directly. I can see it was sent but cannot extract the content. For PDFs, images, or Office docs — suggest uploading to Google Drive via the web app where I can process them.`}}catch(k){if(T&&d.caption)w=d.caption;else{await H(h,E,`⚠️ Received your file but couldn't process it: ${k.message}`);return}}if(!w)return;Ln(h,E).catch(()=>{});let p=await a.prepare("SELECT id FROM threads WHERE user_id = ? AND channel = 'telegram' AND is_archived = 0 ORDER BY updated_at DESC LIMIT 1").bind(u.id).first();p?await a.prepare("UPDATE threads SET updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(p.id).run():p={id:(await a.prepare("INSERT INTO threads (user_id, title, channel) VALUES (?, 'Telegram', 'telegram')").bind(u.id).run()).meta.last_row_id};const f=Cn(u.id,u.username,w,E);f.metadata={thread_id:p.id};let _,x;try{const k=await st(a,u.id,u.pin_hash);_=k.provider,x=k.rotation}catch(k){console.error("Telegram provider setup error:",k);const O=(o=k.message)!=null&&o.includes("No LLM provider")?`⚠️ No AI provider configured yet.

Go to the web app → Settings → Keys → add at least one API key (DeepSeek, Grok, Abacus AI, etc.).`:(l=k.message)!=null&&l.includes("Daily usage limit")?"⚠️ Daily usage limit reached. Your limit resets at midnight.":`⚠️ AI provider error: ${k.message||"Unknown error"}`;await H(h,E,O);return}/\b(write\s+(an?\s+)?(essay|article|report|document|doc|blog|post|summary|draft)|draft\s+(an?\s+)?(essay|article|report|document)|research\s+.{0,60}(save|store|drive|doc))\b/i.test(w)&&await H(h,E,"📝 Working on it— searching and saving to Drive takes about 20 seconds...");const I=25e3;try{const k=await Promise.race([Ut(f,a,_,u,x,r),new Promise((C,N)=>setTimeout(()=>N(new Error("TELEGRAM_TIMEOUT")),I))]),O=Rn(k,"telegram");await H(h,E,O||"(empty response)")}catch(k){console.error("Telegram agent error:",k);const O=k.message==="TELEGRAM_TIMEOUT",C=O?`⏱️ This took longer than Telegram allows (25s limit).

For long essays, please use the web app — it handles long generation without time limits.`:(c=k.message)!=null&&c.includes("API error")?`⚠️ AI provider returned an error. The provider (${_.name}) may be temporarily unavailable. Your message was saved — try again shortly.`:`⚠️ Something went wrong processing your message. Error: ${(k.message||"Unknown").substring(0,200)}`;await H(h,E,C);try{const{logError:N}=await Promise.resolve().then(()=>Rt);await N(a,u.id,"telegram",O?"timeout":"agent_error",k.message||"Agent error",{provider:_.name})}catch{}}}catch(d){console.error("Telegram webhook error:",d);try{const{logError:m}=await Promise.resolve().then(()=>Rt);await m(a,null,"telegram","webhook_error",d.message||"Unknown telegram error")}catch{}}};return e.executionCtx.waitUntil(n()),e.json({ok:!0})});it.post("/setup-webhook",async e=>{var l;const t=(l=e.req.header("Authorization"))==null?void 0:l.replace("Bearer ","");if(!t)return e.json({error:"Auth required"},401);const a=await e.env.DB.prepare(`SELECT s.user_id, u.pin_hash FROM sessions s JOIN users u ON s.user_id = u.id
