...
Note: Always use this date/time as the current time. Do NOT guess or use UTC.`}async function Wt(e,t,a){var c;const n=await(await fetch(`https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(`name='${a.replace(/'/g,"\\'")}' and mimeType='application/vnd.google-apps.folder' and trashed=false`)}&fields=files(id,name)`,{headers:{Authorization:`Bearer ${e}`}})).json();let s;((c=n.files)==null?void 0:c.length)>0?s=n.files[0].id:s=(await(await fetch("https://www.googleapis.com/drive/v3/files",{method:"POST",headers:{Authorization:`Bearer ${e}`,"Content-Type":"application/json"},body:JSON.stringify({name:a,mimeType:"application/vnd.google-apps.folder"})})).json()).id;const l=((await(await fetch(`https://www.googleapis.com/drive/v3/files/${t}?fields=parents`,{headers:{Authorization:`Bearer ${e}`}})).json()).parents||[]).join(",");return await fetch(`https://www.googleapis.com/drive/v3/files/${t}?addParents=${s}&removeParents=${l}`,{method:"PATCH",headers:{Authorization:`Bearer ${e}`,"Content-Type":"application/json"},body:JSON.stringify({})}),{folderId:s,folderName:a}}function ft(e){try{const t=new Date;return new Intl.DateTimeFormat("en-US",{timeZone:e,weekday:"long",year:"numeric",month:"long",day:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:!0}).format(t)}catch{return new Date().toISOString()}}function cn(e){const t=e.toLowerCase().trim(),a=t.match(/(?:remind|alert|notify|tell).*?in\s+(\d+)\s*(minutes?|mins?|hours?|hrs?|h)\b/i);if(a){let n=parseInt(a[1],10);a[2].toLowerCase().startsWith("h")&&(n*=60);const o=e.match(/(?:to|about|that)\s+(.+?)\.?$/i),i=o?o[1].trim():e.replace(/remind.*?(?:in\s+\d+\s*\w+)/i,"").trim()||"Reminder",l=i.length>50?i.substring(0,47)+"...":i;return{args:{name:l.charAt(0).toUpperCase()+l.slice(1),description:i,schedule_type:"once",minutes_from_now:n,action_type:"reminder",action_description:i,schedule_value:""}}}const r=t.match(/(?:remind|alert|notify|tell).*?(?:at|by)\s+(\d{1,2})[:.]?(\d{2})?\s*(am|pm|a\.m\.|p\.m\.)?/i);if(r){let n=parseInt(r[1],10);const s=r[2]?parseInt(r[2],10):0,o=(r[3]||"").replace(/\./g,"").toLowerCase();o==="pm"&&n<12&&(n+=12),o==="am"&&n===12&&(n=0);const i=e.match(/(?:to|about|that)\s+(.+?)\.?$/i),l=i?i[1].trim():"Reminder",c=l.length>50?l.substring(0,47)+"...":l,u=new Date,m=u.getFullYear(),y=String(u.getMonth()+1).padStart(2,"0"),v=String(u.getDate()).padStart(2,"0"),g=`${m}-${y}-${v} ${String(n).padStart(2,"0")}:${String(s).padStart(2,"0")}`;return{args:{name:c.charAt(0).toUpperCase()+c.slice(1),description:l,schedule_type:"once",schedule_value:g,action_type:"reminder",action_description:l}}}return null}async function Ne(e,t,a,r,n,s,o,i,l,c,u,m){const y=Date.now();let v=!0,g="",_="";try{return _=await dn(e,t,a,r,s,o,i,l,c,u,m),_}catch(w){throw v=!1,g=w.message||"Unknown error",w}finally{const w=Date.now()-y;try{await a.prepare(`INSERT INTO tool_execution_log (user_id, agent_type, provider_name, tool_name, tool_args, tool_result, success, error_message, latency_ms, was_enforcement_retry, channel)
