...
- Unread errors: ${(_==null?void 0:_.cnt)||0}`}case"read_sheet":{if(!n)return"Authentication context unavailable.";try{const d=new te(a,r,n,s||"",o||""),y=t.spreadsheet_id;let p=t.range;const m=await d.sheets.getMetadata(y),_=m.sheets;p.includes("!")||(p=`${_[0]}!${p}`);let T;try{T=await d.sheets.readRange(y,p)}catch(O){if((f=O.message)!=null&&f.includes("Unable to parse range")||(v=O.message)!=null&&v.includes("400")){const L=p.includes("!")?p.split("!")[1]:p;p=`${_[0]}!${L}`,T=await d.sheets.readRange(y,p)}else throw O}let S=`[Spreadsheet: "${m.title}" | Reading tab: "${p.split("!")[0]}" | All tabs in this spreadsheet: ${_.map(O=>`"${O}"`).join(", ")}]
...
(Could not move to folder "${t.folder_name}": ${_.message})`}try{await new J(a).store(r,"context",`Document: ${t.title}`,`Document ID: ${p.documentId} | URL: ${p.url}`,6,"working")}catch{}return`Document created: "${t.title}"${m}
ID: ${p.documentId}
URL: ${p.url}`}catch(d){return await A(a,r,"google","create_doc",d.message),`Failed to create document: ${d.message}`}}case"read_doc":{if(!n)return"Authentication context unavailable.";try{const y=await new te(a,r,n,s||"",o||"").docs.readDocument(t.document_id);return`Document: "${y.title}"
...
`,_+=m.sources.map((T,S)=>`[${S+1}] ${T.title}
    ${T.url}`).join(`
`)),_}catch(d){return await A(a,r,"research","research",d.message),`Research error: ${d.message}`}}case"search_places":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured. Add it in Settings → Keys → Google API Key.";const y=await B(d.encrypted_value,n),p=await ja(y,t.query,{type:t.type});return p.error?`Places search failed: ${p.error}`:p.results.length===0?`No places found for "${t.query}".`:p.results.map((m,_)=>{const T=m.rating?` ★${m.rating} (${m.userRatingsTotal||0} reviews)`:"",S=m.openNow!==void 0?m.openNow?" · Open now":" · Closed":"",O=m.googleMapsUri?`
   ${m.googleMapsUri}`:"";return`${_+1}. **${m.name}**${T}${S}
   ${m.address}${O}
   [place_id: ${m.placeId}]`}).join(`

`)}catch(d){return await A(a,r,"google_api","search_places",d.message),`Places search error: ${d.message}`}}case"get_place_details":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const y=await B(d.encrypted_value,n),p=await Pa(y,t.place_id);if(p.error)return`Details lookup failed: ${p.error}`;if(!p.details)return"No details found.";const m=p.details;let _=`**${m.name}**
...
🕐 ${y}${p}`;await fetch(`https://api.telegram.org/bot${E}/sendMessage`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({chat_id:l.telegram_chat_id,text:m,parse_mode:"Markdown"})})}n.push({user_id:l.id,reminders_sent:x.length})}catch(c){n.push({user_id:l.id,status:"error",error:c.message})}return e.json({executed:n.length,results:n})}catch(r){return e.json({error:r.message},500)}});async function Fn(e,t,a,r,n){try{const s=await e.prepare(`SELECT c.encrypted_value, u.pin_hash FROM credentials c 
       JOIN users u ON c.user_id = u.id 
       WHERE c.user_id = ? AND c.service = 'telegram_bot_token'`).bind(t.id).first();if(!s)return;const o=await B(s.encrypted_value,s.pin_hash);await fetch(`https://api.telegram.org/bot${o}/sendMessage`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({chat_id:t.telegram_chat_id,text:`📋 *Briefing*

${a}`,parse_mode:"Markdown",reply_markup:{inline_keyboard:r.map(i=>i.map(l=>({...l,callback_data:`${l.callback_data}:${n}`})))}})}),await e.prepare("UPDATE briefings SET delivered_telegram = 1 WHERE id = ?").bind(n).run()}catch(s){console.error("Telegram briefing error:",s.message)}}V.delete("/briefings/:id",er,async e=>{const t=e.get("user").id,a=e.req.param("id");return await e.env.DB.prepare("DELETE FROM briefings WHERE id = ? AND user_id = ?").bind(a,t).run(),e.json({success:!0})});const ee=new ve;ee.use("/api/*",Ur());ee.route("/api/auth",be);ee.route("/api/chat",j);ee.route("/api/settings",P);ee.route("/api/system",de);ee.route("/api/telegram",rt);ee.route("/api/proactive",V);ee.get("/auth/google/callback",async e=>{const t=new URL(e.req.url),a=t.searchParams.get("code"),r=t.searchParams.get("state"),n=t.searchParams.get("error");if(n)return e.html(Re(!1,`Google denied access: ${n}`));if(!a||!r)return e.html(Re(!1,"Missing authorization code or state parameter."));try{const o=JSON.parse(atob(r)).sessionId;if(!o)return e.html(Re(!1,"Invalid state parameter — missing session."));const i=await e.env.DB.prepare(`SELECT s.*, u.* FROM sessions s JOIN users u ON s.user_id = u.id 
       WHERE s.id = ? AND s.expires_at > datetime('now')`).bind(o).first();if(!i)return e.html(Re(!1,"Session expired. Please log in again and retry."));const l=i.user_id,c=i.pin_hash,u=`${t.protocol}//${t.host}/auth/google/callback`,h=await La(e.env.DB,l,c,a,u,e.env.GOOGLE_CLIENT_ID,e.env.GOOGLE_CLIENT_SECRET);return e.html(Re(!0,`Connected as ${h.email}`,h.email))}catch(s){return e.html(Re(!1,`OAuth failed: ${s.message}`))}});ee.get("/",e=>(e.header("Cache-Control","no-cache, no-store, must-revalidate"),e.header("Pragma","no-cache"),e.header("Expires","0"),e.html(_a())));ee.get("*",e=>e.req.path.startsWith("/api/")?e.json({error:"Not found"},404):(e.header("Cache-Control","no-cache, no-store, must-revalidate"),e.header("Pragma","no-cache"),e.header("Expires","0"),e.html(_a())));function Re(e,t,a){return`<!DOCTYPE html>
