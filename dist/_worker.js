...
`);for(const x of y)if(x.startsWith("data: ")){const _=x.slice(6);if(_==="[DONE]")continue;try{const E=JSON.parse(_);E.type==="content_block_delta"&&((f=E.delta)!=null&&f.text)&&c.enqueue(new TextEncoder().encode("data: "+JSON.stringify({text:E.delta.text})+`
...
`);for(const x of h)if(x.startsWith("data: ")){const _=x.slice(6);if(_==="[DONE]")continue;try{const d=(f=(y=(v=JSON.parse(_).choices)==null?void 0:v[0])==null?void 0:y.delta)==null?void 0:f.content;d&&i.enqueue(new TextEncoder().encode("data: "+JSON.stringify({text:d})+`

`))}catch{}}}})}}function kt(e,t,a,r){const n=Ke[e];if(!n)throw new Error(`Unknown LLM provider: ${e}`);const s=r||n.defaultModel;return n.apiFormat==="anthropic"?new ya(t,s,n.apiBase,a):new ba(t,s,n.apiBase,a)}class wa{constructor(){k(this,"errorLog",new Map);k(this,"usageLog",new Map)}async pickProvider(t){const a=Date.now(),r=t.filter(n=>{const s=this.errorLog.get(n);return s?s.cooldownUntil<=a:!0});return r.length>0?r[0]:null}async recordUsage(t,a){const r=this.usageLog.get(t)||{tokens:0,requests:0};this.usageLog.set(t,{tokens:r.tokens+a,requests:r.requests+1})}async recordError(t,a,r=5){this.errorLog.set(t,{error:a,cooldownUntil:Date.now()+r*60*1e3})}}const $r=["llm_slot_1","llm_slot_2","llm_slot_3"],Mr=["anthropic","openai"];async function et(e,t,a){const{decrypt:r}=await Promise.resolve().then(()=>Lr),n=new wa,s=[];for(const h of $r){const v=await e.prepare("SELECT encrypted_value, label FROM credentials WHERE user_id = ? AND service = ?").bind(t,h).first();if(v)try{const y=await r(v.encrypted_value,a),f=JSON.parse(y);if(f.provider&&f.apiKey&&Ke[f.provider]){const _=f.provider,E=kt(f.provider,f.apiKey,_,f.model);s.push({name:_,provider:E})}}catch(y){console.error(`Failed to load ${h}:`,y)}}const o=new Set(s.map(h=>h.name));for(const h of Mr){if(o.has(h))continue;const v=await e.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(t,h).first();if(v)try{const y=await r(v.encrypted_value,a);if(Ke[h]){const x=kt(h,y,h);s.push({name:h,provider:x})}}catch{console.error(`Failed to decrypt legacy ${h} key`)}}if(s.length===0)throw new Error("No LLM provider configured. Please add at least one API key in Settings → Keys.");const i=s.map(h=>h.name),l=await n.pickProvider(i);if(!l)return console.warn("All providers in cooldown, using first available"),{provider:s[0].provider,rotation:n};const c=s.find(h=>h.name===l);return{provider:Br(c.provider,s,n),rotation:n}}function Br(e,t,a){return t.length<=1?e:{name:e.name,async chat(r,n){try{return await e.chat(r,n)}catch(s){const o=s.message||"";if(!(o.includes("401")||o.includes("403")||o.includes("authentication")||o.includes("credit balance")||o.includes("invalid")&&o.includes("key")||o.includes("properties field not found")))throw s;console.warn(`Provider ${e.name} auth/billing error, trying fallback...`),await a.recordError(e.name,o,1440);const l=t.filter(c=>c.name!==e.name);for(const c of l)try{const u=await c.provider.chat(r,n);return this.name=c.name,u}catch(u){const h=u.message||"";if(h.includes("401")||h.includes("403")||h.includes("authentication")||h.includes("credit balance")||h.includes("properties field not found")){await a.recordError(c.name,h,1440);continue}throw u}throw new Error(`All LLM providers failed. Primary (${e.name}): ${o.substring(0,150)}. Check your API keys in Settings → Keys.`)}},async streamChat(r,n){return await e.streamChat(r,n)}}}const St=Object.freeze(Object.defineProperty({__proto__:null,ClaudeProvider:ya,OpenAICompatibleProvider:ba,ProviderRotation:wa,createProviderFromConfig:kt,createRotatingProvider:et,logError:C},Symbol.toStringTag,{value:"Module"})),wt=20,jr=2e3,Pr=2e3,_a=4;function Ur(e){return Math.ceil(e.length/_a)}function Ut(e,t){const a=t*_a;return e.length<=a?e:e.slice(0,a)+`
...
- Time-aware: reference current date/time when relevant`;default:return""}}const tn=2e3,an=2e3,za=4;function xt(e){return Math.ceil(e.length/za)}function Gt(e,t){const a=t*za;return e.length<=a?e:e.slice(0,a)+`
...
Note: Always use this date/time as the current time. Do NOT guess or use UTC.`}async function zt(e,t,a){var c;const n=await(await fetch(`https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(`name='${a.replace(/'/g,"\\'")}' and mimeType='application/vnd.google-apps.folder' and trashed=false`)}&fields=files(id,name)`,{headers:{Authorization:`Bearer ${e}`}})).json();let s;((c=n.files)==null?void 0:c.length)>0?s=n.files[0].id:s=(await(await fetch("https://www.googleapis.com/drive/v3/files",{method:"POST",headers:{Authorization:`Bearer ${e}`,"Content-Type":"application/json"},body:JSON.stringify({name:a,mimeType:"application/vnd.google-apps.folder"})})).json()).id;const l=((await(await fetch(`https://www.googleapis.com/drive/v3/files/${t}?fields=parents`,{headers:{Authorization:`Bearer ${e}`}})).json()).parents||[]).join(",");return await fetch(`https://www.googleapis.com/drive/v3/files/${t}?addParents=${s}&removeParents=${l}`,{method:"PATCH",headers:{Authorization:`Bearer ${e}`,"Content-Type":"application/json"},body:JSON.stringify({})}),{folderId:s,folderName:a}}function gt(e){try{const t=new Date;return new Intl.DateTimeFormat("en-US",{timeZone:e,weekday:"long",year:"numeric",month:"long",day:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:!0}).format(t)}catch{return new Date().toISOString()}}async function ft(e,t,a,r,n,s,o,i,l,c,u){var v,y,f,x,_,E;const h=new K(a);switch(e){case"create_schedule":{const d=new Date;let g;const p=c||"UTC";if(t.schedule_type==="interval"){const m=parseInt(t.schedule_value,10);g=new Date(d.getTime()+m*60*1e3)}else if(t.schedule_type==="daily"){const[m,w]=t.schedule_value.split(":").map(Number),T=d.toLocaleString("en-US",{timeZone:p}),S=new Date(T),I=new Date(S);I.setHours(m,w,0,0),I<=S&&I.setDate(I.getDate()+1);const A=new Date(I.toLocaleString("en-US",{timeZone:"UTC"})),O=new Date(I.toLocaleString("en-US",{timeZone:p})),N=A.getTime()-O.getTime();g=new Date(I.getTime()+N)}else if(t.schedule_type==="weekly"){const[m,w]=t.schedule_value.split(" "),[T,S]=(w||"00:00").split(":").map(Number),A=["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"].findIndex(Ie=>Ie.toLowerCase()===m.toLowerCase()),O=d.toLocaleString("en-US",{timeZone:p}),N=new Date(O),$=new Date(N);$.setHours(T,S,0,0);let H=(A-$.getDay()+7)%7;H===0&&$<=N&&(H=7),$.setDate($.getDate()+H);const ke=new Date($.toLocaleString("en-US",{timeZone:"UTC"})),te=new Date($.toLocaleString("en-US",{timeZone:p})),Se=ke.getTime()-te.getTime();g=new Date($.getTime()+Se)}else if(t.schedule_type==="once"){const[m,w]=t.schedule_value.split(" "),[T,S,I]=m.split("-").map(Number),[A,O]=(w||"00:00").split(":").map(Number),N=d.toLocaleString("en-US",{timeZone:p}),$=new Date(N),H=new Date($);H.setFullYear(T,S-1,I),H.setHours(A,O,0,0);const ke=new Date(H.toLocaleString("en-US",{timeZone:"UTC"})),te=new Date(H.toLocaleString("en-US",{timeZone:p})),Se=ke.getTime()-te.getTime();g=new Date(H.getTime()+Se)}else g=new Date(d.getTime()+3600*1e3);return await a.prepare(`INSERT INTO cron_jobs (user_id, name, description, schedule_type, schedule_value, action_type, action_config, next_run, state)
...
- Unread errors: ${(w==null?void 0:w.cnt)||0}`}case"read_sheet":{if(!n)return"Authentication context unavailable.";try{const d=new Q(a,r,n,s||"",o||""),g=t.spreadsheet_id;let p=t.range;const m=await d.sheets.getMetadata(g),w=m.sheets;p.includes("!")||(p=`${w[0]}!${p}`);let T;try{T=await d.sheets.readRange(g,p)}catch(I){if((v=I.message)!=null&&v.includes("Unable to parse range")||(y=I.message)!=null&&y.includes("400")){const A=p.includes("!")?p.split("!")[1]:p;p=`${w[0]}!${A}`,T=await d.sheets.readRange(g,p)}else throw I}let S=`[Spreadsheet: "${m.title}" | Reading tab: "${p.split("!")[0]}" | All tabs in this spreadsheet: ${w.map(I=>`"${I}"`).join(", ")}]
...
ID: ${p.spreadsheetId}
URL: ${p.url}`}catch(d){return await C(a,r,"google","create_sheet",d.message),`Failed to create spreadsheet: ${d.message}`}}case"list_calendar_events":{if(!n)return"Authentication context unavailable.";try{const d=new Q(a,r,n,s||"",o||""),g=t.calendar_id||"primary",p=t.days_ahead||7,m=new Date,w=new Date(m.getTime()+p*24*60*60*1e3),T=await d.calendar.listEvents(g,{timeMin:m.toISOString(),timeMax:w.toISOString(),query:t.query});return T.length===0?`No events found in the next ${p} days.`:T.map(S=>{var $;const I=S.start.dateTime||S.start.date||"TBD",A=S.end.dateTime||S.end.date||"",O=S.location?` 📍 ${S.location}`:"",N=(($=S.attendees)==null?void 0:$.map(H=>H.email).join(", "))||"";return`• ${S.summary} — ${I} to ${A}${O}${N?`
  Attendees: ${N}`:""}`}).join(`
`)}catch(d){return await C(a,r,"google","list_calendar",d.message),`Failed to list events: ${d.message}`}}case"create_calendar_event":{if(!n)return"Authentication context unavailable.";try{const d=new Q(a,r,n,s||"",o||""),g=t.calendar_id||"primary",p=await d.calendar.createEvent(g,{summary:t.summary,description:t.description,location:t.location,startDateTime:t.start_datetime,endDateTime:t.end_datetime,attendees:t.attendees});return`Event created: "${p.summary}"
ID: ${p.id}
...
ID: ${p.documentId}
URL: ${p.url}`}catch(d){return await C(a,r,"google","create_doc",d.message),`Failed to create document: ${d.message}`}}case"read_doc":{if(!n)return"Authentication context unavailable.";try{const g=await new Q(a,r,n,s||"",o||"").docs.readDocument(t.document_id);return`Document: "${g.title}"
...
`)}catch(d){return await C(a,r,"search","web_search",d.message),`Web search error: ${d.message}`}case"read_url":try{const d=t.url;if(!d||!d.startsWith("http://")&&!d.startsWith("https://"))return"Invalid URL. Please provide a full URL starting with http:// or https://";const g=Math.min(t.max_length||8e3,15e3),{fetchPageContent:p}=await Promise.resolve().then(()=>Zr),m=await p(d,g);return m.error?`Failed to read page: ${m.error}`:!m.text||m.text.length<20?`Page at ${d} returned no readable content.`:`Content from ${d} (${m.text.length} chars):

${m.text}`}catch(d){return await C(a,r,"search","read_url",d.message),`Read URL error: ${d.message}`}case"research":{if(!u)return"Research tool requires an LLM provider but none is available.";try{const g=Ua(t.query,u,{depth:t.depth||"quick",site:t.site}),p=new Promise(T=>setTimeout(()=>T(null),45e3)),m=await Promise.race([g,p]);if(m===null){const{webSearch:T}=await Promise.resolve().then(()=>Kr),S=await T(t.query,{num:5});if(S.error||S.results.length===0)return"Research timed out and fallback search returned no results. Try rephrasing or asking a more specific question.";let I=`Research took too long, but here are the top search results:
...
— ${T.author} (★${T.rating}, ${T.time}): "${T.text}"`}return w}catch(d){return await C(a,r,"google_api","place_details",d.message),`Place details error: ${d.message}`}}case"get_directions":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const g=await M(d.encrypted_value,n),p=await La(g,t.origin,t.destination,{mode:t.mode||"driving"});if(p.error)return`Directions failed: ${p.error}`;if(!p.route)return"No route found.";const m=p.route;let w=`**${m.startAddress}** → **${m.endAddress}**
`;return w+=`📏 ${m.distance} · ⏱️ ${m.duration}`,m.durationInTraffic&&(w+=` (with traffic: ${m.durationInTraffic})`),w+=`
via ${m.summary}`,w+=`

Steps:`,m.steps.forEach((T,S)=>{w+=`
${S+1}. ${T.instruction} (${T.distance}, ${T.duration})`}),w}catch(d){return await C(a,r,"google_api","directions",d.message),`Directions error: ${d.message}`}}case"get_travel_time":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const g=await M(d.encrypted_value,n),p=await Ba(g,t.origin,t.destination,t.mode||"driving");if(p.error)return`Travel time lookup failed: ${p.error}`;let m=`${t.origin} → ${t.destination}: ${p.distance}, ${p.duration}`;return p.durationInTraffic&&(m+=` (with traffic: ${p.durationInTraffic})`),m}catch(d){return await C(a,r,"google_api","travel_time",d.message),`Travel time error: ${d.message}`}}case"translate_text":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const g=await M(d.encrypted_value,n),p=await Na(g,t.text,t.target_language,t.source_language);return p.error?`Translation failed: ${p.error}`:`[${p.detectedSourceLang||t.source_language||"auto"} → ${t.target_language}]

${p.translatedText}`}catch(d){return await C(a,r,"google_api","translate",d.message),`Translation error: ${d.message}`}}case"search_youtube":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const g=await M(d.encrypted_value,n),p=await Ma(g,t.query,{maxResults:t.max_results||5,order:t.order||"relevance"});return p.error?`YouTube search failed: ${p.error}`:p.results.length===0?`No YouTube results for "${t.query}".`:p.results.map((m,w)=>{var T;return`${w+1}. **${m.title}**
   ${m.channelTitle} · ${((T=m.publishedAt)==null?void 0:T.split("T")[0])||""}
   ${m.description}
   ${m.url}`}).join(`

`)}catch(d){return await C(a,r,"google_api","youtube_search",d.message),`YouTube search error: ${d.message}`}}case"geocode_address":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const g=await M(d.encrypted_value,n),p=await $a(g,t.address);return p.error?`Geocoding failed: ${p.error}`:p.results.length===0?`Location not found: "${t.address}"`:p.results.map((m,w)=>`${w+1}. ${m.address}
   Coordinates: ${m.lat}, ${m.lng}`).join(`
`)}catch(d){return await C(a,r,"google_api","geocode",d.message),`Geocoding error: ${d.message}`}}default:return`Unknown tool: ${e}`}}async function Dt(e,t,a,r,n,s){var x;const o=new K(t),i=(x=e.metadata)==null?void 0:x.thread_id,l=await o.buildContext(r.id),c=await o.getRecentConversations(r.id,25,i),h=[{role:"system",content:Fa(r,l)},...c.map(_=>({role:_.role,content:_.content})),{role:"user",content:e.text}];await o.storeMessage(r.id,e.channel,"user",e.text,"{}",i);const v=10;let y="",f=0;for(let _=0;_<v;_++)try{const E=await a.chat(h,{tools:ht});if(E.usage&&(f+=E.usage.promptTokens+E.usage.completionTokens),E.toolCalls&&E.toolCalls.length>0){E.content&&h.push({role:"assistant",content:E.content});for(const d of E.toolCalls)try{const g=await ft(d.name,d.arguments,t,r.id,r.pin_hash,s==null?void 0:s.GOOGLE_CLIENT_ID,s==null?void 0:s.GOOGLE_CLIENT_SECRET,s==null?void 0:s.GOOGLE_API_KEY,s==null?void 0:s.GOOGLE_CSE_ID,r.timezone,a);h.push({role:"user",content:`[Tool Result for ${d.name}]: ${g}`})}catch(g){await C(t,r.id,"tool",d.name,g.message||"Tool execution failed"),h.push({role:"user",content:`[Tool Error for ${d.name}]: ${g.message||"Execution failed"}`})}continue}y=E.content;break}catch(E){if(n){const d=E.message||"",g=d.includes("401")||d.includes("403")||d.includes("authentication")||d.includes("credit balance"),p=d.includes("429"),m=g?1440:p?10:5;await n.recordError(a.name,d,m)}throw await C(t,r.id,"llm","provider_error",E.message||"Unknown LLM error",{provider:a.name,turn:_}),E}if(n&&f>0)try{await n.recordUsage(a.name,f)}catch{}return await o.storeMessage(r.id,e.channel,"assistant",y,"{}",i),await o.compactHistory(r.id,30),y}const Ft={"claude-sonnet-4-20250514":2e5,"claude-haiku-4-20250514":2e5,"gpt-4o":128e3,"gpt-4o-mini":128e3,"grok-3-mini":131072,"grok-3":131072,"deepseek-chat":64e3,"gemini-2.0-flash":1e6,default:32e3};function rn(e){for(const[t,a]of Object.entries(Ft))if(e.toLowerCase().includes(t.toLowerCase()))return a;return Ft.default}function nn(e,t,a,r){const n=rn(r),s=Math.floor(n*.75),o=[];let i=0,l=!1;const c=xt(e);o.push({role:"system",content:e}),i+=c;const u=xt(a);i+=u;const h=s-i,v=[];let y=0;for(let f=t.length-1;f>=0;f--){const x=t[f],_=xt(x.content);if(y+_<=h)v.unshift({role:x.role,content:x.content}),y+=_;else{l=!0;break}}return o.push(...v),o.push({role:"user",content:a}),i+=y,{maxTokens:n,usedTokens:i,messages:o,wasTruncated:l}}async function*sn(e,t,a,r,n,s){var _;const o=new K(t),i=(_=e.metadata)==null?void 0:_.thread_id;yield{type:"thinking",data:{threadId:i,provider:a.name}};const l=await o.buildContext(r.id),c=await o.getRecentConversations(r.id,20,i),u=Fa(r,l),h=nn(u,c,e.text,a.name);await o.storeMessage(r.id,e.channel,"user",e.text,"{}",i);const v=10;let y="",f=0;const x=[...h.messages];for(let E=0;E<v;E++)try{E>0&&(yield{type:"thinking",data:{threadId:i}});const d=await a.chat(x,{tools:ht});if(d.usage&&(f+=d.usage.promptTokens+d.usage.completionTokens),d.toolCalls&&d.toolCalls.length>0){d.content&&(yield{type:"chunk",data:{text:d.content,threadId:i}},x.push({role:"assistant",content:d.content}));for(const p of d.toolCalls){yield{type:"tool_start",data:{tool:p.name,toolArgs:p.arguments,threadId:i}};try{const m=await ft(p.name,p.arguments,t,r.id,r.pin_hash,s==null?void 0:s.GOOGLE_CLIENT_ID,s==null?void 0:s.GOOGLE_CLIENT_SECRET,s==null?void 0:s.GOOGLE_API_KEY,s==null?void 0:s.GOOGLE_CSE_ID,r.timezone,a);yield{type:"tool_end",data:{tool:p.name,toolResult:m.substring(0,500)+(m.length>500?"...":""),threadId:i}},x.push({role:"user",content:`[Tool Result for ${p.name}]: ${m}`})}catch(m){await C(t,r.id,"tool",p.name,m.message||"Tool execution failed"),yield{type:"tool_end",data:{tool:p.name,toolResult:`Error: ${m.message||"Execution failed"}`,threadId:i}},x.push({role:"user",content:`[Tool Error for ${p.name}]: ${m.message||"Execution failed"}`})}}continue}y=d.content;const g=50;for(let p=0;p<y.length;p+=g)yield{type:"chunk",data:{text:y.substring(p,p+g),threadId:i}},p+g<y.length&&await new Promise(w=>setTimeout(w,10));break}catch(d){if(n){const g=d.message||"",p=g.includes("401")||g.includes("403")||g.includes("authentication")||g.includes("credit balance"),m=g.includes("429"),w=p?1440:m?10:5;await n.recordError(a.name,g,w)}await C(t,r.id,"llm","provider_error",d.message||"Unknown LLM error",{provider:a.name,turn:E}),yield{type:"error",data:{error:d.message||"An error occurred",threadId:i}};return}if(n&&f>0)try{await n.recordUsage(a.name,f)}catch{}await o.storeMessage(r.id,e.channel,"assistant",y,"{}",i),await o.compactHistory(r.id,30),yield{type:"done",data:{threadId:i,provider:a.name,tokenCount:f}}}async function Wa(e,t,a,r,n,s){var u;const o=new K(t),i=(u=e.metadata)==null?void 0:u.thread_id,l=await o.buildContext(r.id),c=Ha(e.text,l);if(c.agent==="conversation")return ln(e,t,a,r,l,n,i);if(c.agent==="multi"||c.confidence<.5)return Dt(e,t,a,r,n,s);try{return await on(c.agent,e,t,a,r,l,n,s,i)}catch(h){return await C(t,r.id,"router","subagent_fallback",`${c.agent} failed: ${h.message}`,{route:c}),Dt(e,t,a,r,n,s)}}async function on(e,t,a,r,n,s,o,i,l){const c=new K(a),u=gt(n.timezone),h=ct(e,n,s,n.timezone,u),v=Ga(e,ht),y=await c.getRecentConversations(n.id,25,l),f=[{role:"system",content:h},...y.map(d=>({role:d.role,content:d.content})),{role:"user",content:t.text}];await c.storeMessage(n.id,t.channel,"user",t.text,"{}",l);const x=10;let _="",E=0;for(let d=0;d<x;d++)try{const g=await r.chat(f,{tools:v.length>0?v:void 0});if(g.usage&&(E+=g.usage.promptTokens+g.usage.completionTokens),g.toolCalls&&g.toolCalls.length>0){g.content&&f.push({role:"assistant",content:g.content});for(const p of g.toolCalls)try{const m=await ft(p.name,p.arguments,a,n.id,n.pin_hash,i==null?void 0:i.GOOGLE_CLIENT_ID,i==null?void 0:i.GOOGLE_CLIENT_SECRET,i==null?void 0:i.GOOGLE_API_KEY,i==null?void 0:i.GOOGLE_CSE_ID,n.timezone,r);f.push({role:"user",content:`[Tool Result for ${p.name}]: ${m}`})}catch(m){await C(a,n.id,"tool",p.name,m.message||"Tool execution failed"),f.push({role:"user",content:`[Tool Error for ${p.name}]: ${m.message||"Execution failed"}`})}continue}_=g.content;break}catch(g){if(o){const p=g.message||"",m=p.includes("401")||p.includes("403")||p.includes("authentication")||p.includes("credit balance"),w=p.includes("429"),T=m?1440:w?10:5;await o.recordError(r.name,p,T)}throw await C(a,n.id,"llm","subagent_error",g.message||"Unknown error",{agent:e,provider:r.name,turn:d}),g}if(o&&E>0)try{await o.recordUsage(r.name,E)}catch{}return await c.storeMessage(n.id,t.channel,"assistant",_,"{}",l),await c.compactHistory(n.id,30),_}async function ln(e,t,a,r,n,s,o){const i=new K(t),l=gt(r.timezone),c=ct("conversation",r,n,r.timezone,l),u=await i.getRecentConversations(r.id,25,o),h=[{role:"system",content:c},...u.map(f=>({role:f.role,content:f.content})),{role:"user",content:e.text}];await i.storeMessage(r.id,e.channel,"user",e.text,"{}",o);let v=0,y="";try{const f=await a.chat(h,{temperature:.8});f.usage&&(v=f.usage.promptTokens+f.usage.completionTokens),y=f.content}catch(f){if(s){const x=f.message||"",_=x.includes("401")||x.includes("403")||x.includes("authentication")||x.includes("credit balance"),E=x.includes("429"),d=_?1440:E?10:5;await s.recordError(a.name,x,d)}throw await C(t,r.id,"llm","conversation_error",f.message,{provider:a.name}),f}if(s&&v>0)try{await s.recordUsage(a.name,v)}catch{}return await i.storeMessage(r.id,e.channel,"assistant",y,"{}",o),await i.compactHistory(r.id,30),y}async function*cn(e,t,a,r,n,s){var d;const o=new K(t),i=(d=e.metadata)==null?void 0:d.thread_id,l=await o.buildContext(r.id),c=Ha(e.text,l);if(yield{type:"thinking",data:{threadId:i,provider:a.name}},c.agent==="multi"||c.confidence<.5){yield*sn(e,t,a,r,n,s);return}const u=gt(r.timezone),h=c.agent==="conversation"?ct("conversation",r,l,r.timezone,u):ct(c.agent,r,l,r.timezone,u),v=Ga(c.agent,ht),y=await o.getRecentConversations(r.id,25,i),f=[{role:"system",content:h},...y.map(g=>({role:g.role,content:g.content})),{role:"user",content:e.text}];await o.storeMessage(r.id,e.channel,"user",e.text,"{}",i);const x=10;let _="",E=0;for(let g=0;g<x;g++)try{g>0&&(yield{type:"thinking",data:{threadId:i}});const p=await a.chat(f,{tools:v.length>0?v:void 0});if(p.usage&&(E+=p.usage.promptTokens+p.usage.completionTokens),p.toolCalls&&p.toolCalls.length>0){p.content&&(yield{type:"chunk",data:{text:p.content,threadId:i}},f.push({role:"assistant",content:p.content}));for(const w of p.toolCalls){yield{type:"tool_start",data:{tool:w.name,toolArgs:w.arguments,threadId:i}};try{const T=await ft(w.name,w.arguments,t,r.id,r.pin_hash,s==null?void 0:s.GOOGLE_CLIENT_ID,s==null?void 0:s.GOOGLE_CLIENT_SECRET,s==null?void 0:s.GOOGLE_API_KEY,s==null?void 0:s.GOOGLE_CSE_ID,r.timezone,a);yield{type:"tool_end",data:{tool:w.name,toolResult:T.substring(0,500)+(T.length>500?"...":""),threadId:i}},f.push({role:"user",content:`[Tool Result for ${w.name}]: ${T}`})}catch(T){await C(t,r.id,"tool",w.name,T.message||"Tool execution failed"),yield{type:"tool_end",data:{tool:w.name,toolResult:`Error: ${T.message||"Execution failed"}`,threadId:i}},f.push({role:"user",content:`[Tool Error for ${w.name}]: ${T.message||"Execution failed"}`})}}continue}_=p.content;const m=50;for(let w=0;w<_.length;w+=m)yield{type:"chunk",data:{text:_.substring(w,w+m),threadId:i}},w+m<_.length&&await new Promise(S=>setTimeout(S,10));break}catch(p){if(n){const m=p.message||"",w=m.includes("401")||m.includes("403")||m.includes("authentication")||m.includes("credit balance"),T=m.includes("429"),S=w?1440:T?10:5;await n.recordError(a.name,m,S)}await C(t,r.id,"llm","subagent_stream_error",p.message||"Unknown error",{agent:c.agent,provider:a.name,turn:g}),yield{type:"error",data:{error:p.message||"An error occurred",threadId:i}};return}if(n&&E>0)try{await n.recordUsage(a.name,E)}catch{}await o.storeMessage(r.id,e.channel,"assistant",_,"{}",i),await o.compactHistory(r.id,30),yield{type:"done",data:{threadId:i,provider:a.name,tokenCount:E}}}const B=new ge;async function dn(e,t){var n;const a=(n=e.req.header("Authorization"))==null?void 0:n.replace("Bearer ","");if(!a)return e.json({error:"Authentication required"},401);const r=await e.env.DB.prepare(`SELECT s.*, u.* FROM sessions s JOIN users u ON s.user_id = u.id 
...
`;for(const c of s)o+=`- ${c.name} (${c.type}, ${Math.round(c.size/1024)}KB, file_id: ${c.file_id})`,c.text_preview&&(o+=`
  Preview: ${c.text_preview.substring(0,300)}...`),o+=`
`;o+="Use parse_document tool to read full file contents. Use drive_upload to upload to Google Drive.]"}let i=n;if(!i){const c=await e.env.DB.prepare("INSERT INTO threads (user_id, title) VALUES (?, ?) RETURNING *").bind(t.id,a.trim().substring(0,60)+(a.trim().length>60?"...":"")).first();i=c==null?void 0:c.id}const l={userId:t.id,username:t.username,channel:r,text:a.trim()+o,sessionId:e.get("sessionId"),timestamp:new Date().toISOString(),metadata:{thread_id:i}};try{const{provider:c,rotation:u}=await et(e.env.DB,t.id,t.pin_hash),h=new ReadableStream({async start(v){const y=new TextEncoder;try{const f=cn(l,e.env.DB,c,t,u,{GOOGLE_CLIENT_ID:e.env.GOOGLE_CLIENT_ID,GOOGLE_CLIENT_SECRET:e.env.GOOGLE_CLIENT_SECRET,GOOGLE_API_KEY:e.env.GOOGLE_API_KEY,GOOGLE_CSE_ID:e.env.GOOGLE_CSE_ID});for await(const x of f)x.data.threadId||(x.data.threadId=i),v.enqueue(y.encode(Wt(x)));i&&await e.env.DB.prepare("UPDATE threads SET message_count = message_count + 2, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(i).run(),v.close()}catch(f){const x={type:"error",data:{error:f.message||"An error occurred",threadId:i}};v.enqueue(y.encode(Wt(x))),v.close()}}});return new Response(h,{headers:{"Content-Type":"text/event-stream","Cache-Control":"no-cache",Connection:"keep-alive","X-Thread-Id":String(i||"")}})}catch(c){console.error("Stream setup error:",c);const u=c.message||"";return u.includes("No LLM provider configured")?e.json({error:"No AI provider configured. Please add at least one API key in Settings → Keys.",type:"no_provider",thread_id:i},400):u.includes("limit reached")?e.json({error:u,type:"cost_limit",thread_id:i},429):e.json({error:"Something went wrong setting up the stream.",details:u,thread_id:i},500)}});B.get("/threads/:id/messages",async e=>{var s;const t=e.get("user"),a=parseInt(e.req.param("id")),r=parseInt(e.req.query("limit")||"50"),n=await e.env.DB.prepare(`SELECT id, role, content, channel, created_at FROM conversations 
