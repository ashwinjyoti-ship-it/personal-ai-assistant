...
- **HARD RULE — no false confirmations**: You have ZERO tool access. You CANNOT set reminders, create schedules, send emails, read sheets, or perform any action. NEVER output phrases like "Reminder set for...", "I've scheduled...", "Done ✅", "Task created", or any language implying an action was completed. If the user wants an action, say: "I can do that — just send your message and I'll take care of it." Do NOT simulate the outcome.`;default:return""}}const ws=Object.freeze(Object.defineProperty({__proto__:null,buildSubAgentPrompt:jn,classifyIntentFast:dr,detectDeterministicOp:$n,detectTierTwoOp:Bn},Symbol.toStringTag,{value:"Module"})),bs=2e3,_s=2e3,Pn=4;function Xt(e){return Math.ceil(e.length/Pn)}function kr(e,t){const r=t*Pn;return e.length<=r?e:e.slice(0,r)+`
...
`,bs):"",i=n!=null&&n.trim()?`## Your Standing Instructions (User-Set — Do Not Duplicate in Memory)
...
${n}
`:"",o=kr(t,_s);return`You are ${a} — a personal AI assistant. Your name is ${a} — always refer to yourself by this name if asked.
...
[...truncated in history to reduce context size]`})}}function Es(e){const t=[];let r=[],n="",a=!1,s=0;const i=e.length;for(;s<i;){const o=e[s];if(a){if(o==='"'){if(e[s+1]==='"'){n+='"',s+=2;continue}a=!1,s++;continue}n+=o,s++;continue}if(o==='"'){a=!0,s++;continue}if(o===","){r.push(n),n="",s++;continue}if(o==="\r"&&e[s+1]===`
`){r.push(n),t.push(r),r=[],n="",s+=2;continue}if(o===`
`||o==="\r"){r.push(n),t.push(r),r=[],n="",s++;continue}n+=o,s++}for((n||r.length)&&(r.push(n),t.push(r));t.length&&t[t.length-1].every(o=>o==="");)t.pop();return t}async function Ts(e,t,r,n,a,s,i,o,l,c,d,m,h){var y,_,w,T,E,R,N,j,G,F,P,q,z,O,W,L;const v=new J(r);switch(e){case"create_schedule":{const u=new Date;let f;const p=c||"UTC";if(t.minutes_from_now&&typeof t.minutes_from_now=="number"&&t.minutes_from_now>0){f=new Date(u.getTime()+t.minutes_from_now*60*1e3);const D=f.toLocaleString("en-US",{timeZone:p,hour12:!1,year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"}).split(", "),[I,M,C]=(D[0]||"").split("/");t.schedule_value=`${C}-${I}-${M} ${D[1]||"00:00"}`,t.schedule_type="once"}else if(t.schedule_type==="interval"){const S=parseInt(t.schedule_value,10);f=new Date(u.getTime()+S*60*1e3)}else if(t.schedule_type==="daily"&&t.action_type==="reminder"){const S=`${t.name||""} ${t.action_description||""}`.toLowerCase();if(/\bevery\b|\bdaily\b|\beach\b|\bmorning\b|\bevening\b|\bnight\b|\bweekday\b|\bweekend\b|\brecurring\b|\brepeat\b/.test(S)){const[I,M]=t.schedule_value.split(":").map(Number),C=u.toLocaleString("en-US",{timeZone:p}),A=new Date(C),B=new Date(A);B.setHours(I,M,0,0),B<=A&&B.setDate(B.getDate()+1);const V=new Date(B.toLocaleString("en-US",{timeZone:"UTC"})),X=new Date(B.toLocaleString("en-US",{timeZone:p})),ee=V.getTime()-X.getTime();f=new Date(B.getTime()+ee)}else{const[I,M]=t.schedule_value.split(":").map(Number),C=u.toLocaleString("en-US",{timeZone:p}),A=new Date(C),B=new Date(A);B.setHours(I,M,0,0),B<=A&&B.setDate(B.getDate()+1);const V=gt=>String(gt).padStart(2,"0"),X=B.getFullYear(),ee=V(B.getMonth()+1),te=V(B.getDate());t.schedule_value=`${X}-${ee}-${te} ${V(I)}:${V(M)}`,t.schedule_type="once";const ae=new Date(B.toLocaleString("en-US",{timeZone:"UTC"})),de=new Date(B.toLocaleString("en-US",{timeZone:p})),Ne=ae.getTime()-de.getTime();f=new Date(B.getTime()+Ne)}}else if(t.schedule_type==="daily"){const[S,D]=t.schedule_value.split(":").map(Number),I=u.toLocaleString("en-US",{timeZone:p}),M=new Date(I),C=new Date(M);C.setHours(S,D,0,0),C<=M&&C.setDate(C.getDate()+1);const A=new Date(C.toLocaleString("en-US",{timeZone:"UTC"})),B=new Date(C.toLocaleString("en-US",{timeZone:p})),V=A.getTime()-B.getTime();f=new Date(C.getTime()+V)}else if(t.schedule_type==="weekly"){const[S,D]=t.schedule_value.split(" "),[I,M]=(D||"00:00").split(":").map(Number),A=["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"].findIndex(Ne=>Ne.toLowerCase()===S.toLowerCase()),B=u.toLocaleString("en-US",{timeZone:p}),V=new Date(B),X=new Date(V);X.setHours(I,M,0,0);let ee=(A-X.getDay()+7)%7;ee===0&&X<=V&&(ee=7),X.setDate(X.getDate()+ee);const te=new Date(X.toLocaleString("en-US",{timeZone:"UTC"})),ae=new Date(X.toLocaleString("en-US",{timeZone:p})),de=te.getTime()-ae.getTime();f=new Date(X.getTime()+de)}else if(t.schedule_type==="once"){const[S,D]=t.schedule_value.split(" "),[I,M,C]=S.split("-").map(Number),[A,B]=(D||"00:00").split(":").map(Number),V=u.toLocaleString("en-US",{timeZone:p}),X=new Date(V),ee=new Date(X);ee.setFullYear(I,M-1,C),ee.setHours(A,B,0,0);const te=new Date(ee.toLocaleString("en-US",{timeZone:"UTC"})),ae=new Date(ee.toLocaleString("en-US",{timeZone:p})),de=te.getTime()-ae.getTime();f=new Date(ee.getTime()+de);const Ne=new Date(u.getTime()+120*1e3);if(f.getTime()<u.getTime()+60*1e3){const gt=f.toISOString();f=Ne;const ft=` [Note: The requested time ${t.schedule_value} in ${p} resolved to ${gt} UTC which is in the past. Auto-adjusted to fire in ~2 minutes at ${f.toISOString()}.]`;t._pastTimeWarning=ft}}else f=new Date(u.getTime()+3600*1e3);if(t.action_type==="custom"&&t.schedule_type==="once"){const S=`${t.name||""} ${t.action_description||t.description||""}`.toLowerCase();/\b(check|search|look\s*up|read|fetch|find|verify|track|scan|review|query|pull|get)\b/i.test(S)||(t.action_type="reminder")}if(await r.prepare("SELECT id FROM cron_jobs WHERE user_id = ? AND name = ? AND schedule_type = ? AND schedule_value = ? AND state != 'completed' LIMIT 1").bind(n,t.name,t.schedule_type,t.schedule_value).first()){const S=f.toLocaleString("en-US",{timeZone:p,weekday:"short",year:"numeric",month:"short",day:"numeric",hour:"2-digit",minute:"2-digit",hour12:!0});return`Schedule already exists: "${t.name}" is already set for ${S} (${p}). No duplicate created.`}await r.prepare(`INSERT INTO cron_jobs (user_id, name, description, schedule_type, schedule_value, action_type, action_config, next_run, state)
...
- Unread errors: ${(b==null?void 0:b.cnt)||0}`}case"read_sheet":{if(!a)return"Authentication context unavailable.";try{const u=new ue(r,n,a,s||"",i||""),f=t.spreadsheet_id;let p=t.range;const g=await u.sheets.getMetadata(f),b=g.sheets;p.includes("!")||(p=`${b[0]}!${p}`);let x;try{x=await u.sheets.readRange(f,p)}catch(D){if((y=D.message)!=null&&y.includes("Unable to parse range")||(_=D.message)!=null&&_.includes("400")){const I=p.includes("!")?p.split("!")[1]:p;p=`${b[0]}!${I}`,x=await u.sheets.readRange(f,p)}else throw D}let S=`[Spreadsheet: "${g.title}" | Reading tab: "${p.split("!")[0]}" | All tabs in this spreadsheet: ${b.map(D=>`"${D}"`).join(", ")}]
...
${S.substring(0,2e4)}`}catch{return`Could not decode text file: ${g}`}if(b==="application/pdf"||b==="application/vnd.openxmlformats-officedocument.wordprocessingml.document"||g.toLowerCase().endsWith(".pdf")||g.toLowerCase().endsWith(".docx")){if(b==="application/vnd.openxmlformats-officedocument.wordprocessingml.document"||g.toLowerCase().endsWith(".docx")){try{const I=await Ln(Buffer.from(x,"base64"));if(I.length>50)return`Document: ${g}

${I.substring(0,2e4)}`}catch{}return`Could not extract text from "${g}". Try uploading to Google Drive and sharing the link — Drive can open and export Word documents directly.`}let S=null,D="claude-haiku-4-5-20251001";for(const I of["llm_slot_1","llm_slot_2","llm_slot_3"])try{const M=await r.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(n,I).first();if(M&&a){const C=await Y(M.encrypted_value,a),A=JSON.parse(C);if(A.provider==="anthropic"){S=A.apiKey,A.model&&(D=A.model);break}}}catch{}if(S)try{const I=f?`Focus specifically on extracting: ${f}`:"Extract and return all readable text content from this document. Preserve structure where relevant.",M=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"x-api-key":S,"anthropic-version":"2023-06-01","anthropic-beta":"pdfs-2024-09-25","content-type":"application/json"},body:JSON.stringify({model:D,max_tokens:4096,messages:[{role:"user",content:[{type:"document",source:{type:"base64",media_type:"application/pdf",data:x}},{type:"text",text:I}]}]})});if(M.ok){const A=((P=(F=(await M.json()).content)==null?void 0:F[0])==null?void 0:P.text)||"";return`Document: ${g}

${A}`}else{const C=await M.text();return`Could not parse ${g} via Anthropic API: ${C.substring(0,200)}`}}catch(I){return`Document parsing error for ${g}: ${I.message}`}return"To parse PDF documents, please configure an Anthropic API key in Settings → Keys. No Anthropic key is currently set."}try{const S=Buffer.from(x,"base64").toString("utf-8").substring(0,2e3);return`Document: ${g} (${b})

Content preview:
${S}`}catch{return`Cannot read file: ${g} (${b})`}}case"search_library":{const u=t.query,f=Math.min(typeof t.limit=="number"?t.limit:10,20);if(!u)return"query is required for search_library.";if(h!=null&&h.ai&&(h!=null&&h.vectorize))try{const{semanticDocumentSearch:S}=await Promise.resolve().then(()=>St),D=await S({DB:r,AI:h.ai,VECTORIZE:h.vectorize},n,u,f);if(D.length>0){const I=D.map(M=>`[id:${M.document_id}] "${M.filename}" (relevance: ${(M.relevance_score*100).toFixed(1)}%)
...
${g.substring(0,2e4)}`:`Document "${p.name}" has no extracted text yet. Ask Karna to parse it with parse_document(file_id="${p.file_id}") to extract the text first.`}case"create_skill":{const u=(q=t.name)==null?void 0:q.trim(),f=(z=t.description)==null?void 0:z.trim(),p=(O=t.instructions)==null?void 0:O.trim();if(!u||!f||!p)return"create_skill requires name, description, and instructions.";let g=u.toLowerCase().replace(/[^a-z0-9\s_-]/g,"").replace(/\s+/g,"_").replace(/_+/g,"_").substring(0,50).replace(/^_|_$/g,"");g||(g=`skill_${Date.now()}`);const b=await r.prepare("SELECT slug FROM user_skills WHERE user_id = ? AND slug LIKE ?").bind(n,`${g}%`).all();(W=b.results)!=null&&W.some(I=>I.slug===g)&&(g=`${g}_${(((L=b.results)==null?void 0:L.length)||0)+1}`);const x=JSON.stringify(t.parameters||{}),S=JSON.stringify(t.required_tools||[]),D=JSON.stringify(t.examples||[]);return await r.prepare(`INSERT INTO user_skills (user_id, name, slug, description, instructions, parameters, required_tools, examples)
...
${a}`:a,h=jn("conversation",n,m,n.timezone,c,e.channel),v=(await o.getRecentConversations(n.id,30,i)).filter(E=>!E.content.startsWith("[Autonomous Scheduled Task]")&&!E.content.startsWith("[Scheduled Reminder]")),y=Un([{role:"system",content:h},...v.map(E=>({role:E.role,content:E.content})),{role:"user",content:e.text}]);await o.storeMessage(n.id,e.channel,"user",e.text,"{}",i);let _=0,w="";try{const E=await r.chat(y,{temperature:.8});E.usage&&(_=E.usage.promptTokens+E.usage.completionTokens),w=E.content}catch(E){if(s){const R=E.message||"",N=R.includes("401")||R.includes("403")||R.includes("authentication")||R.includes("credit balance"),j=R.includes("429"),G=N?1440:j?10:5;await s.recordError(r.name,R,G)}throw await H(t,n.id,"llm","conversation_error",E.message,{provider:r.name}),E}if(s&&_>0)try{await s.recordUsage(r.name,_)}catch{}try{await t.prepare("INSERT INTO llm_calls (user_id, provider_name, agent_type, tokens_used, latency_ms, success, channel) VALUES (?, ?, ?, ?, ?, ?, ?)").bind(n.id,r.name,"conversation",_,Date.now()-l,1,e.channel).run()}catch{}const T=Pt(w);return await o.storeMessage(n.id,e.channel,"assistant",T,"{}",i),T}async function*Rs(e,t,r,n,a,s){var d;const i=new J(t),o=(d=e.metadata)==null?void 0:d.thread_id,l=await i.buildContext(n.id),c=dr(e.text,l);if(yield{type:"thinking",data:{threadId:o,provider:r.name}},c.agent!=="conversation"){yield*Ds(e,t,r,n,a,s);return}try{const m=await zn(e,t,r,n,l,a,o),h=50;for(let v=0;v<m.length;v+=h)yield{type:"chunk",data:{text:m.substring(v,v+h),threadId:o}},v+h<m.length&&await new Promise(y=>setTimeout(y,10))}catch(m){yield{type:"error",data:{error:m.message||"An error occurred",threadId:o}};return}yield{type:"done",data:{threadId:o,provider:r.name,tokenCount:0}}}const ne=new ge;async function Os(e,t){var a;const r=(a=e.req.header("Authorization"))==null?void 0:a.replace("Bearer ","");if(!r)return e.json({error:"Authentication required"},401);const n=await e.env.DB.prepare(`SELECT s.*, u.* FROM sessions s JOIN users u ON s.user_id = u.id 
     WHERE s.id = ? AND s.expires_at > datetime('now')`).bind(r).first();if(!n)return e.json({error:"Invalid session"},401);e.set("user",{id:n.user_id,username:n.username,name:n.name,pin_hash:n.pin_hash,role:n.role,personality_prompt:n.personality_prompt,telegram_chat_id:n.telegram_chat_id,timezone:n.timezone,assistant_name:n.assistant_name||"Karna",created_at:n.created_at,updated_at:n.updated_at}),e.set("sessionId",r),await t()}ne.use("/*",Os);ne.get("/threads",async e=>{const t=e.get("user"),r=e.req.query("archived")==="1",n=parseInt(e.req.query("limit")||"30"),a=await e.env.DB.prepare(`SELECT t.*, 
...
`;for(const c of s)i+=`- ${c.name} (${c.type}, ${Math.round(c.size/1024)}KB, file_id: ${c.file_id})`,c.text_preview&&(i+=`
  Preview: ${c.text_preview.substring(0,300)}...`),i+=`
`;i+="Use parse_document tool to read full file contents. Use drive_upload to upload to Google Drive.]"}let o=a;if(!o){const c=await e.env.DB.prepare("INSERT INTO threads (user_id, title) VALUES (?, ?) RETURNING *").bind(t.id,r.trim().substring(0,60)+(r.trim().length>60?"...":"")).first();o=c==null?void 0:c.id}const l={userId:t.id,username:t.username,channel:n,text:r.trim()+i,sessionId:e.get("sessionId"),timestamp:new Date().toISOString(),metadata:{thread_id:o}};try{const{provider:c,rotation:d}=await Ve(e.env.DB,t.id,t.pin_hash),m=new ReadableStream({async start(h){const v=new TextEncoder;try{const y=Rs(l,e.env.DB,c,t,d,{GOOGLE_CLIENT_ID:e.env.GOOGLE_CLIENT_ID,GOOGLE_CLIENT_SECRET:e.env.GOOGLE_CLIENT_SECRET,GOOGLE_API_KEY:e.env.GOOGLE_API_KEY,GOOGLE_CSE_ID:e.env.GOOGLE_CSE_ID,DOCUMENTS_BUCKET:e.env.DOCUMENTS_BUCKET});for await(const _ of y)_.data.threadId||(_.data.threadId=o),h.enqueue(v.encode(Nr(_)));o&&await e.env.DB.prepare("UPDATE threads SET message_count = message_count + 2, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(o).run(),h.close()}catch(y){const _={type:"error",data:{error:y.message||"An error occurred",threadId:o}};h.enqueue(v.encode(Nr(_))),h.close()}}});return new Response(m,{headers:{"Content-Type":"text/event-stream","Cache-Control":"no-cache",Connection:"keep-alive","X-Thread-Id":String(o||"")}})}catch(c){console.error("Stream setup error:",c);const d=c.message||"";return d.includes("No LLM provider configured")?e.json({error:"No AI provider configured. Please add at least one API key in Settings → Keys.",type:"no_provider",thread_id:o},400):d.includes("429")||d.includes("limit reached")||d.includes("rate limit")||d.includes("Too Many Requests")?e.json({error:"Rate limit reached across all configured providers. Add a second AI provider in Settings → API Keys (Gemini and DeepSeek offer free tiers) to enable automatic failover and avoid this.",type:"rate_limit",thread_id:o},429):e.json({error:"Something went wrong setting up the stream.",details:d,thread_id:o},500)}});ne.get("/threads/:id/messages",async e=>{var s;const t=e.get("user"),r=parseInt(e.req.param("id")),n=parseInt(e.req.query("limit")||"50"),a=await e.env.DB.prepare(`SELECT id, role, content, channel, created_at FROM conversations 
...
- Example: "Couldn't retrieve Amazon order status — requires login."`;function Ns(e,t,r){return r==="reminder"?`[Scheduled Reminder] "${e}": ${t||"Time for your reminder."}`:r==="check_mail"?`[Autonomous Scheduled Task] Execute this task NOW using tools — do NOT just describe what you'd do.
Task: "${e}"
...
${h}`),a++}catch{}}}catch{}return e.json({checked:n,notified:a})});function Is(e,t,r,n){return{userId:e,username:t,channel:"telegram",text:r,sessionId:`telegram-${n}`,timestamp:new Date().toISOString()}}function As(e,t){return e.replace(/\*\*(.*?)\*\*/gs,"*$1*").replace(/^#{1,3}\s+(.+)$/gm,"*$1*").replace(/\[([^\]]+)\]\([^)]+\)/g,"$1").replace(/```(\w*)\n([\s\S]*?)```/g,"```$2```")}const Nt=new ge,Ls=4e3;async function re(e,t,r,n="Markdown",a,s){var c,d;const i=$s(r,Ls),o=[];let l=!0;for(let m=0;m<i.length;m++){const h=i[m];let v=!1,y="";for(let _=0;_<3;_++)try{const w=await fetch(`https://api.telegram.org/bot${e}/sendMessage`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({chat_id:t,text:h,parse_mode:n,disable_web_page_preview:!1})});if(w.ok){v=!0;break}const T=await w.json().catch(()=>null);if(y=`HTTP ${w.status}: ${(T==null?void 0:T.description)||"Unknown error"}`,(c=T==null?void 0:T.description)!=null&&c.includes("parse")||(d=T==null?void 0:T.description)!=null&&d.includes("entities")){if((await fetch(`https://api.telegram.org/bot${e}/sendMessage`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({chat_id:t,text:h})})).ok){v=!0;break}y+=" (plain-text retry also failed)"}if(w.status===429||w.status>=500){const E=Math.pow(2,_)*1e3;await new Promise(R=>setTimeout(R,E));continue}break}catch(w){if(y=`Network error: ${w.message}`,_<2){const T=Math.pow(2,_)*1e3;await new Promise(E=>setTimeout(E,T));continue}}v||(l=!1,o.push(`Chunk ${m+1}/${i.length}: ${y}`))}if(!l&&a&&s&&o.length>0)try{const{logError:m}=await Promise.resolve().then(()=>st);await m(a,s,"telegram","send_failed",o.join(" | "))}catch{}return{success:l,errors:o}}async function Ms(e,t){try{await fetch(`https://api.telegram.org/bot${e}/sendChatAction`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({chat_id:t,action:"typing"})})}catch{}}function $s(e,t){if(e.length<=t)return[e];const r=[];let n=e;for(;n.length>0;){if(n.length<=t){r.push(n);break}let a=n.lastIndexOf(`
`,t);a<t*.3&&(a=n.lastIndexOf(" ",t)),a<t*.3&&(a=t),r.push(n.substring(0,a)),n=n.substring(a).trimStart()}return r}function Bs(e){const t=Date.now()-new Date(e).getTime(),r=Math.floor(t/6e4);if(r<1)return"just now";if(r<60)return`${r}m ago`;const n=Math.floor(r/60);return n<24?`${n}h ago`:`${Math.floor(n/24)}d ago`}async function js(e,t,r){const a={reminder:"⏰",mail:"✉️",calendar:"📅",error:"⚠️",system:"⚙️"}[r.type]||"🔔",s={daily:"📅 Daily",weekly:"📅 Weekly",once:"✓ Once"},i=r.schedule_type?` · _${s[r.schedule_type]||"⏱ Repeating"}_`:"",o=r.body?`
`+r.body.substring(0,150)+(r.body.length>150?"…":""):"",l=`${a} *${r.title}*${i}
_${Bs(r.created_at)}_${o}`,c=Jn(r.id);(await fetch(`https://api.telegram.org/bot${e}/sendMessage`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({chat_id:t,text:l,parse_mode:"Markdown",reply_markup:{inline_keyboard:c}})})).ok||await fetch(`https://api.telegram.org/bot${e}/sendMessage`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({chat_id:t,text:l.replace(/[_*`[\]]/g,""),reply_markup:{inline_keyboard:c}})})}async function Ps(e,t,r,n,a){switch(e.split("@")[0].toLowerCase()){case"/start":{const i=(n==null?void 0:n.name)||"there",o=(n==null?void 0:n.assistant_name)||"Karna",l=`👋 *Hello, ${i}!*
...
For long essays, please use the web app — it handles long generation without time limits.`:(c=O.message)!=null&&c.includes("API error")?`⚠️ AI provider returned an error. The provider (${G.name}) may be temporarily unavailable. Your message was saved — try again shortly.`:`⚠️ Something went wrong processing your message. Error: ${(O.message||"Unknown").substring(0,200)}`,u=await re(R,w,L,"Markdown",r,E.id);z=u.success,u.success||console.error(`[CRITICAL] Error message failed to send to Telegram for user ${E.id}:`,u.errors);try{const{logError:f}=await Promise.resolve().then(()=>st);await f(r,E.id,"telegram",W?"timeout":"agent_error",O.message||"Agent error",{provider:G.name})}catch{}}}catch(d){console.error("Telegram webhook error:",d);try{const{logError:m}=await Promise.resolve().then(()=>st);await m(r,null,"telegram","webhook_error",d.message||"Unknown telegram error")}catch{}}};return e.executionCtx.waitUntil(a()),e.json({ok:!0})});Nt.post("/setup-webhook",async e=>{var l;const t=(l=e.req.header("Authorization"))==null?void 0:l.replace("Bearer ","");if(!t)return e.json({error:"Auth required"},401);const r=await e.env.DB.prepare(`SELECT s.user_id, u.pin_hash FROM sessions s JOIN users u ON s.user_id = u.id
  ...
  `).bind(v,v,h.id).run();const y=await e.prepare(`SELECT c.encrypted_value, u.pin_hash FROM credentials c 
     JOIN users u ON c.user_id = u.id 
     WHERE c.user_id = ? AND c.service = 'telegram_bot_token'`).bind(m.id).first();if(!y)return;const _=await Y(y.encrypted_value,y.pin_hash);try{const T=await fetch(`https://api.telegram.org/bot${_}/answerCallbackQuery`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({callback_query_id:r,text:v?"✅ Checked!":"☐ Unchecked"})});T.ok||console.warn(`[callback answer] Failed to answer callback: HTTP ${T.status}`)}catch(T){console.warn(`[callback answer] Error answering callback: ${T.message}`)}if((w=a.reply_markup)!=null&&w.inline_keyboard){const T=a.reply_markup.inline_keyboard.map(E=>E.map(R=>{var N;if((N=R.callback_data)!=null&&N.includes(c)){const j=v?"✅":"☐",G=R.text.replace(/^[☐✅]\s*/,"");return{...R,text:`${j} ${G}`}}return R}));try{await fetch(`https://api.telegram.org/bot${_}/editMessageReplyMarkup`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({chat_id:l,message_id:a.message_id,reply_markup:{inline_keyboard:T}})})}catch{}}}function Ws(e){const t=new Date,r=new Date(t.toLocaleString("en-US",{timeZone:e})),n=new Date(r);n.setDate(n.getDate()+1),n.setHours(0,0,0,0);const a=new Date(n);a.setHours(23,59,59,999);const s=n.toISOString().split("T")[0];return{start:n.toISOString(),end:a.toISOString(),dateStr:s}}async function qs(e,t,r,n,a,s){try{return(await new cr(e,t,r,n,a).listEvents("primary",{timeMin:s.start,timeMax:s.end,maxResults:50})).map(l=>{var c;return{id:l.id||`google-${Date.now()}`,title:l.summary||"Untitled Event",startTime:l.start.dateTime||l.start.date||"",endTime:l.end.dateTime||l.end.date||"",location:l.location,attendees:(c=l.attendees)==null?void 0:c.map(d=>d.displayName||d.email),source:"google"}})}catch(i){return console.error("Google Calendar fetch error:",i.message),[]}}async function zs(e,t,r,n,a){try{const s=new be(e,t,r,n,a),i=await s.listMessages({query:"is:unread",maxResults:50,labelIds:["INBOX"]}),o=await s.listMessages({query:"is:important is:unread",maxResults:10}),l={};for(const m of i){const h=m.from.split("<")[0].trim()||m.from;l[h]=(l[h]||0)+1}const c=Object.entries(l).sort(([,m],[,h])=>h-m).slice(0,5).map(([m])=>m),d=i.some(m=>m.subject.toLowerCase().includes("urgent")||m.subject.toLowerCase().includes("asap")||m.subject.toLowerCase().includes("immediately"));return{unreadCount:i.length,importantCount:o.length,topSenders:c,hasUrgent:d}}catch(s){return console.error("Gmail fetch error:",s.message),{unreadCount:0,importantCount:0,topSenders:[],hasUrgent:!1}}}async function Ks(e,t){try{const r=await e.prepare(`
...
`)}function Zs(e){const t=[];let r=0;for(const n of e.calendar.google)t.push({type:"calendar",key:n.id,text:`${n.title} - ${new Date(n.startTime).toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"})}`,metadata:{event:n},sortOrder:r++});e.emails.gmail.unreadCount>0&&t.push({type:"email",key:"gmail-unread",text:`Review ${e.emails.gmail.unreadCount} unread Gmail messages`,metadata:{source:"gmail",count:e.emails.gmail.unreadCount},sortOrder:r++});for(const n of e.tasks.items)t.push({type:"task",key:`task-${n}`,text:n,metadata:{},sortOrder:r++});for(const n of e.news.items)t.push({type:"news",key:`news-${n.url}`,text:`📰 ${n.title}`,metadata:{url:n.url,source:n.source},sortOrder:r++});return t}async function Xs(e,t){const r=await e.prepare("SELECT * FROM briefing_preferences WHERE user_id = ?").bind(t).first();if(!r)return{components:{google_calendar:!0,gmail:!0,tasks:!0,news:!0},newsTopics:["AI","LLM","Tools","Agentic Workflows","AI Features"]};let n;try{const s=JSON.parse(r.components);n={google_calendar:s.google_calendar!==!1,gmail:s.gmail!==!1,tasks:s.tasks!==!1,news:s.news!==!1}}catch{n={google_calendar:!0,gmail:!0,tasks:!0,news:!0}}const a=r.news_topics?r.news_topics.split(",").map(s=>s.trim()).filter(Boolean):["AI","LLM","Tools","Agentic Workflows","AI Features"];return{components:n,newsTopics:a}}async function Vn(e,t,r){var E,R;const n=t.timezone||"Asia/Kolkata",a=Ws(n),{components:s,newsTopics:i}=await Xs(e,t.id),o=[],l=[];s.google_calendar&&(o.push(qs(e,t.id,t.pin_hash,r.GOOGLE_CLIENT_ID,r.GOOGLE_CLIENT_SECRET,a)),l.push("googleEvents")),s.gmail&&(o.push(zs(e,t.id,t.pin_hash,r.GOOGLE_CLIENT_ID,r.GOOGLE_CLIENT_SECRET)),l.push("gmailSummary")),s.tasks&&(o.push(Ks(e,t.id)),l.push("tasks")),s.news&&(o.push(Js(i,e,t.id)),l.push("news"));const c=await Promise.all(o),d={};l.forEach((N,j)=>{d[N]=c[j]});const m={unreadCount:0,importantCount:0,topSenders:[],hasUrgent:!1},h={pending:0,dueToday:0,items:[]},v={generatedAt:new Date().toISOString(),targetDate:a.dateStr,calendar:{google:d.googleEvents||[],totalCount:((E=d.googleEvents)==null?void 0:E.length)||0},emails:{gmail:d.gmailSummary||m},tasks:d.tasks||h,news:{items:d.news||[],fetchedAt:new Date().toISOString()},summary:""},y=((R=await e.prepare("SELECT briefing_time FROM briefing_preferences WHERE user_id = ?").bind(t.id).first())==null?void 0:R.briefing_time)||"20:00";v.summary=Vs(v,y);const _=Zs(v),w=await e.prepare(`
  ...
  `).bind(r).all();return{briefing:{...n,content:JSON.parse(n.content_json||"{}")},items:a.results||[]}}async function ei(e,t,r,n){if(!await e.prepare(`
  ...
  `).bind(i,i,n,r).run(),{checked:i===1}}async function ti(e,t,r=10){return((await e.prepare(`
    SELECT b.*, 
...
🕐 ${R}${N}`;await fetch(`https://api.telegram.org/bot${T}/sendMessage`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({chat_id:l.telegram_chat_id,text:j,parse_mode:"Markdown"})})}a.push({user_id:l.id,reminders_sent:_.length})}catch(c){a.push({user_id:l.id,status:"error",error:c.message})}return e.json({executed:a.length,results:a})}catch(n){return e.json({error:n.message},500)}});le.post("/cron/morning-briefing",async e=>{const t=e.req.header("X-Cron-Secret")||"",r=e.env.CRON_SECRET||"karna-cron-default-v1";if(t!==r)return e.json({error:"Unauthorized"},401);try{const n=await e.env.DB.prepare(`
      ...
      FROM users u
      LEFT JOIN briefing_preferences bp ON u.id = bp.user_id
    `).all(),a=[],s=new Date;for(const i of n.results||[]){if(!i.morning_briefing_enabled)continue;const o=i.timezone||"Asia/Kolkata",l=i.morning_briefing_time||"08:00";if(Zn(l,o,s))try{const c=await ta(e.env.DB,i,{GOOGLE_CLIENT_ID:e.env.GOOGLE_CLIENT_ID,GOOGLE_CLIENT_SECRET:e.env.GOOGLE_CLIENT_SECRET});let d={telegram:!0,web:!0};try{d=JSON.parse(i.notification_channels||"{}")}catch{}if(d.telegram!==!1&&i.telegram_chat_id&&c.briefingId){const m=si(c.content);await ea(e.env.DB,i,m),await e.env.DB.prepare("UPDATE briefings SET delivered_telegram = 1 WHERE id = ?").bind(c.briefingId).run()}a.push({user_id:i.id,status:"success",briefing_id:c.briefingId,briefing_time:l,timezone:o})}catch(c){a.push({user_id:i.id,status:"error",error:c.message})}}return e.json({executed:a.length,results:a})}catch(n){return e.json({error:n.message},500)}});le.post("/cron/email-digest",async e=>{const t=e.req.header("X-Cron-Secret")||"",r=e.env.CRON_SECRET||"karna-cron-default-v1";if(t!==r)return e.json({error:"Unauthorized"},401);try{const n=await e.env.DB.prepare(`
    ...
    `).all(),a=[];for(const s of n.results||[]){let i={};try{i=JSON.parse(s.components||"{}")}catch{}if(i.email_digest!==!1)try{const o=await ra(e.env.DB,s,{GOOGLE_CLIENT_ID:e.env.GOOGLE_CLIENT_ID,GOOGLE_CLIENT_SECRET:e.env.GOOGLE_CLIENT_SECRET}),l=`Email Digest — ${new Date().toLocaleDateString("en-US",{weekday:"short",month:"short",day:"numeric"})}`,c=JSON.stringify(o,null,2),d=await e.env.DB.prepare(`INSERT INTO action_items (user_id, type, title, body, priority, source, source_id, action_payload, due_at)
      ...
      FROM users u
      LEFT JOIN briefing_preferences bp ON u.id = bp.user_id
    `).all(),a=[],s=new Date;for(const i of n.results||[]){if(!i.weekly_review_enabled)continue;const o=i.timezone||"Asia/Kolkata",l=i.weekly_review_day_time||"Sunday 20:00";if(ni(l,o,s))try{const c=await ai(e.env.DB,i,{GOOGLE_CLIENT_ID:e.env.GOOGLE_CLIENT_ID,GOOGLE_CLIENT_SECRET:e.env.GOOGLE_CLIENT_SECRET}),d=`Weekly Review — Week of ${new Date().toLocaleDateString("en-US",{month:"short",day:"numeric"})}`,m=JSON.stringify(c,null,2),h=await e.env.DB.prepare(`INSERT INTO action_items (user_id, type, title, body, priority, source, source_id, action_payload, due_at)
           VALUES (?, 'weekly_review', ?, ?, 'normal', 'proactive', ?, ?, ?) RETURNING id`).bind(i.id,d,m,`weekly_review_${Date.now()}`,null,null,null).first();let v={telegram:!0,web:!0};try{v=JSON.parse(i.notification_channels||"{}")}catch{}if(v.telegram!==!1&&i.telegram_chat_id){const y=ii(c);await ea(e.env.DB,i,y)}a.push({user_id:i.id,status:"success",action_item_id:h==null?void 0:h.id})}catch(c){a.push({user_id:i.id,status:"error",error:c.message})}}return e.json({executed:a.length,results:a})}catch(n){return e.json({error:n.message},500)}});async function Qn(e,t,r,n,a){try{const s=await e.prepare(`SELECT c.encrypted_value, u.pin_hash FROM credentials c 
       JOIN users u ON c.user_id = u.id 
