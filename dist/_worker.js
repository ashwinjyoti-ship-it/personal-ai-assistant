...
`),t.trim()}async function za(e,t,a={}){const r=a.maxPages||(a.depth==="thorough"?5:3),n=a.maxResults||(a.depth==="thorough"?8:5),s=await ht(e,{num:n,site:a.site});if(s.error)return{report:"",sources:[],pagesRead:0,error:`Search failed: ${s.error}`};if(s.results.length===0)return{report:`No web results found for "${e}".`,sources:[],pagesRead:0};const i=s.results.slice(0,r).map(async y=>{const v=await Ga(y.link);return{title:y.title,url:y.link,displayLink:y.displayLink,snippet:y.snippet,content:v.text,error:v.error}}),c=(await Promise.all(i)).filter(y=>y.content.length>50);if(c.length===0){const y=s.results.map((g,x)=>`[${x+1}] ${g.title}
${g.snippet}
Source: ${g.link}`).join(`

`);return{report:await Ft(e,y,t,"snippets"),sources:s.results.map(g=>({title:g.title,url:g.link})),pagesRead:0}}const u=c.map((y,v)=>`--- SOURCE ${v+1}: ${y.title} (${y.displayLink}) ---
${y.content}
--- END SOURCE ${v+1} ---`).join(`

`);return{report:await Ft(e,u,t,"full"),sources:c.map(y=>({title:y.title,url:y.url})),pagesRead:c.length}}async function Ft(e,t,a,r){const s=`You are a research analyst. Your job is to synthesize web research into clear, actionable reports.
...
- Unread errors: ${(_==null?void 0:_.cnt)||0}`}case"read_sheet":{if(!n)return"Authentication context unavailable.";try{const d=new te(a,r,n,s||"",o||""),f=t.spreadsheet_id;let p=t.range;const h=await d.sheets.getMetadata(f),_=h.sheets;p.includes("!")||(p=`${_[0]}!${p}`);let E;try{E=await d.sheets.readRange(f,p)}catch(S){if((y=S.message)!=null&&y.includes("Unable to parse range")||(v=S.message)!=null&&v.includes("400")){const O=p.includes("!")?p.split("!")[1]:p;p=`${_[0]}!${O}`,E=await d.sheets.readRange(f,p)}else throw S}let k=`[Spreadsheet: "${h.title}" | Reading tab: "${p.split("!")[0]}" | All tabs in this spreadsheet: ${_.map(S=>`"${S}"`).join(", ")}]
...
ID: ${p.spreadsheetId}
URL: ${p.url}`}catch(d){return await R(a,r,"google","create_sheet",d.message),`Failed to create spreadsheet: ${d.message}`}}case"list_calendar_events":{if(!n)return"Authentication context unavailable.";try{const d=new te(a,r,n,s||"",o||""),f=t.calendar_id||"primary",p=t.days_ahead||7,h=new Date,_=new Date(h.getTime()+p*24*60*60*1e3),E=await d.calendar.listEvents(f,{timeMin:h.toISOString(),timeMax:_.toISOString(),query:t.query});return E.length===0?`No events found in the next ${p} days.`:E.map(k=>{var $;const S=k.start.dateTime||k.start.date||"TBD",O=k.end.dateTime||k.end.date||"",C=k.location?` 📍 ${k.location}`:"",N=(($=k.attendees)==null?void 0:$.map(q=>q.email).join(", "))||"";return`• ${k.summary} — ${S} to ${O}${C}${N?`
  Attendees: ${N}`:""}`}).join(`
...
`)}catch(d){return await R(a,r,"google_api","search_places",d.message),`Places search error: ${d.message}`}}case"get_place_details":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const f=await M(d.encrypted_value,n),p=await $a(f,t.place_id);if(p.error)return`Details lookup failed: ${p.error}`;if(!p.details)return"No details found.";const h=p.details;let _=`**${h.name}**
...
— ${E.author} (★${E.rating}, ${E.time}): "${E.text}"`}return _}catch(d){return await R(a,r,"google_api","place_details",d.message),`Place details error: ${d.message}`}}case"get_directions":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const f=await M(d.encrypted_value,n),p=await Ma(f,t.origin,t.destination,{mode:t.mode||"driving"});if(p.error)return`Directions failed: ${p.error}`;if(!p.route)return"No route found.";const h=p.route;let _=`**${h.startAddress}** → **${h.endAddress}**
...
${p.translatedText}`}catch(d){return await R(a,r,"google_api","translate",d.message),`Translation error: ${d.message}`}}case"search_youtube":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const f=await M(d.encrypted_value,n),p=await Pa(f,t.query,{maxResults:t.max_results||5,order:t.order||"relevance"});return p.error?`YouTube search failed: ${p.error}`:p.results.length===0?`No YouTube results for "${t.query}".`:p.results.map((h,_)=>{var E;return`${_+1}. **${h.title}**
...
`;for(const c of s)o+=`- ${c.name} (${c.type}, ${Math.round(c.size/1024)}KB, file_id: ${c.file_id})`,c.text_preview&&(o+=`
  Preview: ${c.text_preview.substring(0,300)}...`),o+=`
`;o+="Use parse_document tool to read full file contents. Use drive_upload to upload to Google Drive.]"}let i=n;if(!i){const c=await e.env.DB.prepare("INSERT INTO threads (user_id, title) VALUES (?, ?) RETURNING *").bind(t.id,a.trim().substring(0,60)+(a.trim().length>60?"...":"")).first();i=c==null?void 0:c.id}const l={userId:t.id,username:t.username,channel:r,text:a.trim()+o,sessionId:e.get("sessionId"),timestamp:new Date().toISOString(),metadata:{thread_id:i}};try{const{provider:c,rotation:u}=await tt(e.env.DB,t.id,t.pin_hash),m=new ReadableStream({async start(y){const v=new TextEncoder;try{const g=gn(l,e.env.DB,c,t,u,{GOOGLE_CLIENT_ID:e.env.GOOGLE_CLIENT_ID,GOOGLE_CLIENT_SECRET:e.env.GOOGLE_CLIENT_SECRET,GOOGLE_API_KEY:e.env.GOOGLE_API_KEY,GOOGLE_CSE_ID:e.env.GOOGLE_CSE_ID});for await(const x of g)x.data.threadId||(x.data.threadId=i),y.enqueue(v.encode(Yt(x)));i&&await e.env.DB.prepare("UPDATE threads SET message_count = message_count + 2, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(i).run(),y.close()}catch(g){const x={type:"error",data:{error:g.message||"An error occurred",threadId:i}};y.enqueue(v.encode(Yt(x))),y.close()}}});return new Response(m,{headers:{"Content-Type":"text/event-stream","Cache-Control":"no-cache",Connection:"keep-alive","X-Thread-Id":String(i||"")}})}catch(c){console.error("Stream setup error:",c);const u=c.message||"";return u.includes("No LLM provider configured")?e.json({error:"No AI provider configured. Please add at least one API key in Settings → Keys.",type:"no_provider",thread_id:i},400):u.includes("limit reached")?e.json({error:u,type:"cost_limit",thread_id:i},429):e.json({error:"Something went wrong setting up the stream.",details:u,thread_id:i},500)}});B.get("/threads/:id/messages",async e=>{var s;const t=e.get("user"),a=parseInt(e.req.param("id")),r=parseInt(e.req.query("limit")||"50"),n=await e.env.DB.prepare(`SELECT id, role, content, channel, created_at FROM conversations 
...
Status: ✅ Online`;await G(a,t,u)}catch{await G(a,t,"✅ Online — but had trouble fetching stats.")}return!0}case"/new":return r?(await n.prepare("UPDATE threads SET is_archived = 1 WHERE user_id = ? AND channel = 'telegram' AND is_archived = 0").bind(r.id).run(),await G(a,t,"🆕 Starting fresh conversation. Your next message begins a new thread."),!0):(await G(a,t,"⚠️ Account not linked."),!0);default:return!1}}at.post("/webhook",async e=>{var t,a,r,n,s;try{const o=await e.req.json();if(o.callback_query)return await Sn(e.env.DB,o.callback_query),e.json({ok:!0});const i=o.message;if(!i)return e.json({ok:!0});const l=!!i.text,c=!!i.voice,u=!!i.document,m=!!i.photo,y=!!i.caption;if(!l&&!c&&!u&&!m)return e.json({ok:!0});const v=String(i.chat.id);let g=i.text||"";const x=await e.env.DB.prepare("SELECT * FROM users WHERE telegram_chat_id = ?").bind(v).first();let w=null;if(x){const h=await e.env.DB.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(x.id,"telegram_bot_token").first();h&&(w=await M(h.encrypted_value,x.pin_hash))}if(!w){const h=await e.env.DB.prepare(`SELECT c.encrypted_value, u.pin_hash FROM credentials c 
         JOIN users u ON c.user_id = u.id 
         WHERE c.service = 'telegram_bot_token' LIMIT 1`).first();h&&(w=await M(h.encrypted_value,h.pin_hash))}if(!w)return e.json({ok:!0,message:"Bot token not configured"});if(g.startsWith("/")&&await kn(g,v,w,x,e.env.DB))return e.json({ok:!0});if(!x)return await G(w,v,`⚠️ Your account isn't linked yet.

Your Telegram Chat ID is: \`${v}\`

Go to the web app → Settings → Profile → set your Telegram Chat ID to this value.`),e.json({ok:!0});if(i.voice&&w&&x)try{await G(w,v,"🎤 Processing voice note...");const _=await(await fetch(`https://api.telegram.org/bot${w}/getFile?file_id=${i.voice.file_id}`)).json();if(_.ok&&((t=_.result)!=null&&t.file_path)){const k=await(await fetch(`https://api.telegram.org/file/bot${w}/${_.result.file_path}`)).blob();let S="",O="",C="whisper-1";const N=await e.env.DB.prepare("SELECT service, encrypted_value FROM credentials WHERE user_id = ? AND (service IN ('llm_slot_1', 'llm_slot_2', 'llm_slot_3', 'openai'))").bind(x.id).all();for(const F of N.results){const De=await M(F.encrypted_value,x.pin_hash);if(F.service==="openai"){S="https://api.openai.com/v1/audio/transcriptions",O=De;break}else if(F.service.startsWith("llm_slot_"))try{const ue=JSON.parse(De);if(ue.provider==="openai"){S="https://api.openai.com/v1/audio/transcriptions",O=ue.apiKey;break}else if(ue.provider==="groq"){S="https://api.groq.com/openai/v1/audio/transcriptions",O=ue.apiKey,C="whisper-large-v3";break}}catch{}}if(!S)return await G(w,v,"⚠️ To use voice notes, configure an OpenAI API key in your LLM slots (Settings → Keys)."),e.json({ok:!0});const $=new FormData;$.append("file",k,"voice.ogg"),$.append("model",C),$.append("language","en");const q=await fetch(S,{method:"POST",headers:{Authorization:`Bearer ${O}`},body:$});if(!q.ok){const F=await q.text();return await G(w,v,`⚠️ Transcription failed: ${q.status} ${F}`),e.json({ok:!0})}g=(await q.json()).text,await G(w,v,`🗣️ *You said:* ${g}`)}}catch(h){return await G(w,v,`⚠️ Failed to process voice note: ${h.message}`),e.json({ok:!0})}if((u||m)&&w&&x)try{let h,_="unknown",E="unknown",k=0;if(u)h=i.document.file_id,_=i.document.file_name||"document",E=i.document.mime_type||"unknown",k=i.document.file_size||0;else if(m){const S=i.photo[i.photo.length-1];h=S.file_id,_="photo.jpg",E="image/jpeg",k=S.file_size||0}if(h){const O=await(await fetch(`https://api.telegram.org/bot${w}/getFile?file_id=${h}`)).json();let C="";if(O.ok&&((a=O.result)!=null&&a.file_path)&&(/\.(txt|csv|json|md|xml|html|log|yaml|yml|tsv|ini|cfg|conf|py|js|ts|sh|sql)$/i.test(_)||/^text\/|application\/json|application\/xml|application\/csv/i.test(E))&&k<5e4)try{C=await(await fetch(`https://api.telegram.org/file/bot${w}/${O.result.file_path}`)).text()}catch{}const N=i.caption||"",$=`[Telegram file received: "${_}" (${E}, ${Math.round(k/1024)}KB)]`;C?g=`${N?N+`
