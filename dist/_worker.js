...
`);return En(r,di)}catch{return""}}class X{constructor(t){this.db=t}async store(t,n,s,r,a=5,i="working"){const o=await this.db.prepare("SELECT id FROM memory WHERE user_id = ? AND type = ? AND title = ?").bind(t,n,s).first();o?await this.db.prepare("UPDATE memory SET content = ?, importance = ?, tier = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(r,a,i,o.id).run():await this.db.prepare("INSERT INTO memory (user_id, type, title, content, importance, tier) VALUES (?, ?, ?, ?, ?, ?)").bind(t,n,s,r,a,i).run(),i==="working"&&await this.enforceWorkingMemoryCap(t)}async cleanupDoneTasks(t){await this.db.prepare(`UPDATE memory SET tier = 'long_term', updated_at = CURRENT_TIMESTAMP
       ...
       AND updated_at < datetime('now', '-7 days')`).bind(t).run()}async enforceWorkingMemoryCap(t){const n=await this.db.prepare("SELECT COUNT(*) as cnt FROM memory WHERE user_id = ? AND tier = 'working'").bind(t).first();if(((n==null?void 0:n.cnt)||0)>hn){const s=((n==null?void 0:n.cnt)||0)-hn;await this.db.prepare(`UPDATE memory SET tier = 'long_term', updated_at = CURRENT_TIMESTAMP
         ...
         )`).bind(t,t,s).run()}}async getWorkingMemory(t){return(await this.db.prepare("SELECT * FROM memory WHERE user_id = ? AND tier = 'working' ORDER BY importance DESC, updated_at DESC LIMIT ?").bind(t,hn).all()).results||[]}async getAll(t,n,s=50){return n?(await this.db.prepare("SELECT * FROM memory WHERE user_id = ? AND type = ? ORDER BY tier ASC, importance DESC, updated_at DESC LIMIT ?").bind(t,n,s).all()).results||[]:(await this.db.prepare("SELECT * FROM memory WHERE user_id = ? ORDER BY tier ASC, importance DESC, updated_at DESC LIMIT ?").bind(t,s).all()).results||[]}async search(t,n,s=10){return this.searchMemoryByTier(t,n,s)}async searchLongTerm(t,n,s=5){return this.searchMemoryByTier(t,n,s,"long_term")}async searchMemoryByTier(t,n,s,r){const a=r?" AND tier = ?":"",i=(y,h)=>r?[t,r,y,y,h]:[t,y,y,h],l=(await this.db.prepare(`SELECT * FROM memory WHERE user_id = ?${a} AND (title LIKE ? OR content LIKE ?) ORDER BY importance DESC LIMIT ?`).bind(...i(`%${n}%`,s)).all()).results||[];if(l.length>0)return await this.touchMemories(t,l.map(y=>y.id)),l;const c=n.split(/\s+/).filter(y=>y.length>2);if(c.length===0)return[];const d=new Map,u=new Map;for(const y of c){const h=await this.db.prepare(`SELECT * FROM memory WHERE user_id = ?${a} AND (title LIKE ? OR content LIKE ?) LIMIT ?`).bind(...i(`%${y}%`,s*2)).all();for(const v of h.results||[])d.set(v.id,(d.get(v.id)||0)+1),u.set(v.id,v)}const g=[...u.values()].sort((y,h)=>(d.get(h.id)||0)-(d.get(y.id)||0)).slice(0,s);return g.length>0&&await this.touchMemories(t,g.map(y=>y.id)),g}async touchMemories(t,n){for(const s of n)await this.db.prepare("UPDATE memory SET updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?").bind(s,t).run()}async update(t,n,s){await this.db.prepare("UPDATE memory SET content = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?").bind(s,t,n).run()}async promote(t,n){await this.db.prepare("UPDATE memory SET tier = 'working', updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?").bind(t,n).run(),await this.enforceWorkingMemoryCap(n)}async demote(t,n){await this.db.prepare("UPDATE memory SET tier = 'long_term', updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?").bind(t,n).run()}async remove(t,n){await this.db.prepare("DELETE FROM memory WHERE id = ? AND user_id = ?").bind(t,n).run()}async buildContext(t){const n=await this.getWorkingMemory(t);if(n.length===0)return"";const s={};for(const a of n)s[a.type]||(s[a.type]=[]),s[a.type].push(a);let r=`
...
Generate exactly 4-5 specific sub-queries that together cover all important angles (definition, current state, comparisons, recent developments, expert analysis). Return ONLY a JSON array of strings, nothing else.`,400),l=as(o,[e,`${e} overview`,`${e} examples`,`${e} latest`]).slice(0,5),c=await Promise.all(l.map(w=>i(w,5,"thorough"))),d=new Set,u=[];for(const w of c)for(const b of w)d.has(b.result.url)||u.length>=20||(d.add(b.result.url),u.push(b));let g="";for(let w=0;w<u.length;w++){const b=u[w],S=`[${w+1}] ${b.result.title}
${b.result.url}

${b.content}

`;if(g.length+S.length>18e4)break;g+=S,a.push({url:b.result.url,title:b.result.title})}const y=await Yt(t,"You are a research analyst identifying information gaps.",`I'm researching: ${e}

Here's what I've found so far:
${g.slice(0,6e4)}

Identify 2-3 specific information gaps or important angles not yet covered. Return ONLY a JSON array of follow-up search queries.`,300),h=as(y,[]).slice(0,3);if(h.length>0){let w=0;const b=await Promise.all(h.map(S=>i(S,4,"thorough")));for(const S of b)for(const R of S){if(w>=5||d.has(R.result.url))continue;d.add(R.result.url);const N=`[${a.length+1}] ${R.result.title}
${R.result.url}

${R.content}

`;if(g.length+N.length>18e4)break;g+=N,a.push({url:R.result.url,title:R.result.title}),u.push(R),w++}}return{report:await Yt(t,`You are an expert research analyst producing a comprehensive report. Use this exact structure:
...
Sources:
${g}`,4096),sources:a,pagesRead:u.length}}async function fr(e,t,n={}){const s=n.depth||"quick",r=async(h,v)=>(await Lt(h,{num:v,site:n.site,googleApiKey:n.googleApiKey,googleCseId:n.googleCseId})).results||[];if(n.anthropicKey)try{const h=await Ai(e,n.anthropicKey,n.tavilyKey||null,s,r);if(h.report.trim())return{report:h.report,sources:h.sources.map(v=>({title:v.title,url:v.url})),pagesRead:h.pagesRead}}catch(h){console.warn("[conductResearch] Opus path failed:",h.message)}if(n.tavilyKey)try{const h=await hr(e,n.tavilyKey,s);if(h.results.length>0){const v=s==="thorough"?8:5,b=(await Promise.all(h.results.slice(0,v).map(async N=>({result:N,content:await gr(N)})))).filter(N=>N.content.length>50),S=b.map(N=>({title:N.result.title,url:N.result.url}));if(b.length>0){const N=b.map((M,G)=>`--- SOURCE ${G+1}: ${M.result.title} ---
${M.content}
--- END SOURCE ${G+1} ---`).join(`

`);return{report:await Jt(e,N,t,"full"),sources:S,pagesRead:b.length}}const R=h.results.map((N,U)=>`[${U+1}] ${N.title}
${N.content}
Source: ${N.url}`).join(`

`);return{report:await Jt(e,R,t,"snippets"),sources:h.results.map(N=>({title:N.title,url:N.url})),pagesRead:0}}}catch(h){console.warn("[conductResearch] Tavily path failed:",h.message)}const a=n.maxPages||(s==="thorough"?5:3),i=n.maxResults||(s==="thorough"?8:5),o=await Lt(e,{num:i,site:n.site,googleApiKey:n.googleApiKey,googleCseId:n.googleCseId});if(o.error)return{report:"",sources:[],pagesRead:0,error:`Search failed: ${o.error}`};if(o.results.length===0)return{report:`No web results found for "${e}".`,sources:[],pagesRead:0};const c=o.results.slice(0,a).map(async h=>{const v=await An(h.link);return{title:h.title,url:h.link,displayLink:h.displayLink,snippet:h.snippet,content:v.text,error:v.error}}),u=(await Promise.all(c)).filter(h=>h.content.length>50);if(u.length===0){const h=o.results.map((w,b)=>`[${b+1}] ${w.title}
${w.snippet}
Source: ${w.link}`).join(`

`);return{report:await Jt(e,h,t,"snippets"),sources:o.results.map(w=>({title:w.title,url:w.link})),pagesRead:0}}const g=u.map((h,v)=>`--- SOURCE ${v+1}: ${h.title} (${h.displayLink}) ---
${h.content}
--- END SOURCE ${v+1} ---`).join(`

`);return{report:await Jt(e,g,t,"full"),sources:u.map(h=>({title:h.title,url:h.url})),pagesRead:u.length}}async function Jt(e,t,n,s){const a=`You are a research analyst. Your job is to synthesize web research into clear, actionable reports.
...
If a captcha was encountered, set captcha_required to true and populate whatever tracking data was visible before the captcha appeared.`}async function Er(e){const t=e instanceof Buffer?new Uint8Array(e):e,n=new DataView(t.buffer,t.byteOffset,t.byteLength);let s=0;for(;s<t.length-30&&n.getUint32(s,!0)===67324752;){const r=n.getUint16(s+6,!0),a=n.getUint16(s+8,!0),i=n.getUint32(s+18,!0),o=n.getUint32(s+22,!0),l=n.getUint16(s+26,!0),c=n.getUint16(s+28,!0),d=new TextDecoder().decode(t.slice(s+30,s+30+l)),u=s+30+l+c;if(d==="word/document.xml"){const g=t.slice(u,u+i);let y;if(a===0)y=g;else{const w=new DecompressionStream("deflate-raw"),b=w.writable.getWriter();b.write(g),b.close();const S=w.readable.getReader(),R=[];let B=!1;for(;!B;){const M=await S.read();M.done?B=!0:R.push(M.value)}const N=R.reduce((M,G)=>M+G.length,0);y=new Uint8Array(o||N);let U=0;for(const M of R)y.set(M,U),U+=M.length}return new TextDecoder().decode(y).replace(/<\/w:p>/g,`
`).replace(/<\/w:tr>/g,`
...
${n}
`:"";let u="";try{const y=new Date;u=new Intl.DateTimeFormat("en-GB",{timeZone:t.timezone,day:"numeric",month:"short",year:"numeric"}).format(y)}catch{u=""}const g=`
## Current User
- **Name**: ${t.name}
- **Timezone**: ${t.timezone}
- **Time**: ${r}
...
`}catch{return""}}async function Ki(e,t,n){var i;let s=0,r=0,a=0;try{const o=await e.prepare(`SELECT us.id, us.user_id, us.name, us.instructions, us.refinement_count,
       ...
       WHERE us.user_id = ? AND us.is_auto = 1 AND us.enabled = 1
         ...
         ORDER BY created_at DESC LIMIT 3`).bind(l.id).all();if((c.results??[]).length<2||l.refinement_count>=kr){await e.prepare("UPDATE user_skills SET enabled = 0, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(l.id).run(),await e.prepare(`INSERT INTO notifications (user_id, type, title, body, source, is_read)
           VALUES (?, 'warning', ?, ?, 'skills', 0)`).bind(l.user_id,`Auto-skill retired: ${l.name}`,`"${l.name}" had a ${Math.round(l.confidence_score*100)}% success rate and couldn't be improved — disabled. Check Settings → Skills to manage it.`).run(),a++;continue}const d=c.results.map(y=>y.user_message_sample),u=JSON.parse(c.results[0].tool_sequence),g=[{role:"system",content:"You are a workflow optimizer. Rewrite a skill procedure so it is more reliable, based on examples that previously succeeded."},{role:"user",content:`Skill "${l.name}" has a ${Math.round(l.confidence_score*100)}% success rate.

Current instructions:
${l.instructions}

Recent successful examples:
${d.map((y,h)=>`${h+1}. "${y}"`).join(`
`)}
Tools used: ${u.join(" → ")}

Rewrite the instructions to be clearer and more reliable. Keep them under 200 words.
Respond with EXACTLY:
REWRITTEN_INSTRUCTIONS: <revised instructions>`}];try{const v=(((i=(await t.chat(g,{tools:[]})).content)==null?void 0:i.trim())??"").match(/^REWRITTEN_INSTRUCTIONS:\s*([\s\S]+)$/m);v&&v[1].trim()&&(await e.prepare(`UPDATE user_skills
...
- Unread errors: ${(_==null?void 0:_.cnt)||0}`}case"read_sheet":{if(!r)return"Authentication context unavailable.";try{const m=new we(n,s,r,a||"",i||""),E=t.spreadsheet_id;let p=t.range;const f=await m.sheets.getMetadata(E),_=f.sheets;p.includes("!")||(p=`${_[0]}!${p}`);let T;try{T=await m.sheets.readRange(E,p)}catch(D){if((w=D.message)!=null&&w.includes("Unable to parse range")||(b=D.message)!=null&&b.includes("400")){const A=p.includes("!")?p.split("!")[1]:p;p=`${_[0]}!${A}`,T=await m.sheets.readRange(E,p)}else throw D}let x=`[Spreadsheet: "${f.title}" | Reading tab: "${p.split("!")[0]}" | All tabs in this spreadsheet: ${_.map(D=>`"${D}"`).join(", ")}]
...
The sheet write has been saved to temporary memory. After reconnecting, tell me "write the pending sheet data" to complete this.`:"")}const p=t.values;let f=t.range;const x=Math.max(...p.map(C=>C.length))+4,D=p.map(C=>{const I=[...C];for(;I.length<x;)I.push("");return I}),A=f.match(/^(.+!)?([A-Z]+)(\d+):([A-Z]+)(\d+)$/i);if(A){const C=A[1]||"",I=A[2],L=A[3],P=A[5],Q=I.toUpperCase().charCodeAt(0)-64+x-1,Y=Q<=26?String.fromCharCode(64+Q):"Z";f=`${C}${I}${L}:${Y}${P}`}const O=await m.sheets.writeRange(t.spreadsheet_id,f,D);try{const C=new X(n),I=await C.search(s,`Pending sheet write: ${t.spreadsheet_id}`);for(const L of I)L.title.startsWith(`Pending sheet write: ${t.spreadsheet_id}`)&&await C.remove(L.id,s)}catch{}return`Written ${O.updatedCells} cells to ${f}.`}catch(m){return await H(n,s,"google","write_sheet",m.message),`Failed to write sheet: ${m.message}`}}case"append_sheet":{if(!r)return"Authentication context unavailable.";try{const m=new we(n,s,r,a||"",i||"");if(!(await m.isConnected()).connected){if(t.spreadsheet_id&&t.range&&t.values)try{await new X(n).store(s,"context",`Pending sheet append: ${t.spreadsheet_id} — ${t.range}`,JSON.stringify({tool:"append_sheet",spreadsheet_id:t.spreadsheet_id,range:t.range,values:t.values}),9,"working")}catch{}return"Google account not connected. Please go to Settings → Keys → Google Workspace and connect your account."+(t.spreadsheet_id&&t.range?`

The append data has been saved to temporary memory. After reconnecting, tell me "append the pending sheet data" to complete this.`:"")}const p=await m.sheets.appendRows(t.spreadsheet_id,t.range,t.values);try{const f=new X(n),_=await f.search(s,`Pending sheet append: ${t.spreadsheet_id}`);for(const T of _)T.title.startsWith(`Pending sheet append: ${t.spreadsheet_id}`)&&await f.remove(T.id,s)}catch{}return`Appended ${p.updatedCells} cells to ${t.range}.`}catch(m){return await H(n,s,"google","append_sheet",m.message),`Failed to append to sheet: ${m.message}`}}case"create_sheet":{if(!r)return"Authentication context unavailable.";try{const m=new we(n,s,r,a||"",i||"");if(!(await m.isConnected()).connected){if(t.title)try{await new X(n).store(s,"context",`Pending spreadsheet create: "${t.title}"`,JSON.stringify({tool:"create_sheet",title:t.title,sheet_names:t.sheet_names??null,folder_name:t.folder_name??null}),9,"working")}catch{}return'Google account not connected. Please go to Settings → Keys → Google Workspace and click "Connect Google Account" to sign in with your Google account first.'+(t.title?`

The spreadsheet request has been saved to temporary memory. After reconnecting, tell me "create the pending spreadsheet" and I'll complete this automatically.`:"")}const p=await m.sheets.createSpreadsheet(t.title,t.sheet_names);let f="";if(t.folder_name)try{const{token:_}=await(await Promise.resolve().then(()=>et)).getGoogleAuth(n,s,r,a||"",i||"");f=`
Folder: "${(await vn(_,p.spreadsheetId,t.folder_name)).folderName}"`}catch(_){f=`
(Note: spreadsheet saved to Drive root — could not place in folder "${t.folder_name}": ${_.message})`}try{await new X(n).store(s,"context",`Spreadsheet: ${t.title}`,`Spreadsheet ID: ${p.spreadsheetId} | URL: ${p.url} | Sheets: ${(t.sheet_names||["Sheet1"]).join(", ")}`,7,"working")}catch{}return`Spreadsheet created: "${t.title}"${f}
ID: ${p.spreadsheetId}
URL: ${p.url}`}catch(m){return await H(n,s,"google","create_sheet",m.message),`Failed to create spreadsheet: ${m.message}`}}case"list_calendar_events":{if(!r)return"Authentication context unavailable.";try{const m=new we(n,s,r,a||"",i||""),E=t.calendar_id||"primary",p=t.days_ahead||7,f=new Date,_=new Date(f.getTime()+p*24*60*60*1e3),T=await m.calendar.listEvents(E,{timeMin:f.toISOString(),timeMax:_.toISOString(),query:t.query});return T.length===0?`No events found in the next ${p} days.`:T.map(x=>{var I;const D=x.start.dateTime||x.start.date||"TBD",A=x.end.dateTime||x.end.date||"",O=x.location?` 📍 ${x.location}`:"",C=((I=x.attendees)==null?void 0:I.map(L=>L.email).join(", "))||"";return`• ${x.summary} — ${D} to ${A}${O}${C?`
  Attendees: ${C}`:""}`}).join(`
...
The calendar event has been saved to temporary memory. After reconnecting, tell me "create the pending event" and I'll add it to your calendar.`:"")}const p=t.calendar_id||"primary",f=await m.calendar.createEvent(p,{summary:t.summary,description:t.description,location:t.location,startDateTime:t.start_datetime,endDateTime:t.end_datetime,attendees:t.attendees});try{const _=new X(n),T=await _.search(s,`Pending calendar event: "${t.summary}"`);for(const x of T)x.title.startsWith(`Pending calendar event: "${t.summary}"`)&&await _.remove(x.id,s)}catch{}return`Event created: "${f.summary}"
ID: ${f.id}
...
Use append_to_doc with the document ID above to add content.`}await H(n,s,"google","create_doc_append_fallback",`Formatted append failed, used plain text: ${x.message}`)}}let f="";if(t.folder_name)try{const{token:_}=await(await Promise.resolve().then(()=>et)).getGoogleAuth(n,s,r,a||"",i||"");f=`
Folder: "${(await vn(_,p.documentId,t.folder_name)).folderName}"`}catch(_){f=`
(Note: document saved to Drive root — could not place in folder "${t.folder_name}": ${_.message})`}try{await new X(n).store(s,"context",`Document: ${t.title}`,`Document ID: ${p.documentId} | URL: ${p.url}`,6,"working")}catch{}try{const _=t.content;await n.prepare(`INSERT OR IGNORE INTO document_library (user_id, source, drive_file_id, name, summary, extracted_text, status)
           VALUES (?, 'drive', ?, ?, ?, ?, 'parsed')`).bind(s,p.documentId,t.title,_?_.substring(0,500):null,_?_.substring(0,5e4):null).run()}catch{}try{const _=new X(n),T=await _.search(s,`Pending Google Doc save: "${t.title}"`);for(const x of T)x.title.startsWith(`Pending Google Doc save: "${t.title}"`)&&await _.remove(x.id,s)}catch{}return`Document created: "${t.title}"${f}
ID: ${p.documentId}
...
---
💡 *Say "save as note" to store this report in your notes.*`;try{const A=new X(n),O=x.report.substring(0,600);await A.store(s,"context",`Research: ${t.query.substring(0,80)}`,O,6,"long_term")}catch{}return D}catch(m){return await H(n,s,"research","research",m.message),`Research error: ${m.message}`}}case"save_note":try{const m=(t.content||"").trim();if(!m)return"Note content cannot be empty.";const E=t.source||"manual",p=["manual","research","chat"].includes(E)?E:"manual",f=await n.prepare(`INSERT INTO notes (user_id, title, content, tags, source, source_query, is_pinned)
           VALUES (?, ?, ?, ?, ?, ?, 0) RETURNING id, title`).bind(s,(t.title||"").trim(),m,(t.tags||"").trim(),p,(t.source_query||"").trim()).first();return`Note saved (ID ${f==null?void 0:f.id}): "${(f==null?void 0:f.title)||"Untitled"}"`}catch(m){return`Failed to save note: ${m.message}`}case"search_notes":try{const m=(t.query||"").trim();if(!m)return"Search query is required.";const E=`%${m}%`,f=(await n.prepare(`SELECT id, title, content, tags, is_pinned, updated_at FROM notes
           WHERE user_id = ? AND (title LIKE ? OR content LIKE ? OR tags LIKE ?)
           ORDER BY updated_at DESC LIMIT 20`).bind(s,E,E,E).all()).results||[];return f.length===0?`No notes found matching "${m}".`:f.map(_=>`[#${_.id}] ${_.is_pinned?"📌 ":""}${_.title||"Untitled"} (${_.updated_at})
${_.content.slice(0,200)}${_.content.length>200?"...":""}${_.tags?`
Tags: ${_.tags}`:""}`).join(`

`)}catch(m){return`Note search failed: ${m.message}`}case"list_notes":try{const m=Math.min(t.limit||10,50),E=t.tag,p=t.pinned_only===!0,f=["user_id = ?"],_=[s];E&&(f.push("tags LIKE ?"),_.push(`%${E}%`)),p&&f.push("is_pinned = 1"),_.push(m);const x=(await n.prepare(`SELECT id, title, content, tags, is_pinned, updated_at FROM notes
           WHERE ${f.join(" AND ")} ORDER BY is_pinned DESC, updated_at DESC LIMIT ?`).bind(..._).all()).results||[];return x.length===0?"No notes found.":x.map(D=>`[#${D.id}] ${D.is_pinned?"📌 ":""}${D.title||"Untitled"} (${D.updated_at})
${D.content.slice(0,150)}${D.content.length>150?"...":""}`).join(`

`)}catch(m){return`Failed to list notes: ${m.message}`}case"delete_note":try{const m=t.id;return m?(await n.prepare("DELETE FROM notes WHERE id = ? AND user_id = ?").bind(m,s).run()).meta.changes?`Note #${m} deleted.`:`Note #${m} not found.`:"Note ID is required."}catch(m){return`Failed to delete note: ${m.message}`}case"browser_task":{if(!r)return"Authentication context unavailable.";try{const m=await n.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(s,"browser_use_api_key").first();if(!m)return"Browser Use API key not configured. Add it in Settings → API Keys → Browser Use API Key (get one at cloud.browser-use.com).";const E=(await J(m.encrypted_value,r)).trim();let p,f=t.task,_,T;if(!t.site_name)try{const L=await n.prepare("SELECT name FROM site_credentials WHERE user_id = ?").bind(s).all(),P=f.toLowerCase(),K=(L.results||[]).find(Q=>P.includes(Q.name.toLowerCase()));K&&(t={...t,site_name:K.name},os("browser_task auto-vault: inferred site_name from task text",{siteName:K.name,userId:s}))}catch{}if(t.site_name)try{const L=await n.prepare("SELECT id, encrypted_blob FROM site_credentials WHERE user_id = ? AND name = ? COLLATE NOCASE").bind(s,t.site_name).first();if(L){const P=JSON.parse(await J(L.encrypted_blob,r));p={username:P.username,password:P.password},T=P.sessionId,_=L.id,f=`${f}

When prompted to log in, use username {username} and password {password}.`}}catch{}const x=async L=>{if(_)try{const P=await n.prepare("SELECT encrypted_blob FROM site_credentials WHERE id = ? AND user_id = ?").bind(_,s).first();if(!P)return;const K=JSON.parse(await J(P.encrypted_blob,r));K.sessionId=L,await n.prepare("UPDATE site_credentials SET encrypted_blob = ? WHERE id = ? AND user_id = ?").bind(await kt(JSON.stringify(K),r),_,s).run()}catch{}},D=async()=>{if(!(!_||!T))try{const L=await n.prepare("SELECT encrypted_blob FROM site_credentials WHERE id = ? AND user_id = ?").bind(_,s).first();if(!L)return;const P=JSON.parse(await J(L.encrypted_blob,r));delete P.sessionId,await n.prepare("UPDATE site_credentials SET encrypted_blob = ? WHERE id = ? AND user_id = ?").bind(await kt(JSON.stringify(P),r),_,s).run()}catch{}},A=/blue[\s-]?dart[\s\S]{0,100}?(\d{10,11})|(\d{10,11})[\s\S]{0,100}?blue[\s-]?dart/i.exec(f);if(A){const L=A[1]||A[2];f=Pi(L)}const O=void 0;h&&(h.apiKey=E,T&&!h.sessionId?(h.sessionId=T,h.persistSession=!0):!h.sessionId&&_&&(h.sessionId=await $i(E)??void 0)),os("browser_task starting",{userId:s,channel:y,timeoutMs:O??3e5,sessionId:h==null?void 0:h.sessionId,vaultSession:!!T});const C=await br(f,E,{secrets:p,sessionId:h==null?void 0:h.sessionId,timeoutMs:O});if(C.status==="completed"){const L=(h==null?void 0:h.sessionId)??void 0;return _&&L&&(h&&(h.persistSession=!0),await x(L)),(W=C.output)!=null&&W.includes('"captcha_required": true')?"Captcha detected — manual verification required. The site blocked automated access. Please try completing it manually or try again later.":C.output??"[NO-OUTPUT] Browser task completed but returned no content — do NOT invent or summarise what the site may have contained. Tell the user the browser returned nothing and suggest they try again."}if(C.status==="timeout"){h&&(h.hasActiveTask=!0);try{await new X(n).store(s,"context",`Browser task in progress: ${C.taskId}`,JSON.stringify({task_id:C.taskId,task:t.task}),9,"working")}catch{}try{const L=(t.task||"").substring(0,200);await n.prepare("INSERT INTO pending_browser_tasks (user_id, task_id, task_description, thread_id, channel) VALUES (?, ?, ?, ?, ?)").bind(s,C.taskId,L,(h==null?void 0:h.threadId)??null,y).run()}catch{}return`[BROWSER_TIMEOUT:${C.taskId}] Browser task did not finish within the time limit. Tell the user: "The browser is still working — I'll send you a notification as soon as it's done. No need to follow up."`}T&&E&&an(T,E).catch(()=>{}),h&&(h.persistSession=!1),await D();const I=[C.error,C.output].filter(Boolean).join(" — ");return`Browser task failed (ID: \`${C.taskId}\`): ${I||"No details returned."} | Operator hint: Check Browser Use dashboard — taskId=${C.taskId}`}catch(m){return await H(n,s,"browser","browser_task",m.message),`Browser task error: ${m.message}`}}case"browser_task_status":{if(!r)return"Authentication context unavailable.";try{const m=await n.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(s,"browser_use_api_key").first();if(!m)return"Browser Use API key not configured.";const E=await J(m.encrypted_value,r),p=await _r(t.task_id,E);if(p.done){try{const f=new X(n),_=await f.search(s,`Browser task in progress: ${t.task_id}`);for(const T of _)await f.remove(T.id,s)}catch{}return p.status==="finished"||p.status==="completed"?p.output?p.output:'[NO-OUTPUT] Browser task finished but returned no content. Do NOT invent or infer what emails or page data might have said. Tell the user: "The browser finished but returned no content — the site may have blocked automation or the login failed. Would you like to try again?"':`Browser task ended with status "${p.status}" and no output. Do NOT retry — report this to the user.`}return`[still-running] Browser task has not finished yet (status: ${p.status}). STOP — do not call browser_task_status again. Tell the user: "The browser is still working — I'll send you a notification as soon as it's done. No need to follow up."`}catch(m){return await H(n,s,"browser","browser_task_status",m.message),`Browser status check error: ${m.message}`}}case"vault_lookup":try{const m=(t.site_name||"").trim();if(!m)return"No site name provided.";const p=((await n.prepare("SELECT name FROM site_credentials WHERE user_id = ? AND name LIKE ? COLLATE NOCASE").bind(s,`%${m}%`).all()).results||[]).map(f=>f.name);return p.length===0?`No vault entries found matching "${m}".`:`Vault entries matching "${m}": ${p.join(", ")}. Use site_name="${p[0]}" in browser_task to inject credentials automatically.`}catch{return"vault_lookup: could not query Secret Vault (table may not exist — run migrations)."}case"search_places":{if(!r)return"Authentication context unavailable.";try{const m=await n.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(s,"google_api_key").first();if(!m)return"Google API Key not configured. Add it in Settings → Keys → Google API Key.";const E=await J(m.encrypted_value,r),p=await ir(E,t.query,{type:t.type});return p.error?`Places search failed: ${p.error}`:p.results.length===0?`No places found for "${t.query}".`:p.results.map((f,_)=>{const T=f.rating?` ★${f.rating} (${f.userRatingsTotal||0} reviews)`:"",x=f.openNow!==void 0?f.openNow?" · Open now":" · Closed":"",D=f.googleMapsUri?`
   ${f.googleMapsUri}`:"";return`${_+1}. **${f.name}**${T}${x}
   ${f.address}${D}
   [place_id: ${f.placeId}]`}).join(`

`)}catch(m){return await H(n,s,"google_api","search_places",m.message),`Places search error: ${m.message}`}}case"get_place_details":{if(!r)return"Authentication context unavailable.";try{const m=await n.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(s,"google_api_key").first();if(!m)return"Google API Key not configured.";const E=await J(m.encrypted_value,r),p=await or(E,t.place_id);if(p.error)return`Details lookup failed: ${p.error}`;if(!p.details)return"No details found.";const f=p.details;let _=`**${f.name}**
...
Recent Reviews:`;for(const T of f.reviews)_+=`
— ${T.author} (★${T.rating}, ${T.time}): "${T.text}"`}return _}catch(m){return await H(n,s,"google_api","place_details",m.message),`Place details error: ${m.message}`}}case"get_directions":{if(!r)return"Authentication context unavailable.";try{const m=await n.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(s,"google_api_key").first();if(!m)return"Google API Key not configured.";const E=await J(m.encrypted_value,r),p=await lr(E,t.origin,t.destination,{mode:t.mode||"driving"});if(p.error)return`Directions failed: ${p.error}`;if(!p.route)return"No route found.";const f=p.route;let _=`**${f.startAddress}** → **${f.endAddress}**
`;return _+=`📏 ${f.distance} · ⏱️ ${f.duration}`,f.durationInTraffic&&(_+=` (with traffic: ${f.durationInTraffic})`),_+=`
via ${f.summary}`,_+=`

Steps:`,f.steps.forEach((T,x)=>{_+=`
${x+1}. ${T.instruction} (${T.distance}, ${T.duration})`}),_}catch(m){return await H(n,s,"google_api","directions",m.message),`Directions error: ${m.message}`}}case"get_travel_time":{if(!r)return"Authentication context unavailable.";try{const m=await n.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(s,"google_api_key").first();if(!m)return"Google API Key not configured.";const E=await J(m.encrypted_value,r),p=await pr(E,t.origin,t.destination,t.mode||"driving");if(p.error)return`Travel time lookup failed: ${p.error}`;let f=`${t.origin} → ${t.destination}: ${p.distance}, ${p.duration}`;return p.durationInTraffic&&(f+=` (with traffic: ${p.durationInTraffic})`),f}catch(m){return await H(n,s,"google_api","travel_time",m.message),`Travel time error: ${m.message}`}}case"translate_text":{if(!r)return"Authentication context unavailable.";try{const m=await n.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(s,"google_api_key").first();if(!m)return"Google API Key not configured.";const E=await J(m.encrypted_value,r),p=await cr(E,t.text,t.target_language,t.source_language);return p.error?`Translation failed: ${p.error}`:`[${p.detectedSourceLang||t.source_language||"auto"} → ${t.target_language}]

${p.translatedText}`}catch(m){return await H(n,s,"google_api","translate",m.message),`Translation error: ${m.message}`}}case"search_youtube":{if(!r)return"Authentication context unavailable.";try{const m=await n.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(s,"google_api_key").first();if(!m)return"Google API Key not configured.";const E=await J(m.encrypted_value,r),p=await ur(E,t.query,{maxResults:t.max_results||5,order:t.order||"relevance"});return p.error?`YouTube search failed: ${p.error}`:p.results.length===0?`No YouTube results for "${t.query}".`:p.results.map((f,_)=>{var T;return`${_+1}. **${f.title}**
   ${f.channelTitle} · ${((T=f.publishedAt)==null?void 0:T.split("T")[0])||""}
   ${f.description}
   ${f.url}`}).join(`
...
Use read_library_file with the id to get the full document text.`}}catch{}const p=await n.prepare(`
        ...
        LIMIT ?
      `).bind(s,`%${m}%`,`%${m}%`,`%${m}%`,E).all(),f=await n.prepare(`
        ...
        ORDER BY dl.created_at DESC
        LIMIT ?
      `).bind(s,s,`%${m}%`,s,`%${m}%`,`%${m}%`,`%${m}%`,E).all(),_=[...p.results||[],...f.results||[]].slice(0,E);if(_.length===0)return`No documents found matching "${m}" in your library.`;const T=_.map(x=>{const D=(x.summary||x.dl_extracted||"").substring(0,200);return`[id:${x.id}] "${x.name}" (source: ${x.source}, status: ${x.status})
...
Use read_library_file with the id to get full text.`}case"read_library_file":{const m=String(t.id_or_name||"").trim();if(!m)return"id_or_name is required for read_library_file.";const E=parseInt(m,10);let p=null;if(isNaN(E)||(p=await n.prepare(`SELECT dl.id, dl.name, dl.extracted_text, dl.summary, dl.file_id
           ...
           FROM document_library dl WHERE dl.user_id = ? AND dl.name LIKE ? LIMIT 1`).bind(s,`%${m}%`).first()),!p)return`Document "${m}" not found. Use search_library to find available documents.`;let f=p.extracted_text||null;if(!f&&p.file_id){const _=await n.prepare("SELECT extracted_text FROM uploaded_files WHERE id = ? AND user_id = ?").bind(p.file_id,s).first();f=(_==null?void 0:_.extracted_text)||null}return f||(f=p.summary||null),f?`Document: ${p.name}

${f.substring(0,2e4)}`:`Document "${p.name}" has no extracted text yet. Ask Karna to parse it with parse_document(file_id="${p.file_id}") to extract the text first.`}case"create_skill":{const m=(Z=t.name)==null?void 0:Z.trim(),E=(te=t.description)==null?void 0:te.trim(),p=(ie=t.instructions)==null?void 0:ie.trim();if(!m||!E||!p)return"create_skill requires name, description, and instructions.";let f=m.toLowerCase().replace(/[^a-z0-9\s_-]/g,"").replace(/\s+/g,"_").replace(/_+/g,"_").substring(0,50).replace(/^_|_$/g,"");f||(f=`skill_${Date.now()}`);const _=await n.prepare("SELECT slug FROM user_skills WHERE user_id = ? AND slug LIKE ?").bind(s,`${f}%`).all();(re=_.results)!=null&&re.some(A=>A.slug===f)&&(f=`${f}_${(((me=_.results)==null?void 0:me.length)||0)+1}`);const T=JSON.stringify(t.parameters||{}),x=JSON.stringify(t.required_tools||[]),D=JSON.stringify(t.examples||[]);return await n.prepare(`INSERT INTO user_skills (user_id, name, slug, description, instructions, parameters, required_tools, examples)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).bind(s,m,f,E,p,T,x,D).run(),`Skill created: **${m}** (invoke as: "${f}")

You can now ask me to run "${m}" at any time. The skill will appear in Settings → Skills. You can also say "run my ${m} skill" to execute it.`}case"list_skills":{const E=t.include_disabled===!0?"SELECT id, name, slug, description, enabled, usage_count, last_used_at FROM user_skills WHERE user_id = ? ORDER BY created_at DESC":"SELECT id, name, slug, description, enabled, usage_count, last_used_at FROM user_skills WHERE user_id = ? AND enabled = 1 ORDER BY created_at DESC",f=(await n.prepare(E).bind(s).all()).results||[];if(f.length===0)return`You haven't created any custom skills yet. Ask me to create one: "Create a skill that..."`;const _=f.map(T=>`• **${T.name}** (${T.slug}): ${T.description} [used ${T.usage_count} times${T.enabled?"":" — disabled"}]`).join(`
`);return`Your custom skills (${f.length}):

${_}`}default:{const m=e,E=await n.prepare("SELECT id, name, description, instructions, required_tools FROM user_skills WHERE user_id = ? AND slug = ? AND enabled = 1").bind(s,m).first();if(E){await n.prepare("UPDATE user_skills SET usage_count = usage_count + 1, last_used_at = CURRENT_TIMESTAMP WHERE id = ?").bind(E.id).run();const p=(()=>{try{return JSON.parse(E.required_tools).join(", ")}catch{return""}})(),f=Object.keys(t).length>0?`
...
Execute the steps above using your available tools. When complete, provide a clear summary of what was done and any results.`}return`Unknown tool: ${e}`}}}async function Hr(e,t,n,s,r){if(t.length>0&&t[t.length-1].role==="user"){const a="(Previous request did not complete. Please try again.)";await e.storeMessage(n,s,"assistant",a,"{}",r),t.push({id:-1,user_id:n,channel:s,role:"assistant",content:a,metadata:"{}",token_estimate:a.length,created_at:new Date().toISOString()})}}function Fr(e){for(let t=e.length-1;t>=0;t--)if(e[t].role==="assistant"){const n=typeof e[t].content=="string"?e[t].content:"";n.length<300&&/^(now let me|let me |i'll |i will |i'm going to|let's |to do this)/i.test(n.trim())&&(e[t]={...e[t],content:"(My previous response was cut off before completing. Starting fresh.)"});break}}async function ds(e,t,n,s,r,a,i){var ie,re,me,m,E;const o=new X(t),l=(ie=e.metadata)==null?void 0:ie.thread_id,c=Date.now(),[d,u,g,y]=await Promise.all([o.buildContext(s.id),On(t,s.id),Wn(t,s.id),Or(t,s.id)]),h=await o.getRecentConversations(s.id,30,l);await Hr(o,h,s.id,e.channel,l);const v=Br(s,d,e.channel,g,y,u),w=Fn(h),b=qn([{role:"system",content:v},...Hn(h),{role:"user",content:e.text}]);$r(b,w),Fr(b);const S=[/\bmy (address|phone|email|budget|preferences?|settings?|rules?|sheet|doc|folder|password|login|account)\b/i,/\b(do you remember|last time|what did i|you said|i told you|my usual|like before|same as last|remind me what|what was the|previously)\b/i],R=(d.match(/^- /gm)||[]).length;if(S.some(p=>p.test(e.text))||R<3)try{const p=await o.searchLongTerm(s.id,e.text,5);if(p.length>0){const f=p.map(_=>`- [${_.type}] ${_.title}: ${_.content}`).join(`
...
${f}]`})}}catch{}await o.storeMessage(s.id,e.channel,"user",e.text,"{}",l);const N=(i==null?void 0:i.maxTurns)??10,U=(i==null?void 0:i.tools)??await Gn(t,s.id);let M="",G=0;const W=[];let ne,$=0,z=0;const q={hasActiveTask:!1,persistSession:!1,threadId:l,channel:e.channel};for(let p=0;p<N;p++){$=p+1;try{p>0&&Ur(b);const f=await n.chat(b,{tools:U,toolChoice:p===0&&(i!=null&&i.forceToolUseOnFirstTurn)?"required":void 0});if(f.usage&&(G+=f.usage.promptTokens+f.usage.completionTokens),f.toolCalls&&f.toolCalls.length>0){const _=f.content||"(tools executed)";b.push({role:"assistant",content:_});for(const x of f.toolCalls)W.push(x.name);const T=await Promise.all(f.toolCalls.map(async x=>{try{const D=await Bt(x.name,x.arguments,t,s.id,{agentType:"full",providerName:n.name,channel:e.channel},s.pin_hash,a==null?void 0:a.GOOGLE_CLIENT_ID,a==null?void 0:a.GOOGLE_CLIENT_SECRET,a==null?void 0:a.GOOGLE_API_KEY,a==null?void 0:a.GOOGLE_CSE_ID,s.timezone,n,a==null?void 0:a.DOCUMENTS_BUCKET,{ai:a==null?void 0:a.AI,vectorize:a==null?void 0:a.VECTORIZE},q);ne=Mr(x.name,x.arguments,D,ne);const A=["parse_document","drive_read_file","read_library_file"].includes(x.name)?2e4:8e3,O=D.length>A?D.substring(0,A)+`
...
`)){const g=u.trim().split("|");if(g.length<4)continue;const[y,h,v,w]=g,b=["fact","preference","context","decision","summary","task"].find(R=>R===y.trim().toLowerCase());if(!b||!(h!=null&&h.trim())||!(v!=null&&v.trim()))continue;const S=Math.min(10,Math.max(1,parseInt(w)||5));await s.store(n.id,b,h.trim(),v.trim(),S,"long_term")}}const us={"claude-opus-4-8":1e6,"claude-sonnet-4-6":1e6,"claude-haiku-4-5":2e5,"gpt-4o":128e3,"gpt-4o-mini":128e3,"grok-3-mini":131072,"grok-3":131072,"deepseek-chat":64e3,"gemini-2.0-flash":1e6,default:32e3};function yo(e){for(const[t,n]of Object.entries(us))if(e.toLowerCase().includes(t.toLowerCase()))return n;return us.default}function vo(e,t,n,s){const r=yo(s),a=Math.floor(r*.75),i=[];let o=0,l=!1;const c=fn(e);i.push({role:"system",content:e}),o+=c;const d=fn(n);o+=d;const u=a-o,g=[];let y=0;for(let h=t.length-1;h>=0;h--){const v=t[h],w=fn(v.content);if(y+w<=u)g.unshift({role:v.role,content:v.content}),y+=w;else{l=!0;break}}return i.push(...g),i.push({role:"user",content:n}),o+=y,{maxTokens:r,usedTokens:o,messages:i,wasTruncated:l}}async function*wo(e,t,n,s,r,a){var me,m,E;const i=new X(t),o=(me=e.metadata)==null?void 0:me.thread_id,l=Date.now();yield{type:"thinking",data:{threadId:o,provider:n.name}};const[c,d,u,g]=await Promise.all([i.buildContext(s.id),On(t,s.id),Wn(t,s.id),Or(t,s.id)]),y=await i.getRecentConversations(s.id,30,o);await Hr(i,y,s.id,e.channel,o);const h=Br(s,c,e.channel,u,g,d),v=Fn(y),w=qn([...Hn(y)]);let b=e.text;v&&(b=`${xn}

${e.text}`);const S=vo(h,w.map(p=>({role:p.role,content:p.content})),b,n.name);await i.storeMessage(s.id,e.channel,"user",e.text,"{}",o);const R=await Gn(t,s.id),B=10;let N="",U=0;const M=[...S.messages],G=[];let W,ne=0,$=0;const z={hasActiveTask:!1,persistSession:!1,threadId:o,channel:e.channel},q=[/\bmy (address|phone|email|budget|preferences?|settings?|rules?|sheet|doc|folder|password|login|account)\b/i,/\b(do you remember|last time|what did i|you said|i told you|my usual|like before|same as last|remind me what|what was the|previously)\b/i],Z=(c.match(/^- /gm)||[]).length;if(q.some(p=>p.test(e.text))||Z<3||v)try{const p=await i.searchLongTerm(s.id,e.text,5);if(p.length>0){const f=p.map(_=>`- [${_.type}] ${_.title}: ${_.content}`).join(`
...
`;for(const c of a)i+=`- ${c.name} (${c.type}, ${Math.round(c.size/1024)}KB, file_id: ${c.file_id})`,c.text_preview&&(i+=`
  Preview: ${c.text_preview.substring(0,300)}...`),i+=`
`;i+="Use parse_document tool to read full file contents. Use drive_upload to upload to Google Drive.]"}let o=r;if(!o){const c=await e.env.DB.prepare("INSERT INTO threads (user_id, title) VALUES (?, ?) RETURNING *").bind(t.id,n.trim().substring(0,60)+(n.trim().length>60?"...":"")).first();o=c==null?void 0:c.id}const l={userId:t.id,username:t.username,channel:s,text:n.trim()+i,sessionId:e.get("sessionId"),timestamp:new Date().toISOString(),metadata:{thread_id:o}};try{const{provider:c,rotation:d}=await Ve(e.env.DB,t.id,t.pin_hash),u=new ReadableStream({async start(g){const y=new TextEncoder;try{const h=bo(l,e.env.DB,c,t,d,{GOOGLE_CLIENT_ID:e.env.GOOGLE_CLIENT_ID,GOOGLE_CLIENT_SECRET:e.env.GOOGLE_CLIENT_SECRET,GOOGLE_API_KEY:e.env.GOOGLE_API_KEY,GOOGLE_CSE_ID:e.env.GOOGLE_CSE_ID,DOCUMENTS_BUCKET:e.env.DOCUMENTS_BUCKET});for await(const v of h)v.data.threadId||(v.data.threadId=o),g.enqueue(y.encode(ps(v)));o&&await e.env.DB.prepare("UPDATE threads SET message_count = message_count + 2, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(o).run(),g.close()}catch(h){const v={type:"error",data:{error:h.message||"An error occurred",threadId:o}};g.enqueue(y.encode(ps(v))),g.close()}}});return new Response(u,{headers:{"Content-Type":"text/event-stream","Cache-Control":"no-cache",Connection:"keep-alive","X-Thread-Id":String(o||"")}})}catch(c){console.error("Stream setup error:",c);const d=c.message||"";return d.includes("No LLM provider configured")?e.json({error:"No AI provider configured. Please add at least one API key in Settings → Keys.",type:"no_provider",thread_id:o},400):d.includes("429")||d.includes("limit reached")||d.includes("rate limit")||d.includes("Too Many Requests")?e.json({error:"Rate limit reached across all configured providers. Add a second AI provider in Settings → API Keys (Gemini and DeepSeek offer free tiers) to enable automatic failover and avoid this.",type:"rate_limit",thread_id:o},429):e.json({error:"Something went wrong setting up the stream.",details:d,thread_id:o},500)}});le.get("/threads/:id/messages",async e=>{var a;const t=e.get("user"),n=parseInt(e.req.param("id")),s=parseInt(e.req.query("limit")||"50"),r=await e.env.DB.prepare(`SELECT id, role, content, channel, created_at FROM conversations 
...
CRITICAL SAFETY RULE: This is a RECURRING scheduled task. You MUST NOT call gmail_send or gmail_draft — sending emails on every cron tick spams recipients. Report findings as text only.`:""}${Ot}`:`[Scheduled task "${e}"]: ${t||"Execute this scheduled task."}${Ot}`}Ne.post("/cron/check-browser-tasks",async e=>{const t=e.req.header("X-Cron-Secret")||"",n=e.env.CRON_SECRET||"karna-cron-default-v1";if(t!==n)return e.json({error:"Unauthorized"},401);let s=0,r=0;try{const a=await e.env.DB.prepare(`SELECT pbt.id, pbt.user_id, pbt.task_id, pbt.task_description,
       ...
       LIMIT 10`).all();for(const i of a.results||[]){s++;try{const o=await e.env.DB.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = 'browser_use_api_key'").bind(i.user_id).first();if(!o)continue;const l=(await J(o.encrypted_value,i.pin_hash)).trim(),c=await _r(i.task_id,l,{waitMs:8e3});if(!c.done||!(await e.env.DB.prepare(`UPDATE pending_browser_tasks SET notified = 1
           WHERE user_id = ? AND task_id = ? AND notified = 0`).bind(i.user_id,i.task_id).run()).meta.changes)continue;const u=i.task_description?`"${i.task_description.substring(0,80)}${i.task_description.length>80?"...":""}"`:"Your browser task";let g,y;if(c.status==="finished"&&c.output?(g="Browser task completed",y=`${u} finished.

${c.output.substring(0,500)}${c.output.length>500?"...":""}`):c.status==="finished"?(g="Browser task completed (no output)",y=`${u} finished, but the browser returned no readable content. You may want to retry.`):(g="Browser task ended",y=`${u} ended with status "${c.status}". Check the browser dashboard for details.`),i.thread_id){const h=c.status==="finished"&&c.output?c.output.substring(0,8e3):y,v=Math.ceil(h.length/4);try{await e.env.DB.prepare(`INSERT INTO conversations (user_id, channel, role, content, metadata, token_estimate, thread_id)
               VALUES (?, ?, 'assistant', ?, '{}', ?, ?)`).bind(i.user_id,i.channel||"web",h,v,i.thread_id).run()}catch{}}i.pin_hash&&await Ze(e.env.DB,i.user_id,g,y,{pinHash:i.pin_hash,tags:["browser","karna"]}),r++,await new Promise(h=>setTimeout(h,200))}catch{}}}catch{}return e.json({checked:s,notified:r})});Ne.get("/scorecard/weekly",async e=>{var u;const t=(u=e.req.header("Authorization"))==null?void 0:u.replace("Bearer ","");if(!t)return e.json({error:"Auth required"},401);const n=await e.env.DB.prepare("SELECT s.user_id FROM sessions s WHERE s.id = ? AND s.expires_at > datetime('now')").bind(t).first();if(!n)return e.json({error:"Invalid session"},401);const s=n.user_id,r=new Date(Date.now()-10080*60*1e3).toISOString(),[a,i,o]=await Promise.all([e.env.DB.prepare(`SELECT COUNT(*) as total,
...
Status: ✅ Online`,u=await ae(n,t,d,"Markdown",r,s.id);u.success||console.warn(`[/status] Failed to send message: ${u.errors.join(" | ")}`)}catch{const o=await ae(n,t,"✅ Online — but had trouble fetching stats.","Markdown",r,s==null?void 0:s.id);o.success||console.warn(`[/status error] Failed to send message: ${o.errors.join(" | ")}`)}return!0}case"/new":{if(!s){const o=await ae(n,t,"⚠️ Account not linked.","Markdown",r);return o.success||console.warn(`[/new] Failed to send message: ${o.errors.join(" | ")}`),!0}await r.prepare("UPDATE threads SET is_archived = 1 WHERE user_id = ? AND channel = 'telegram' AND is_archived = 0").bind(s.id).run();const i=await ae(n,t,"🆕 Starting fresh conversation. Your next message begins a new thread.","Markdown",r,s.id);return i.success||console.warn(`[/new] Failed to send message: ${i.errors.join(" | ")}`),!0}case"/tasks":case"/task":{if(!s){const i=await ae(n,t,"⚠️ Account not linked.","Markdown",r);return i.success||console.warn(`[/tasks] Failed to send message: ${i.errors.join(" | ")}`),!0}try{const o=(await r.prepare(`
          ...
          ORDER BY
            ...
            importance DESC
          LIMIT 20
        `).bind(s.id).all()).results||[];if(o.length===0){const h=await ae(n,t,"✅ No open tasks. You're all clear.","Markdown",r,s.id);return h.success||console.warn(`[/tasks] Failed to send message: ${h.errors.join(" | ")}`),!0}const l=new Date,c=l.toISOString().slice(0,10),d=new Date(l);d.setDate(d.getDate()+1);const u=d.toISOString().slice(0,10),g=[`📋 *Open Tasks (${o.length})*
`];for(const h of o){let v="";if(h.due_date){const w=h.due_date.slice(0,10);w<c?v=" ⚠️ _overdue_":w===c?v=" 🔴 _due today_":w===u?v=" 🟡 _due tomorrow_":v=` _${new Date(h.due_date).toLocaleDateString("en-GB",{day:"numeric",month:"short"})}_`}g.push(`☐ ${h.title}${v}`)}g.push(`
_Say "mark [task] as done" to close a task._`);const y=await ae(n,t,g.join(`
...
If you requested a browser task, the result will arrive as a notification when ready. For other long tasks, try the web app.`:(c=$.message)!=null&&c.includes("API error")?`⚠️ AI provider returned an error. The provider (${U.name}) may be temporarily unavailable. Your message was saved — try again shortly.`:`⚠️ Something went wrong processing your message. Error: ${($.message||"Unknown").substring(0,200)}`,Z=await ae(R,w,q,"Markdown",n,S.id);ne=Z.success,Z.success||console.error(`[CRITICAL] Error message failed to send to Telegram for user ${S.id}:`,Z.errors);try{const{logError:te}=await Promise.resolve().then(()=>vt);await te(n,S.id,"telegram",z?"timeout":"agent_error",$.message||"Agent error",{provider:U.name})}catch{}}}catch(d){console.error("Telegram webhook error:",d);try{const{logError:u}=await Promise.resolve().then(()=>vt);await u(n,null,"telegram","webhook_error",d.message||"Unknown telegram error")}catch{}}}function ys(e){if(!e)return null;if(e.startsWith("cron:")){const t=parseInt(e.replace("cron:",""),10);return isNaN(t)?null:t}return null}function Fo(e){const t=new Date(new Date().toLocaleString("en-US",{timeZone:e})),n=new Date(t);n.setDate(n.getDate()+1),n.setHours(9,0,0,0);const s=new Date(n.toLocaleString("en-US",{timeZone:"UTC"})),r=new Date(n.toLocaleString("en-US",{timeZone:e}));return new Date(n.getTime()+(s.getTime()-r.getTime())).toISOString()}const Kr=e=>[[{text:"✅ Seen",callback_data:`notif_seen:${e}`},{text:"⏰ Snooze",callback_data:`notif_snooze_menu:${e}`},{text:"✓ Done",callback_data:`notif_done:${e}`}]],qo=e=>[[{text:"10 minutes",callback_data:`notif_snooze:${e}:10m`},{text:"1 hour",callback_data:`notif_snooze:${e}:1h`}],[{text:"Tomorrow 9 AM",callback_data:`notif_snooze:${e}:tomorrow`},{text:"← Back",callback_data:`notif_back:${e}`}]];async function mt(e,t,n){const s=await De(`https://api.telegram.org/bot${e}/answerCallbackQuery`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({callback_query_id:t,text:n})});s.ok||console.warn("[answerCallback]",t,s.status)}async function pt(e,t,n,s){const r=await De(`https://api.telegram.org/bot${e}/editMessageReplyMarkup`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({chat_id:t,message_id:n,reply_markup:s?{inline_keyboard:s}:{}})});r.ok||console.warn("[editKeyboard]",t,n,r.status)}async function Go(e,t,n,s,r,a,i,o,l,c){const d=await e.prepare("SELECT * FROM notifications WHERE id = ? AND user_id = ?").bind(o,a).first();if(!d){await mt(t,n,"Notification not found — may have already been actioned."),await pt(t,s,r,null);return}if(i==="notif_seen")await e.prepare("DELETE FROM notifications WHERE id = ? AND user_id = ?").bind(o,a).run(),await mt(t,n,"✅ Dismissed"),await pt(t,s,r,null);else if(i==="notif_snooze_menu")await mt(t,n),await pt(t,s,r,qo(o));else if(i==="notif_back")await mt(t,n),await pt(t,s,r,Kr(o));else if(i==="notif_snooze"){const u=ys(d.source);u&&await e.prepare("UPDATE cron_jobs SET state = 'completed', enabled = 0, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?").bind(u,a).run();let g,y;l==="10m"?(g=new Date(Date.now()+600*1e3).toISOString(),y="10 minutes"):l==="1h"?(g=new Date(Date.now()+3600*1e3).toISOString(),y="1 hour"):(g=Fo(c||"UTC"),y="tomorrow 9 AM"),await e.prepare(`INSERT INTO cron_jobs (user_id, name, description, schedule_type, schedule_value, action_type, action_config, next_run, enabled, state)
  ...
  `).bind(y,y,g.id).run();const h=await e.prepare(`SELECT c.encrypted_value, u.pin_hash FROM credentials c 
     ...
     WHERE s.id = ? AND s.expires_at > datetime('now')`).bind(t).first();if(!n)return e.json({error:"Invalid session"},401);const s=await e.env.DB.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(n.user_id,"telegram_bot_token").first();if(!s)return e.json({configured:!1,error:"Bot token not set"});const r=await J(s.encrypted_value,n.pin_hash);try{const g=await(await fetch(`https://api.telegram.org/bot${r}/getWebhookInfo`)).json();return e.json({configured:!0,webhook_url:((i=g.result)==null?void 0:i.url)||"",has_webhook:!!((o=g.result)!=null&&o.url),pending_updates:((l=g.result)==null?void 0:l.pending_update_count)||0,last_error:((c=g.result)==null?void 0:c.last_error_message)||"",last_error_date:((d=g.result)==null?void 0:d.last_error_date)||null})}catch(u){return e.json({configured:!0,error:u.message})}});Gt.post("/detect-chat-id",async e=>{var a,i;const t=(a=e.req.header("Authorization"))==null?void 0:a.replace("Bearer ","");if(!t)return e.json({error:"Auth required"},401);const n=await e.env.DB.prepare(`SELECT s.user_id, u.pin_hash FROM sessions s JOIN users u ON s.user_id = u.id
     WHERE s.id = ? AND s.expires_at > datetime('now')`).bind(t).first();if(!n)return e.json({error:"Invalid session"},401);const s=await e.env.DB.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(n.user_id,"telegram_bot_token").first();if(!s)return e.json({error:"Bot token not configured"},400);const r=await J(s.encrypted_value,n.pin_hash);try{const c=((i=(await(await fetch(`https://api.telegram.org/bot${r}/getWebhookInfo`)).json()).result)==null?void 0:i.url)||"";await fetch(`https://api.telegram.org/bot${r}/deleteWebhook`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({drop_pending_updates:!1})}),await new Promise(w=>setTimeout(w,500));const u=await(await fetch(`https://api.telegram.org/bot${r}/getUpdates?limit=10&timeout=0`)).json();c&&await fetch(`https://api.telegram.org/bot${r}/setWebhook`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:c,allowed_updates:[...zr]})});const g=u.result||[];if(g.length===0)return e.json({found:!1,message:"No messages found. Send any message to your bot on Telegram first, then try again within 30 seconds."});const y=[],h=new Set;for(let w=g.length-1;w>=0;w--){const b=g[w].message;if(b&&b.chat){const S=String(b.chat.id);h.has(S)||(h.add(S),y.push({chat_id:S,name:[b.chat.first_name,b.chat.last_name].filter(Boolean).join(" ")||b.chat.title||"Unknown",username:b.chat.username||"",date:new Date((b.date||0)*1e3).toISOString()}))}}if(y.length===0)return e.json({found:!1,message:"No chat messages found in updates. Try sending a message to the bot and click detect again."});const v=y[0].chat_id;return await e.env.DB.prepare("UPDATE users SET telegram_chat_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(v,n.user_id).run(),e.json({found:!0,chat_id:v,name:y[0].name,all_chats:y,message:`Chat ID ${v} detected and saved to your profile.`})}catch(o){return e.json({error:`Detection failed: ${o.message}`},500)}});function Ko(e){const t=new Date,n=new Date(t.toLocaleString("en-US",{timeZone:e})),s=new Date(n);s.setDate(s.getDate()+1),s.setHours(0,0,0,0);const r=new Date(s);r.setHours(23,59,59,999);const a=s.toISOString().split("T")[0];return{start:s.toISOString(),end:r.toISOString(),dateStr:a}}function on(e,t=new Date){return new Intl.DateTimeFormat("en-CA",{timeZone:e||"Asia/Kolkata",year:"numeric",month:"2-digit",day:"2-digit"}).format(t)}async function Yo(e,t,n,s,r,a){try{return(await new Cn(e,t,n,s,r).listEvents("primary",{timeMin:a.start,timeMax:a.end,maxResults:50})).map(l=>{var c;return{id:l.id||`google-${Date.now()}`,title:l.summary||"Untitled Event",startTime:l.start.dateTime||l.start.date||"",endTime:l.end.dateTime||l.end.date||"",location:l.location,attendees:(c=l.attendees)==null?void 0:c.map(d=>d.displayName||d.email),source:"google"}})}catch(i){return console.error("Google Calendar fetch error:",i.message),[]}}async function Jo(e,t,n,s,r){try{const a=new Re(e,t,n,s,r),i=await a.listMessages({query:"is:unread",maxResults:50,labelIds:["INBOX"]}),o=await a.listMessages({query:"is:important is:unread",maxResults:10}),l={};for(const u of i){const g=u.from.split("<")[0].trim()||u.from;l[g]=(l[g]||0)+1}const c=Object.entries(l).sort(([,u],[,g])=>g-u).slice(0,5).map(([u])=>u),d=i.some(u=>u.subject.toLowerCase().includes("urgent")||u.subject.toLowerCase().includes("asap")||u.subject.toLowerCase().includes("immediately"));return{unreadCount:i.length,importantCount:o.length,topSenders:c,hasUrgent:d}}catch(a){return console.error("Gmail fetch error:",a.message),{unreadCount:0,importantCount:0,topSenders:[],hasUrgent:!1}}}async function Vo(e,t){try{const n=await e.prepare(`
...
🕐 ${b}${S}`;await Ze(e.env.DB,l.id,R,B,{pinHash:l.pin_hash,priority:"high",tags:["calendar","karna"]})}r.push({user_id:l.id,reminders_sent:v.length})}catch(c){r.push({user_id:l.id,status:"error",error:c.message})}return e.json({executed:r.length,results:r})}catch(s){return e.json({error:s.message},500)}});fe.post("/cron/morning-briefing",async e=>{const t=e.req.header("X-Cron-Secret")||"",n=e.env.CRON_SECRET||"karna-cron-default-v1";if(t!==n)return e.json({error:"Unauthorized"},401);try{const s=await e.env.DB.prepare(`
      ...
      FROM users u
      LEFT JOIN briefing_preferences bp ON u.id = bp.user_id
    `).all(),r=[],a=new Date;for(const i of s.results||[]){if(!i.morning_briefing_enabled)continue;const o=i.timezone||"Asia/Kolkata",l=i.morning_briefing_time||"08:00";if(!Jr(l,o,a))continue;const c=on(o,a);if(!await e.env.DB.prepare("SELECT 1 FROM briefings WHERE user_id = ? AND briefing_type = 'morning' AND briefing_date = ? LIMIT 1").bind(i.id,c).first())try{const u=await Zr(e.env.DB,i,{GOOGLE_CLIENT_ID:e.env.GOOGLE_CLIENT_ID,GOOGLE_CLIENT_SECRET:e.env.GOOGLE_CLIENT_SECRET});let g={telegram:!0,web:!0};try{g=JSON.parse(i.notification_channels||"{}")}catch{}if(g.web!==!1&&u.briefingId){const y=dl(u.content);await Ze(e.env.DB,i.id,"Morning Briefing",y,{pinHash:i.pin_hash,tags:["briefing","karna"]}),await e.env.DB.prepare("UPDATE briefings SET delivered_telegram = 1 WHERE id = ?").bind(u.briefingId).run()}r.push({user_id:i.id,status:"success",briefing_id:u.briefingId,briefing_time:l,timezone:o})}catch(u){r.push({user_id:i.id,status:"error",error:u.message})}}return e.json({executed:r.length,results:r})}catch(s){return e.json({error:s.message},500)}});fe.post("/cron/email-digest",async e=>{const t=e.req.header("X-Cron-Secret")||"",n=e.env.CRON_SECRET||"karna-cron-default-v1";if(t!==n)return e.json({error:"Unauthorized"},401);try{const s=await e.env.DB.prepare(`
...
`,i);if(l>a+1800/2)i=l+2;else{const c=e.lastIndexOf(". ",i);c>a+1800/2&&(i=c+2)}}const o=e.slice(a,i).trim();o.length>=100&&r.push(o),a=i-200,a<=0&&(a=i)}return r}async function El(e,t,n,s){if(!e.AI||!e.VECTORIZE)return;const r=na(s);if(r.length===0)return;const a=await e.DB.prepare("SELECT vector_id FROM document_chunks WHERE document_id = ?").bind(n).all();a.results.length>0&&(await e.VECTORIZE.deleteByIds(a.results.map(d=>d.vector_id)),await e.DB.prepare("DELETE FROM document_chunks WHERE document_id = ?").bind(n).run());const o=(await e.AI.run("@cf/baai/bge-large-en-v1.5",{text:r})).data,l=r.map((d,u)=>`doc_${n}_${u}`);await e.VECTORIZE.insert(r.map((d,u)=>({id:l[u],values:o[u],metadata:{userId:String(t),documentId:String(n)}})));const c=e.DB.prepare("INSERT INTO document_chunks (user_id, document_id, chunk_index, text, vector_id) VALUES (?, ?, ?, ?, ?)");await e.DB.batch(r.map((d,u)=>c.bind(t,n,u,d,l[u])))}async function Tl(e,t,n,s=5){if(!e.AI||!e.VECTORIZE)return[];const a=(await e.AI.run("@cf/baai/bge-large-en-v1.5",{text:[n]})).data[0],i=await e.VECTORIZE.query(a,{topK:s*3,filter:{userId:String(t)}});if(!i.matches||i.matches.length===0)return[];const o=i.matches.map(v=>v.id),l=new Map(i.matches.map(v=>[v.id,v.score])),c=o.map(()=>"?").join(","),u=((await e.DB.prepare(`SELECT dc.text, dc.vector_id, dc.document_id, dl.name, dc.chunk_index
     ...
     WHERE dc.vector_id IN (${c}) AND dc.user_id = ?`).bind(...o,t).all()).results||[]).map(v=>({filename:v.name,relevance_score:l.get(v.vector_id)??0,chunk:v.text,document_id:v.document_id,chunk_index:v.chunk_index,retrieval_method:"vector"})),y=((await e.DB.prepare(`SELECT dc.text, dc.document_id, dc.chunk_index, dl.name
     FROM document_chunks dc
     JOIN document_library dl ON dc.document_id = dl.id
     WHERE dc.user_id = ? AND dc.text LIKE ?
     ORDER BY dc.chunk_index DESC
     LIMIT ?`).bind(t,`%${n.substring(0,80)}%`,s*2).all()).results||[]).map(v=>({filename:v.name,relevance_score:.55,chunk:v.text,document_id:v.document_id,chunk_index:v.chunk_index,retrieval_method:"keyword"})),h=new Map;for(const v of[...u,...y]){const w=`${v.document_id}:${v.chunk_index}`;if(!h.has(w))h.set(w,v);else{const b=h.get(w);h.set(w,{...b,relevance_score:Math.max(b.relevance_score,v.relevance_score),retrieval_method:"hybrid"})}}return[...h.values()].sort((v,w)=>w.relevance_score-v.relevance_score).slice(0,s)}const Ke=Object.freeze(Object.defineProperty({__proto__:null,chunkText:na,indexDocumentChunks:El,semanticDocumentSearch:Tl},Symbol.toStringTag,{value:"Module"}));export{bs as default};
