...
`),t.trim()}async function Ua(e,t,a={}){const r=a.maxPages||(a.depth==="thorough"?5:3),n=a.maxResults||(a.depth==="thorough"?8:5),s=await mt(e,{num:n,site:a.site});if(s.error)return{report:"",sources:[],pagesRead:0,error:`Search failed: ${s.error}`};if(s.results.length===0)return{report:`No web results found for "${e}".`,sources:[],pagesRead:0};const i=s.results.slice(0,r).map(async v=>{const y=await Pa(v.link);return{title:v.title,url:v.link,displayLink:v.displayLink,snippet:v.snippet,content:y.text,error:y.error}}),c=(await Promise.all(i)).filter(v=>v.content.length>50);if(c.length===0){const v=s.results.map((f,_)=>`[${_+1}] ${f.title}
${f.snippet}
Source: ${f.link}`).join(`

`);return{report:await Ht(e,v,t,"snippets"),sources:s.results.map(f=>({title:f.title,url:f.link})),pagesRead:0}}const u=c.map((v,y)=>`--- SOURCE ${y+1}: ${v.title} (${v.displayLink}) ---
${v.content}
--- END SOURCE ${y+1} ---`).join(`

`);return{report:await Ht(e,u,t,"full"),sources:c.map(v=>({title:v.title,url:v.url})),pagesRead:c.length}}async function Ht(e,t,a,r){const s=`You are a research analyst. Your job is to synthesize web research into clear, actionable reports.
...
Note: Always use this date/time as the current time. Do NOT guess or use UTC.`}async function zt(e,t,a){var c;const n=await(await fetch(`https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(`name='${a.replace(/'/g,"\\'")}' and mimeType='application/vnd.google-apps.folder' and trashed=false`)}&fields=files(id,name)`,{headers:{Authorization:`Bearer ${e}`}})).json();let s;((c=n.files)==null?void 0:c.length)>0?s=n.files[0].id:s=(await(await fetch("https://www.googleapis.com/drive/v3/files",{method:"POST",headers:{Authorization:`Bearer ${e}`,"Content-Type":"application/json"},body:JSON.stringify({name:a,mimeType:"application/vnd.google-apps.folder"})})).json()).id;const l=((await(await fetch(`https://www.googleapis.com/drive/v3/files/${t}?fields=parents`,{headers:{Authorization:`Bearer ${e}`}})).json()).parents||[]).join(",");return await fetch(`https://www.googleapis.com/drive/v3/files/${t}?addParents=${s}&removeParents=${l}`,{method:"PATCH",headers:{Authorization:`Bearer ${e}`,"Content-Type":"application/json"},body:JSON.stringify({})}),{folderId:s,folderName:a}}function gt(e){try{const t=new Date;return new Intl.DateTimeFormat("en-US",{timeZone:e,weekday:"long",year:"numeric",month:"long",day:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:!0}).format(t)}catch{return new Date().toISOString()}}async function ft(e,t,a,r,n,s,o,i,l,c,u){var v,y,f,_,x,E;const h=new K(a);switch(e){case"create_schedule":{const d=new Date;let g;const m=c||"UTC";if(t.schedule_type==="interval"){const p=parseInt(t.schedule_value,10);g=new Date(d.getTime()+p*60*1e3)}else if(t.schedule_type==="daily"){const[p,w]=t.schedule_value.split(":").map(Number),T=d.toLocaleString("en-US",{timeZone:m}),S=new Date(T),I=new Date(S);I.setHours(p,w,0,0),I<=S&&I.setDate(I.getDate()+1);const A=new Date(I.toLocaleString("en-US",{timeZone:"UTC"})),O=new Date(I.toLocaleString("en-US",{timeZone:m})),N=A.getTime()-O.getTime();g=new Date(I.getTime()+N)}else if(t.schedule_type==="weekly"){const[p,w]=t.schedule_value.split(" "),[T,S]=(w||"00:00").split(":").map(Number),A=["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"].findIndex(Ie=>Ie.toLowerCase()===p.toLowerCase()),O=d.toLocaleString("en-US",{timeZone:m}),N=new Date(O),$=new Date(N);$.setHours(T,S,0,0);let H=(A-$.getDay()+7)%7;H===0&&$<=N&&(H=7),$.setDate($.getDate()+H);const ke=new Date($.toLocaleString("en-US",{timeZone:"UTC"})),te=new Date($.toLocaleString("en-US",{timeZone:m})),Se=ke.getTime()-te.getTime();g=new Date($.getTime()+Se)}else if(t.schedule_type==="once"){const[p,w]=t.schedule_value.split(" "),[T,S,I]=p.split("-").map(Number),[A,O]=(w||"00:00").split(":").map(Number),N=d.toLocaleString("en-US",{timeZone:m}),$=new Date(N),H=new Date($);H.setFullYear(T,S-1,I),H.setHours(A,O,0,0);const ke=new Date(H.toLocaleString("en-US",{timeZone:"UTC"})),te=new Date(H.toLocaleString("en-US",{timeZone:m})),Se=ke.getTime()-te.getTime();g=new Date(H.getTime()+Se)}else g=new Date(d.getTime()+3600*1e3);return await a.prepare(`INSERT INTO cron_jobs (user_id, name, description, schedule_type, schedule_value, action_type, action_config, next_run, state)
...
`)}case"toggle_schedule":{const d=t.enabled?1:0,g=d?"active":"paused";return await a.prepare("UPDATE cron_jobs SET enabled = ?, state = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?").bind(d,g,t.job_id,r).run(),`Schedule ${t.job_id} ${d?"enabled (active)":"paused"}.`}case"update_schedule_state":{const d=["created","active","reminding","paused","completed"],g=t.state;if(!d.includes(g))return`Invalid state "${g}". Valid states: ${d.join(", ")}`;const m=g==="completed"||g==="paused"?0:1;return await a.prepare("UPDATE cron_jobs SET state = ?, enabled = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?").bind(g,m,t.job_id,r).run(),`Schedule ${t.job_id} state updated to "${g}".`}case"delete_schedule":return await a.prepare("DELETE FROM cron_jobs WHERE id = ? AND user_id = ?").bind(t.job_id,r).run(),`Schedule ${t.job_id} deleted.`;case"store_memory":{const d=t.importance||5,g=d>=7?"working":"long_term";return await h.store(r,t.type,t.title,t.content,d,g),`Stored in ${g==="working"?"working":"long-term"} memory: [${t.type}] ${t.title} (importance: ${d})`}case"search_memory":{const d=await h.search(r,t.query);return d.length===0?"No matching memories found.":d.map(g=>`[${g.tier||"long_term"}] [${g.type}] **${g.title}**: ${g.content}`).join(`
`)}case"get_system_status":{const d=await a.prepare("SELECT COUNT(*) as cnt FROM cron_jobs WHERE user_id = ? AND enabled = 1").bind(r).first(),g=await a.prepare("SELECT COUNT(*) as cnt FROM memory WHERE user_id = ?").bind(r).first(),m=await a.prepare("SELECT COUNT(*) as cnt FROM memory WHERE user_id = ? AND tier = 'working'").bind(r).first(),p=await a.prepare("SELECT COUNT(*) as cnt FROM conversations WHERE user_id = ?").bind(r).first(),w=await a.prepare("SELECT COUNT(*) as cnt FROM error_log WHERE (user_id = ? OR user_id IS NULL) AND acknowledged = 0").bind(r).first();return`System Status:
- Active schedules: ${(d==null?void 0:d.cnt)||0}
- Memory: ${(m==null?void 0:m.cnt)||0} working / ${(g==null?void 0:g.cnt)||0} total
- Total messages: ${(p==null?void 0:p.cnt)||0}
- Unread errors: ${(w==null?void 0:w.cnt)||0}`}case"read_sheet":{if(!n)return"Authentication context unavailable.";try{const d=new Q(a,r,n,s||"",o||""),g=t.spreadsheet_id;let m=t.range;const p=await d.sheets.getMetadata(g),w=p.sheets;m.includes("!")||(m=`${w[0]}!${m}`);let T;try{T=await d.sheets.readRange(g,m)}catch(I){if((v=I.message)!=null&&v.includes("Unable to parse range")||(y=I.message)!=null&&y.includes("400")){const A=m.includes("!")?m.split("!")[1]:m;m=`${w[0]}!${A}`,T=await d.sheets.readRange(g,m)}else throw I}let S=`[Spreadsheet: "${p.title}" | Reading tab: "${m.split("!")[0]}" | All tabs in this spreadsheet: ${w.map(I=>`"${I}"`).join(", ")}]
...
`)}catch(d){return await C(a,r,"google","read_sheet",d.message),`Failed to read sheet: ${d.message}`}}case"write_sheet":{if(!n)return"Authentication context unavailable.";try{return`Written ${(await new Q(a,r,n,s||"",o||"").sheets.writeRange(t.spreadsheet_id,t.range,t.values)).updatedCells} cells to ${t.range}.`}catch(d){return await C(a,r,"google","write_sheet",d.message),`Failed to write sheet: ${d.message}`}}case"append_sheet":{if(!n)return"Authentication context unavailable.";try{return`Appended ${(await new Q(a,r,n,s||"",o||"").sheets.appendRows(t.spreadsheet_id,t.range,t.values)).updatedCells} cells to ${t.range}.`}catch(d){return await C(a,r,"google","append_sheet",d.message),`Failed to append to sheet: ${d.message}`}}case"create_sheet":{if(!n)return"Authentication context unavailable.";try{const d=new Q(a,r,n,s||"",o||"");if(!(await d.isConnected()).connected)return'Google account not connected. Please go to Settings → Keys → Google Workspace and click "Connect Google Account" to sign in with your Google account first.';const m=await d.sheets.createSpreadsheet(t.title,t.sheet_names);let p="";if(t.folder_name)try{const{token:w}=await(await Promise.resolve().then(()=>nt)).getGoogleAuth(a,r,n,s||"",o||"");p=`
Folder: "${(await zt(w,m.spreadsheetId,t.folder_name)).folderName}"`}catch(w){p=`
(Could not move to folder "${t.folder_name}": ${w.message})`}try{await new K(a).store(r,"context",`Spreadsheet: ${t.title}`,`Spreadsheet ID: ${m.spreadsheetId} | URL: ${m.url} | Sheets: ${(t.sheet_names||["Sheet1"]).join(", ")}`,7,"working")}catch{}return`Spreadsheet created: "${t.title}"${p}
ID: ${m.spreadsheetId}
URL: ${m.url}`}catch(d){return await C(a,r,"google","create_sheet",d.message),`Failed to create spreadsheet: ${d.message}`}}case"list_calendar_events":{if(!n)return"Authentication context unavailable.";try{const d=new Q(a,r,n,s||"",o||""),g=t.calendar_id||"primary",m=t.days_ahead||7,p=new Date,w=new Date(p.getTime()+m*24*60*60*1e3),T=await d.calendar.listEvents(g,{timeMin:p.toISOString(),timeMax:w.toISOString(),query:t.query});return T.length===0?`No events found in the next ${m} days.`:T.map(S=>{var $;const I=S.start.dateTime||S.start.date||"TBD",A=S.end.dateTime||S.end.date||"",O=S.location?` 📍 ${S.location}`:"",N=(($=S.attendees)==null?void 0:$.map(H=>H.email).join(", "))||"";return`• ${S.summary} — ${I} to ${A}${O}${N?`
  Attendees: ${N}`:""}`}).join(`
`)}catch(d){return await C(a,r,"google","list_calendar",d.message),`Failed to list events: ${d.message}`}}case"create_calendar_event":{if(!n)return"Authentication context unavailable.";try{const d=new Q(a,r,n,s||"",o||""),g=t.calendar_id||"primary",m=await d.calendar.createEvent(g,{summary:t.summary,description:t.description,location:t.location,startDateTime:t.start_datetime,endDateTime:t.end_datetime,attendees:t.attendees});return`Event created: "${m.summary}"
ID: ${m.id}
Start: ${m.start.dateTime||m.start.date}`}catch(d){return await C(a,r,"google","create_event",d.message),`Failed to create event: ${d.message}`}}case"create_doc":{if(!n)return"Authentication context unavailable.";try{const d=new Q(a,r,n,s||"",o||"");if(!(await d.isConnected()).connected)return'Google account not connected. Please go to Settings → Keys → Google Workspace and click "Connect Google Account" to sign in with your Google account first.';const m=await d.docs.createDocument(t.title);t.content&&await d.docs.appendText(m.documentId,t.content);let p="";if(t.folder_name)try{const{token:w}=await(await Promise.resolve().then(()=>nt)).getGoogleAuth(a,r,n,s||"",o||"");p=`
Folder: "${(await zt(w,m.documentId,t.folder_name)).folderName}"`}catch(w){p=`
(Could not move to folder "${t.folder_name}": ${w.message})`}try{await new K(a).store(r,"context",`Document: ${t.title}`,`Document ID: ${m.documentId} | URL: ${m.url}`,6,"working")}catch{}return`Document created: "${t.title}"${p}
ID: ${m.documentId}
URL: ${m.url}`}catch(d){return await C(a,r,"google","create_doc",d.message),`Failed to create document: ${d.message}`}}case"read_doc":{if(!n)return"Authentication context unavailable.";try{const g=await new Q(a,r,n,s||"",o||"").docs.readDocument(t.document_id);return`Document: "${g.title}"

${g.content}`}catch(d){return await C(a,r,"google","read_doc",d.message),`Failed to read document: ${d.message}`}}case"append_to_doc":{if(!n)return"Authentication context unavailable.";try{const d=new Q(a,r,n,s||"",o||"");if(!(await d.isConnected()).connected)return'Google account not connected. Please go to Settings → Keys → Google Workspace and click "Connect Google Account" first.';await d.docs.appendText(t.document_id,t.content);let m=t.document_id;try{m=(await d.docs.readDocument(t.document_id)).title}catch{}return`Content appended to "${m}".
URL: https://docs.google.com/document/d/${t.document_id}/edit`}catch(d){return await C(a,r,"google","append_to_doc",d.message),`Failed to append to document: ${d.message}`}}case"gmail_list":{if(!n)return"Authentication context unavailable.";try{const g=await new re(a,r,n,s||"",o||"").listMessages({maxResults:t.max_results||10,query:t.query});return g.length===0?"No messages found.":g.map((m,p)=>`${m.isUnread?"● ":"  "}${p+1}. **${m.subject}**
   From: ${m.from}
   Date: ${m.date}
   ${m.snippet}
   [id: ${m.id}]`).join(`

`)}catch(d){return await C(a,r,"gmail","list",d.message),(f=d.message)!=null&&f.includes("not connected")?d.message:`Gmail list error: ${d.message}`}}case"gmail_read":{if(!n)return"Authentication context unavailable.";try{const d=new re(a,r,n,s||"",o||""),g=await d.getMessage(t.message_id);if(!g)return"Message not found.";const m=await d.getMessageBody(t.message_id);return`**${g.subject}**
From: ${g.from}
To: ${g.to}
Date: ${g.date}

${m}`}catch(d){return await C(a,r,"gmail","read",d.message),`Gmail read error: ${d.message}`}}case"gmail_search":{if(!n)return"Authentication context unavailable.";try{const g=await new re(a,r,n,s||"",o||"").search(t.query,t.max_results||10);return g.length===0?`No results for: ${t.query}`:g.map((m,p)=>`${m.isUnread?"● ":"  "}${p+1}. **${m.subject}**
   From: ${m.from}
   Date: ${m.date}
   ${m.snippet}
   [id: ${m.id}]`).join(`

`)}catch(d){return await C(a,r,"gmail","search",d.message),`Gmail search error: ${d.message}`}}case"gmail_send":{if(!n)return"Authentication context unavailable.";try{const g=await new re(a,r,n,s||"",o||"").send(t.to,t.subject,t.body,{cc:t.cc});return`Email sent successfully to ${t.to}. Subject: "${t.subject}" [Message ID: ${g.id}]`}catch(d){return await C(a,r,"gmail","send",d.message),`Gmail send error: ${d.message}`}}case"gmail_draft":{if(!n)return"Authentication context unavailable.";try{const g=await new re(a,r,n,s||"",o||"").createDraft(t.to,t.subject,t.body);return`Draft created. To: ${t.to}, Subject: "${t.subject}" — Review and send from Gmail. [Draft ID: ${g.id}]`}catch(d){return await C(a,r,"gmail","draft",d.message),`Gmail draft error: ${d.message}`}}case"gmail_modify":{if(!n)return"Authentication context unavailable.";try{return await new re(a,r,n,s||"",o||"").modifyMessage(t.message_id,t.action),`Message ${t.message_id} successfully ${t.action}ed.`}catch(d){return await C(a,r,"gmail","modify",d.message),`Gmail modify error: ${d.message}`}}case"gmail_unread_count":{if(!n)return"Authentication context unavailable.";try{const g=await new re(a,r,n,s||"",o||"").getUnreadCount();return`You have ${g} unread email${g!==1?"s":""} in Gmail.`}catch(d){return(_=d.message)!=null&&_.includes("not connected")?d.message:`Gmail error: ${d.message}`}}case"drive_list":{if(!n)return"Authentication context unavailable.";try{const{token:d}=await(await Promise.resolve().then(()=>nt)).getGoogleAuth(a,r,n,s||"",o||""),g=new URLSearchParams;g.set("pageSize",String(t.max_results||10)),g.set("fields","files(id,name,mimeType,modifiedTime,size,webViewLink)"),g.set("orderBy","modifiedTime desc");let m="";t.folder_id?m=`'${t.folder_id}' in parents and trashed = false`:t.query?m=`${t.query} and trashed = false`:m="trashed = false",g.set("q",m);const p=await fetch(`https://www.googleapis.com/drive/v3/files?${g}`,{headers:{Authorization:`Bearer ${d}`}});if(!p.ok)throw new Error(`Drive API error (${p.status})`);const w=await p.json();return(x=w.files)!=null&&x.length?w.files.map((T,S)=>{var N,$;const I=((N=T.mimeType)==null?void 0:N.split(".").pop())||T.mimeType,A=T.size?`${(parseInt(T.size)/1024).toFixed(1)} KB`:"",O=(($=T.modifiedTime)==null?void 0:$.split("T")[0])||"";return`${S+1}. **${T.name}** (${I})
   ${A} · Modified: ${O}
   ${T.webViewLink||""}`}).join(`

`):"No files found."}catch(d){return await C(a,r,"google","drive_list",d.message),`Drive list error: ${d.message}`}}case"drive_search":{if(!n)return"Authentication context unavailable.";try{const{token:d}=await(await Promise.resolve().then(()=>nt)).getGoogleAuth(a,r,n,s||"",o||""),g=`fullText contains '${t.query.replace(/'/g,"\\'")}' and trashed = false`,m=new URLSearchParams;m.set("q",g),m.set("pageSize",String(t.max_results||10)),m.set("fields","files(id,name,mimeType,modifiedTime,size,webViewLink)"),m.set("orderBy","modifiedTime desc");const p=await fetch(`https://www.googleapis.com/drive/v3/files?${m}`,{headers:{Authorization:`Bearer ${d}`}});if(!p.ok)throw new Error(`Drive API error (${p.status})`);const w=await p.json();return(E=w.files)!=null&&E.length?w.files.map((T,S)=>{var O,N;const I=((O=T.mimeType)==null?void 0:O.split(".").pop())||T.mimeType,A=((N=T.modifiedTime)==null?void 0:N.split("T")[0])||"";return`${S+1}. **${T.name}** (${I}) — Modified: ${A}
   ${T.webViewLink||""}`}).join(`

`):`No files found for: "${t.query}"`}catch(d){return await C(a,r,"google","drive_search",d.message),`Drive search error: ${d.message}`}}case"web_search":try{const d=await mt(t.query,{num:t.num_results||5,site:t.site});return d.error?`Web search failed: ${d.error}`:d.results.length===0?`No results found for "${t.query}".`:d.results.map((g,m)=>`${m+1}. **${g.title}**
   ${g.link}
   ${g.snippet}`).join(`

`)}catch(d){return await C(a,r,"search","web_search",d.message),`Web search error: ${d.message}`}case"read_url":try{const d=t.url;if(!d||!d.startsWith("http://")&&!d.startsWith("https://"))return"Invalid URL. Please provide a full URL starting with http:// or https://";const g=Math.min(t.max_length||8e3,15e3),{fetchPageContent:m}=await Promise.resolve().then(()=>Zr),p=await m(d,g);return p.error?`Failed to read page: ${p.error}`:!p.text||p.text.length<20?`Page at ${d} returned no readable content.`:`Content from ${d} (${p.text.length} chars):

${p.text}`}catch(d){return await C(a,r,"search","read_url",d.message),`Read URL error: ${d.message}`}case"research":{if(!u)return"Research tool requires an LLM provider but none is available.";try{const g=Ua(t.query,u,{depth:t.depth||"quick",site:t.site}),m=new Promise(T=>setTimeout(()=>T(null),45e3)),p=await Promise.race([g,m]);if(p===null){const{webSearch:T}=await Promise.resolve().then(()=>Kr),S=await T(t.query,{num:5});if(S.error||S.results.length===0)return"Research timed out and fallback search returned no results. Try rephrasing or asking a more specific question.";let I=`Research took too long, but here are the top search results:

`;return I+=S.results.map((A,O)=>`${O+1}. **${A.title}**
   ${A.snippet}
   ${A.link}`).join(`

`),I}if(p.error)return`Research failed: ${p.error}`;let w=p.report;return p.sources.length>0&&(w+=`

---
**Sources** (`+p.pagesRead+` pages read):
`,w+=p.sources.map((T,S)=>`[${S+1}] ${T.title}
    ${T.url}`).join(`
`)),w}catch(d){return await C(a,r,"research","research",d.message),`Research error: ${d.message}`}}case"search_places":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured. Add it in Settings → Keys → Google API Key.";const g=await M(d.encrypted_value,n),m=await Oa(g,t.query,{type:t.type});return m.error?`Places search failed: ${m.error}`:m.results.length===0?`No places found for "${t.query}".`:m.results.map((p,w)=>{const T=p.rating?` ★${p.rating} (${p.userRatingsTotal||0} reviews)`:"",S=p.openNow!==void 0?p.openNow?" · Open now":" · Closed":"",I=p.googleMapsUri?`
   ${p.googleMapsUri}`:"";return`${w+1}. **${p.name}**${T}${S}
   ${p.address}${I}
   [place_id: ${p.placeId}]`}).join(`

`)}catch(d){return await C(a,r,"google_api","search_places",d.message),`Places search error: ${d.message}`}}case"get_place_details":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const g=await M(d.encrypted_value,n),m=await Aa(g,t.place_id);if(m.error)return`Details lookup failed: ${m.error}`;if(!m.details)return"No details found.";const p=m.details;let w=`**${p.name}**
📍 ${p.address}`;if(p.phone&&(w+=`
📞 ${p.phone}`),p.website&&(w+=`
🌐 ${p.website}`),p.rating&&(w+=`
★ ${p.rating}`),p.googleMapsUri&&(w+=`
📌 ${p.googleMapsUri}`),p.openingHours&&(w+=`

Opening Hours:
${p.openingHours.join(`
`)}`),p.reviews&&p.reviews.length>0){w+=`

Recent Reviews:`;for(const T of p.reviews)w+=`
— ${T.author} (★${T.rating}, ${T.time}): "${T.text}"`}return w}catch(d){return await C(a,r,"google_api","place_details",d.message),`Place details error: ${d.message}`}}case"get_directions":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const g=await M(d.encrypted_value,n),m=await La(g,t.origin,t.destination,{mode:t.mode||"driving"});if(m.error)return`Directions failed: ${m.error}`;if(!m.route)return"No route found.";const p=m.route;let w=`**${p.startAddress}** → **${p.endAddress}**
`;return w+=`📏 ${p.distance} · ⏱️ ${p.duration}`,p.durationInTraffic&&(w+=` (with traffic: ${p.durationInTraffic})`),w+=`
via ${p.summary}`,w+=`

Steps:`,p.steps.forEach((T,S)=>{w+=`
${S+1}. ${T.instruction} (${T.distance}, ${T.duration})`}),w}catch(d){return await C(a,r,"google_api","directions",d.message),`Directions error: ${d.message}`}}case"get_travel_time":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const g=await M(d.encrypted_value,n),m=await Ba(g,t.origin,t.destination,t.mode||"driving");if(m.error)return`Travel time lookup failed: ${m.error}`;let p=`${t.origin} → ${t.destination}: ${m.distance}, ${m.duration}`;return m.durationInTraffic&&(p+=` (with traffic: ${m.durationInTraffic})`),p}catch(d){return await C(a,r,"google_api","travel_time",d.message),`Travel time error: ${d.message}`}}case"translate_text":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const g=await M(d.encrypted_value,n),m=await Na(g,t.text,t.target_language,t.source_language);return m.error?`Translation failed: ${m.error}`:`[${m.detectedSourceLang||t.source_language||"auto"} → ${t.target_language}]

${m.translatedText}`}catch(d){return await C(a,r,"google_api","translate",d.message),`Translation error: ${d.message}`}}case"search_youtube":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const g=await M(d.encrypted_value,n),m=await Ma(g,t.query,{maxResults:t.max_results||5,order:t.order||"relevance"});return m.error?`YouTube search failed: ${m.error}`:m.results.length===0?`No YouTube results for "${t.query}".`:m.results.map((p,w)=>{var T;return`${w+1}. **${p.title}**
   ${p.channelTitle} · ${((T=p.publishedAt)==null?void 0:T.split("T")[0])||""}
   ${p.description}
   ${p.url}`}).join(`

`)}catch(d){return await C(a,r,"google_api","youtube_search",d.message),`YouTube search error: ${d.message}`}}case"geocode_address":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const g=await M(d.encrypted_value,n),m=await $a(g,t.address);return m.error?`Geocoding failed: ${m.error}`:m.results.length===0?`Location not found: "${t.address}"`:m.results.map((p,w)=>`${w+1}. ${p.address}
   Coordinates: ${p.lat}, ${p.lng}`).join(`
`)}catch(d){return await C(a,r,"google_api","geocode",d.message),`Geocoding error: ${d.message}`}}default:return`Unknown tool: ${e}`}}async function Dt(e,t,a,r,n,s){var _;const o=new K(t),i=(_=e.metadata)==null?void 0:_.thread_id,l=await o.buildContext(r.id),c=await o.getRecentConversations(r.id,25,i),h=[{role:"system",content:Fa(r,l)},...c.map(x=>({role:x.role,content:x.content})),{role:"user",content:e.text}];await o.storeMessage(r.id,e.channel,"user",e.text,"{}",i);const v=10;let y="",f=0;for(let x=0;x<v;x++)try{const E=await a.chat(h,{tools:ht});if(E.usage&&(f+=E.usage.promptTokens+E.usage.completionTokens),E.toolCalls&&E.toolCalls.length>0){E.content&&h.push({role:"assistant",content:E.content});for(const d of E.toolCalls)try{const g=await ft(d.name,d.arguments,t,r.id,r.pin_hash,s==null?void 0:s.GOOGLE_CLIENT_ID,s==null?void 0:s.GOOGLE_CLIENT_SECRET,s==null?void 0:s.GOOGLE_API_KEY,s==null?void 0:s.GOOGLE_CSE_ID,r.timezone,a);h.push({role:"user",content:`[Tool Result for ${d.name}]: ${g}`})}catch(g){await C(t,r.id,"tool",d.name,g.message||"Tool execution failed"),h.push({role:"user",content:`[Tool Error for ${d.name}]: ${g.message||"Execution failed"}`})}continue}y=E.content;break}catch(E){if(n){const d=E.message||"",g=d.includes("401")||d.includes("403")||d.includes("authentication")||d.includes("credit balance"),m=d.includes("429"),p=g?1440:m?10:5;await n.recordError(a.name,d,p)}throw await C(t,r.id,"llm","provider_error",E.message||"Unknown LLM error",{provider:a.name,turn:x}),E}if(n&&f>0)try{await n.recordUsage(a.name,f)}catch{}return await o.storeMessage(r.id,e.channel,"assistant",y,"{}",i),await o.compactHistory(r.id,30),y}const Ft={"claude-sonnet-4-20250514":2e5,"claude-haiku-4-20250514":2e5,"gpt-4o":128e3,"gpt-4o-mini":128e3,"grok-3-mini":131072,"grok-3":131072,"deepseek-chat":64e3,"gemini-2.0-flash":1e6,default:32e3};function rn(e){for(const[t,a]of Object.entries(Ft))if(e.toLowerCase().includes(t.toLowerCase()))return a;return Ft.default}function nn(e,t,a,r){const n=rn(r),s=Math.floor(n*.75),o=[];let i=0,l=!1;const c=_t(e);o.push({role:"system",content:e}),i+=c;const u=_t(a);i+=u;const h=s-i,v=[];let y=0;for(let f=t.length-1;f>=0;f--){const _=t[f],x=_t(_.content);if(y+x<=h)v.unshift({role:_.role,content:_.content}),y+=x;else{l=!0;break}}return o.push(...v),o.push({role:"user",content:a}),i+=y,{maxTokens:n,usedTokens:i,messages:o,wasTruncated:l}}async function*sn(e,t,a,r,n,s){var x;const o=new K(t),i=(x=e.metadata)==null?void 0:x.thread_id;yield{type:"thinking",data:{threadId:i,provider:a.name}};const l=await o.buildContext(r.id),c=await o.getRecentConversations(r.id,20,i),u=Fa(r,l),h=nn(u,c,e.text,a.name);await o.storeMessage(r.id,e.channel,"user",e.text,"{}",i);const v=10;let y="",f=0;const _=[...h.messages];for(let E=0;E<v;E++)try{E>0&&(yield{type:"thinking",data:{threadId:i}});const d=await a.chat(_,{tools:ht});if(d.usage&&(f+=d.usage.promptTokens+d.usage.completionTokens),d.toolCalls&&d.toolCalls.length>0){d.content&&(yield{type:"chunk",data:{text:d.content,threadId:i}},_.push({role:"assistant",content:d.content}));for(const m of d.toolCalls){yield{type:"tool_start",data:{tool:m.name,toolArgs:m.arguments,threadId:i}};try{const p=await ft(m.name,m.arguments,t,r.id,r.pin_hash,s==null?void 0:s.GOOGLE_CLIENT_ID,s==null?void 0:s.GOOGLE_CLIENT_SECRET,s==null?void 0:s.GOOGLE_API_KEY,s==null?void 0:s.GOOGLE_CSE_ID,r.timezone,a);yield{type:"tool_end",data:{tool:m.name,toolResult:p.substring(0,500)+(p.length>500?"...":""),threadId:i}},_.push({role:"user",content:`[Tool Result for ${m.name}]: ${p}`})}catch(p){await C(t,r.id,"tool",m.name,p.message||"Tool execution failed"),yield{type:"tool_end",data:{tool:m.name,toolResult:`Error: ${p.message||"Execution failed"}`,threadId:i}},_.push({role:"user",content:`[Tool Error for ${m.name}]: ${p.message||"Execution failed"}`})}}continue}y=d.content;const g=50;for(let m=0;m<y.length;m+=g)yield{type:"chunk",data:{text:y.substring(m,m+g),threadId:i}},m+g<y.length&&await new Promise(w=>setTimeout(w,10));break}catch(d){if(n){const g=d.message||"",m=g.includes("401")||g.includes("403")||g.includes("authentication")||g.includes("credit balance"),p=g.includes("429"),w=m?1440:p?10:5;await n.recordError(a.name,g,w)}await C(t,r.id,"llm","provider_error",d.message||"Unknown LLM error",{provider:a.name,turn:E}),yield{type:"error",data:{error:d.message||"An error occurred",threadId:i}};return}if(n&&f>0)try{await n.recordUsage(a.name,f)}catch{}await o.storeMessage(r.id,e.channel,"assistant",y,"{}",i),await o.compactHistory(r.id,30),yield{type:"done",data:{threadId:i,provider:a.name,tokenCount:f}}}async function Wa(e,t,a,r,n,s){var u;const o=new K(t),i=(u=e.metadata)==null?void 0:u.thread_id,l=await o.buildContext(r.id),c=Ha(e.text,l);if(c.agent==="conversation")return ln(e,t,a,r,l,n,i);if(c.agent==="multi"||c.confidence<.5)return Dt(e,t,a,r,n,s);try{return await on(c.agent,e,t,a,r,l,n,s,i)}catch(h){return await C(t,r.id,"router","subagent_fallback",`${c.agent} failed: ${h.message}`,{route:c}),Dt(e,t,a,r,n,s)}}async function on(e,t,a,r,n,s,o,i,l){const c=new K(a),u=gt(n.timezone),h=ct(e,n,s,n.timezone,u),v=Ga(e,ht),y=await c.getRecentConversations(n.id,25,l),f=[{role:"system",content:h},...y.map(g=>({role:g.role,content:g.content})),{role:"user",content:t.text}];await c.storeMessage(n.id,t.channel,"user",t.text,"{}",l);const _=10;let x="",E=0,d="";for(let g=0;g<_;g++)try{const m=await r.chat(f,{tools:v.length>0?v:void 0});if(m.usage&&(E+=m.usage.promptTokens+m.usage.completionTokens),m.toolCalls&&m.toolCalls.length>0){m.content&&(d=m.content,f.push({role:"assistant",content:m.content}));for(const p of m.toolCalls)try{const w=await ft(p.name,p.arguments,a,n.id,n.pin_hash,i==null?void 0:i.GOOGLE_CLIENT_ID,i==null?void 0:i.GOOGLE_CLIENT_SECRET,i==null?void 0:i.GOOGLE_API_KEY,i==null?void 0:i.GOOGLE_CSE_ID,n.timezone,r);f.push({role:"user",content:`[Tool Result for ${p.name}]: ${w}`})}catch(w){await C(a,n.id,"tool",p.name,w.message||"Tool execution failed"),f.push({role:"user",content:`[Tool Error for ${p.name}]: ${w.message||"Execution failed"}`})}continue}x=m.content,!x&&d&&(x=d);break}catch(m){if(o){const p=m.message||"",w=p.includes("401")||p.includes("403")||p.includes("authentication")||p.includes("credit balance"),T=p.includes("429"),S=w?1440:T?10:5;await o.recordError(r.name,p,S)}throw await C(a,n.id,"llm","subagent_error",m.message||"Unknown error",{agent:e,provider:r.name,turn:g}),m}if(o&&E>0)try{await o.recordUsage(r.name,E)}catch{}return await c.storeMessage(n.id,t.channel,"assistant",x,"{}",l),await c.compactHistory(n.id,30),x}async function ln(e,t,a,r,n,s,o){const i=new K(t),l=gt(r.timezone),c=ct("conversation",r,n,r.timezone,l),u=await i.getRecentConversations(r.id,25,o),h=[{role:"system",content:c},...u.map(f=>({role:f.role,content:f.content})),{role:"user",content:e.text}];await i.storeMessage(r.id,e.channel,"user",e.text,"{}",o);let v=0,y="";try{const f=await a.chat(h,{temperature:.8});f.usage&&(v=f.usage.promptTokens+f.usage.completionTokens),y=f.content}catch(f){if(s){const _=f.message||"",x=_.includes("401")||_.includes("403")||_.includes("authentication")||_.includes("credit balance"),E=_.includes("429"),d=x?1440:E?10:5;await s.recordError(a.name,_,d)}throw await C(t,r.id,"llm","conversation_error",f.message,{provider:a.name}),f}if(s&&v>0)try{await s.recordUsage(a.name,v)}catch{}return await i.storeMessage(r.id,e.channel,"assistant",y,"{}",o),await i.compactHistory(r.id,30),y}async function*cn(e,t,a,r,n,s){var d;const o=new K(t),i=(d=e.metadata)==null?void 0:d.thread_id,l=await o.buildContext(r.id),c=Ha(e.text,l);if(yield{type:"thinking",data:{threadId:i,provider:a.name}},c.agent==="multi"||c.confidence<.5){yield*sn(e,t,a,r,n,s);return}const u=gt(r.timezone),h=c.agent==="conversation"?ct("conversation",r,l,r.timezone,u):ct(c.agent,r,l,r.timezone,u),v=Ga(c.agent,ht),y=await o.getRecentConversations(r.id,25,i),f=[{role:"system",content:h},...y.map(g=>({role:g.role,content:g.content})),{role:"user",content:e.text}];await o.storeMessage(r.id,e.channel,"user",e.text,"{}",i);const _=10;let x="",E=0;for(let g=0;g<_;g++)try{g>0&&(yield{type:"thinking",data:{threadId:i}});const m=await a.chat(f,{tools:v.length>0?v:void 0});if(m.usage&&(E+=m.usage.promptTokens+m.usage.completionTokens),m.toolCalls&&m.toolCalls.length>0){m.content&&(yield{type:"chunk",data:{text:m.content,threadId:i}},f.push({role:"assistant",content:m.content}));for(const w of m.toolCalls){yield{type:"tool_start",data:{tool:w.name,toolArgs:w.arguments,threadId:i}};try{const T=await ft(w.name,w.arguments,t,r.id,r.pin_hash,s==null?void 0:s.GOOGLE_CLIENT_ID,s==null?void 0:s.GOOGLE_CLIENT_SECRET,s==null?void 0:s.GOOGLE_API_KEY,s==null?void 0:s.GOOGLE_CSE_ID,r.timezone,a);yield{type:"tool_end",data:{tool:w.name,toolResult:T.substring(0,500)+(T.length>500?"...":""),threadId:i}},f.push({role:"user",content:`[Tool Result for ${w.name}]: ${T}`})}catch(T){await C(t,r.id,"tool",w.name,T.message||"Tool execution failed"),yield{type:"tool_end",data:{tool:w.name,toolResult:`Error: ${T.message||"Execution failed"}`,threadId:i}},f.push({role:"user",content:`[Tool Error for ${w.name}]: ${T.message||"Execution failed"}`})}}continue}x=m.content;const p=50;for(let w=0;w<x.length;w+=p)yield{type:"chunk",data:{text:x.substring(w,w+p),threadId:i}},w+p<x.length&&await new Promise(S=>setTimeout(S,10));break}catch(m){if(n){const p=m.message||"",w=p.includes("401")||p.includes("403")||p.includes("authentication")||p.includes("credit balance"),T=p.includes("429"),S=w?1440:T?10:5;await n.recordError(a.name,p,S)}await C(t,r.id,"llm","subagent_stream_error",m.message||"Unknown error",{agent:c.agent,provider:a.name,turn:g}),yield{type:"error",data:{error:m.message||"An error occurred",threadId:i}};return}if(n&&E>0)try{await n.recordUsage(a.name,E)}catch{}await o.storeMessage(r.id,e.channel,"assistant",x,"{}",i),await o.compactHistory(r.id,30),yield{type:"done",data:{threadId:i,provider:a.name,tokenCount:E}}}const B=new ge;async function dn(e,t){var n;const a=(n=e.req.header("Authorization"))==null?void 0:n.replace("Bearer ","");if(!a)return e.json({error:"Authentication required"},401);const r=await e.env.DB.prepare(`SELECT s.*, u.* FROM sessions s JOIN users u ON s.user_id = u.id 
     WHERE s.id = ? AND s.expires_at > datetime('now')`).bind(a).first();if(!r)return e.json({error:"Invalid session"},401);e.set("user",{id:r.user_id,username:r.username,name:r.name,pin_hash:r.pin_hash,role:r.role,personality_prompt:r.personality_prompt,telegram_chat_id:r.telegram_chat_id,timezone:r.timezone,assistant_name:r.assistant_name||"Karna",created_at:r.created_at,updated_at:r.updated_at}),e.set("sessionId",a),await t()}B.use("/*",dn);B.get("/threads",async e=>{const t=e.get("user"),a=e.req.query("archived")==="1",r=parseInt(e.req.query("limit")||"30"),n=await e.env.DB.prepare(`SELECT t.*, 
...
`;for(const c of s)o+=`- ${c.name} (${c.type}, ${Math.round(c.size/1024)}KB, file_id: ${c.file_id})`,c.text_preview&&(o+=`
  Preview: ${c.text_preview.substring(0,300)}...`),o+=`
`;o+="Use parse_document tool to read full file contents. Use drive_upload to upload to Google Drive.]"}let i=n;if(!i){const c=await e.env.DB.prepare("INSERT INTO threads (user_id, title) VALUES (?, ?) RETURNING *").bind(t.id,a.trim().substring(0,60)+(a.trim().length>60?"...":"")).first();i=c==null?void 0:c.id}const l={userId:t.id,username:t.username,channel:r,text:a.trim()+o,sessionId:e.get("sessionId"),timestamp:new Date().toISOString(),metadata:{thread_id:i}};try{const{provider:c,rotation:u}=await et(e.env.DB,t.id,t.pin_hash),h=new ReadableStream({async start(v){const y=new TextEncoder;try{const f=cn(l,e.env.DB,c,t,u,{GOOGLE_CLIENT_ID:e.env.GOOGLE_CLIENT_ID,GOOGLE_CLIENT_SECRET:e.env.GOOGLE_CLIENT_SECRET,GOOGLE_API_KEY:e.env.GOOGLE_API_KEY,GOOGLE_CSE_ID:e.env.GOOGLE_CSE_ID});for await(const _ of f)_.data.threadId||(_.data.threadId=i),v.enqueue(y.encode(Wt(_)));i&&await e.env.DB.prepare("UPDATE threads SET message_count = message_count + 2, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(i).run(),v.close()}catch(f){const _={type:"error",data:{error:f.message||"An error occurred",threadId:i}};v.enqueue(y.encode(Wt(_))),v.close()}}});return new Response(h,{headers:{"Content-Type":"text/event-stream","Cache-Control":"no-cache",Connection:"keep-alive","X-Thread-Id":String(i||"")}})}catch(c){console.error("Stream setup error:",c);const u=c.message||"";return u.includes("No LLM provider configured")?e.json({error:"No AI provider configured. Please add at least one API key in Settings → Keys.",type:"no_provider",thread_id:i},400):u.includes("limit reached")?e.json({error:u,type:"cost_limit",thread_id:i},429):e.json({error:"Something went wrong setting up the stream.",details:u,thread_id:i},500)}});B.get("/threads/:id/messages",async e=>{var s;const t=e.get("user"),a=parseInt(e.req.param("id")),r=parseInt(e.req.query("limit")||"50"),n=await e.env.DB.prepare(`SELECT id, role, content, channel, created_at FROM conversations 
...
`:""}${$}
Note: This file type cannot be read directly. I can see it was sent but cannot extract the content. For PDFs, images, or Office docs — suggest uploading to Google Drive via the web app where I can process them.`}}catch(p){if(v&&i.caption)f=i.caption;else return await z(x,y,`⚠️ Received your file but couldn't process it: ${p.message}`),e.json({ok:!0})}if(!f)return e.json({ok:!0});await fn(x,y);let E=await e.env.DB.prepare("SELECT id FROM threads WHERE user_id = ? AND channel = 'telegram' AND is_archived = 0 ORDER BY updated_at DESC LIMIT 1").bind(_.id).first();E?await e.env.DB.prepare("UPDATE threads SET updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(E.id).run():E={id:(await e.env.DB.prepare("INSERT INTO threads (user_id, title, channel) VALUES (?, 'Telegram', 'telegram')").bind(_.id).run()).meta.last_row_id};const d=mn(_.id,_.username,f,y);d.metadata={thread_id:E.id};let g,m;try{const p=await et(e.env.DB,_.id,_.pin_hash);g=p.provider,m=p.rotation}catch(p){console.error("Telegram provider setup error:",p);const w=(r=p.message)!=null&&r.includes("No LLM provider")?`⚠️ No AI provider configured yet.

Go to the web app → Settings → Keys → add at least one API key (DeepSeek, Grok, Abacus AI, etc.).`:(n=p.message)!=null&&n.includes("Daily usage limit")?"⚠️ Daily usage limit reached. Your limit resets at midnight.":`⚠️ AI provider error: ${p.message||"Unknown error"}`;return await z(x,y,w),e.json({ok:!0})}try{const p=await Wa(d,e.env.DB,g,_,m,{GOOGLE_CLIENT_ID:e.env.GOOGLE_CLIENT_ID,GOOGLE_CLIENT_SECRET:e.env.GOOGLE_CLIENT_SECRET,GOOGLE_API_KEY:e.env.GOOGLE_API_KEY,GOOGLE_CSE_ID:e.env.GOOGLE_CSE_ID}),w=hn(p,"telegram");await z(x,y,w||"(empty response)")}catch(p){console.error("Telegram agent error:",p);const w=(s=p.message)!=null&&s.includes("API error")?`⚠️ AI provider returned an error. The provider (${g.name}) may be temporarily unavailable. Your message was saved — try again shortly.`:`⚠️ Something went wrong processing your message. Error: ${(p.message||"Unknown").substring(0,200)}`;await z(x,y,w);try{const{logError:T}=await Promise.resolve().then(()=>St);await T(e.env.DB,_.id,"telegram","agent_error",p.message||"Agent error",{provider:g.name})}catch{}}return e.json({ok:!0})}catch(o){console.error("Telegram webhook error:",o);try{const{logError:i}=await Promise.resolve().then(()=>St);await i(e.env.DB,null,"telegram","webhook_error",o.message||"Unknown telegram error")}catch{}return e.json({ok:!0,error:o.message})}});tt.post("/setup-webhook",async e=>{var l;const t=(l=e.req.header("Authorization"))==null?void 0:l.replace("Bearer ","");if(!t)return e.json({error:"Auth required"},401);const a=await e.env.DB.prepare(`SELECT s.user_id, u.pin_hash FROM sessions s JOIN users u ON s.user_id = u.id
...
`)}function Sn(e){const t=[];let a=0;for(const r of e.calendar.google)t.push({type:"calendar",key:r.id,text:`${r.title} - ${new Date(r.startTime).toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"})}`,metadata:{event:r},sortOrder:a++});e.emails.gmail.unreadCount>0&&t.push({type:"email",key:"gmail-unread",text:`Review ${e.emails.gmail.unreadCount} unread Gmail messages`,metadata:{source:"gmail",count:e.emails.gmail.unreadCount},sortOrder:a++});for(const r of e.tasks.items)t.push({type:"task",key:`task-${r}`,text:r,metadata:{},sortOrder:a++});for(const r of e.news.items)t.push({type:"news",key:`news-${r.url}`,text:`📰 ${r.title}`,metadata:{url:r.url,source:r.source},sortOrder:a++});return t}async function In(e,t){const a=await e.prepare("SELECT * FROM briefing_preferences WHERE user_id = ?").bind(t).first();if(!a)return{components:{google_calendar:!0,gmail:!0,tasks:!0,news:!0},newsTopics:["AI","LLM","Tools","Agentic Workflows","AI Features"]};let r;try{r=JSON.parse(a.components)}catch{r={google_calendar:!0,gmail:!0,tasks:!0,news:!0}}const n=a.news_topics?a.news_topics.split(",").map(s=>s.trim()).filter(Boolean):["AI","LLM","Tools","Agentic Workflows","AI Features"];return{components:r,newsTopics:n}}async function qa(e,t,a){var E;const r=t.timezone||"Asia/Kolkata",n=wn(r),{components:s,newsTopics:o}=await In(e,t.id),i=[],l=[];s.google_calendar&&(i.push(xn(e,t.id,t.pin_hash,a.GOOGLE_CLIENT_ID,a.GOOGLE_CLIENT_SECRET,n)),l.push("googleEvents")),s.gmail&&(i.push(_n(e,t.id,t.pin_hash,a.GOOGLE_CLIENT_ID,a.GOOGLE_CLIENT_SECRET)),l.push("gmailSummary")),s.tasks&&(i.push(En(e,t.id)),l.push("tasks")),s.news&&(i.push(Tn(o)),l.push("news"));const c=await Promise.all(i),u={};l.forEach((d,g)=>{u[d]=c[g]});const h={unreadCount:0,importantCount:0,topSenders:[],hasUrgent:!1},v={pending:0,dueToday:0,items:[]},y={generatedAt:new Date().toISOString(),targetDate:n.dateStr,calendar:{google:u.googleEvents||[],totalCount:((E=u.googleEvents)==null?void 0:E.length)||0},emails:{gmail:u.gmailSummary||h},tasks:u.tasks||v,news:{items:u.news||[],fetchedAt:new Date().toISOString()},summary:""};y.summary=kn(y);const f=Sn(y),_=await e.prepare(`
  ...
  `).bind(o,o,r,a).run(),{checked:o===1}}async function Rn(e,t,a=10){return((await e.prepare(`
    ...
    LIMIT ?
  `).bind(t,a).all()).results||[]).map(n=>({...n,content:JSON.parse(n.content_json||"{}")}))}function On(e,t,a=new Date){const r=new Date(a.toLocaleString("en-US",{timeZone:t})),n=r.getHours(),s=r.getMinutes(),[o,i]=e.split(":").map(Number),l=n*60+s,c=o*60+i;return l===c}function An(e,t){const a=e.summary,r=[];for(const n of t.slice(0,10))r.push([{text:`☐ ${n.text.substring(0,40)}${n.text.length>40?"...":""}`,callback_data:`briefing_toggle:${n.key}`}]);return{text:a,inlineKeyboard:r}}const J=new ge;async function Ka(e,t){var n;if(e.req.path.includes("/cron/"))return t();const a=(n=e.req.header("Authorization"))==null?void 0:n.replace("Bearer ","");if(!a)return e.json({error:"Authentication required"},401);const r=await e.env.DB.prepare(`SELECT s.*, u.* FROM sessions s JOIN users u ON s.user_id = u.id 
    ...
    `).all(),n=[],s=new Date;for(const o of r.results||[]){if(!o.briefing_enabled)continue;const i=o.timezone||"Asia/Kolkata",l=o.briefing_time||"20:00";if(On(l,i,s))try{const c=await qa(e.env.DB,o,{GOOGLE_CLIENT_ID:e.env.GOOGLE_CLIENT_ID,GOOGLE_CLIENT_SECRET:e.env.GOOGLE_CLIENT_SECRET});if(o.telegram_chat_id){const{text:u,inlineKeyboard:h}=An(c.content,c.items);await Ln(e.env.DB,o,u,h,c.briefingId)}n.push({user_id:o.id,status:"success",briefing_id:c.briefingId,briefing_time:l,timezone:i})}catch(c){n.push({user_id:o.id,status:"error",error:c.message})}}return e.json({executed:n.length,results:n})}catch(r){return e.json({error:r.message},500)}});J.post("/cron/meeting-reminders",async e=>{const t=e.req.header("X-Cron-Secret")||"",a=e.env.CRON_SECRET||"karna-cron-default-v1";if(t!==a)return e.json({error:"Unauthorized"},401);try{const r=await e.env.DB.prepare("SELECT * FROM users WHERE telegram_chat_id IS NOT NULL").all(),n=[],s=new Date,o=new Date(s.getTime()+600*1e3).toISOString(),i=new Date(s.getTime()+900*1e3).toISOString();for(const l of r.results||[])try{const c=await e.env.DB.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = 'google_oauth_tokens'").bind(l.id).first();if(!c)continue;const u=await M(c.encrypted_value,l.pin_hash),v=JSON.parse(u).access_token;if(!v)continue;const y=await fetch(`https://www.googleapis.com/calendar/v3/calendars/primary/events?singleEvents=true&orderBy=startTime&timeMin=${encodeURIComponent(s.toISOString())}&timeMax=${encodeURIComponent(i)}&maxResults=10`,{headers:{Authorization:`Bearer ${v}`}});if(!y.ok)continue;const _=((await y.json()).items||[]).filter(d=>{var m;const g=(m=d.start)==null?void 0:m.dateTime;return g?g>=s.toISOString()&&g<=o:!1});if(_.length===0){n.push({user_id:l.id,reminders_sent:0});continue}const x=await e.env.DB.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = 'telegram_bot_token'").bind(l.id).first();if(!x)continue;const E=await M(x.encrypted_value,l.pin_hash);for(const d of _){const g=new Date(d.start.dateTime).toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"}),m=d.location?`
📍 ${d.location}`:"",p=`⏰ Meeting in 10 minutes!

*${d.summary||"Untitled Event"}*
🕐 ${g}${m}`;await fetch(`https://api.telegram.org/bot${E}/sendMessage`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({chat_id:l.telegram_chat_id,text:p,parse_mode:"Markdown"})})}n.push({user_id:l.id,reminders_sent:_.length})}catch(c){n.push({user_id:l.id,status:"error",error:c.message})}return e.json({executed:n.length,results:n})}catch(r){return e.json({error:r.message},500)}});async function Ln(e,t,a,r,n){try{const s=await e.prepare(`SELECT c.encrypted_value, u.pin_hash FROM credentials c 
       JOIN users u ON c.user_id = u.id 
