...
`)}function Mn(e){var t;for(let n=e.length-1;n>=0;n--){const s=e[n];if(s.role==="assistant")return((t=An(s.metadata).tools)==null?void 0:t.includes("research"))??!1}return!1}function Rr(e,t,n,s){return e!=="research"||/^(Research failed|Research error|Research timed out|\[Tool Error)/i.test(n)?s:{query:String(t.query||""),report:n.substring(0,kr)}}function Or(e,t){if(!t)return;const n=e[e.length-1];(n==null?void 0:n.role)!=="user"||typeof n.content!="string"||n.content.startsWith(bn)||(e[e.length-1]={role:"user",content:`${bn}
...
Use read_library_file with the id to get the full document text.`}}catch{}const h=await n.prepare(`
      ...
      `).bind(s,`%${m}%`,`%${m}%`,`%${m}%`,y).all(),v=await n.prepare(`
        ...
        ORDER BY dl.created_at DESC
        LIMIT ?
      `).bind(s,s,`%${m}%`,s,`%${m}%`,`%${m}%`,`%${m}%`,y).all(),_=[...h.results||[],...v.results||[]].slice(0,y);if(_.length===0)return`No documents found matching "${m}" in your library.`;const E=_.map(x=>{const D=(x.summary||x.dl_extracted||"").substring(0,200);return`[id:${x.id}] "${x.name}" (source: ${x.source}, status: ${x.status})
...
${v.substring(0,2e4)}`:`Document "${h.name}" has no extracted text yet. Ask Karna to parse it with parse_document(file_id="${h.file_id}") to extract the text first.`}case"create_skill":{const m=(W=t.name)==null?void 0:W.trim(),y=(te=t.description)==null?void 0:te.trim(),h=(le=t.instructions)==null?void 0:le.trim();if(!m||!y||!h)return"create_skill requires name, description, and instructions.";let v=m.toLowerCase().replace(/[^a-z0-9\s_-]/g,"").replace(/\s+/g,"_").replace(/_+/g,"_").substring(0,50).replace(/^_|_$/g,"");v||(v=`skill_${Date.now()}`);const _=await n.prepare("SELECT slug FROM user_skills WHERE user_id = ? AND slug LIKE ?").bind(s,`${v}%`).all();(ne=_.results)!=null&&ne.some(O=>O.slug===v)&&(v=`${v}_${(((me=_.results)==null?void 0:me.length)||0)+1}`);const E=JSON.stringify(t.parameters||{}),x=JSON.stringify(t.required_tools||[]),D=JSON.stringify(t.examples||[]);return await n.prepare(`INSERT INTO user_skills (user_id, name, slug, description, instructions, parameters, required_tools, examples)
