...
${a}`,parse_mode:"Markdown"})})}}catch{}}async function Hs(e,t){const r=await e.prepare(`
  ...
  `).bind(t).all(),a={};for(const s of r.results||[])a[s.pattern_type]={data:JSON.parse(s.data_json),confidence:s.confidence};return a}const P=new he;async function Fs(e,t){var s;const r=(s=e.req.header("Authorization"))==null?void 0:s.replace("Bearer ","");if(!r)return e.json({error:"Authentication required"},401);const a=await e.env.DB.prepare(`SELECT s.*, u.* FROM sessions s JOIN users u ON s.user_id = u.id 
