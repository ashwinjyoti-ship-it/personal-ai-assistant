...
<body>
  ...
  <\/script>
     ...
     WHERE s.id = ? AND s.expires_at > datetime('now')`).bind(t).first();return a?e.json({user:{id:a.uid,username:a.username,name:a.name,role:a.role,timezone:a.timezone}}):e.json({error:"Invalid or expired session"},401)});const Je={anthropic:{id:"anthropic",label:"Anthropic Claude",apiBase:"https://api.anthropic.com",apiFormat:"anthropic",defaultModel:"claude-sonnet-4-20250514",keyPlaceholder:"sk-ant-api03-...",modelHint:"claude-sonnet-4-20250514, claude-haiku-4-20250514",validatePath:"/v1/messages"},openai:{id:"openai",label:"OpenAI GPT",apiBase:"https://api.openai.com",apiFormat:"openai-compatible",defaultModel:"gpt-4o",keyPlaceholder:"sk-...",modelHint:"gpt-4o, gpt-4o-mini, o3-mini",validatePath:"/v1/models"},grok:{id:"grok",label:"xAI Grok",apiBase:"https://api.x.ai",apiFormat:"openai-compatible",defaultModel:"grok-3-mini",keyPlaceholder:"xai-...",modelHint:"grok-3-mini, grok-3",validatePath:"/v1/models"},deepseek:{id:"deepseek",label:"DeepSeek",apiBase:"https://api.deepseek.com",apiFormat:"openai-compatible",defaultModel:"deepseek-chat",keyPlaceholder:"sk-...",modelHint:"deepseek-chat, deepseek-reasoner",validatePath:"/v1/models"},gemini:{id:"gemini",label:"Google Gemini",apiBase:"https://generativelanguage.googleapis.com/v1beta/openai",apiFormat:"openai-compatible",defaultModel:"gemini-2.0-flash",keyPlaceholder:"AIzaSy...",modelHint:"gemini-2.0-flash, gemini-2.5-pro-preview",validatePath:"/models"},openrouter:{id:"openrouter",label:"OpenRouter",apiBase:"https://openrouter.ai/api",apiFormat:"openai-compatible",defaultModel:"anthropic/claude-sonnet-4",keyPlaceholder:"sk-or-...",modelHint:"e.g. deepseek/deepseek-chat, meta-llama/llama-3.1-70b",validatePath:"/v1/models"},abacus:{id:"abacus",label:"Abacus AI (RouteLLM)",apiBase:"https://routellm.abacus.ai",apiFormat:"openai-compatible",defaultModel:"route-llm",keyPlaceholder:"Your Abacus API key",modelHint:"route-llm (auto), grok-4, deepseek-v3.2, claude-4-5-sonnet, gemini-3-flash",validatePath:"/v1/models"}};async function R(e,t,a,r,n,s={}){try{await e.prepare("INSERT INTO error_log (user_id, source, error_type, message, details) VALUES (?, ?, ?, ?, ?)").bind(t,a,r,n,JSON.stringify(s)).run()}catch(o){console.error("Failed to log error:",o)}}async function Et(e,t,a,r,n,s){try{const o=`provider_alert:${r}:${a}`;if(await e.prepare(`SELECT id FROM error_log WHERE user_id = ? AND source = 'provider_alert'
       AND error_type = ? AND created_at > datetime('now', '-1 hour') LIMIT 1`).bind(t,o).first())return;await R(e,t,"provider_alert",o,`${r} failed: ${s.substring(0,200)}`,{alertType:a,failedProvider:r,fallbackProvider:n});let l;a==="all_providers_down"?l=`🚨 All LLM providers failed
...
`),t.trim()}async function Ka(e,t,a={}){const r=a.maxPages||(a.depth==="thorough"?5:3),n=a.maxResults||(a.depth==="thorough"?8:5),s=await gt(e,{num:n,site:a.site});if(s.error)return{report:"",sources:[],pagesRead:0,error:`Search failed: ${s.error}`};if(s.results.length===0)return{report:`No web results found for "${e}".`,sources:[],pagesRead:0};const i=s.results.slice(0,r).map(async y=>{const v=await qa(y.link);return{title:y.title,url:y.link,displayLink:y.displayLink,snippet:y.snippet,content:v.text,error:v.error}}),c=(await Promise.all(i)).filter(y=>y.content.length>50);if(c.length===0){const y=s.results.map((f,x)=>`[${x+1}] ${f.title}
${f.snippet}
Source: ${f.link}`).join(`

`);return{report:await qt(e,y,t,"snippets"),sources:s.results.map(f=>({title:f.title,url:f.link})),pagesRead:0}}const u=c.map((y,v)=>`--- SOURCE ${v+1}: ${y.title} (${y.displayLink}) ---
${y.content}
--- END SOURCE ${v+1} ---`).join(`

`);return{report:await qt(e,u,t,"full"),sources:c.map(y=>({title:y.title,url:y.url})),pagesRead:c.length}}async function qt(e,t,a,r){const s=`You are a research analyst. Your job is to synthesize web research into clear, actionable reports.
...
- Unread errors: ${(_==null?void 0:_.cnt)||0}`}case"read_sheet":{if(!n)return"Authentication context unavailable.";try{const d=new te(a,r,n,s||"",o||""),g=t.spreadsheet_id;let p=t.range;const h=await d.sheets.getMetadata(g),_=h.sheets;p.includes("!")||(p=`${_[0]}!${p}`);let T;try{T=await d.sheets.readRange(g,p)}catch(k){if((y=k.message)!=null&&y.includes("Unable to parse range")||(v=k.message)!=null&&v.includes("400")){const I=p.includes("!")?p.split("!")[1]:p;p=`${_[0]}!${I}`,T=await d.sheets.readRange(g,p)}else throw k}let D=`[Spreadsheet: "${h.title}" | Reading tab: "${p.split("!")[0]}" | All tabs in this spreadsheet: ${_.map(k=>`"${k}"`).join(", ")}]
...
`)}catch(d){return await R(a,r,"search","web_search",d.message),`Web search error: ${d.message}`}case"read_url":try{const d=t.url;if(!d||!d.startsWith("http://")&&!d.startsWith("https://"))return"Invalid URL. Please provide a full URL starting with http:// or https://";const g=Math.min(t.max_length||8e3,15e3),{fetchPageContent:p}=await Promise.resolve().then(()=>on),h=await p(d,g);return h.error?`Failed to read page: ${h.error}`:!h.text||h.text.length<20?`Page at ${d} returned no readable content.`:`Content from ${d} (${h.text.length} chars):

${h.text}`}catch(d){return await R(a,r,"search","read_url",d.message),`Read URL error: ${d.message}`}case"research":{if(!u)return"Research tool requires an LLM provider but none is available.";try{const g=Ka(t.query,u,{depth:t.depth||"quick",site:t.site}),p=new Promise(T=>setTimeout(()=>T(null),45e3)),h=await Promise.race([g,p]);if(h===null){const{webSearch:T}=await Promise.resolve().then(()=>tn),D=await T(t.query,{num:5});if(D.error||D.results.length===0)return"Research timed out and fallback search returned no results. Try rephrasing or asking a more specific question.";let k=`Research took too long, but here are the top search results:
...
— ${T.author} (★${T.rating}, ${T.time}): "${T.text}"`}return _}catch(d){return await R(a,r,"google_api","place_details",d.message),`Place details error: ${d.message}`}}case"get_directions":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const g=await M(d.encrypted_value,n),p=await Ua(g,t.origin,t.destination,{mode:t.mode||"driving"});if(p.error)return`Directions failed: ${p.error}`;if(!p.route)return"No route found.";const h=p.route;let _=`**${h.startAddress}** → **${h.endAddress}**
`;return _+=`📏 ${h.distance} · ⏱️ ${h.duration}`,h.durationInTraffic&&(_+=` (with traffic: ${h.durationInTraffic})`),_+=`
via ${h.summary}`,_+=`

Steps:`,h.steps.forEach((T,D)=>{_+=`
${D+1}. ${T.instruction} (${T.distance}, ${T.duration})`}),_}catch(d){return await R(a,r,"google_api","directions",d.message),`Directions error: ${d.message}`}}case"get_travel_time":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const g=await M(d.encrypted_value,n),p=await Fa(g,t.origin,t.destination,t.mode||"driving");if(p.error)return`Travel time lookup failed: ${p.error}`;let h=`${t.origin} → ${t.destination}: ${p.distance}, ${p.duration}`;return p.durationInTraffic&&(h+=` (with traffic: ${p.durationInTraffic})`),h}catch(d){return await R(a,r,"google_api","travel_time",d.message),`Travel time error: ${d.message}`}}case"translate_text":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const g=await M(d.encrypted_value,n),p=await Ha(g,t.text,t.target_language,t.source_language);return p.error?`Translation failed: ${p.error}`:`[${p.detectedSourceLang||t.source_language||"auto"} → ${t.target_language}]

${p.translatedText}`}catch(d){return await R(a,r,"google_api","translate",d.message),`Translation error: ${d.message}`}}case"search_youtube":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const g=await M(d.encrypted_value,n),p=await za(g,t.query,{maxResults:t.max_results||5,order:t.order||"relevance"});return p.error?`YouTube search failed: ${p.error}`:p.results.length===0?`No YouTube results for "${t.query}".`:p.results.map((h,_)=>{var T;return`${_+1}. **${h.title}**
   ${h.channelTitle} · ${((T=h.publishedAt)==null?void 0:T.split("T")[0])||""}
   ${h.description}
   ${h.url}`}).join(`

`)}catch(d){return await R(a,r,"google_api","youtube_search",d.message),`YouTube search error: ${d.message}`}}case"geocode_address":{if(!n)return"Authentication context unavailable.";try{const d=await a.prepare("SELECT encrypted_value FROM credentials WHERE user_id = ? AND service = ?").bind(r,"google_api_key").first();if(!d)return"Google API Key not configured.";const g=await M(d.encrypted_value,n),p=await Ga(g,t.address);return p.error?`Geocoding failed: ${p.error}`:p.results.length===0?`Location not found: "${t.address}"`:p.results.map((h,_)=>`${_+1}. ${h.address}
   Coordinates: ${h.lat}, ${h.lng}`).join(`
`)}catch(d){return await R(a,r,"google_api","geocode",d.message),`Geocoding error: ${d.message}`}}default:return`Unknown tool: ${e}`}}async function Jt(e,t,a,r,n,s){var E;const o=new J(t),i=(E=e.metadata)==null?void 0:E.thread_id,l=await o.buildContext(r.id),c=await o.getRecentConversations(r.id,25,i),m=[{role:"system",content:Xa(r,l)},...c.map(d=>({role:d.role,content:d.content})),{role:"user",content:e.text}];await o.storeMessage(r.id,e.channel,"user",e.text,"{}",i);const y=10;let v="",f=0;const x=[];for(let d=0;d<y;d++)try{const g=await a.chat(m,{tools:ft});if(g.usage&&(f+=g.usage.promptTokens+g.usage.completionTokens),g.toolCalls&&g.toolCalls.length>0){g.content&&m.push({role:"assistant",content:g.content});for(const p of g.toolCalls){x.push(p.name);try{const h=await Ne(p.name,p.arguments,t,r.id,{agentType:"full",providerName:a.name,channel:e.channel},r.pin_hash,s==null?void 0:s.GOOGLE_CLIENT_ID,s==null?void 0:s.GOOGLE_CLIENT_SECRET,s==null?void 0:s.GOOGLE_API_KEY,s==null?void 0:s.GOOGLE_CSE_ID,r.timezone,a);m.push({role:"user",content:`[Tool Result for ${p.name}]: ${h}`})}catch(h){await R(t,r.id,"tool",p.name,h.message||"Tool execution failed"),m.push({role:"user",content:`[Tool Error for ${p.name}]: ${h.message||"Execution failed"}`})}}continue}v=g.content;break}catch(g){if(n){const p=g.message||"",h=p.includes("401")||p.includes("403")||p.includes("authentication")||p.includes("credit balance"),_=p.includes("429"),T=h?1440:_?10:5;await n.recordError(a.name,p,T)}throw await R(t,r.id,"llm","provider_error",g.message||"Unknown LLM error",{provider:a.name,turn:d}),g}if(n&&f>0)try{await n.recordUsage(a.name,f)}catch{}const w=x.length>0?`[TOOLS_USED: ${[...new Set(x)].join(", ")}] `:"";return await o.storeMessage(r.id,e.channel,"assistant",w+v,"{}",i),await o.compactHistory(r.id,30),v}const Vt={"claude-sonnet-4-20250514":2e5,"claude-haiku-4-20250514":2e5,"gpt-4o":128e3,"gpt-4o-mini":128e3,"grok-3-mini":131072,"grok-3":131072,"deepseek-chat":64e3,"gemini-2.0-flash":1e6,default:32e3};function hn(e){for(const[t,a]of Object.entries(Vt))if(e.toLowerCase().includes(t.toLowerCase()))return a;return Vt.default}function gn(e,t,a,r){const n=hn(r),s=Math.floor(n*.75),o=[];let i=0,l=!1;const c=St(e);o.push({role:"system",content:e}),i+=c;const u=St(a);i+=u;const m=s-i,y=[];let v=0;for(let f=t.length-1;f>=0;f--){const x=t[f],w=St(x.content);if(v+w<=m)y.unshift({role:x.role,content:x.content}),v+=w;else{l=!0;break}}return o.push(...y),o.push({role:"user",content:a}),i+=v,{maxTokens:n,usedTokens:i,messages:o,wasTruncated:l}}async function*fn(e,t,a,r,n,s){var w;const o=new J(t),i=(w=e.metadata)==null?void 0:w.thread_id;yield{type:"thinking",data:{threadId:i,provider:a.name}};const l=await o.buildContext(r.id),c=await o.getRecentConversations(r.id,20,i),u=Xa(r,l),m=gn(u,c,e.text,a.name);await o.storeMessage(r.id,e.channel,"user",e.text,"{}",i);const y=10;let v="",f=0;const x=[...m.messages];for(let E=0;E<y;E++)try{E>0&&(yield{type:"thinking",data:{threadId:i}});const d=await a.chat(x,{tools:ft});if(d.usage&&(f+=d.usage.promptTokens+d.usage.completionTokens),d.toolCalls&&d.toolCalls.length>0){d.content&&(yield{type:"chunk",data:{text:d.content,threadId:i}},x.push({role:"assistant",content:d.content}));for(const p of d.toolCalls){yield{type:"tool_start",data:{tool:p.name,toolArgs:p.arguments,threadId:i}};try{const h=await Ne(p.name,p.arguments,t,r.id,{agentType:"full",providerName:a.name,channel:e.channel},r.pin_hash,s==null?void 0:s.GOOGLE_CLIENT_ID,s==null?void 0:s.GOOGLE_CLIENT_SECRET,s==null?void 0:s.GOOGLE_API_KEY,s==null?void 0:s.GOOGLE_CSE_ID,r.timezone,a);yield{type:"tool_end",data:{tool:p.name,toolResult:h.substring(0,500)+(h.length>500?"...":""),threadId:i}},x.push({role:"user",content:`[Tool Result for ${p.name}]: ${h}`})}catch(h){await R(t,r.id,"tool",p.name,h.message||"Tool execution failed"),yield{type:"tool_end",data:{tool:p.name,toolResult:`Error: ${h.message||"Execution failed"}`,threadId:i}},x.push({role:"user",content:`[Tool Error for ${p.name}]: ${h.message||"Execution failed"}`})}}continue}v=d.content;const g=50;for(let p=0;p<v.length;p+=g)yield{type:"chunk",data:{text:v.substring(p,p+g),threadId:i}},p+g<v.length&&await new Promise(_=>setTimeout(_,10));break}catch(d){if(n){const g=d.message||"",p=g.includes("401")||g.includes("403")||g.includes("authentication")||g.includes("credit balance"),h=g.includes("429"),_=p?1440:h?10:5;await n.recordError(a.name,g,_)}await R(t,r.id,"llm","provider_error",d.message||"Unknown LLM error",{provider:a.name,turn:E}),yield{type:"error",data:{error:d.message||"An error occurred",threadId:i}};return}if(n&&f>0)try{await n.recordUsage(a.name,f)}catch{}await o.storeMessage(r.id,e.channel,"assistant",v,"{}",i),await o.compactHistory(r.id,30),yield{type:"done",data:{threadId:i,provider:a.name,tokenCount:f}}}async function jt(e,t,a,r,n,s){var u;const o=new J(t),i=(u=e.metadata)==null?void 0:u.thread_id,l=await o.buildContext(r.id),c=Ya(e.text,l);if(c.agent==="conversation")return vn(e,t,a,r,l,n,i);if(c.agent==="multi"||c.confidence<.5)return Jt(e,t,a,r,n,s);try{return await yn(c.agent,e,t,a,r,l,n,s,i)}catch(m){return await R(t,r.id,"router","subagent_fallback",`${c.agent} failed: ${m.message}`,{route:c}),Jt(e,t,a,r,n,s)}}async function yn(e,t,a,r,n,s,o,i,l){const c=new J(a),u=yt(n.timezone),m=ut(e,n,s,n.timezone,u),y=Ja(e,ft),v=(await c.getRecentConversations(n.id,25,l)).map(k=>({...k,content:k.content.replace(/^\[TOOLS_USED: [^\]]+\]\s*/i,"")})),f=[{role:"system",content:m},...v.map(k=>({role:k.role,content:k.content})),{role:"user",content:t.text}];await c.storeMessage(n.id,t.channel,"user",t.text,"{}",l);const x=10;let w="",E=0,d="",g=!1;const p=[];for(let k=0;k<x;k++)try{const I=await r.chat(f,{tools:y.length>0?y:void 0});if(I.usage&&(E+=I.usage.promptTokens+I.usage.completionTokens),I.toolCalls&&I.toolCalls.length>0){g=!0,I.content&&(d=I.content,f.push({role:"assistant",content:I.content}));for(const O of I.toolCalls){p.push(O.name);try{const L=await Ne(O.name,O.arguments,a,n.id,{agentType:e,providerName:r.name,channel:t.channel},n.pin_hash,i==null?void 0:i.GOOGLE_CLIENT_ID,i==null?void 0:i.GOOGLE_CLIENT_SECRET,i==null?void 0:i.GOOGLE_API_KEY,i==null?void 0:i.GOOGLE_CSE_ID,n.timezone,r);f.push({role:"user",content:`[Tool Result for ${O.name}]: ${L}`})}catch(L){await R(a,n.id,"tool",O.name,L.message||"Tool execution failed"),f.push({role:"user",content:`[Tool Error for ${O.name}]: ${L.message||"Execution failed"}`})}}continue}w=I.content,!w&&d&&(w=d);break}catch(I){if(o){const O=I.message||"",L=O.includes("401")||O.includes("403")||O.includes("authentication")||O.includes("credit balance"),$=O.includes("429"),z=L?1440:$?10:5;await o.recordError(r.name,O,z)}throw await R(a,n.id,"llm","subagent_error",I.message||"Unknown error",{agent:e,provider:r.name,turn:k}),I}const h=["scheduler","workspace","research"],_=e==="scheduler"&&!p.includes("create_schedule")&&!p.includes("list_schedules")&&!p.includes("toggle_schedule")&&!p.includes("update_schedule_state")&&!p.includes("delete_schedule")&&/\b(remind|in\s+\d+|at\s+\d{1,2}[:.]\d{0,2}|timer|alarm|schedule|tell\s+me\s+in|notify|alert|ping)\b/i.test(t.text);if(!g&&h.includes(e)&&y.length>0||_){try{await a.prepare(`INSERT INTO tool_execution_log (user_id, agent_type, provider_name, tool_name, tool_args, tool_result, success, error_message, latency_ms, was_enforcement_retry, channel)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(n.id,e,r.name,"__enforcement_trigger",JSON.stringify({userMessage:t.text.substring(0,200)}),w.substring(0,200),0,"LLM narrated without calling tools",0,0,t.channel||"web").run()}catch{}try{f.push({role:"assistant",content:w}),f.push({role:"user",content:e==="scheduler"&&_?`[SYSTEM OVERRIDE] You called ${p.join(", ")||"no tools"} but did NOT call create_schedule. The user wants a reminder or schedule created. Call create_schedule NOW with the correct parameters. Do NOT respond with text saying a reminder is set — actually create it.`:`[SYSTEM OVERRIDE] You responded with text but did NOT call any tool. This is a ${e} request — you MUST use your tools. Do NOT repeat your text response. Call the appropriate tool NOW (e.g., ${y.slice(0,3).map(I=>I.name).join(", ")}). The user is waiting for an actual action, not a description of what you would do.`});const k=await r.chat(f,{tools:y});if(k.usage&&(E+=k.usage.promptTokens+k.usage.completionTokens),k.toolCalls&&k.toolCalls.length>0){g=!0;for(const O of k.toolCalls){p.push(O.name);try{const L=await Ne(O.name,O.arguments,a,n.id,{agentType:e,providerName:r.name,channel:t.channel,isEnforcementRetry:!0},n.pin_hash,i==null?void 0:i.GOOGLE_CLIENT_ID,i==null?void 0:i.GOOGLE_CLIENT_SECRET,i==null?void 0:i.GOOGLE_API_KEY,i==null?void 0:i.GOOGLE_CSE_ID,n.timezone,r);f.push({role:"user",content:`[Tool Result for ${O.name}]: ${L}`})}catch(L){f.push({role:"user",content:`[Tool Error for ${O.name}]: ${L.message||"Execution failed"}`})}}const I=await r.chat(f,{tools:y});I.content&&(w=I.content),I.usage&&(E+=I.usage.promptTokens+I.usage.completionTokens)}if(!p.includes("create_schedule")&&e==="scheduler")try{const I=pn(t.text);if(I){const O=await Ne("create_schedule",I.args,a,n.id,{agentType:"scheduler",providerName:"programmatic_fallback",channel:t.channel,isEnforcementRetry:!0},n.pin_hash,i==null?void 0:i.GOOGLE_CLIENT_ID,i==null?void 0:i.GOOGLE_CLIENT_SECRET,i==null?void 0:i.GOOGLE_API_KEY,i==null?void 0:i.GOOGLE_CSE_ID,n.timezone,r);g=!0,p.push("create_schedule"),w=O}}catch(I){await R(a,n.id,"tool_enforcement","scheduler_fallback",`Programmatic scheduler fallback failed: ${I.message||"Unknown"}`)}}catch(k){await R(a,n.id,"tool_enforcement",e,`Tool enforcement retry failed: ${k.message||"Unknown"}`,{originalResponse:w.substring(0,200)})}}if(o&&E>0)try{await o.recordUsage(r.name,E)}catch{}const D=p.length>0?`[TOOLS_USED: ${[...new Set(p)].join(", ")}] `:"";return await c.storeMessage(n.id,t.channel,"assistant",D+w,"{}",l),await c.compactHistory(n.id,30),w}async function vn(e,t,a,r,n,s,o){const i=new J(t),l=yt(r.timezone),c=ut("conversation",r,n,r.timezone,l),u=await i.getRecentConversations(r.id,25,o),m=[{role:"system",content:c},...u.map(f=>({role:f.role,content:f.content})),{role:"user",content:e.text}];await i.storeMessage(r.id,e.channel,"user",e.text,"{}",o);let y=0,v="";try{const f=await a.chat(m,{temperature:.8});f.usage&&(y=f.usage.promptTokens+f.usage.completionTokens),v=f.content}catch(f){if(s){const x=f.message||"",w=x.includes("401")||x.includes("403")||x.includes("authentication")||x.includes("credit balance"),E=x.includes("429"),d=w?1440:E?10:5;await s.recordError(a.name,x,d)}throw await R(t,r.id,"llm","conversation_error",f.message,{provider:a.name}),f}if(s&&y>0)try{await s.recordUsage(a.name,y)}catch{}return await i.storeMessage(r.id,e.channel,"assistant",v,"{}",o),await i.compactHistory(r.id,30),v}async function*bn(e,t,a,r,n,s){var d;const o=new J(t),i=(d=e.metadata)==null?void 0:d.thread_id,l=await o.buildContext(r.id),c=Ya(e.text,l);if(yield{type:"thinking",data:{threadId:i,provider:a.name}},c.agent==="multi"||c.confidence<.5){yield*fn(e,t,a,r,n,s);return}const u=yt(r.timezone),m=c.agent==="conversation"?ut("conversation",r,l,r.timezone,u):ut(c.agent,r,l,r.timezone,u),y=Ja(c.agent,ft),v=await o.getRecentConversations(r.id,25,i),f=[{role:"system",content:m},...v.map(g=>({role:g.role,content:g.content})),{role:"user",content:e.text}];await o.storeMessage(r.id,e.channel,"user",e.text,"{}",i);const x=10;let w="",E=0;for(let g=0;g<x;g++)try{g>0&&(yield{type:"thinking",data:{threadId:i}});const p=await a.chat(f,{tools:y.length>0?y:void 0});if(p.usage&&(E+=p.usage.promptTokens+p.usage.completionTokens),p.toolCalls&&p.toolCalls.length>0){p.content&&(yield{type:"chunk",data:{text:p.content,threadId:i}},f.push({role:"assistant",content:p.content}));for(const _ of p.toolCalls){yield{type:"tool_start",data:{tool:_.name,toolArgs:_.arguments,threadId:i}};try{const T=await Ne(_.name,_.arguments,t,r.id,{agentType:c.agent,providerName:a.name,channel:e.channel},r.pin_hash,s==null?void 0:s.GOOGLE_CLIENT_ID,s==null?void 0:s.GOOGLE_CLIENT_SECRET,s==null?void 0:s.GOOGLE_API_KEY,s==null?void 0:s.GOOGLE_CSE_ID,r.timezone,a);yield{type:"tool_end",data:{tool:_.name,toolResult:T.substring(0,500)+(T.length>500?"...":""),threadId:i}},f.push({role:"user",content:`[Tool Result for ${_.name}]: ${T}`})}catch(T){await R(t,r.id,"tool",_.name,T.message||"Tool execution failed"),yield{type:"tool_end",data:{tool:_.name,toolResult:`Error: ${T.message||"Execution failed"}`,threadId:i}},f.push({role:"user",content:`[Tool Error for ${_.name}]: ${T.message||"Execution failed"}`})}}continue}w=p.content;const h=50;for(let _=0;_<w.length;_+=h)yield{type:"chunk",data:{text:w.substring(_,_+h),threadId:i}},_+h<w.length&&await new Promise(D=>setTimeout(D,10));break}catch(p){if(n){const h=p.message||"",_=h.includes("401")||h.includes("403")||h.includes("authentication")||h.includes("credit balance"),T=h.includes("429"),D=_?1440:T?10:5;await n.recordError(a.name,h,D)}await R(t,r.id,"llm","subagent_stream_error",p.message||"Unknown error",{agent:c.agent,provider:a.name,turn:g}),yield{type:"error",data:{error:p.message||"An error occurred",threadId:i}};return}if(n&&E>0)try{await n.recordUsage(a.name,E)}catch{}await o.storeMessage(r.id,e.channel,"assistant",w,"{}",i),await o.compactHistory(r.id,30),yield{type:"done",data:{threadId:i,provider:a.name,tokenCount:E}}}const B=new ve;async function wn(e,t){var n;const a=(n=e.req.header("Authorization"))==null?void 0:n.replace("Bearer ","");if(!a)return e.json({error:"Authentication required"},401);const r=await e.env.DB.prepare(`SELECT s.*, u.* FROM sessions s JOIN users u ON s.user_id = u.id 
     WHERE s.id = ? AND s.expires_at > datetime('now')`).bind(a).first();if(!r)return e.json({error:"Invalid session"},401);e.set("user",{id:r.user_id,username:r.username,name:r.name,pin_hash:r.pin_hash,role:r.role,personality_prompt:r.personality_prompt,telegram_chat_id:r.telegram_chat_id,timezone:r.timezone,assistant_name:r.assistant_name||"Karna",created_at:r.created_at,updated_at:r.updated_at}),e.set("sessionId",a),await t()}B.use("/*",wn);B.get("/threads",async e=>{const t=e.get("user"),a=e.req.query("archived")==="1",r=parseInt(e.req.query("limit")||"30"),n=await e.env.DB.prepare(`SELECT t.*, 
...
`;for(const c of s)o+=`- ${c.name} (${c.type}, ${Math.round(c.size/1024)}KB, file_id: ${c.file_id})`,c.text_preview&&(o+=`
  Preview: ${c.text_preview.substring(0,300)}...`),o+=`
`;o+="Use parse_document tool to read full file contents. Use drive_upload to upload to Google Drive.]"}let i=n;if(!i){const c=await e.env.DB.prepare("INSERT INTO threads (user_id, title) VALUES (?, ?) RETURNING *").bind(t.id,a.trim().substring(0,60)+(a.trim().length>60?"...":"")).first();i=c==null?void 0:c.id}const l={userId:t.id,username:t.username,channel:r,text:a.trim()+o,sessionId:e.get("sessionId"),timestamp:new Date().toISOString(),metadata:{thread_id:i}};try{const{provider:c,rotation:u}=await at(e.env.DB,t.id,t.pin_hash),m=new ReadableStream({async start(y){const v=new TextEncoder;try{const f=bn(l,e.env.DB,c,t,u,{GOOGLE_CLIENT_ID:e.env.GOOGLE_CLIENT_ID,GOOGLE_CLIENT_SECRET:e.env.GOOGLE_CLIENT_SECRET,GOOGLE_API_KEY:e.env.GOOGLE_API_KEY,GOOGLE_CSE_ID:e.env.GOOGLE_CSE_ID});for await(const x of f)x.data.threadId||(x.data.threadId=i),y.enqueue(v.encode(Xt(x)));i&&await e.env.DB.prepare("UPDATE threads SET message_count = message_count + 2, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(i).run(),y.close()}catch(f){const x={type:"error",data:{error:f.message||"An error occurred",threadId:i}};y.enqueue(v.encode(Xt(x))),y.close()}}});return new Response(m,{headers:{"Content-Type":"text/event-stream","Cache-Control":"no-cache",Connection:"keep-alive","X-Thread-Id":String(i||"")}})}catch(c){console.error("Stream setup error:",c);const u=c.message||"";return u.includes("No LLM provider configured")?e.json({error:"No AI provider configured. Please add at least one API key in Settings → Keys.",type:"no_provider",thread_id:i},400):u.includes("limit reached")?e.json({error:u,type:"cost_limit",thread_id:i},429):e.json({error:"Something went wrong setting up the stream.",details:u,thread_id:i},500)}});B.get("/threads/:id/messages",async e=>{var s;const t=e.get("user"),a=parseInt(e.req.param("id")),r=parseInt(e.req.query("limit")||"50"),n=await e.env.DB.prepare(`SELECT id, role, content, channel, created_at FROM conversations 
...
- Example: "No events at Tata Theatre tomorrow."`;function En(e,t,a){return a==="reminder"?`[Scheduled Reminder] "${e}": ${t||"Time for your reminder."}`:a==="check_mail"?`[Autonomous Scheduled Task] Execute this task NOW using tools — do NOT just describe what you'd do.
Task: "${e}"
