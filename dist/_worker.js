...
Go to the web app → Settings → Keys → add at least one API key (DeepSeek, Grok, Abacus AI, etc.).`:(c=S.message)!=null&&c.includes("Daily usage limit")?"⚠️ Daily usage limit reached. Your limit resets at midnight.":`⚠️ AI provider error: ${S.message||"Unknown error"}`;await P(h,b,k);return}/\b(write\s+(an?\s+)?(essay|article|report|document|doc|blog|post|summary|draft)|draft\s+(an?\s+)?(essay|article|report|document))\b/i.test(T)&&await P(h,b,"📝 Writing and saving to Drive— this takes about 20 seconds...");const R=25e3;try{const S=await Promise.race([Gt(y,a,_,u,x,r),new Promise((O,D)=>setTimeout(()=>D(new Error("TELEGRAM_TIMEOUT")),R))]),k=Rs(S,"telegram");await P(h,b,k||"(empty response)")}catch(S){console.error("Telegram agent error:",S);const k=S.message==="TELEGRAM_TIMEOUT",O=k?`⏱️ This took longer than Telegram allows (25s limit).
